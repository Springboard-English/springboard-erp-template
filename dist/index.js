import * as h from "react";
import Ve, { useState as de, useLayoutEffect as Mo, useContext as gt, createContext as zt, forwardRef as ar, createElement as _r, useEffect as ae, useCallback as Re, useMemo as ue, useRef as ge, Fragment as Gu, useId as Yu } from "react";
import * as Wa from "react-dom";
import Hu, { unstable_batchedUpdates as Fu, createPortal as Lu } from "react-dom";
let Ua = "https://api.springboard.vn";
function _b(e) {
  Ua = e.baseUrl;
}
const Ln = {
  get baseURL() {
    return Ua;
  },
  endpoints: {
    login: "/login/password",
    authenticateGoogle: "/authenticate/google",
    currentUser: "/users/me",
    refresh: "/refresh",
    logout: "/logout",
    resetPassword: "/reset/password",
    resetPasswordAuthorised: "/authenticate/reset",
    sessions: "/users/me/sessions",
    managementSessions: "/sessions",
    managementTests: "/tests",
    managementClasses: "/classes",
    managementGroups: "/groups",
    managementUsers: "/users",
    managementAccounts: "/accounts",
    managementDayblocks: "/dayblocks",
    managementSchedules: "/schedules",
    managementFeedbacks: "/feedbacks",
    managementRegistrations: "/registrations",
    ownRegistrations: "/users/me/registrations",
    ownAssessments: "/users/me/assessments",
    ownNotifications: "/users/me/notifications",
    recordings: "/users/me/recordings"
  }
};
function nt(e) {
  return `${Ln.baseURL}${Ln.endpoints[e]}`;
}
function Qu(e) {
  return `${Ln.baseURL}/notifications/${encodeURIComponent(e)}`;
}
function Zu(e) {
  return e && e.__esModule && Object.prototype.hasOwnProperty.call(e, "default") ? e.default : e;
}
var Mn = { exports: {} }, Ht = {};
var Os;
function Ju() {
  if (Os) return Ht;
  Os = 1;
  var e = /* @__PURE__ */ Symbol.for("react.transitional.element"), t = /* @__PURE__ */ Symbol.for("react.fragment");
  function n(r, o, s) {
    var i = null;
    if (s !== void 0 && (i = "" + s), o.key !== void 0 && (i = "" + o.key), "key" in o) {
      s = {};
      for (var c in o)
        c !== "key" && (s[c] = o[c]);
    } else s = o;
    return o = s.ref, {
      $$typeof: e,
      type: r,
      key: i,
      ref: o !== void 0 ? o : null,
      props: s
    };
  }
  return Ht.Fragment = t, Ht.jsx = n, Ht.jsxs = n, Ht;
}
var Ft = {};
var zs;
function Wu() {
  return zs || (zs = 1, process.env.NODE_ENV !== "production" && (function() {
    function e(y) {
      if (y == null) return null;
      if (typeof y == "function")
        return y.$$typeof === z ? null : y.displayName || y.name || null;
      if (typeof y == "string") return y;
      switch (y) {
        case v:
          return "Fragment";
        case A:
          return "Profiler";
        case b:
          return "StrictMode";
        case P:
          return "Suspense";
        case D:
          return "SuspenseList";
        case Y:
          return "Activity";
      }
      if (typeof y == "object")
        switch (typeof y.tag == "number" && console.error(
          "Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."
        ), y.$$typeof) {
          case g:
            return "Portal";
          case E:
            return y.displayName || "Context";
          case w:
            return (y._context.displayName || "Context") + ".Consumer";
          case C:
            var G = y.render;
            return y = y.displayName, y || (y = G.displayName || G.name || "", y = y !== "" ? "ForwardRef(" + y + ")" : "ForwardRef"), y;
          case M:
            return G = y.displayName || null, G !== null ? G : e(y.type) || "Memo";
          case k:
            G = y._payload, y = y._init;
            try {
              return e(y(G));
            } catch {
            }
        }
      return null;
    }
    function t(y) {
      return "" + y;
    }
    function n(y) {
      try {
        t(y);
        var G = !1;
      } catch {
        G = !0;
      }
      if (G) {
        G = console;
        var Q = G.error, W = typeof Symbol == "function" && Symbol.toStringTag && y[Symbol.toStringTag] || y.constructor.name || "Object";
        return Q.call(
          G,
          "The provided key is an unsupported type %s. This value must be coerced to a string before using it here.",
          W
        ), t(y);
      }
    }
    function r(y) {
      if (y === v) return "<>";
      if (typeof y == "object" && y !== null && y.$$typeof === k)
        return "<...>";
      try {
        var G = e(y);
        return G ? "<" + G + ">" : "<...>";
      } catch {
        return "<...>";
      }
    }
    function o() {
      var y = Z.A;
      return y === null ? null : y.getOwner();
    }
    function s() {
      return Error("react-stack-top-frame");
    }
    function i(y) {
      if (q.call(y, "key")) {
        var G = Object.getOwnPropertyDescriptor(y, "key").get;
        if (G && G.isReactWarning) return !1;
      }
      return y.key !== void 0;
    }
    function c(y, G) {
      function Q() {
        j || (j = !0, console.error(
          "%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://react.dev/link/special-props)",
          G
        ));
      }
      Q.isReactWarning = !0, Object.defineProperty(y, "key", {
        get: Q,
        configurable: !0
      });
    }
    function u() {
      var y = e(this.type);
      return O[y] || (O[y] = !0, console.error(
        "Accessing element.ref was removed in React 19. ref is now a regular prop. It will be removed from the JSX Element type in a future release."
      )), y = this.props.ref, y !== void 0 ? y : null;
    }
    function l(y, G, Q, W, X, _) {
      var J = Q.ref;
      return y = {
        $$typeof: x,
        type: y,
        key: G,
        props: Q,
        _owner: W
      }, (J !== void 0 ? J : null) !== null ? Object.defineProperty(y, "ref", {
        enumerable: !1,
        get: u
      }) : Object.defineProperty(y, "ref", { enumerable: !1, value: null }), y._store = {}, Object.defineProperty(y._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: 0
      }), Object.defineProperty(y, "_debugInfo", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: null
      }), Object.defineProperty(y, "_debugStack", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: X
      }), Object.defineProperty(y, "_debugTask", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: _
      }), Object.freeze && (Object.freeze(y.props), Object.freeze(y)), y;
    }
    function f(y, G, Q, W, X, _) {
      var J = G.children;
      if (J !== void 0)
        if (W)
          if (T(J)) {
            for (W = 0; W < J.length; W++)
              d(J[W]);
            Object.freeze && Object.freeze(J);
          } else
            console.error(
              "React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead."
            );
        else d(J);
      if (q.call(G, "key")) {
        J = e(y);
        var oe = Object.keys(G).filter(function(ne) {
          return ne !== "key";
        });
        W = 0 < oe.length ? "{key: someKey, " + oe.join(": ..., ") + ": ...}" : "{key: someKey}", I[J + W] || (oe = 0 < oe.length ? "{" + oe.join(": ..., ") + ": ...}" : "{}", console.error(
          `A props object containing a "key" prop is being spread into JSX:
  let props = %s;
  <%s {...props} />
React keys must be passed directly to JSX without using spread:
  let props = %s;
  <%s key={someKey} {...props} />`,
          W,
          J,
          oe,
          J
        ), I[J + W] = !0);
      }
      if (J = null, Q !== void 0 && (n(Q), J = "" + Q), i(G) && (n(G.key), J = "" + G.key), "key" in G) {
        Q = {};
        for (var ee in G)
          ee !== "key" && (Q[ee] = G[ee]);
      } else Q = G;
      return J && c(
        Q,
        typeof y == "function" ? y.displayName || y.name || "Unknown" : y
      ), l(
        y,
        J,
        Q,
        o(),
        X,
        _
      );
    }
    function d(y) {
      m(y) ? y._store && (y._store.validated = 1) : typeof y == "object" && y !== null && y.$$typeof === k && (y._payload.status === "fulfilled" ? m(y._payload.value) && y._payload.value._store && (y._payload.value._store.validated = 1) : y._store && (y._store.validated = 1));
    }
    function m(y) {
      return typeof y == "object" && y !== null && y.$$typeof === x;
    }
    var p = Ve, x = /* @__PURE__ */ Symbol.for("react.transitional.element"), g = /* @__PURE__ */ Symbol.for("react.portal"), v = /* @__PURE__ */ Symbol.for("react.fragment"), b = /* @__PURE__ */ Symbol.for("react.strict_mode"), A = /* @__PURE__ */ Symbol.for("react.profiler"), w = /* @__PURE__ */ Symbol.for("react.consumer"), E = /* @__PURE__ */ Symbol.for("react.context"), C = /* @__PURE__ */ Symbol.for("react.forward_ref"), P = /* @__PURE__ */ Symbol.for("react.suspense"), D = /* @__PURE__ */ Symbol.for("react.suspense_list"), M = /* @__PURE__ */ Symbol.for("react.memo"), k = /* @__PURE__ */ Symbol.for("react.lazy"), Y = /* @__PURE__ */ Symbol.for("react.activity"), z = /* @__PURE__ */ Symbol.for("react.client.reference"), Z = p.__CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, q = Object.prototype.hasOwnProperty, T = Array.isArray, R = console.createTask ? console.createTask : function() {
      return null;
    };
    p = {
      react_stack_bottom_frame: function(y) {
        return y();
      }
    };
    var j, O = {}, N = p.react_stack_bottom_frame.bind(
      p,
      s
    )(), S = R(r(s)), I = {};
    Ft.Fragment = v, Ft.jsx = function(y, G, Q) {
      var W = 1e4 > Z.recentlyCreatedOwnerStacks++;
      return f(
        y,
        G,
        Q,
        !1,
        W ? Error("react-stack-top-frame") : N,
        W ? R(r(y)) : S
      );
    }, Ft.jsxs = function(y, G, Q) {
      var W = 1e4 > Z.recentlyCreatedOwnerStacks++;
      return f(
        y,
        G,
        Q,
        !0,
        W ? Error("react-stack-top-frame") : N,
        W ? R(r(y)) : S
      );
    };
  })()), Ft;
}
var Bs;
function Uu() {
  return Bs || (Bs = 1, process.env.NODE_ENV === "production" ? Mn.exports = Ju() : Mn.exports = Wu()), Mn.exports;
}
var a = Uu();
function Va(e) {
  var t, n, r = "";
  if (typeof e == "string" || typeof e == "number") r += e;
  else if (typeof e == "object") if (Array.isArray(e)) {
    var o = e.length;
    for (t = 0; t < o; t++) e[t] && (n = Va(e[t])) && (r && (r += " "), r += n);
  } else for (n in e) e[n] && (r && (r += " "), r += n);
  return r;
}
function qa() {
  for (var e, t, n = 0, r = "", o = arguments.length; n < o; n++) (e = arguments[n]) && (t = Va(e)) && (r && (r += " "), r += t);
  return r;
}
const Ts = (e) => typeof e == "boolean" ? `${e}` : e === 0 ? "0" : e, Gs = qa, Vu = (e, t) => (n) => {
  var r;
  if (t?.variants == null) return Gs(e, n?.class, n?.className);
  const { variants: o, defaultVariants: s } = t, i = Object.keys(o).map((l) => {
    const f = n?.[l], d = s?.[l];
    if (f === null) return null;
    const m = Ts(f) || Ts(d);
    return o[l][m];
  }), c = n && Object.entries(n).reduce((l, f) => {
    let [d, m] = f;
    return m === void 0 || (l[d] = m), l;
  }, {}), u = t == null || (r = t.compoundVariants) === null || r === void 0 ? void 0 : r.reduce((l, f) => {
    let { class: d, className: m, ...p } = f;
    return Object.entries(p).every((x) => {
      let [g, v] = x;
      return Array.isArray(v) ? v.includes({
        ...s,
        ...c
      }[g]) : {
        ...s,
        ...c
      }[g] === v;
    }) ? [
      ...l,
      d,
      m
    ] : l;
  }, []);
  return Gs(e, i, u, n?.class, n?.className);
};
function Ys(e, t) {
  if (typeof e == "function")
    return e(t);
  e != null && (e.current = t);
}
function ir(...e) {
  return (t) => {
    let n = !1;
    const r = e.map((o) => {
      const s = Ys(o, t);
      return !n && typeof s == "function" && (n = !0), s;
    });
    if (n)
      return () => {
        for (let o = 0; o < r.length; o++) {
          const s = r[o];
          typeof s == "function" ? s() : Ys(e[o], null);
        }
      };
  };
}
function ce(...e) {
  return h.useCallback(ir(...e), e);
}
// @__NO_SIDE_EFFECTS__
function ft(e) {
  const t = /* @__PURE__ */ Xu(e), n = h.forwardRef((r, o) => {
    const { children: s, ...i } = r, c = h.Children.toArray(s), u = c.find(_u);
    if (u) {
      const l = u.props.children, f = c.map((d) => d === u ? h.Children.count(l) > 1 ? h.Children.only(null) : h.isValidElement(l) ? l.props.children : null : d);
      return /* @__PURE__ */ a.jsx(t, { ...i, ref: o, children: h.isValidElement(l) ? h.cloneElement(l, void 0, f) : null });
    }
    return /* @__PURE__ */ a.jsx(t, { ...i, ref: o, children: s });
  });
  return n.displayName = `${e}.Slot`, n;
}
var qu = /* @__PURE__ */ ft("Slot");
// @__NO_SIDE_EFFECTS__
function Xu(e) {
  const t = h.forwardRef((n, r) => {
    const { children: o, ...s } = n;
    if (h.isValidElement(o)) {
      const i = ed(o), c = $u(s, o.props);
      return o.type !== h.Fragment && (c.ref = r ? ir(r, i) : i), h.cloneElement(o, c);
    }
    return h.Children.count(o) > 1 ? h.Children.only(null) : null;
  });
  return t.displayName = `${e}.SlotClone`, t;
}
var Ku = /* @__PURE__ */ Symbol("radix.slottable");
function _u(e) {
  return h.isValidElement(e) && typeof e.type == "function" && "__radixId" in e.type && e.type.__radixId === Ku;
}
function $u(e, t) {
  const n = { ...t };
  for (const r in t) {
    const o = e[r], s = t[r];
    /^on[A-Z]/.test(r) ? o && s ? n[r] = (...c) => {
      const u = s(...c);
      return o(...c), u;
    } : o && (n[r] = o) : r === "style" ? n[r] = { ...o, ...s } : r === "className" && (n[r] = [o, s].filter(Boolean).join(" "));
  }
  return { ...e, ...n };
}
function ed(e) {
  let t = Object.getOwnPropertyDescriptor(e.props, "ref")?.get, n = t && "isReactWarning" in t && t.isReactWarning;
  return n ? e.ref : (t = Object.getOwnPropertyDescriptor(e, "ref")?.get, n = t && "isReactWarning" in t && t.isReactWarning, n ? e.props.ref : e.props.ref || e.ref);
}
var td = [
  "a",
  "button",
  "div",
  "form",
  "h2",
  "h3",
  "img",
  "input",
  "label",
  "li",
  "nav",
  "ol",
  "p",
  "select",
  "span",
  "svg",
  "ul"
], re = td.reduce((e, t) => {
  const n = /* @__PURE__ */ ft(`Primitive.${t}`), r = h.forwardRef((o, s) => {
    const { asChild: i, ...c } = o, u = i ? n : t;
    return typeof window < "u" && (window[/* @__PURE__ */ Symbol.for("radix-ui")] = !0), /* @__PURE__ */ a.jsx(u, { ...c, ref: s });
  });
  return r.displayName = `Primitive.${t}`, { ...e, [t]: r };
}, {});
function Xa(e, t) {
  e && Wa.flushSync(() => e.dispatchEvent(t));
}
function nd(e, t) {
  const n = h.createContext(t), r = (s) => {
    const { children: i, ...c } = s, u = h.useMemo(() => c, Object.values(c));
    return /* @__PURE__ */ a.jsx(n.Provider, { value: u, children: i });
  };
  r.displayName = e + "Provider";
  function o(s) {
    const i = h.useContext(n);
    if (i) return i;
    if (t !== void 0) return t;
    throw new Error(`\`${s}\` must be used within \`${e}\``);
  }
  return [r, o];
}
function xt(e, t = []) {
  let n = [];
  function r(s, i) {
    const c = h.createContext(i), u = n.length;
    n = [...n, i];
    const l = (d) => {
      const { scope: m, children: p, ...x } = d, g = m?.[e]?.[u] || c, v = h.useMemo(() => x, Object.values(x));
      return /* @__PURE__ */ a.jsx(g.Provider, { value: v, children: p });
    };
    l.displayName = s + "Provider";
    function f(d, m) {
      const p = m?.[e]?.[u] || c, x = h.useContext(p);
      if (x) return x;
      if (i !== void 0) return i;
      throw new Error(`\`${d}\` must be used within \`${s}\``);
    }
    return [l, f];
  }
  const o = () => {
    const s = n.map((i) => h.createContext(i));
    return function(c) {
      const u = c?.[e] || s;
      return h.useMemo(
        () => ({ [`__scope${e}`]: { ...c, [e]: u } }),
        [c, u]
      );
    };
  };
  return o.scopeName = e, [r, rd(o, ...t)];
}
function rd(...e) {
  const t = e[0];
  if (e.length === 1) return t;
  const n = () => {
    const r = e.map((o) => ({
      useScope: o(),
      scopeName: o.scopeName
    }));
    return function(s) {
      const i = r.reduce((c, { useScope: u, scopeName: l }) => {
        const d = u(s)[`__scope${l}`];
        return { ...c, ...d };
      }, {});
      return h.useMemo(() => ({ [`__scope${t.scopeName}`]: i }), [i]);
    };
  };
  return n.scopeName = t.scopeName, n;
}
function Ka(e) {
  const t = e + "CollectionProvider", [n, r] = xt(t), [o, s] = n(
    t,
    { collectionRef: { current: null }, itemMap: /* @__PURE__ */ new Map() }
  ), i = (g) => {
    const { scope: v, children: b } = g, A = Ve.useRef(null), w = Ve.useRef(/* @__PURE__ */ new Map()).current;
    return /* @__PURE__ */ a.jsx(o, { scope: v, itemMap: w, collectionRef: A, children: b });
  };
  i.displayName = t;
  const c = e + "CollectionSlot", u = /* @__PURE__ */ ft(c), l = Ve.forwardRef(
    (g, v) => {
      const { scope: b, children: A } = g, w = s(c, b), E = ce(v, w.collectionRef);
      return /* @__PURE__ */ a.jsx(u, { ref: E, children: A });
    }
  );
  l.displayName = c;
  const f = e + "CollectionItemSlot", d = "data-radix-collection-item", m = /* @__PURE__ */ ft(f), p = Ve.forwardRef(
    (g, v) => {
      const { scope: b, children: A, ...w } = g, E = Ve.useRef(null), C = ce(v, E), P = s(f, b);
      return Ve.useEffect(() => (P.itemMap.set(E, { ref: E, ...w }), () => {
        P.itemMap.delete(E);
      })), /* @__PURE__ */ a.jsx(m, { [d]: "", ref: C, children: A });
    }
  );
  p.displayName = f;
  function x(g) {
    const v = s(e + "CollectionConsumer", g);
    return Ve.useCallback(() => {
      const A = v.collectionRef.current;
      if (!A) return [];
      const w = Array.from(A.querySelectorAll(`[${d}]`));
      return Array.from(v.itemMap.values()).sort(
        (P, D) => w.indexOf(P.ref.current) - w.indexOf(D.ref.current)
      );
    }, [v.collectionRef, v.itemMap]);
  }
  return [
    { Provider: i, Slot: l, ItemSlot: p },
    x,
    r
  ];
}
function U(e, t, { checkForDefaultPrevented: n = !0 } = {}) {
  return function(o) {
    if (e?.(o), n === !1 || !o.defaultPrevented)
      return t?.(o);
  };
}
var $e = globalThis?.document ? h.useLayoutEffect : () => {
}, od = h[" useInsertionEffect ".trim().toString()] || $e;
function fn({
  prop: e,
  defaultProp: t,
  onChange: n = () => {
  },
  caller: r
}) {
  const [o, s, i] = sd({
    defaultProp: t,
    onChange: n
  }), c = e !== void 0, u = c ? e : o;
  {
    const f = h.useRef(e !== void 0);
    h.useEffect(() => {
      const d = f.current;
      d !== c && console.warn(
        `${r} is changing from ${d ? "controlled" : "uncontrolled"} to ${c ? "controlled" : "uncontrolled"}. Components should not switch from controlled to uncontrolled (or vice versa). Decide between using a controlled or uncontrolled value for the lifetime of the component.`
      ), f.current = c;
    }, [c, r]);
  }
  const l = h.useCallback(
    (f) => {
      if (c) {
        const d = ad(f) ? f(e) : f;
        d !== e && i.current?.(d);
      } else
        s(f);
    },
    [c, e, s, i]
  );
  return [u, l];
}
function sd({
  defaultProp: e,
  onChange: t
}) {
  const [n, r] = h.useState(e), o = h.useRef(n), s = h.useRef(t);
  return od(() => {
    s.current = t;
  }, [t]), h.useEffect(() => {
    o.current !== n && (s.current?.(n), o.current = n);
  }, [n, o]), [n, r, s];
}
function ad(e) {
  return typeof e == "function";
}
function id(e, t) {
  return h.useReducer((n, r) => t[n][r] ?? n, e);
}
var Te = (e) => {
  const { present: t, children: n } = e, r = cd(t), o = typeof n == "function" ? n({ present: r.isPresent }) : h.Children.only(n), s = ce(r.ref, ld(o));
  return typeof n == "function" || r.isPresent ? h.cloneElement(o, { ref: s }) : null;
};
Te.displayName = "Presence";
function cd(e) {
  const [t, n] = h.useState(), r = h.useRef(null), o = h.useRef(e), s = h.useRef("none"), i = e ? "mounted" : "unmounted", [c, u] = id(i, {
    mounted: {
      UNMOUNT: "unmounted",
      ANIMATION_OUT: "unmountSuspended"
    },
    unmountSuspended: {
      MOUNT: "mounted",
      ANIMATION_END: "unmounted"
    },
    unmounted: {
      MOUNT: "mounted"
    }
  });
  return h.useEffect(() => {
    const l = jn(r.current);
    s.current = c === "mounted" ? l : "none";
  }, [c]), $e(() => {
    const l = r.current, f = o.current;
    if (f !== e) {
      const m = s.current, p = jn(l);
      e ? u("MOUNT") : p === "none" || l?.display === "none" ? u("UNMOUNT") : u(f && m !== p ? "ANIMATION_OUT" : "UNMOUNT"), o.current = e;
    }
  }, [e, u]), $e(() => {
    if (t) {
      let l;
      const f = t.ownerDocument.defaultView ?? window, d = (p) => {
        const g = jn(r.current).includes(CSS.escape(p.animationName));
        if (p.target === t && g && (u("ANIMATION_END"), !o.current)) {
          const v = t.style.animationFillMode;
          t.style.animationFillMode = "forwards", l = f.setTimeout(() => {
            t.style.animationFillMode === "forwards" && (t.style.animationFillMode = v);
          });
        }
      }, m = (p) => {
        p.target === t && (s.current = jn(r.current));
      };
      return t.addEventListener("animationstart", m), t.addEventListener("animationcancel", d), t.addEventListener("animationend", d), () => {
        f.clearTimeout(l), t.removeEventListener("animationstart", m), t.removeEventListener("animationcancel", d), t.removeEventListener("animationend", d);
      };
    } else
      u("ANIMATION_END");
  }, [t, u]), {
    isPresent: ["mounted", "unmountSuspended"].includes(c),
    ref: h.useCallback((l) => {
      r.current = l ? getComputedStyle(l) : null, n(l);
    }, [])
  };
}
function jn(e) {
  return e?.animationName || "none";
}
function ld(e) {
  let t = Object.getOwnPropertyDescriptor(e.props, "ref")?.get, n = t && "isReactWarning" in t && t.isReactWarning;
  return n ? e.ref : (t = Object.getOwnPropertyDescriptor(e, "ref")?.get, n = t && "isReactWarning" in t && t.isReactWarning, n ? e.props.ref : e.props.ref || e.ref);
}
var ud = h[" useId ".trim().toString()] || (() => {
}), dd = 0;
function Fe(e) {
  const [t, n] = h.useState(ud());
  return $e(() => {
    n((r) => r ?? String(dd++));
  }, [e]), e || (t ? `radix-${t}` : "");
}
var fd = h.createContext(void 0);
function _a(e) {
  const t = h.useContext(fd);
  return e || t || "ltr";
}
function ze(e) {
  const t = h.useRef(e);
  return h.useEffect(() => {
    t.current = e;
  }), h.useMemo(() => (...n) => t.current?.(...n), []);
}
function md(e, t = globalThis?.document) {
  const n = ze(e);
  h.useEffect(() => {
    const r = (o) => {
      o.key === "Escape" && n(o);
    };
    return t.addEventListener("keydown", r, { capture: !0 }), () => t.removeEventListener("keydown", r, { capture: !0 });
  }, [n, t]);
}
var hd = "DismissableLayer", $r = "dismissableLayer.update", pd = "dismissableLayer.pointerDownOutside", gd = "dismissableLayer.focusOutside", Hs, $a = h.createContext({
  layers: /* @__PURE__ */ new Set(),
  layersWithOutsidePointerEventsDisabled: /* @__PURE__ */ new Set(),
  branches: /* @__PURE__ */ new Set()
}), cr = h.forwardRef(
  (e, t) => {
    const {
      disableOutsidePointerEvents: n = !1,
      onEscapeKeyDown: r,
      onPointerDownOutside: o,
      onFocusOutside: s,
      onInteractOutside: i,
      onDismiss: c,
      ...u
    } = e, l = h.useContext($a), [f, d] = h.useState(null), m = f?.ownerDocument ?? globalThis?.document, [, p] = h.useState({}), x = ce(t, (D) => d(D)), g = Array.from(l.layers), [v] = [...l.layersWithOutsidePointerEventsDisabled].slice(-1), b = g.indexOf(v), A = f ? g.indexOf(f) : -1, w = l.layersWithOutsidePointerEventsDisabled.size > 0, E = A >= b, C = bd((D) => {
      const M = D.target, k = [...l.branches].some((Y) => Y.contains(M));
      !E || k || (o?.(D), i?.(D), D.defaultPrevented || c?.());
    }, m), P = wd((D) => {
      const M = D.target;
      [...l.branches].some((Y) => Y.contains(M)) || (s?.(D), i?.(D), D.defaultPrevented || c?.());
    }, m);
    return md((D) => {
      A === l.layers.size - 1 && (r?.(D), !D.defaultPrevented && c && (D.preventDefault(), c()));
    }, m), h.useEffect(() => {
      if (f)
        return n && (l.layersWithOutsidePointerEventsDisabled.size === 0 && (Hs = m.body.style.pointerEvents, m.body.style.pointerEvents = "none"), l.layersWithOutsidePointerEventsDisabled.add(f)), l.layers.add(f), Fs(), () => {
          n && l.layersWithOutsidePointerEventsDisabled.size === 1 && (m.body.style.pointerEvents = Hs);
        };
    }, [f, m, n, l]), h.useEffect(() => () => {
      f && (l.layers.delete(f), l.layersWithOutsidePointerEventsDisabled.delete(f), Fs());
    }, [f, l]), h.useEffect(() => {
      const D = () => p({});
      return document.addEventListener($r, D), () => document.removeEventListener($r, D);
    }, []), /* @__PURE__ */ a.jsx(
      re.div,
      {
        ...u,
        ref: x,
        style: {
          pointerEvents: w ? E ? "auto" : "none" : void 0,
          ...e.style
        },
        onFocusCapture: U(e.onFocusCapture, P.onFocusCapture),
        onBlurCapture: U(e.onBlurCapture, P.onBlurCapture),
        onPointerDownCapture: U(
          e.onPointerDownCapture,
          C.onPointerDownCapture
        )
      }
    );
  }
);
cr.displayName = hd;
var xd = "DismissableLayerBranch", vd = h.forwardRef((e, t) => {
  const n = h.useContext($a), r = h.useRef(null), o = ce(t, r);
  return h.useEffect(() => {
    const s = r.current;
    if (s)
      return n.branches.add(s), () => {
        n.branches.delete(s);
      };
  }, [n.branches]), /* @__PURE__ */ a.jsx(re.div, { ...e, ref: o });
});
vd.displayName = xd;
function bd(e, t = globalThis?.document) {
  const n = ze(e), r = h.useRef(!1), o = h.useRef(() => {
  });
  return h.useEffect(() => {
    const s = (c) => {
      if (c.target && !r.current) {
        let u = function() {
          ei(
            pd,
            n,
            l,
            { discrete: !0 }
          );
        };
        const l = { originalEvent: c };
        c.pointerType === "touch" ? (t.removeEventListener("click", o.current), o.current = u, t.addEventListener("click", o.current, { once: !0 })) : u();
      } else
        t.removeEventListener("click", o.current);
      r.current = !1;
    }, i = window.setTimeout(() => {
      t.addEventListener("pointerdown", s);
    }, 0);
    return () => {
      window.clearTimeout(i), t.removeEventListener("pointerdown", s), t.removeEventListener("click", o.current);
    };
  }, [t, n]), {
    // ensures we check React component tree (not just DOM tree)
    onPointerDownCapture: () => r.current = !0
  };
}
function wd(e, t = globalThis?.document) {
  const n = ze(e), r = h.useRef(!1);
  return h.useEffect(() => {
    const o = (s) => {
      s.target && !r.current && ei(gd, n, { originalEvent: s }, {
        discrete: !1
      });
    };
    return t.addEventListener("focusin", o), () => t.removeEventListener("focusin", o);
  }, [t, n]), {
    onFocusCapture: () => r.current = !0,
    onBlurCapture: () => r.current = !1
  };
}
function Fs() {
  const e = new CustomEvent($r);
  document.dispatchEvent(e);
}
function ei(e, t, n, { discrete: r }) {
  const o = n.originalEvent.target, s = new CustomEvent(e, { bubbles: !1, cancelable: !0, detail: n });
  t && o.addEventListener(e, t, { once: !0 }), r ? Xa(o, s) : o.dispatchEvent(s);
}
var kr = "focusScope.autoFocusOnMount", Dr = "focusScope.autoFocusOnUnmount", Ls = { bubbles: !1, cancelable: !0 }, Ad = "FocusScope", lr = h.forwardRef((e, t) => {
  const {
    loop: n = !1,
    trapped: r = !1,
    onMountAutoFocus: o,
    onUnmountAutoFocus: s,
    ...i
  } = e, [c, u] = h.useState(null), l = ze(o), f = ze(s), d = h.useRef(null), m = ce(t, (g) => u(g)), p = h.useRef({
    paused: !1,
    pause() {
      this.paused = !0;
    },
    resume() {
      this.paused = !1;
    }
  }).current;
  h.useEffect(() => {
    if (r) {
      let g = function(w) {
        if (p.paused || !c) return;
        const E = w.target;
        c.contains(E) ? d.current = E : qe(d.current, { select: !0 });
      }, v = function(w) {
        if (p.paused || !c) return;
        const E = w.relatedTarget;
        E !== null && (c.contains(E) || qe(d.current, { select: !0 }));
      }, b = function(w) {
        if (document.activeElement === document.body)
          for (const C of w)
            C.removedNodes.length > 0 && qe(c);
      };
      document.addEventListener("focusin", g), document.addEventListener("focusout", v);
      const A = new MutationObserver(b);
      return c && A.observe(c, { childList: !0, subtree: !0 }), () => {
        document.removeEventListener("focusin", g), document.removeEventListener("focusout", v), A.disconnect();
      };
    }
  }, [r, c, p.paused]), h.useEffect(() => {
    if (c) {
      Zs.add(p);
      const g = document.activeElement;
      if (!c.contains(g)) {
        const b = new CustomEvent(kr, Ls);
        c.addEventListener(kr, l), c.dispatchEvent(b), b.defaultPrevented || (yd(Nd(ti(c)), { select: !0 }), document.activeElement === g && qe(c));
      }
      return () => {
        c.removeEventListener(kr, l), setTimeout(() => {
          const b = new CustomEvent(Dr, Ls);
          c.addEventListener(Dr, f), c.dispatchEvent(b), b.defaultPrevented || qe(g ?? document.body, { select: !0 }), c.removeEventListener(Dr, f), Zs.remove(p);
        }, 0);
      };
    }
  }, [c, l, f, p]);
  const x = h.useCallback(
    (g) => {
      if (!n && !r || p.paused) return;
      const v = g.key === "Tab" && !g.altKey && !g.ctrlKey && !g.metaKey, b = document.activeElement;
      if (v && b) {
        const A = g.currentTarget, [w, E] = Ed(A);
        w && E ? !g.shiftKey && b === E ? (g.preventDefault(), n && qe(w, { select: !0 })) : g.shiftKey && b === w && (g.preventDefault(), n && qe(E, { select: !0 })) : b === A && g.preventDefault();
      }
    },
    [n, r, p.paused]
  );
  return /* @__PURE__ */ a.jsx(re.div, { tabIndex: -1, ...i, ref: m, onKeyDown: x });
});
lr.displayName = Ad;
function yd(e, { select: t = !1 } = {}) {
  const n = document.activeElement;
  for (const r of e)
    if (qe(r, { select: t }), document.activeElement !== n) return;
}
function Ed(e) {
  const t = ti(e), n = Qs(t, e), r = Qs(t.reverse(), e);
  return [n, r];
}
function ti(e) {
  const t = [], n = document.createTreeWalker(e, NodeFilter.SHOW_ELEMENT, {
    acceptNode: (r) => {
      const o = r.tagName === "INPUT" && r.type === "hidden";
      return r.disabled || r.hidden || o ? NodeFilter.FILTER_SKIP : r.tabIndex >= 0 ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
    }
  });
  for (; n.nextNode(); ) t.push(n.currentNode);
  return t;
}
function Qs(e, t) {
  for (const n of e)
    if (!Md(n, { upTo: t })) return n;
}
function Md(e, { upTo: t }) {
  if (getComputedStyle(e).visibility === "hidden") return !0;
  for (; e; ) {
    if (t !== void 0 && e === t) return !1;
    if (getComputedStyle(e).display === "none") return !0;
    e = e.parentElement;
  }
  return !1;
}
function jd(e) {
  return e instanceof HTMLInputElement && "select" in e;
}
function qe(e, { select: t = !1 } = {}) {
  if (e && e.focus) {
    const n = document.activeElement;
    e.focus({ preventScroll: !0 }), e !== n && jd(e) && t && e.select();
  }
}
var Zs = Cd();
function Cd() {
  let e = [];
  return {
    add(t) {
      const n = e[0];
      t !== n && n?.pause(), e = Js(e, t), e.unshift(t);
    },
    remove(t) {
      e = Js(e, t), e[0]?.resume();
    }
  };
}
function Js(e, t) {
  const n = [...e], r = n.indexOf(t);
  return r !== -1 && n.splice(r, 1), n;
}
function Nd(e) {
  return e.filter((t) => t.tagName !== "A");
}
var Id = "Portal", ur = h.forwardRef((e, t) => {
  const { container: n, ...r } = e, [o, s] = h.useState(!1);
  $e(() => s(!0), []);
  const i = n || o && globalThis?.document?.body;
  return i ? Hu.createPortal(/* @__PURE__ */ a.jsx(re.div, { ...r, ref: t }), i) : null;
});
ur.displayName = Id;
var Rr = 0;
function jo() {
  h.useEffect(() => {
    const e = document.querySelectorAll("[data-radix-focus-guard]");
    return document.body.insertAdjacentElement("afterbegin", e[0] ?? Ws()), document.body.insertAdjacentElement("beforeend", e[1] ?? Ws()), Rr++, () => {
      Rr === 1 && document.querySelectorAll("[data-radix-focus-guard]").forEach((t) => t.remove()), Rr--;
    };
  }, []);
}
function Ws() {
  const e = document.createElement("span");
  return e.setAttribute("data-radix-focus-guard", ""), e.tabIndex = 0, e.style.outline = "none", e.style.opacity = "0", e.style.position = "fixed", e.style.pointerEvents = "none", e;
}
var De = function() {
  return De = Object.assign || function(t) {
    for (var n, r = 1, o = arguments.length; r < o; r++) {
      n = arguments[r];
      for (var s in n) Object.prototype.hasOwnProperty.call(n, s) && (t[s] = n[s]);
    }
    return t;
  }, De.apply(this, arguments);
};
function ni(e, t) {
  var n = {};
  for (var r in e) Object.prototype.hasOwnProperty.call(e, r) && t.indexOf(r) < 0 && (n[r] = e[r]);
  if (e != null && typeof Object.getOwnPropertySymbols == "function")
    for (var o = 0, r = Object.getOwnPropertySymbols(e); o < r.length; o++)
      t.indexOf(r[o]) < 0 && Object.prototype.propertyIsEnumerable.call(e, r[o]) && (n[r[o]] = e[r[o]]);
  return n;
}
function Sd(e, t, n) {
  if (n || arguments.length === 2) for (var r = 0, o = t.length, s; r < o; r++)
    (s || !(r in t)) && (s || (s = Array.prototype.slice.call(t, 0, r)), s[r] = t[r]);
  return e.concat(s || Array.prototype.slice.call(t));
}
var Bn = "right-scroll-bar-position", Tn = "width-before-scroll-bar", kd = "with-scroll-bars-hidden", Dd = "--removed-body-scroll-bar-size";
function Pr(e, t) {
  return typeof e == "function" ? e(t) : e && (e.current = t), e;
}
function Rd(e, t) {
  var n = de(function() {
    return {
      // value
      value: e,
      // last callback
      callback: t,
      // "memoized" public interface
      facade: {
        get current() {
          return n.value;
        },
        set current(r) {
          var o = n.value;
          o !== r && (n.value = r, n.callback(r, o));
        }
      }
    };
  })[0];
  return n.callback = t, n.facade;
}
var Pd = typeof window < "u" ? h.useLayoutEffect : h.useEffect, Us = /* @__PURE__ */ new WeakMap();
function Od(e, t) {
  var n = Rd(null, function(r) {
    return e.forEach(function(o) {
      return Pr(o, r);
    });
  });
  return Pd(function() {
    var r = Us.get(n);
    if (r) {
      var o = new Set(r), s = new Set(e), i = n.current;
      o.forEach(function(c) {
        s.has(c) || Pr(c, null);
      }), s.forEach(function(c) {
        o.has(c) || Pr(c, i);
      });
    }
    Us.set(n, e);
  }, [e]), n;
}
function zd(e) {
  return e;
}
function Bd(e, t) {
  t === void 0 && (t = zd);
  var n = [], r = !1, o = {
    read: function() {
      if (r)
        throw new Error("Sidecar: could not `read` from an `assigned` medium. `read` could be used only with `useMedium`.");
      return n.length ? n[n.length - 1] : e;
    },
    useMedium: function(s) {
      var i = t(s, r);
      return n.push(i), function() {
        n = n.filter(function(c) {
          return c !== i;
        });
      };
    },
    assignSyncMedium: function(s) {
      for (r = !0; n.length; ) {
        var i = n;
        n = [], i.forEach(s);
      }
      n = {
        push: function(c) {
          return s(c);
        },
        filter: function() {
          return n;
        }
      };
    },
    assignMedium: function(s) {
      r = !0;
      var i = [];
      if (n.length) {
        var c = n;
        n = [], c.forEach(s), i = n;
      }
      var u = function() {
        var f = i;
        i = [], f.forEach(s);
      }, l = function() {
        return Promise.resolve().then(u);
      };
      l(), n = {
        push: function(f) {
          i.push(f), l();
        },
        filter: function(f) {
          return i = i.filter(f), n;
        }
      };
    }
  };
  return o;
}
function Td(e) {
  e === void 0 && (e = {});
  var t = Bd(null);
  return t.options = De({ async: !0, ssr: !1 }, e), t;
}
var ri = function(e) {
  var t = e.sideCar, n = ni(e, ["sideCar"]);
  if (!t)
    throw new Error("Sidecar: please provide `sideCar` property to import the right car");
  var r = t.read();
  if (!r)
    throw new Error("Sidecar medium not found");
  return h.createElement(r, De({}, n));
};
ri.isSideCarExport = !0;
function Gd(e, t) {
  return e.useMedium(t), ri;
}
var oi = Td(), Or = function() {
}, dr = h.forwardRef(function(e, t) {
  var n = h.useRef(null), r = h.useState({
    onScrollCapture: Or,
    onWheelCapture: Or,
    onTouchMoveCapture: Or
  }), o = r[0], s = r[1], i = e.forwardProps, c = e.children, u = e.className, l = e.removeScrollBar, f = e.enabled, d = e.shards, m = e.sideCar, p = e.noRelative, x = e.noIsolation, g = e.inert, v = e.allowPinchZoom, b = e.as, A = b === void 0 ? "div" : b, w = e.gapMode, E = ni(e, ["forwardProps", "children", "className", "removeScrollBar", "enabled", "shards", "sideCar", "noRelative", "noIsolation", "inert", "allowPinchZoom", "as", "gapMode"]), C = m, P = Od([n, t]), D = De(De({}, E), o);
  return h.createElement(
    h.Fragment,
    null,
    f && h.createElement(C, { sideCar: oi, removeScrollBar: l, shards: d, noRelative: p, noIsolation: x, inert: g, setCallbacks: s, allowPinchZoom: !!v, lockRef: n, gapMode: w }),
    i ? h.cloneElement(h.Children.only(c), De(De({}, D), { ref: P })) : h.createElement(A, De({}, D, { className: u, ref: P }), c)
  );
});
dr.defaultProps = {
  enabled: !0,
  removeScrollBar: !0,
  inert: !1
};
dr.classNames = {
  fullWidth: Tn,
  zeroRight: Bn
};
var Yd = function() {
  if (typeof __webpack_nonce__ < "u")
    return __webpack_nonce__;
};
function Hd() {
  if (!document)
    return null;
  var e = document.createElement("style");
  e.type = "text/css";
  var t = Yd();
  return t && e.setAttribute("nonce", t), e;
}
function Fd(e, t) {
  e.styleSheet ? e.styleSheet.cssText = t : e.appendChild(document.createTextNode(t));
}
function Ld(e) {
  var t = document.head || document.getElementsByTagName("head")[0];
  t.appendChild(e);
}
var Qd = function() {
  var e = 0, t = null;
  return {
    add: function(n) {
      e == 0 && (t = Hd()) && (Fd(t, n), Ld(t)), e++;
    },
    remove: function() {
      e--, !e && t && (t.parentNode && t.parentNode.removeChild(t), t = null);
    }
  };
}, Zd = function() {
  var e = Qd();
  return function(t, n) {
    h.useEffect(function() {
      return e.add(t), function() {
        e.remove();
      };
    }, [t && n]);
  };
}, si = function() {
  var e = Zd(), t = function(n) {
    var r = n.styles, o = n.dynamic;
    return e(r, o), null;
  };
  return t;
}, Jd = {
  left: 0,
  top: 0,
  right: 0,
  gap: 0
}, zr = function(e) {
  return parseInt(e || "", 10) || 0;
}, Wd = function(e) {
  var t = window.getComputedStyle(document.body), n = t[e === "padding" ? "paddingLeft" : "marginLeft"], r = t[e === "padding" ? "paddingTop" : "marginTop"], o = t[e === "padding" ? "paddingRight" : "marginRight"];
  return [zr(n), zr(r), zr(o)];
}, Ud = function(e) {
  if (e === void 0 && (e = "margin"), typeof window > "u")
    return Jd;
  var t = Wd(e), n = document.documentElement.clientWidth, r = window.innerWidth;
  return {
    left: t[0],
    top: t[1],
    right: t[2],
    gap: Math.max(0, r - n + t[2] - t[0])
  };
}, Vd = si(), jt = "data-scroll-locked", qd = function(e, t, n, r) {
  var o = e.left, s = e.top, i = e.right, c = e.gap;
  return n === void 0 && (n = "margin"), `
  .`.concat(kd, ` {
   overflow: hidden `).concat(r, `;
   padding-right: `).concat(c, "px ").concat(r, `;
  }
  body[`).concat(jt, `] {
    overflow: hidden `).concat(r, `;
    overscroll-behavior: contain;
    `).concat([
    t && "position: relative ".concat(r, ";"),
    n === "margin" && `
    padding-left: `.concat(o, `px;
    padding-top: `).concat(s, `px;
    padding-right: `).concat(i, `px;
    margin-left:0;
    margin-top:0;
    margin-right: `).concat(c, "px ").concat(r, `;
    `),
    n === "padding" && "padding-right: ".concat(c, "px ").concat(r, ";")
  ].filter(Boolean).join(""), `
  }
  
  .`).concat(Bn, ` {
    right: `).concat(c, "px ").concat(r, `;
  }
  
  .`).concat(Tn, ` {
    margin-right: `).concat(c, "px ").concat(r, `;
  }
  
  .`).concat(Bn, " .").concat(Bn, ` {
    right: 0 `).concat(r, `;
  }
  
  .`).concat(Tn, " .").concat(Tn, ` {
    margin-right: 0 `).concat(r, `;
  }
  
  body[`).concat(jt, `] {
    `).concat(Dd, ": ").concat(c, `px;
  }
`);
}, Vs = function() {
  var e = parseInt(document.body.getAttribute(jt) || "0", 10);
  return isFinite(e) ? e : 0;
}, Xd = function() {
  h.useEffect(function() {
    return document.body.setAttribute(jt, (Vs() + 1).toString()), function() {
      var e = Vs() - 1;
      e <= 0 ? document.body.removeAttribute(jt) : document.body.setAttribute(jt, e.toString());
    };
  }, []);
}, Kd = function(e) {
  var t = e.noRelative, n = e.noImportant, r = e.gapMode, o = r === void 0 ? "margin" : r;
  Xd();
  var s = h.useMemo(function() {
    return Ud(o);
  }, [o]);
  return h.createElement(Vd, { styles: qd(s, !t, o, n ? "" : "!important") });
}, eo = !1;
if (typeof window < "u")
  try {
    var Cn = Object.defineProperty({}, "passive", {
      get: function() {
        return eo = !0, !0;
      }
    });
    window.addEventListener("test", Cn, Cn), window.removeEventListener("test", Cn, Cn);
  } catch {
    eo = !1;
  }
var wt = eo ? { passive: !1 } : !1, _d = function(e) {
  return e.tagName === "TEXTAREA";
}, ai = function(e, t) {
  if (!(e instanceof Element))
    return !1;
  var n = window.getComputedStyle(e);
  return (
    // not-not-scrollable
    n[t] !== "hidden" && // contains scroll inside self
    !(n.overflowY === n.overflowX && !_d(e) && n[t] === "visible")
  );
}, $d = function(e) {
  return ai(e, "overflowY");
}, ef = function(e) {
  return ai(e, "overflowX");
}, qs = function(e, t) {
  var n = t.ownerDocument, r = t;
  do {
    typeof ShadowRoot < "u" && r instanceof ShadowRoot && (r = r.host);
    var o = ii(e, r);
    if (o) {
      var s = ci(e, r), i = s[1], c = s[2];
      if (i > c)
        return !0;
    }
    r = r.parentNode;
  } while (r && r !== n.body);
  return !1;
}, tf = function(e) {
  var t = e.scrollTop, n = e.scrollHeight, r = e.clientHeight;
  return [
    t,
    n,
    r
  ];
}, nf = function(e) {
  var t = e.scrollLeft, n = e.scrollWidth, r = e.clientWidth;
  return [
    t,
    n,
    r
  ];
}, ii = function(e, t) {
  return e === "v" ? $d(t) : ef(t);
}, ci = function(e, t) {
  return e === "v" ? tf(t) : nf(t);
}, rf = function(e, t) {
  return e === "h" && t === "rtl" ? -1 : 1;
}, of = function(e, t, n, r, o) {
  var s = rf(e, window.getComputedStyle(t).direction), i = s * r, c = n.target, u = t.contains(c), l = !1, f = i > 0, d = 0, m = 0;
  do {
    if (!c)
      break;
    var p = ci(e, c), x = p[0], g = p[1], v = p[2], b = g - v - s * x;
    (x || b) && ii(e, c) && (d += b, m += x);
    var A = c.parentNode;
    c = A && A.nodeType === Node.DOCUMENT_FRAGMENT_NODE ? A.host : A;
  } while (
    // portaled content
    !u && c !== document.body || // self content
    u && (t.contains(c) || t === c)
  );
  return (f && Math.abs(d) < 1 || !f && Math.abs(m) < 1) && (l = !0), l;
}, Nn = function(e) {
  return "changedTouches" in e ? [e.changedTouches[0].clientX, e.changedTouches[0].clientY] : [0, 0];
}, Xs = function(e) {
  return [e.deltaX, e.deltaY];
}, Ks = function(e) {
  return e && "current" in e ? e.current : e;
}, sf = function(e, t) {
  return e[0] === t[0] && e[1] === t[1];
}, af = function(e) {
  return `
  .block-interactivity-`.concat(e, ` {pointer-events: none;}
  .allow-interactivity-`).concat(e, ` {pointer-events: all;}
`);
}, cf = 0, At = [];
function lf(e) {
  var t = h.useRef([]), n = h.useRef([0, 0]), r = h.useRef(), o = h.useState(cf++)[0], s = h.useState(si)[0], i = h.useRef(e);
  h.useEffect(function() {
    i.current = e;
  }, [e]), h.useEffect(function() {
    if (e.inert) {
      document.body.classList.add("block-interactivity-".concat(o));
      var g = Sd([e.lockRef.current], (e.shards || []).map(Ks), !0).filter(Boolean);
      return g.forEach(function(v) {
        return v.classList.add("allow-interactivity-".concat(o));
      }), function() {
        document.body.classList.remove("block-interactivity-".concat(o)), g.forEach(function(v) {
          return v.classList.remove("allow-interactivity-".concat(o));
        });
      };
    }
  }, [e.inert, e.lockRef.current, e.shards]);
  var c = h.useCallback(function(g, v) {
    if ("touches" in g && g.touches.length === 2 || g.type === "wheel" && g.ctrlKey)
      return !i.current.allowPinchZoom;
    var b = Nn(g), A = n.current, w = "deltaX" in g ? g.deltaX : A[0] - b[0], E = "deltaY" in g ? g.deltaY : A[1] - b[1], C, P = g.target, D = Math.abs(w) > Math.abs(E) ? "h" : "v";
    if ("touches" in g && D === "h" && P.type === "range")
      return !1;
    var M = window.getSelection(), k = M && M.anchorNode, Y = k ? k === P || k.contains(P) : !1;
    if (Y)
      return !1;
    var z = qs(D, P);
    if (!z)
      return !0;
    if (z ? C = D : (C = D === "v" ? "h" : "v", z = qs(D, P)), !z)
      return !1;
    if (!r.current && "changedTouches" in g && (w || E) && (r.current = C), !C)
      return !0;
    var Z = r.current || C;
    return of(Z, v, g, Z === "h" ? w : E);
  }, []), u = h.useCallback(function(g) {
    var v = g;
    if (!(!At.length || At[At.length - 1] !== s)) {
      var b = "deltaY" in v ? Xs(v) : Nn(v), A = t.current.filter(function(C) {
        return C.name === v.type && (C.target === v.target || v.target === C.shadowParent) && sf(C.delta, b);
      })[0];
      if (A && A.should) {
        v.cancelable && v.preventDefault();
        return;
      }
      if (!A) {
        var w = (i.current.shards || []).map(Ks).filter(Boolean).filter(function(C) {
          return C.contains(v.target);
        }), E = w.length > 0 ? c(v, w[0]) : !i.current.noIsolation;
        E && v.cancelable && v.preventDefault();
      }
    }
  }, []), l = h.useCallback(function(g, v, b, A) {
    var w = { name: g, delta: v, target: b, should: A, shadowParent: uf(b) };
    t.current.push(w), setTimeout(function() {
      t.current = t.current.filter(function(E) {
        return E !== w;
      });
    }, 1);
  }, []), f = h.useCallback(function(g) {
    n.current = Nn(g), r.current = void 0;
  }, []), d = h.useCallback(function(g) {
    l(g.type, Xs(g), g.target, c(g, e.lockRef.current));
  }, []), m = h.useCallback(function(g) {
    l(g.type, Nn(g), g.target, c(g, e.lockRef.current));
  }, []);
  h.useEffect(function() {
    return At.push(s), e.setCallbacks({
      onScrollCapture: d,
      onWheelCapture: d,
      onTouchMoveCapture: m
    }), document.addEventListener("wheel", u, wt), document.addEventListener("touchmove", u, wt), document.addEventListener("touchstart", f, wt), function() {
      At = At.filter(function(g) {
        return g !== s;
      }), document.removeEventListener("wheel", u, wt), document.removeEventListener("touchmove", u, wt), document.removeEventListener("touchstart", f, wt);
    };
  }, []);
  var p = e.removeScrollBar, x = e.inert;
  return h.createElement(
    h.Fragment,
    null,
    x ? h.createElement(s, { styles: af(o) }) : null,
    p ? h.createElement(Kd, { noRelative: e.noRelative, gapMode: e.gapMode }) : null
  );
}
function uf(e) {
  for (var t = null; e !== null; )
    e instanceof ShadowRoot && (t = e.host, e = e.host), e = e.parentNode;
  return t;
}
const df = Gd(oi, lf);
var fr = h.forwardRef(function(e, t) {
  return h.createElement(dr, De({}, e, { ref: t, sideCar: df }));
});
fr.classNames = dr.classNames;
var ff = function(e) {
  if (typeof document > "u")
    return null;
  var t = Array.isArray(e) ? e[0] : e;
  return t.ownerDocument.body;
}, yt = /* @__PURE__ */ new WeakMap(), In = /* @__PURE__ */ new WeakMap(), Sn = {}, Br = 0, li = function(e) {
  return e && (e.host || li(e.parentNode));
}, mf = function(e, t) {
  return t.map(function(n) {
    if (e.contains(n))
      return n;
    var r = li(n);
    return r && e.contains(r) ? r : (console.error("aria-hidden", n, "in not contained inside", e, ". Doing nothing"), null);
  }).filter(function(n) {
    return !!n;
  });
}, hf = function(e, t, n, r) {
  var o = mf(t, Array.isArray(e) ? e : [e]);
  Sn[n] || (Sn[n] = /* @__PURE__ */ new WeakMap());
  var s = Sn[n], i = [], c = /* @__PURE__ */ new Set(), u = new Set(o), l = function(d) {
    !d || c.has(d) || (c.add(d), l(d.parentNode));
  };
  o.forEach(l);
  var f = function(d) {
    !d || u.has(d) || Array.prototype.forEach.call(d.children, function(m) {
      if (c.has(m))
        f(m);
      else
        try {
          var p = m.getAttribute(r), x = p !== null && p !== "false", g = (yt.get(m) || 0) + 1, v = (s.get(m) || 0) + 1;
          yt.set(m, g), s.set(m, v), i.push(m), g === 1 && x && In.set(m, !0), v === 1 && m.setAttribute(n, "true"), x || m.setAttribute(r, "true");
        } catch (b) {
          console.error("aria-hidden: cannot operate on ", m, b);
        }
    });
  };
  return f(t), c.clear(), Br++, function() {
    i.forEach(function(d) {
      var m = yt.get(d) - 1, p = s.get(d) - 1;
      yt.set(d, m), s.set(d, p), m || (In.has(d) || d.removeAttribute(r), In.delete(d)), p || d.removeAttribute(n);
    }), Br--, Br || (yt = /* @__PURE__ */ new WeakMap(), yt = /* @__PURE__ */ new WeakMap(), In = /* @__PURE__ */ new WeakMap(), Sn = {});
  };
}, Co = function(e, t, n) {
  n === void 0 && (n = "data-aria-hidden");
  var r = Array.from(Array.isArray(e) ? e : [e]), o = ff(e);
  return o ? (r.push.apply(r, Array.from(o.querySelectorAll("[aria-live], script"))), hf(r, o, n, "aria-hidden")) : function() {
    return null;
  };
}, mr = "Dialog", [ui] = xt(mr), [pf, Se] = ui(mr), di = (e) => {
  const {
    __scopeDialog: t,
    children: n,
    open: r,
    defaultOpen: o,
    onOpenChange: s,
    modal: i = !0
  } = e, c = h.useRef(null), u = h.useRef(null), [l, f] = fn({
    prop: r,
    defaultProp: o ?? !1,
    onChange: s,
    caller: mr
  });
  return /* @__PURE__ */ a.jsx(
    pf,
    {
      scope: t,
      triggerRef: c,
      contentRef: u,
      contentId: Fe(),
      titleId: Fe(),
      descriptionId: Fe(),
      open: l,
      onOpenChange: f,
      onOpenToggle: h.useCallback(() => f((d) => !d), [f]),
      modal: i,
      children: n
    }
  );
};
di.displayName = mr;
var fi = "DialogTrigger", mi = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, ...r } = e, o = Se(fi, n), s = ce(t, o.triggerRef);
    return /* @__PURE__ */ a.jsx(
      re.button,
      {
        type: "button",
        "aria-haspopup": "dialog",
        "aria-expanded": o.open,
        "aria-controls": o.contentId,
        "data-state": So(o.open),
        ...r,
        ref: s,
        onClick: U(e.onClick, o.onOpenToggle)
      }
    );
  }
);
mi.displayName = fi;
var No = "DialogPortal", [gf, hi] = ui(No, {
  forceMount: void 0
}), pi = (e) => {
  const { __scopeDialog: t, forceMount: n, children: r, container: o } = e, s = Se(No, t);
  return /* @__PURE__ */ a.jsx(gf, { scope: t, forceMount: n, children: h.Children.map(r, (i) => /* @__PURE__ */ a.jsx(Te, { present: n || s.open, children: /* @__PURE__ */ a.jsx(ur, { asChild: !0, container: o, children: i }) })) });
};
pi.displayName = No;
var Qn = "DialogOverlay", gi = h.forwardRef(
  (e, t) => {
    const n = hi(Qn, e.__scopeDialog), { forceMount: r = n.forceMount, ...o } = e, s = Se(Qn, e.__scopeDialog);
    return s.modal ? /* @__PURE__ */ a.jsx(Te, { present: r || s.open, children: /* @__PURE__ */ a.jsx(vf, { ...o, ref: t }) }) : null;
  }
);
gi.displayName = Qn;
var xf = /* @__PURE__ */ ft("DialogOverlay.RemoveScroll"), vf = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, ...r } = e, o = Se(Qn, n);
    return (
      // Make sure `Content` is scrollable even when it doesn't live inside `RemoveScroll`
      // ie. when `Overlay` and `Content` are siblings
      /* @__PURE__ */ a.jsx(fr, { as: xf, allowPinchZoom: !0, shards: [o.contentRef], children: /* @__PURE__ */ a.jsx(
        re.div,
        {
          "data-state": So(o.open),
          ...r,
          ref: t,
          style: { pointerEvents: "auto", ...r.style }
        }
      ) })
    );
  }
), mt = "DialogContent", xi = h.forwardRef(
  (e, t) => {
    const n = hi(mt, e.__scopeDialog), { forceMount: r = n.forceMount, ...o } = e, s = Se(mt, e.__scopeDialog);
    return /* @__PURE__ */ a.jsx(Te, { present: r || s.open, children: s.modal ? /* @__PURE__ */ a.jsx(bf, { ...o, ref: t }) : /* @__PURE__ */ a.jsx(wf, { ...o, ref: t }) });
  }
);
xi.displayName = mt;
var bf = h.forwardRef(
  (e, t) => {
    const n = Se(mt, e.__scopeDialog), r = h.useRef(null), o = ce(t, n.contentRef, r);
    return h.useEffect(() => {
      const s = r.current;
      if (s) return Co(s);
    }, []), /* @__PURE__ */ a.jsx(
      vi,
      {
        ...e,
        ref: o,
        trapFocus: n.open,
        disableOutsidePointerEvents: !0,
        onCloseAutoFocus: U(e.onCloseAutoFocus, (s) => {
          s.preventDefault(), n.triggerRef.current?.focus();
        }),
        onPointerDownOutside: U(e.onPointerDownOutside, (s) => {
          const i = s.detail.originalEvent, c = i.button === 0 && i.ctrlKey === !0;
          (i.button === 2 || c) && s.preventDefault();
        }),
        onFocusOutside: U(
          e.onFocusOutside,
          (s) => s.preventDefault()
        )
      }
    );
  }
), wf = h.forwardRef(
  (e, t) => {
    const n = Se(mt, e.__scopeDialog), r = h.useRef(!1), o = h.useRef(!1);
    return /* @__PURE__ */ a.jsx(
      vi,
      {
        ...e,
        ref: t,
        trapFocus: !1,
        disableOutsidePointerEvents: !1,
        onCloseAutoFocus: (s) => {
          e.onCloseAutoFocus?.(s), s.defaultPrevented || (r.current || n.triggerRef.current?.focus(), s.preventDefault()), r.current = !1, o.current = !1;
        },
        onInteractOutside: (s) => {
          e.onInteractOutside?.(s), s.defaultPrevented || (r.current = !0, s.detail.originalEvent.type === "pointerdown" && (o.current = !0));
          const i = s.target;
          n.triggerRef.current?.contains(i) && s.preventDefault(), s.detail.originalEvent.type === "focusin" && o.current && s.preventDefault();
        }
      }
    );
  }
), vi = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, trapFocus: r, onOpenAutoFocus: o, onCloseAutoFocus: s, ...i } = e, c = Se(mt, n), u = h.useRef(null), l = ce(t, u);
    return jo(), /* @__PURE__ */ a.jsxs(a.Fragment, { children: [
      /* @__PURE__ */ a.jsx(
        lr,
        {
          asChild: !0,
          loop: !0,
          trapped: r,
          onMountAutoFocus: o,
          onUnmountAutoFocus: s,
          children: /* @__PURE__ */ a.jsx(
            cr,
            {
              role: "dialog",
              id: c.contentId,
              "aria-describedby": c.descriptionId,
              "aria-labelledby": c.titleId,
              "data-state": So(c.open),
              ...i,
              ref: l,
              onDismiss: () => c.onOpenChange(!1)
            }
          )
        }
      ),
      /* @__PURE__ */ a.jsxs(a.Fragment, { children: [
        /* @__PURE__ */ a.jsx(Af, { titleId: c.titleId }),
        /* @__PURE__ */ a.jsx(Ef, { contentRef: u, descriptionId: c.descriptionId })
      ] })
    ] });
  }
), Io = "DialogTitle", bi = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, ...r } = e, o = Se(Io, n);
    return /* @__PURE__ */ a.jsx(re.h2, { id: o.titleId, ...r, ref: t });
  }
);
bi.displayName = Io;
var wi = "DialogDescription", Ai = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, ...r } = e, o = Se(wi, n);
    return /* @__PURE__ */ a.jsx(re.p, { id: o.descriptionId, ...r, ref: t });
  }
);
Ai.displayName = wi;
var yi = "DialogClose", Ei = h.forwardRef(
  (e, t) => {
    const { __scopeDialog: n, ...r } = e, o = Se(yi, n);
    return /* @__PURE__ */ a.jsx(
      re.button,
      {
        type: "button",
        ...r,
        ref: t,
        onClick: U(e.onClick, () => o.onOpenChange(!1))
      }
    );
  }
);
Ei.displayName = yi;
function So(e) {
  return e ? "open" : "closed";
}
var Mi = "DialogTitleWarning", [$b, ji] = nd(Mi, {
  contentName: mt,
  titleName: Io,
  docsSlug: "dialog"
}), Af = ({ titleId: e }) => {
  const t = ji(Mi), n = `\`${t.contentName}\` requires a \`${t.titleName}\` for the component to be accessible for screen reader users.

If you want to hide the \`${t.titleName}\`, you can wrap it with our VisuallyHidden component.

For more information, see https://radix-ui.com/primitives/docs/components/${t.docsSlug}`;
  return h.useEffect(() => {
    e && (document.getElementById(e) || console.error(n));
  }, [n, e]), null;
}, yf = "DialogDescriptionWarning", Ef = ({ contentRef: e, descriptionId: t }) => {
  const r = `Warning: Missing \`Description\` or \`aria-describedby={undefined}\` for {${ji(yf).contentName}}.`;
  return h.useEffect(() => {
    const o = e.current?.getAttribute("aria-describedby");
    t && o && (document.getElementById(t) || console.warn(r));
  }, [r, e, t]), null;
}, Ci = di, Ni = mi, Ii = pi, Si = gi, ki = xi, Di = bi, Ri = Ai, mn = Ei;
function Mf(e) {
  const [t, n] = h.useState(void 0);
  return $e(() => {
    if (e) {
      n({ width: e.offsetWidth, height: e.offsetHeight });
      const r = new ResizeObserver((o) => {
        if (!Array.isArray(o) || !o.length)
          return;
        const s = o[0];
        let i, c;
        if ("borderBoxSize" in s) {
          const u = s.borderBoxSize, l = Array.isArray(u) ? u[0] : u;
          i = l.inlineSize, c = l.blockSize;
        } else
          i = e.offsetWidth, c = e.offsetHeight;
        n({ width: i, height: c });
      });
      return r.observe(e, { box: "border-box" }), () => r.unobserve(e);
    } else
      n(void 0);
  }, [e]), t;
}
const jf = ["top", "right", "bottom", "left"], et = Math.min, xe = Math.max, Zn = Math.round, kn = Math.floor, Oe = (e) => ({
  x: e,
  y: e
}), Cf = {
  left: "right",
  right: "left",
  bottom: "top",
  top: "bottom"
};
function to(e, t, n) {
  return xe(e, et(t, n));
}
function Le(e, t) {
  return typeof e == "function" ? e(t) : e;
}
function Qe(e) {
  return e.split("-")[0];
}
function Bt(e) {
  return e.split("-")[1];
}
function ko(e) {
  return e === "x" ? "y" : "x";
}
function Do(e) {
  return e === "y" ? "height" : "width";
}
function Pe(e) {
  const t = e[0];
  return t === "t" || t === "b" ? "y" : "x";
}
function Ro(e) {
  return ko(Pe(e));
}
function Nf(e, t, n) {
  n === void 0 && (n = !1);
  const r = Bt(e), o = Ro(e), s = Do(o);
  let i = o === "x" ? r === (n ? "end" : "start") ? "right" : "left" : r === "start" ? "bottom" : "top";
  return t.reference[s] > t.floating[s] && (i = Jn(i)), [i, Jn(i)];
}
function If(e) {
  const t = Jn(e);
  return [no(e), t, no(t)];
}
function no(e) {
  return e.includes("start") ? e.replace("start", "end") : e.replace("end", "start");
}
const _s = ["left", "right"], $s = ["right", "left"], Sf = ["top", "bottom"], kf = ["bottom", "top"];
function Df(e, t, n) {
  switch (e) {
    case "top":
    case "bottom":
      return n ? t ? $s : _s : t ? _s : $s;
    case "left":
    case "right":
      return t ? Sf : kf;
    default:
      return [];
  }
}
function Rf(e, t, n, r) {
  const o = Bt(e);
  let s = Df(Qe(e), n === "start", r);
  return o && (s = s.map((i) => i + "-" + o), t && (s = s.concat(s.map(no)))), s;
}
function Jn(e) {
  const t = Qe(e);
  return Cf[t] + e.slice(t.length);
}
function Pf(e) {
  return {
    top: 0,
    right: 0,
    bottom: 0,
    left: 0,
    ...e
  };
}
function Pi(e) {
  return typeof e != "number" ? Pf(e) : {
    top: e,
    right: e,
    bottom: e,
    left: e
  };
}
function Wn(e) {
  const {
    x: t,
    y: n,
    width: r,
    height: o
  } = e;
  return {
    width: r,
    height: o,
    top: n,
    left: t,
    right: t + r,
    bottom: n + o,
    x: t,
    y: n
  };
}
function ea(e, t, n) {
  let {
    reference: r,
    floating: o
  } = e;
  const s = Pe(t), i = Ro(t), c = Do(i), u = Qe(t), l = s === "y", f = r.x + r.width / 2 - o.width / 2, d = r.y + r.height / 2 - o.height / 2, m = r[c] / 2 - o[c] / 2;
  let p;
  switch (u) {
    case "top":
      p = {
        x: f,
        y: r.y - o.height
      };
      break;
    case "bottom":
      p = {
        x: f,
        y: r.y + r.height
      };
      break;
    case "right":
      p = {
        x: r.x + r.width,
        y: d
      };
      break;
    case "left":
      p = {
        x: r.x - o.width,
        y: d
      };
      break;
    default:
      p = {
        x: r.x,
        y: r.y
      };
  }
  switch (Bt(t)) {
    case "start":
      p[i] -= m * (n && l ? -1 : 1);
      break;
    case "end":
      p[i] += m * (n && l ? -1 : 1);
      break;
  }
  return p;
}
async function Of(e, t) {
  var n;
  t === void 0 && (t = {});
  const {
    x: r,
    y: o,
    platform: s,
    rects: i,
    elements: c,
    strategy: u
  } = e, {
    boundary: l = "clippingAncestors",
    rootBoundary: f = "viewport",
    elementContext: d = "floating",
    altBoundary: m = !1,
    padding: p = 0
  } = Le(t, e), x = Pi(p), v = c[m ? d === "floating" ? "reference" : "floating" : d], b = Wn(await s.getClippingRect({
    element: (n = await (s.isElement == null ? void 0 : s.isElement(v))) == null || n ? v : v.contextElement || await (s.getDocumentElement == null ? void 0 : s.getDocumentElement(c.floating)),
    boundary: l,
    rootBoundary: f,
    strategy: u
  })), A = d === "floating" ? {
    x: r,
    y: o,
    width: i.floating.width,
    height: i.floating.height
  } : i.reference, w = await (s.getOffsetParent == null ? void 0 : s.getOffsetParent(c.floating)), E = await (s.isElement == null ? void 0 : s.isElement(w)) ? await (s.getScale == null ? void 0 : s.getScale(w)) || {
    x: 1,
    y: 1
  } : {
    x: 1,
    y: 1
  }, C = Wn(s.convertOffsetParentRelativeRectToViewportRelativeRect ? await s.convertOffsetParentRelativeRectToViewportRelativeRect({
    elements: c,
    rect: A,
    offsetParent: w,
    strategy: u
  }) : A);
  return {
    top: (b.top - C.top + x.top) / E.y,
    bottom: (C.bottom - b.bottom + x.bottom) / E.y,
    left: (b.left - C.left + x.left) / E.x,
    right: (C.right - b.right + x.right) / E.x
  };
}
const zf = 50, Bf = async (e, t, n) => {
  const {
    placement: r = "bottom",
    strategy: o = "absolute",
    middleware: s = [],
    platform: i
  } = n, c = i.detectOverflow ? i : {
    ...i,
    detectOverflow: Of
  }, u = await (i.isRTL == null ? void 0 : i.isRTL(t));
  let l = await i.getElementRects({
    reference: e,
    floating: t,
    strategy: o
  }), {
    x: f,
    y: d
  } = ea(l, r, u), m = r, p = 0;
  const x = {};
  for (let g = 0; g < s.length; g++) {
    const v = s[g];
    if (!v)
      continue;
    const {
      name: b,
      fn: A
    } = v, {
      x: w,
      y: E,
      data: C,
      reset: P
    } = await A({
      x: f,
      y: d,
      initialPlacement: r,
      placement: m,
      strategy: o,
      middlewareData: x,
      rects: l,
      platform: c,
      elements: {
        reference: e,
        floating: t
      }
    });
    f = w ?? f, d = E ?? d, x[b] = {
      ...x[b],
      ...C
    }, P && p < zf && (p++, typeof P == "object" && (P.placement && (m = P.placement), P.rects && (l = P.rects === !0 ? await i.getElementRects({
      reference: e,
      floating: t,
      strategy: o
    }) : P.rects), {
      x: f,
      y: d
    } = ea(l, m, u)), g = -1);
  }
  return {
    x: f,
    y: d,
    placement: m,
    strategy: o,
    middlewareData: x
  };
}, Tf = (e) => ({
  name: "arrow",
  options: e,
  async fn(t) {
    const {
      x: n,
      y: r,
      placement: o,
      rects: s,
      platform: i,
      elements: c,
      middlewareData: u
    } = t, {
      element: l,
      padding: f = 0
    } = Le(e, t) || {};
    if (l == null)
      return {};
    const d = Pi(f), m = {
      x: n,
      y: r
    }, p = Ro(o), x = Do(p), g = await i.getDimensions(l), v = p === "y", b = v ? "top" : "left", A = v ? "bottom" : "right", w = v ? "clientHeight" : "clientWidth", E = s.reference[x] + s.reference[p] - m[p] - s.floating[x], C = m[p] - s.reference[p], P = await (i.getOffsetParent == null ? void 0 : i.getOffsetParent(l));
    let D = P ? P[w] : 0;
    (!D || !await (i.isElement == null ? void 0 : i.isElement(P))) && (D = c.floating[w] || s.floating[x]);
    const M = E / 2 - C / 2, k = D / 2 - g[x] / 2 - 1, Y = et(d[b], k), z = et(d[A], k), Z = Y, q = D - g[x] - z, T = D / 2 - g[x] / 2 + M, R = to(Z, T, q), j = !u.arrow && Bt(o) != null && T !== R && s.reference[x] / 2 - (T < Z ? Y : z) - g[x] / 2 < 0, O = j ? T < Z ? T - Z : T - q : 0;
    return {
      [p]: m[p] + O,
      data: {
        [p]: R,
        centerOffset: T - R - O,
        ...j && {
          alignmentOffset: O
        }
      },
      reset: j
    };
  }
}), Gf = function(e) {
  return e === void 0 && (e = {}), {
    name: "flip",
    options: e,
    async fn(t) {
      var n, r;
      const {
        placement: o,
        middlewareData: s,
        rects: i,
        initialPlacement: c,
        platform: u,
        elements: l
      } = t, {
        mainAxis: f = !0,
        crossAxis: d = !0,
        fallbackPlacements: m,
        fallbackStrategy: p = "bestFit",
        fallbackAxisSideDirection: x = "none",
        flipAlignment: g = !0,
        ...v
      } = Le(e, t);
      if ((n = s.arrow) != null && n.alignmentOffset)
        return {};
      const b = Qe(o), A = Pe(c), w = Qe(c) === c, E = await (u.isRTL == null ? void 0 : u.isRTL(l.floating)), C = m || (w || !g ? [Jn(c)] : If(c)), P = x !== "none";
      !m && P && C.push(...Rf(c, g, x, E));
      const D = [c, ...C], M = await u.detectOverflow(t, v), k = [];
      let Y = ((r = s.flip) == null ? void 0 : r.overflows) || [];
      if (f && k.push(M[b]), d) {
        const T = Nf(o, i, E);
        k.push(M[T[0]], M[T[1]]);
      }
      if (Y = [...Y, {
        placement: o,
        overflows: k
      }], !k.every((T) => T <= 0)) {
        var z, Z;
        const T = (((z = s.flip) == null ? void 0 : z.index) || 0) + 1, R = D[T];
        if (R && (!(d === "alignment" ? A !== Pe(R) : !1) || // We leave the current main axis only if every placement on that axis
        // overflows the main axis.
        Y.every((N) => Pe(N.placement) === A ? N.overflows[0] > 0 : !0)))
          return {
            data: {
              index: T,
              overflows: Y
            },
            reset: {
              placement: R
            }
          };
        let j = (Z = Y.filter((O) => O.overflows[0] <= 0).sort((O, N) => O.overflows[1] - N.overflows[1])[0]) == null ? void 0 : Z.placement;
        if (!j)
          switch (p) {
            case "bestFit": {
              var q;
              const O = (q = Y.filter((N) => {
                if (P) {
                  const S = Pe(N.placement);
                  return S === A || // Create a bias to the `y` side axis due to horizontal
                  // reading directions favoring greater width.
                  S === "y";
                }
                return !0;
              }).map((N) => [N.placement, N.overflows.filter((S) => S > 0).reduce((S, I) => S + I, 0)]).sort((N, S) => N[1] - S[1])[0]) == null ? void 0 : q[0];
              O && (j = O);
              break;
            }
            case "initialPlacement":
              j = c;
              break;
          }
        if (o !== j)
          return {
            reset: {
              placement: j
            }
          };
      }
      return {};
    }
  };
};
function ta(e, t) {
  return {
    top: e.top - t.height,
    right: e.right - t.width,
    bottom: e.bottom - t.height,
    left: e.left - t.width
  };
}
function na(e) {
  return jf.some((t) => e[t] >= 0);
}
const Yf = function(e) {
  return e === void 0 && (e = {}), {
    name: "hide",
    options: e,
    async fn(t) {
      const {
        rects: n,
        platform: r
      } = t, {
        strategy: o = "referenceHidden",
        ...s
      } = Le(e, t);
      switch (o) {
        case "referenceHidden": {
          const i = await r.detectOverflow(t, {
            ...s,
            elementContext: "reference"
          }), c = ta(i, n.reference);
          return {
            data: {
              referenceHiddenOffsets: c,
              referenceHidden: na(c)
            }
          };
        }
        case "escaped": {
          const i = await r.detectOverflow(t, {
            ...s,
            altBoundary: !0
          }), c = ta(i, n.floating);
          return {
            data: {
              escapedOffsets: c,
              escaped: na(c)
            }
          };
        }
        default:
          return {};
      }
    }
  };
}, Oi = /* @__PURE__ */ new Set(["left", "top"]);
async function Hf(e, t) {
  const {
    placement: n,
    platform: r,
    elements: o
  } = e, s = await (r.isRTL == null ? void 0 : r.isRTL(o.floating)), i = Qe(n), c = Bt(n), u = Pe(n) === "y", l = Oi.has(i) ? -1 : 1, f = s && u ? -1 : 1, d = Le(t, e);
  let {
    mainAxis: m,
    crossAxis: p,
    alignmentAxis: x
  } = typeof d == "number" ? {
    mainAxis: d,
    crossAxis: 0,
    alignmentAxis: null
  } : {
    mainAxis: d.mainAxis || 0,
    crossAxis: d.crossAxis || 0,
    alignmentAxis: d.alignmentAxis
  };
  return c && typeof x == "number" && (p = c === "end" ? x * -1 : x), u ? {
    x: p * f,
    y: m * l
  } : {
    x: m * l,
    y: p * f
  };
}
const Ff = function(e) {
  return e === void 0 && (e = 0), {
    name: "offset",
    options: e,
    async fn(t) {
      var n, r;
      const {
        x: o,
        y: s,
        placement: i,
        middlewareData: c
      } = t, u = await Hf(t, e);
      return i === ((n = c.offset) == null ? void 0 : n.placement) && (r = c.arrow) != null && r.alignmentOffset ? {} : {
        x: o + u.x,
        y: s + u.y,
        data: {
          ...u,
          placement: i
        }
      };
    }
  };
}, Lf = function(e) {
  return e === void 0 && (e = {}), {
    name: "shift",
    options: e,
    async fn(t) {
      const {
        x: n,
        y: r,
        placement: o,
        platform: s
      } = t, {
        mainAxis: i = !0,
        crossAxis: c = !1,
        limiter: u = {
          fn: (b) => {
            let {
              x: A,
              y: w
            } = b;
            return {
              x: A,
              y: w
            };
          }
        },
        ...l
      } = Le(e, t), f = {
        x: n,
        y: r
      }, d = await s.detectOverflow(t, l), m = Pe(Qe(o)), p = ko(m);
      let x = f[p], g = f[m];
      if (i) {
        const b = p === "y" ? "top" : "left", A = p === "y" ? "bottom" : "right", w = x + d[b], E = x - d[A];
        x = to(w, x, E);
      }
      if (c) {
        const b = m === "y" ? "top" : "left", A = m === "y" ? "bottom" : "right", w = g + d[b], E = g - d[A];
        g = to(w, g, E);
      }
      const v = u.fn({
        ...t,
        [p]: x,
        [m]: g
      });
      return {
        ...v,
        data: {
          x: v.x - n,
          y: v.y - r,
          enabled: {
            [p]: i,
            [m]: c
          }
        }
      };
    }
  };
}, Qf = function(e) {
  return e === void 0 && (e = {}), {
    options: e,
    fn(t) {
      const {
        x: n,
        y: r,
        placement: o,
        rects: s,
        middlewareData: i
      } = t, {
        offset: c = 0,
        mainAxis: u = !0,
        crossAxis: l = !0
      } = Le(e, t), f = {
        x: n,
        y: r
      }, d = Pe(o), m = ko(d);
      let p = f[m], x = f[d];
      const g = Le(c, t), v = typeof g == "number" ? {
        mainAxis: g,
        crossAxis: 0
      } : {
        mainAxis: 0,
        crossAxis: 0,
        ...g
      };
      if (u) {
        const w = m === "y" ? "height" : "width", E = s.reference[m] - s.floating[w] + v.mainAxis, C = s.reference[m] + s.reference[w] - v.mainAxis;
        p < E ? p = E : p > C && (p = C);
      }
      if (l) {
        var b, A;
        const w = m === "y" ? "width" : "height", E = Oi.has(Qe(o)), C = s.reference[d] - s.floating[w] + (E && ((b = i.offset) == null ? void 0 : b[d]) || 0) + (E ? 0 : v.crossAxis), P = s.reference[d] + s.reference[w] + (E ? 0 : ((A = i.offset) == null ? void 0 : A[d]) || 0) - (E ? v.crossAxis : 0);
        x < C ? x = C : x > P && (x = P);
      }
      return {
        [m]: p,
        [d]: x
      };
    }
  };
}, Zf = function(e) {
  return e === void 0 && (e = {}), {
    name: "size",
    options: e,
    async fn(t) {
      var n, r;
      const {
        placement: o,
        rects: s,
        platform: i,
        elements: c
      } = t, {
        apply: u = () => {
        },
        ...l
      } = Le(e, t), f = await i.detectOverflow(t, l), d = Qe(o), m = Bt(o), p = Pe(o) === "y", {
        width: x,
        height: g
      } = s.floating;
      let v, b;
      d === "top" || d === "bottom" ? (v = d, b = m === (await (i.isRTL == null ? void 0 : i.isRTL(c.floating)) ? "start" : "end") ? "left" : "right") : (b = d, v = m === "end" ? "top" : "bottom");
      const A = g - f.top - f.bottom, w = x - f.left - f.right, E = et(g - f[v], A), C = et(x - f[b], w), P = !t.middlewareData.shift;
      let D = E, M = C;
      if ((n = t.middlewareData.shift) != null && n.enabled.x && (M = w), (r = t.middlewareData.shift) != null && r.enabled.y && (D = A), P && !m) {
        const Y = xe(f.left, 0), z = xe(f.right, 0), Z = xe(f.top, 0), q = xe(f.bottom, 0);
        p ? M = x - 2 * (Y !== 0 || z !== 0 ? Y + z : xe(f.left, f.right)) : D = g - 2 * (Z !== 0 || q !== 0 ? Z + q : xe(f.top, f.bottom));
      }
      await u({
        ...t,
        availableWidth: M,
        availableHeight: D
      });
      const k = await i.getDimensions(c.floating);
      return x !== k.width || g !== k.height ? {
        reset: {
          rects: !0
        }
      } : {};
    }
  };
};
function hr() {
  return typeof window < "u";
}
function Tt(e) {
  return zi(e) ? (e.nodeName || "").toLowerCase() : "#document";
}
function be(e) {
  var t;
  return (e == null || (t = e.ownerDocument) == null ? void 0 : t.defaultView) || window;
}
function Ge(e) {
  var t;
  return (t = (zi(e) ? e.ownerDocument : e.document) || window.document) == null ? void 0 : t.documentElement;
}
function zi(e) {
  return hr() ? e instanceof Node || e instanceof be(e).Node : !1;
}
function Ce(e) {
  return hr() ? e instanceof Element || e instanceof be(e).Element : !1;
}
function Ze(e) {
  return hr() ? e instanceof HTMLElement || e instanceof be(e).HTMLElement : !1;
}
function ra(e) {
  return !hr() || typeof ShadowRoot > "u" ? !1 : e instanceof ShadowRoot || e instanceof be(e).ShadowRoot;
}
function hn(e) {
  const {
    overflow: t,
    overflowX: n,
    overflowY: r,
    display: o
  } = Ne(e);
  return /auto|scroll|overlay|hidden|clip/.test(t + r + n) && o !== "inline" && o !== "contents";
}
function Jf(e) {
  return /^(table|td|th)$/.test(Tt(e));
}
function pr(e) {
  try {
    if (e.matches(":popover-open"))
      return !0;
  } catch {
  }
  try {
    return e.matches(":modal");
  } catch {
    return !1;
  }
}
const Wf = /transform|translate|scale|rotate|perspective|filter/, Uf = /paint|layout|strict|content/, it = (e) => !!e && e !== "none";
let Tr;
function Po(e) {
  const t = Ce(e) ? Ne(e) : e;
  return it(t.transform) || it(t.translate) || it(t.scale) || it(t.rotate) || it(t.perspective) || !Oo() && (it(t.backdropFilter) || it(t.filter)) || Wf.test(t.willChange || "") || Uf.test(t.contain || "");
}
function Vf(e) {
  let t = tt(e);
  for (; Ze(t) && !Rt(t); ) {
    if (Po(t))
      return t;
    if (pr(t))
      return null;
    t = tt(t);
  }
  return null;
}
function Oo() {
  return Tr == null && (Tr = typeof CSS < "u" && CSS.supports && CSS.supports("-webkit-backdrop-filter", "none")), Tr;
}
function Rt(e) {
  return /^(html|body|#document)$/.test(Tt(e));
}
function Ne(e) {
  return be(e).getComputedStyle(e);
}
function gr(e) {
  return Ce(e) ? {
    scrollLeft: e.scrollLeft,
    scrollTop: e.scrollTop
  } : {
    scrollLeft: e.scrollX,
    scrollTop: e.scrollY
  };
}
function tt(e) {
  if (Tt(e) === "html")
    return e;
  const t = (
    // Step into the shadow DOM of the parent of a slotted node.
    e.assignedSlot || // DOM Element detected.
    e.parentNode || // ShadowRoot detected.
    ra(e) && e.host || // Fallback.
    Ge(e)
  );
  return ra(t) ? t.host : t;
}
function Bi(e) {
  const t = tt(e);
  return Rt(t) ? e.ownerDocument ? e.ownerDocument.body : e.body : Ze(t) && hn(t) ? t : Bi(t);
}
function _t(e, t, n) {
  var r;
  t === void 0 && (t = []), n === void 0 && (n = !0);
  const o = Bi(e), s = o === ((r = e.ownerDocument) == null ? void 0 : r.body), i = be(o);
  if (s) {
    const c = ro(i);
    return t.concat(i, i.visualViewport || [], hn(o) ? o : [], c && n ? _t(c) : []);
  } else
    return t.concat(o, _t(o, [], n));
}
function ro(e) {
  return e.parent && Object.getPrototypeOf(e.parent) ? e.frameElement : null;
}
function Ti(e) {
  const t = Ne(e);
  let n = parseFloat(t.width) || 0, r = parseFloat(t.height) || 0;
  const o = Ze(e), s = o ? e.offsetWidth : n, i = o ? e.offsetHeight : r, c = Zn(n) !== s || Zn(r) !== i;
  return c && (n = s, r = i), {
    width: n,
    height: r,
    $: c
  };
}
function zo(e) {
  return Ce(e) ? e : e.contextElement;
}
function Ct(e) {
  const t = zo(e);
  if (!Ze(t))
    return Oe(1);
  const n = t.getBoundingClientRect(), {
    width: r,
    height: o,
    $: s
  } = Ti(t);
  let i = (s ? Zn(n.width) : n.width) / r, c = (s ? Zn(n.height) : n.height) / o;
  return (!i || !Number.isFinite(i)) && (i = 1), (!c || !Number.isFinite(c)) && (c = 1), {
    x: i,
    y: c
  };
}
const qf = /* @__PURE__ */ Oe(0);
function Gi(e) {
  const t = be(e);
  return !Oo() || !t.visualViewport ? qf : {
    x: t.visualViewport.offsetLeft,
    y: t.visualViewport.offsetTop
  };
}
function Xf(e, t, n) {
  return t === void 0 && (t = !1), !n || t && n !== be(e) ? !1 : t;
}
function ht(e, t, n, r) {
  t === void 0 && (t = !1), n === void 0 && (n = !1);
  const o = e.getBoundingClientRect(), s = zo(e);
  let i = Oe(1);
  t && (r ? Ce(r) && (i = Ct(r)) : i = Ct(e));
  const c = Xf(s, n, r) ? Gi(s) : Oe(0);
  let u = (o.left + c.x) / i.x, l = (o.top + c.y) / i.y, f = o.width / i.x, d = o.height / i.y;
  if (s) {
    const m = be(s), p = r && Ce(r) ? be(r) : r;
    let x = m, g = ro(x);
    for (; g && r && p !== x; ) {
      const v = Ct(g), b = g.getBoundingClientRect(), A = Ne(g), w = b.left + (g.clientLeft + parseFloat(A.paddingLeft)) * v.x, E = b.top + (g.clientTop + parseFloat(A.paddingTop)) * v.y;
      u *= v.x, l *= v.y, f *= v.x, d *= v.y, u += w, l += E, x = be(g), g = ro(x);
    }
  }
  return Wn({
    width: f,
    height: d,
    x: u,
    y: l
  });
}
function xr(e, t) {
  const n = gr(e).scrollLeft;
  return t ? t.left + n : ht(Ge(e)).left + n;
}
function Yi(e, t) {
  const n = e.getBoundingClientRect(), r = n.left + t.scrollLeft - xr(e, n), o = n.top + t.scrollTop;
  return {
    x: r,
    y: o
  };
}
function Kf(e) {
  let {
    elements: t,
    rect: n,
    offsetParent: r,
    strategy: o
  } = e;
  const s = o === "fixed", i = Ge(r), c = t ? pr(t.floating) : !1;
  if (r === i || c && s)
    return n;
  let u = {
    scrollLeft: 0,
    scrollTop: 0
  }, l = Oe(1);
  const f = Oe(0), d = Ze(r);
  if ((d || !d && !s) && ((Tt(r) !== "body" || hn(i)) && (u = gr(r)), d)) {
    const p = ht(r);
    l = Ct(r), f.x = p.x + r.clientLeft, f.y = p.y + r.clientTop;
  }
  const m = i && !d && !s ? Yi(i, u) : Oe(0);
  return {
    width: n.width * l.x,
    height: n.height * l.y,
    x: n.x * l.x - u.scrollLeft * l.x + f.x + m.x,
    y: n.y * l.y - u.scrollTop * l.y + f.y + m.y
  };
}
function _f(e) {
  return Array.from(e.getClientRects());
}
function $f(e) {
  const t = Ge(e), n = gr(e), r = e.ownerDocument.body, o = xe(t.scrollWidth, t.clientWidth, r.scrollWidth, r.clientWidth), s = xe(t.scrollHeight, t.clientHeight, r.scrollHeight, r.clientHeight);
  let i = -n.scrollLeft + xr(e);
  const c = -n.scrollTop;
  return Ne(r).direction === "rtl" && (i += xe(t.clientWidth, r.clientWidth) - o), {
    width: o,
    height: s,
    x: i,
    y: c
  };
}
const oa = 25;
function em(e, t) {
  const n = be(e), r = Ge(e), o = n.visualViewport;
  let s = r.clientWidth, i = r.clientHeight, c = 0, u = 0;
  if (o) {
    s = o.width, i = o.height;
    const f = Oo();
    (!f || f && t === "fixed") && (c = o.offsetLeft, u = o.offsetTop);
  }
  const l = xr(r);
  if (l <= 0) {
    const f = r.ownerDocument, d = f.body, m = getComputedStyle(d), p = f.compatMode === "CSS1Compat" && parseFloat(m.marginLeft) + parseFloat(m.marginRight) || 0, x = Math.abs(r.clientWidth - d.clientWidth - p);
    x <= oa && (s -= x);
  } else l <= oa && (s += l);
  return {
    width: s,
    height: i,
    x: c,
    y: u
  };
}
function tm(e, t) {
  const n = ht(e, !0, t === "fixed"), r = n.top + e.clientTop, o = n.left + e.clientLeft, s = Ze(e) ? Ct(e) : Oe(1), i = e.clientWidth * s.x, c = e.clientHeight * s.y, u = o * s.x, l = r * s.y;
  return {
    width: i,
    height: c,
    x: u,
    y: l
  };
}
function sa(e, t, n) {
  let r;
  if (t === "viewport")
    r = em(e, n);
  else if (t === "document")
    r = $f(Ge(e));
  else if (Ce(t))
    r = tm(t, n);
  else {
    const o = Gi(e);
    r = {
      x: t.x - o.x,
      y: t.y - o.y,
      width: t.width,
      height: t.height
    };
  }
  return Wn(r);
}
function Hi(e, t) {
  const n = tt(e);
  return n === t || !Ce(n) || Rt(n) ? !1 : Ne(n).position === "fixed" || Hi(n, t);
}
function nm(e, t) {
  const n = t.get(e);
  if (n)
    return n;
  let r = _t(e, [], !1).filter((c) => Ce(c) && Tt(c) !== "body"), o = null;
  const s = Ne(e).position === "fixed";
  let i = s ? tt(e) : e;
  for (; Ce(i) && !Rt(i); ) {
    const c = Ne(i), u = Po(i);
    !u && c.position === "fixed" && (o = null), (s ? !u && !o : !u && c.position === "static" && !!o && (o.position === "absolute" || o.position === "fixed") || hn(i) && !u && Hi(e, i)) ? r = r.filter((f) => f !== i) : o = c, i = tt(i);
  }
  return t.set(e, r), r;
}
function rm(e) {
  let {
    element: t,
    boundary: n,
    rootBoundary: r,
    strategy: o
  } = e;
  const i = [...n === "clippingAncestors" ? pr(t) ? [] : nm(t, this._c) : [].concat(n), r], c = sa(t, i[0], o);
  let u = c.top, l = c.right, f = c.bottom, d = c.left;
  for (let m = 1; m < i.length; m++) {
    const p = sa(t, i[m], o);
    u = xe(p.top, u), l = et(p.right, l), f = et(p.bottom, f), d = xe(p.left, d);
  }
  return {
    width: l - d,
    height: f - u,
    x: d,
    y: u
  };
}
function om(e) {
  const {
    width: t,
    height: n
  } = Ti(e);
  return {
    width: t,
    height: n
  };
}
function sm(e, t, n) {
  const r = Ze(t), o = Ge(t), s = n === "fixed", i = ht(e, !0, s, t);
  let c = {
    scrollLeft: 0,
    scrollTop: 0
  };
  const u = Oe(0);
  function l() {
    u.x = xr(o);
  }
  if (r || !r && !s)
    if ((Tt(t) !== "body" || hn(o)) && (c = gr(t)), r) {
      const p = ht(t, !0, s, t);
      u.x = p.x + t.clientLeft, u.y = p.y + t.clientTop;
    } else o && l();
  s && !r && o && l();
  const f = o && !r && !s ? Yi(o, c) : Oe(0), d = i.left + c.scrollLeft - u.x - f.x, m = i.top + c.scrollTop - u.y - f.y;
  return {
    x: d,
    y: m,
    width: i.width,
    height: i.height
  };
}
function Gr(e) {
  return Ne(e).position === "static";
}
function aa(e, t) {
  if (!Ze(e) || Ne(e).position === "fixed")
    return null;
  if (t)
    return t(e);
  let n = e.offsetParent;
  return Ge(e) === n && (n = n.ownerDocument.body), n;
}
function Fi(e, t) {
  const n = be(e);
  if (pr(e))
    return n;
  if (!Ze(e)) {
    let o = tt(e);
    for (; o && !Rt(o); ) {
      if (Ce(o) && !Gr(o))
        return o;
      o = tt(o);
    }
    return n;
  }
  let r = aa(e, t);
  for (; r && Jf(r) && Gr(r); )
    r = aa(r, t);
  return r && Rt(r) && Gr(r) && !Po(r) ? n : r || Vf(e) || n;
}
const am = async function(e) {
  const t = this.getOffsetParent || Fi, n = this.getDimensions, r = await n(e.floating);
  return {
    reference: sm(e.reference, await t(e.floating), e.strategy),
    floating: {
      x: 0,
      y: 0,
      width: r.width,
      height: r.height
    }
  };
};
function im(e) {
  return Ne(e).direction === "rtl";
}
const cm = {
  convertOffsetParentRelativeRectToViewportRelativeRect: Kf,
  getDocumentElement: Ge,
  getClippingRect: rm,
  getOffsetParent: Fi,
  getElementRects: am,
  getClientRects: _f,
  getDimensions: om,
  getScale: Ct,
  isElement: Ce,
  isRTL: im
};
function Li(e, t) {
  return e.x === t.x && e.y === t.y && e.width === t.width && e.height === t.height;
}
function lm(e, t) {
  let n = null, r;
  const o = Ge(e);
  function s() {
    var c;
    clearTimeout(r), (c = n) == null || c.disconnect(), n = null;
  }
  function i(c, u) {
    c === void 0 && (c = !1), u === void 0 && (u = 1), s();
    const l = e.getBoundingClientRect(), {
      left: f,
      top: d,
      width: m,
      height: p
    } = l;
    if (c || t(), !m || !p)
      return;
    const x = kn(d), g = kn(o.clientWidth - (f + m)), v = kn(o.clientHeight - (d + p)), b = kn(f), w = {
      rootMargin: -x + "px " + -g + "px " + -v + "px " + -b + "px",
      threshold: xe(0, et(1, u)) || 1
    };
    let E = !0;
    function C(P) {
      const D = P[0].intersectionRatio;
      if (D !== u) {
        if (!E)
          return i();
        D ? i(!1, D) : r = setTimeout(() => {
          i(!1, 1e-7);
        }, 1e3);
      }
      D === 1 && !Li(l, e.getBoundingClientRect()) && i(), E = !1;
    }
    try {
      n = new IntersectionObserver(C, {
        ...w,
        // Handle <iframe>s
        root: o.ownerDocument
      });
    } catch {
      n = new IntersectionObserver(C, w);
    }
    n.observe(e);
  }
  return i(!0), s;
}
function um(e, t, n, r) {
  r === void 0 && (r = {});
  const {
    ancestorScroll: o = !0,
    ancestorResize: s = !0,
    elementResize: i = typeof ResizeObserver == "function",
    layoutShift: c = typeof IntersectionObserver == "function",
    animationFrame: u = !1
  } = r, l = zo(e), f = o || s ? [...l ? _t(l) : [], ...t ? _t(t) : []] : [];
  f.forEach((b) => {
    o && b.addEventListener("scroll", n, {
      passive: !0
    }), s && b.addEventListener("resize", n);
  });
  const d = l && c ? lm(l, n) : null;
  let m = -1, p = null;
  i && (p = new ResizeObserver((b) => {
    let [A] = b;
    A && A.target === l && p && t && (p.unobserve(t), cancelAnimationFrame(m), m = requestAnimationFrame(() => {
      var w;
      (w = p) == null || w.observe(t);
    })), n();
  }), l && !u && p.observe(l), t && p.observe(t));
  let x, g = u ? ht(e) : null;
  u && v();
  function v() {
    const b = ht(e);
    g && !Li(g, b) && n(), g = b, x = requestAnimationFrame(v);
  }
  return n(), () => {
    var b;
    f.forEach((A) => {
      o && A.removeEventListener("scroll", n), s && A.removeEventListener("resize", n);
    }), d?.(), (b = p) == null || b.disconnect(), p = null, u && cancelAnimationFrame(x);
  };
}
const dm = Ff, fm = Lf, mm = Gf, hm = Zf, pm = Yf, ia = Tf, gm = Qf, xm = (e, t, n) => {
  const r = /* @__PURE__ */ new Map(), o = {
    platform: cm,
    ...n
  }, s = {
    ...o.platform,
    _c: r
  };
  return Bf(e, t, {
    ...o,
    platform: s
  });
};
var vm = typeof document < "u", bm = function() {
}, Gn = vm ? Mo : bm;
function Un(e, t) {
  if (e === t)
    return !0;
  if (typeof e != typeof t)
    return !1;
  if (typeof e == "function" && e.toString() === t.toString())
    return !0;
  let n, r, o;
  if (e && t && typeof e == "object") {
    if (Array.isArray(e)) {
      if (n = e.length, n !== t.length) return !1;
      for (r = n; r-- !== 0; )
        if (!Un(e[r], t[r]))
          return !1;
      return !0;
    }
    if (o = Object.keys(e), n = o.length, n !== Object.keys(t).length)
      return !1;
    for (r = n; r-- !== 0; )
      if (!{}.hasOwnProperty.call(t, o[r]))
        return !1;
    for (r = n; r-- !== 0; ) {
      const s = o[r];
      if (!(s === "_owner" && e.$$typeof) && !Un(e[s], t[s]))
        return !1;
    }
    return !0;
  }
  return e !== e && t !== t;
}
function Qi(e) {
  return typeof window > "u" ? 1 : (e.ownerDocument.defaultView || window).devicePixelRatio || 1;
}
function ca(e, t) {
  const n = Qi(e);
  return Math.round(t * n) / n;
}
function Yr(e) {
  const t = h.useRef(e);
  return Gn(() => {
    t.current = e;
  }), t;
}
function wm(e) {
  e === void 0 && (e = {});
  const {
    placement: t = "bottom",
    strategy: n = "absolute",
    middleware: r = [],
    platform: o,
    elements: {
      reference: s,
      floating: i
    } = {},
    transform: c = !0,
    whileElementsMounted: u,
    open: l
  } = e, [f, d] = h.useState({
    x: 0,
    y: 0,
    strategy: n,
    placement: t,
    middlewareData: {},
    isPositioned: !1
  }), [m, p] = h.useState(r);
  Un(m, r) || p(r);
  const [x, g] = h.useState(null), [v, b] = h.useState(null), A = h.useCallback((N) => {
    N !== P.current && (P.current = N, g(N));
  }, []), w = h.useCallback((N) => {
    N !== D.current && (D.current = N, b(N));
  }, []), E = s || x, C = i || v, P = h.useRef(null), D = h.useRef(null), M = h.useRef(f), k = u != null, Y = Yr(u), z = Yr(o), Z = Yr(l), q = h.useCallback(() => {
    if (!P.current || !D.current)
      return;
    const N = {
      placement: t,
      strategy: n,
      middleware: m
    };
    z.current && (N.platform = z.current), xm(P.current, D.current, N).then((S) => {
      const I = {
        ...S,
        // The floating element's position may be recomputed while it's closed
        // but still mounted (such as when transitioning out). To ensure
        // `isPositioned` will be `false` initially on the next open, avoid
        // setting it to `true` when `open === false` (must be specified).
        isPositioned: Z.current !== !1
      };
      T.current && !Un(M.current, I) && (M.current = I, Wa.flushSync(() => {
        d(I);
      }));
    });
  }, [m, t, n, z, Z]);
  Gn(() => {
    l === !1 && M.current.isPositioned && (M.current.isPositioned = !1, d((N) => ({
      ...N,
      isPositioned: !1
    })));
  }, [l]);
  const T = h.useRef(!1);
  Gn(() => (T.current = !0, () => {
    T.current = !1;
  }), []), Gn(() => {
    if (E && (P.current = E), C && (D.current = C), E && C) {
      if (Y.current)
        return Y.current(E, C, q);
      q();
    }
  }, [E, C, q, Y, k]);
  const R = h.useMemo(() => ({
    reference: P,
    floating: D,
    setReference: A,
    setFloating: w
  }), [A, w]), j = h.useMemo(() => ({
    reference: E,
    floating: C
  }), [E, C]), O = h.useMemo(() => {
    const N = {
      position: n,
      left: 0,
      top: 0
    };
    if (!j.floating)
      return N;
    const S = ca(j.floating, f.x), I = ca(j.floating, f.y);
    return c ? {
      ...N,
      transform: "translate(" + S + "px, " + I + "px)",
      ...Qi(j.floating) >= 1.5 && {
        willChange: "transform"
      }
    } : {
      position: n,
      left: S,
      top: I
    };
  }, [n, c, j.floating, f.x, f.y]);
  return h.useMemo(() => ({
    ...f,
    update: q,
    refs: R,
    elements: j,
    floatingStyles: O
  }), [f, q, R, j, O]);
}
const Am = (e) => {
  function t(n) {
    return {}.hasOwnProperty.call(n, "current");
  }
  return {
    name: "arrow",
    options: e,
    fn(n) {
      const {
        element: r,
        padding: o
      } = typeof e == "function" ? e(n) : e;
      return r && t(r) ? r.current != null ? ia({
        element: r.current,
        padding: o
      }).fn(n) : {} : r ? ia({
        element: r,
        padding: o
      }).fn(n) : {};
    }
  };
}, ym = (e, t) => {
  const n = dm(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
}, Em = (e, t) => {
  const n = fm(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
}, Mm = (e, t) => ({
  fn: gm(e).fn,
  options: [e, t]
}), jm = (e, t) => {
  const n = mm(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
}, Cm = (e, t) => {
  const n = hm(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
}, Nm = (e, t) => {
  const n = pm(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
}, Im = (e, t) => {
  const n = Am(e);
  return {
    name: n.name,
    fn: n.fn,
    options: [e, t]
  };
};
var Sm = "Arrow", Zi = h.forwardRef((e, t) => {
  const { children: n, width: r = 10, height: o = 5, ...s } = e;
  return /* @__PURE__ */ a.jsx(
    re.svg,
    {
      ...s,
      ref: t,
      width: r,
      height: o,
      viewBox: "0 0 30 10",
      preserveAspectRatio: "none",
      children: e.asChild ? n : /* @__PURE__ */ a.jsx("polygon", { points: "0,0 30,0 15,10" })
    }
  );
});
Zi.displayName = Sm;
var km = Zi, Bo = "Popper", [Ji, vr] = xt(Bo), [Dm, Wi] = Ji(Bo), Ui = (e) => {
  const { __scopePopper: t, children: n } = e, [r, o] = h.useState(null);
  return /* @__PURE__ */ a.jsx(Dm, { scope: t, anchor: r, onAnchorChange: o, children: n });
};
Ui.displayName = Bo;
var Vi = "PopperAnchor", qi = h.forwardRef(
  (e, t) => {
    const { __scopePopper: n, virtualRef: r, ...o } = e, s = Wi(Vi, n), i = h.useRef(null), c = ce(t, i), u = h.useRef(null);
    return h.useEffect(() => {
      const l = u.current;
      u.current = r?.current || i.current, l !== u.current && s.onAnchorChange(u.current);
    }), r ? null : /* @__PURE__ */ a.jsx(re.div, { ...o, ref: c });
  }
);
qi.displayName = Vi;
var To = "PopperContent", [Rm, Pm] = Ji(To), Xi = h.forwardRef(
  (e, t) => {
    const {
      __scopePopper: n,
      side: r = "bottom",
      sideOffset: o = 0,
      align: s = "center",
      alignOffset: i = 0,
      arrowPadding: c = 0,
      avoidCollisions: u = !0,
      collisionBoundary: l = [],
      collisionPadding: f = 0,
      sticky: d = "partial",
      hideWhenDetached: m = !1,
      updatePositionStrategy: p = "optimized",
      onPlaced: x,
      ...g
    } = e, v = Wi(To, n), [b, A] = h.useState(null), w = ce(t, (J) => A(J)), [E, C] = h.useState(null), P = Mf(E), D = P?.width ?? 0, M = P?.height ?? 0, k = r + (s !== "center" ? "-" + s : ""), Y = typeof f == "number" ? f : { top: 0, right: 0, bottom: 0, left: 0, ...f }, z = Array.isArray(l) ? l : [l], Z = z.length > 0, q = {
      padding: Y,
      boundary: z.filter(zm),
      // with `strategy: 'fixed'`, this is the only way to get it to respect boundaries
      altBoundary: Z
    }, { refs: T, floatingStyles: R, placement: j, isPositioned: O, middlewareData: N } = wm({
      // default to `fixed` strategy so users don't have to pick and we also avoid focus scroll issues
      strategy: "fixed",
      placement: k,
      whileElementsMounted: (...J) => um(...J, {
        animationFrame: p === "always"
      }),
      elements: {
        reference: v.anchor
      },
      middleware: [
        ym({ mainAxis: o + M, alignmentAxis: i }),
        u && Em({
          mainAxis: !0,
          crossAxis: !1,
          limiter: d === "partial" ? Mm() : void 0,
          ...q
        }),
        u && jm({ ...q }),
        Cm({
          ...q,
          apply: ({ elements: J, rects: oe, availableWidth: ee, availableHeight: ne }) => {
            const { width: le, height: he } = oe.reference, fe = J.floating.style;
            fe.setProperty("--radix-popper-available-width", `${ee}px`), fe.setProperty("--radix-popper-available-height", `${ne}px`), fe.setProperty("--radix-popper-anchor-width", `${le}px`), fe.setProperty("--radix-popper-anchor-height", `${he}px`);
          }
        }),
        E && Im({ element: E, padding: c }),
        Bm({ arrowWidth: D, arrowHeight: M }),
        m && Nm({ strategy: "referenceHidden", ...q })
      ]
    }), [S, I] = $i(j), y = ze(x);
    $e(() => {
      O && y?.();
    }, [O, y]);
    const G = N.arrow?.x, Q = N.arrow?.y, W = N.arrow?.centerOffset !== 0, [X, _] = h.useState();
    return $e(() => {
      b && _(window.getComputedStyle(b).zIndex);
    }, [b]), /* @__PURE__ */ a.jsx(
      "div",
      {
        ref: T.setFloating,
        "data-radix-popper-content-wrapper": "",
        style: {
          ...R,
          transform: O ? R.transform : "translate(0, -200%)",
          // keep off the page when measuring
          minWidth: "max-content",
          zIndex: X,
          "--radix-popper-transform-origin": [
            N.transformOrigin?.x,
            N.transformOrigin?.y
          ].join(" "),
          // hide the content if using the hide middleware and should be hidden
          // set visibility to hidden and disable pointer events so the UI behaves
          // as if the PopperContent isn't there at all
          ...N.hide?.referenceHidden && {
            visibility: "hidden",
            pointerEvents: "none"
          }
        },
        dir: e.dir,
        children: /* @__PURE__ */ a.jsx(
          Rm,
          {
            scope: n,
            placedSide: S,
            onArrowChange: C,
            arrowX: G,
            arrowY: Q,
            shouldHideArrow: W,
            children: /* @__PURE__ */ a.jsx(
              re.div,
              {
                "data-side": S,
                "data-align": I,
                ...g,
                ref: w,
                style: {
                  ...g.style,
                  // if the PopperContent hasn't been placed yet (not all measurements done)
                  // we prevent animations so that users's animation don't kick in too early referring wrong sides
                  animation: O ? void 0 : "none"
                }
              }
            )
          }
        )
      }
    );
  }
);
Xi.displayName = To;
var Ki = "PopperArrow", Om = {
  top: "bottom",
  right: "left",
  bottom: "top",
  left: "right"
}, _i = h.forwardRef(function(t, n) {
  const { __scopePopper: r, ...o } = t, s = Pm(Ki, r), i = Om[s.placedSide];
  return (
    // we have to use an extra wrapper because `ResizeObserver` (used by `useSize`)
    // doesn't report size as we'd expect on SVG elements.
    // it reports their bounding box which is effectively the largest path inside the SVG.
    /* @__PURE__ */ a.jsx(
      "span",
      {
        ref: s.onArrowChange,
        style: {
          position: "absolute",
          left: s.arrowX,
          top: s.arrowY,
          [i]: 0,
          transformOrigin: {
            top: "",
            right: "0 0",
            bottom: "center 0",
            left: "100% 0"
          }[s.placedSide],
          transform: {
            top: "translateY(100%)",
            right: "translateY(50%) rotate(90deg) translateX(-50%)",
            bottom: "rotate(180deg)",
            left: "translateY(50%) rotate(-90deg) translateX(50%)"
          }[s.placedSide],
          visibility: s.shouldHideArrow ? "hidden" : void 0
        },
        children: /* @__PURE__ */ a.jsx(
          km,
          {
            ...o,
            ref: n,
            style: {
              ...o.style,
              // ensures the element can be measured correctly (mostly for if SVG)
              display: "block"
            }
          }
        )
      }
    )
  );
});
_i.displayName = Ki;
function zm(e) {
  return e !== null;
}
var Bm = (e) => ({
  name: "transformOrigin",
  options: e,
  fn(t) {
    const { placement: n, rects: r, middlewareData: o } = t, i = o.arrow?.centerOffset !== 0, c = i ? 0 : e.arrowWidth, u = i ? 0 : e.arrowHeight, [l, f] = $i(n), d = { start: "0%", center: "50%", end: "100%" }[f], m = (o.arrow?.x ?? 0) + c / 2, p = (o.arrow?.y ?? 0) + u / 2;
    let x = "", g = "";
    return l === "bottom" ? (x = i ? d : `${m}px`, g = `${-u}px`) : l === "top" ? (x = i ? d : `${m}px`, g = `${r.floating.height + u}px`) : l === "right" ? (x = `${-u}px`, g = i ? d : `${p}px`) : l === "left" && (x = `${r.floating.width + u}px`, g = i ? d : `${p}px`), { data: { x, y: g } };
  }
});
function $i(e) {
  const [t, n = "center"] = e.split("-");
  return [t, n];
}
var Go = Ui, Yo = qi, ec = Xi, tc = _i, Hr = "rovingFocusGroup.onEntryFocus", Tm = { bubbles: !1, cancelable: !0 }, pn = "RovingFocusGroup", [oo, nc, Gm] = Ka(pn), [Ym, rc] = xt(
  pn,
  [Gm]
), [Hm, Fm] = Ym(pn), oc = h.forwardRef(
  (e, t) => /* @__PURE__ */ a.jsx(oo.Provider, { scope: e.__scopeRovingFocusGroup, children: /* @__PURE__ */ a.jsx(oo.Slot, { scope: e.__scopeRovingFocusGroup, children: /* @__PURE__ */ a.jsx(Lm, { ...e, ref: t }) }) })
);
oc.displayName = pn;
var Lm = h.forwardRef((e, t) => {
  const {
    __scopeRovingFocusGroup: n,
    orientation: r,
    loop: o = !1,
    dir: s,
    currentTabStopId: i,
    defaultCurrentTabStopId: c,
    onCurrentTabStopIdChange: u,
    onEntryFocus: l,
    preventScrollOnEntryFocus: f = !1,
    ...d
  } = e, m = h.useRef(null), p = ce(t, m), x = _a(s), [g, v] = fn({
    prop: i,
    defaultProp: c ?? null,
    onChange: u,
    caller: pn
  }), [b, A] = h.useState(!1), w = ze(l), E = nc(n), C = h.useRef(!1), [P, D] = h.useState(0);
  return h.useEffect(() => {
    const M = m.current;
    if (M)
      return M.addEventListener(Hr, w), () => M.removeEventListener(Hr, w);
  }, [w]), /* @__PURE__ */ a.jsx(
    Hm,
    {
      scope: n,
      orientation: r,
      dir: x,
      loop: o,
      currentTabStopId: g,
      onItemFocus: h.useCallback(
        (M) => v(M),
        [v]
      ),
      onItemShiftTab: h.useCallback(() => A(!0), []),
      onFocusableItemAdd: h.useCallback(
        () => D((M) => M + 1),
        []
      ),
      onFocusableItemRemove: h.useCallback(
        () => D((M) => M - 1),
        []
      ),
      children: /* @__PURE__ */ a.jsx(
        re.div,
        {
          tabIndex: b || P === 0 ? -1 : 0,
          "data-orientation": r,
          ...d,
          ref: p,
          style: { outline: "none", ...e.style },
          onMouseDown: U(e.onMouseDown, () => {
            C.current = !0;
          }),
          onFocus: U(e.onFocus, (M) => {
            const k = !C.current;
            if (M.target === M.currentTarget && k && !b) {
              const Y = new CustomEvent(Hr, Tm);
              if (M.currentTarget.dispatchEvent(Y), !Y.defaultPrevented) {
                const z = E().filter((j) => j.focusable), Z = z.find((j) => j.active), q = z.find((j) => j.id === g), R = [Z, q, ...z].filter(
                  Boolean
                ).map((j) => j.ref.current);
                ic(R, f);
              }
            }
            C.current = !1;
          }),
          onBlur: U(e.onBlur, () => A(!1))
        }
      )
    }
  );
}), sc = "RovingFocusGroupItem", ac = h.forwardRef(
  (e, t) => {
    const {
      __scopeRovingFocusGroup: n,
      focusable: r = !0,
      active: o = !1,
      tabStopId: s,
      children: i,
      ...c
    } = e, u = Fe(), l = s || u, f = Fm(sc, n), d = f.currentTabStopId === l, m = nc(n), { onFocusableItemAdd: p, onFocusableItemRemove: x, currentTabStopId: g } = f;
    return h.useEffect(() => {
      if (r)
        return p(), () => x();
    }, [r, p, x]), /* @__PURE__ */ a.jsx(
      oo.ItemSlot,
      {
        scope: n,
        id: l,
        focusable: r,
        active: o,
        children: /* @__PURE__ */ a.jsx(
          re.span,
          {
            tabIndex: d ? 0 : -1,
            "data-orientation": f.orientation,
            ...c,
            ref: t,
            onMouseDown: U(e.onMouseDown, (v) => {
              r ? f.onItemFocus(l) : v.preventDefault();
            }),
            onFocus: U(e.onFocus, () => f.onItemFocus(l)),
            onKeyDown: U(e.onKeyDown, (v) => {
              if (v.key === "Tab" && v.shiftKey) {
                f.onItemShiftTab();
                return;
              }
              if (v.target !== v.currentTarget) return;
              const b = Jm(v, f.orientation, f.dir);
              if (b !== void 0) {
                if (v.metaKey || v.ctrlKey || v.altKey || v.shiftKey) return;
                v.preventDefault();
                let w = m().filter((E) => E.focusable).map((E) => E.ref.current);
                if (b === "last") w.reverse();
                else if (b === "prev" || b === "next") {
                  b === "prev" && w.reverse();
                  const E = w.indexOf(v.currentTarget);
                  w = f.loop ? Wm(w, E + 1) : w.slice(E + 1);
                }
                setTimeout(() => ic(w));
              }
            }),
            children: typeof i == "function" ? i({ isCurrentTabStop: d, hasTabStop: g != null }) : i
          }
        )
      }
    );
  }
);
ac.displayName = sc;
var Qm = {
  ArrowLeft: "prev",
  ArrowUp: "prev",
  ArrowRight: "next",
  ArrowDown: "next",
  PageUp: "first",
  Home: "first",
  PageDown: "last",
  End: "last"
};
function Zm(e, t) {
  return t !== "rtl" ? e : e === "ArrowLeft" ? "ArrowRight" : e === "ArrowRight" ? "ArrowLeft" : e;
}
function Jm(e, t, n) {
  const r = Zm(e.key, n);
  if (!(t === "vertical" && ["ArrowLeft", "ArrowRight"].includes(r)) && !(t === "horizontal" && ["ArrowUp", "ArrowDown"].includes(r)))
    return Qm[r];
}
function ic(e, t = !1) {
  const n = document.activeElement;
  for (const r of e)
    if (r === n || (r.focus({ preventScroll: t }), document.activeElement !== n)) return;
}
function Wm(e, t) {
  return e.map((n, r) => e[(t + r) % e.length]);
}
var Um = oc, Vm = ac, so = ["Enter", " "], qm = ["ArrowDown", "PageUp", "Home"], cc = ["ArrowUp", "PageDown", "End"], Xm = [...qm, ...cc], Km = {
  ltr: [...so, "ArrowRight"],
  rtl: [...so, "ArrowLeft"]
}, _m = {
  ltr: ["ArrowLeft"],
  rtl: ["ArrowRight"]
}, gn = "Menu", [$t, $m, eh] = Ka(gn), [vt, lc] = xt(gn, [
  eh,
  vr,
  rc
]), xn = vr(), uc = rc(), [dc, rt] = vt(gn), [th, vn] = vt(gn), fc = (e) => {
  const { __scopeMenu: t, open: n = !1, children: r, dir: o, onOpenChange: s, modal: i = !0 } = e, c = xn(t), [u, l] = h.useState(null), f = h.useRef(!1), d = ze(s), m = _a(o);
  return h.useEffect(() => {
    const p = () => {
      f.current = !0, document.addEventListener("pointerdown", x, { capture: !0, once: !0 }), document.addEventListener("pointermove", x, { capture: !0, once: !0 });
    }, x = () => f.current = !1;
    return document.addEventListener("keydown", p, { capture: !0 }), () => {
      document.removeEventListener("keydown", p, { capture: !0 }), document.removeEventListener("pointerdown", x, { capture: !0 }), document.removeEventListener("pointermove", x, { capture: !0 });
    };
  }, []), /* @__PURE__ */ a.jsx(Go, { ...c, children: /* @__PURE__ */ a.jsx(
    dc,
    {
      scope: t,
      open: n,
      onOpenChange: d,
      content: u,
      onContentChange: l,
      children: /* @__PURE__ */ a.jsx(
        th,
        {
          scope: t,
          onClose: h.useCallback(() => d(!1), [d]),
          isUsingKeyboardRef: f,
          dir: m,
          modal: i,
          children: r
        }
      )
    }
  ) });
};
fc.displayName = gn;
var nh = "MenuAnchor", Ho = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, ...r } = e, o = xn(n);
    return /* @__PURE__ */ a.jsx(Yo, { ...o, ...r, ref: t });
  }
);
Ho.displayName = nh;
var Fo = "MenuPortal", [rh, mc] = vt(Fo, {
  forceMount: void 0
}), hc = (e) => {
  const { __scopeMenu: t, forceMount: n, children: r, container: o } = e, s = rt(Fo, t);
  return /* @__PURE__ */ a.jsx(rh, { scope: t, forceMount: n, children: /* @__PURE__ */ a.jsx(Te, { present: n || s.open, children: /* @__PURE__ */ a.jsx(ur, { asChild: !0, container: o, children: r }) }) });
};
hc.displayName = Fo;
var ye = "MenuContent", [oh, Lo] = vt(ye), pc = h.forwardRef(
  (e, t) => {
    const n = mc(ye, e.__scopeMenu), { forceMount: r = n.forceMount, ...o } = e, s = rt(ye, e.__scopeMenu), i = vn(ye, e.__scopeMenu);
    return /* @__PURE__ */ a.jsx($t.Provider, { scope: e.__scopeMenu, children: /* @__PURE__ */ a.jsx(Te, { present: r || s.open, children: /* @__PURE__ */ a.jsx($t.Slot, { scope: e.__scopeMenu, children: i.modal ? /* @__PURE__ */ a.jsx(sh, { ...o, ref: t }) : /* @__PURE__ */ a.jsx(ah, { ...o, ref: t }) }) }) });
  }
), sh = h.forwardRef(
  (e, t) => {
    const n = rt(ye, e.__scopeMenu), r = h.useRef(null), o = ce(t, r);
    return h.useEffect(() => {
      const s = r.current;
      if (s) return Co(s);
    }, []), /* @__PURE__ */ a.jsx(
      Qo,
      {
        ...e,
        ref: o,
        trapFocus: n.open,
        disableOutsidePointerEvents: n.open,
        disableOutsideScroll: !0,
        onFocusOutside: U(
          e.onFocusOutside,
          (s) => s.preventDefault(),
          { checkForDefaultPrevented: !1 }
        ),
        onDismiss: () => n.onOpenChange(!1)
      }
    );
  }
), ah = h.forwardRef((e, t) => {
  const n = rt(ye, e.__scopeMenu);
  return /* @__PURE__ */ a.jsx(
    Qo,
    {
      ...e,
      ref: t,
      trapFocus: !1,
      disableOutsidePointerEvents: !1,
      disableOutsideScroll: !1,
      onDismiss: () => n.onOpenChange(!1)
    }
  );
}), ih = /* @__PURE__ */ ft("MenuContent.ScrollLock"), Qo = h.forwardRef(
  (e, t) => {
    const {
      __scopeMenu: n,
      loop: r = !1,
      trapFocus: o,
      onOpenAutoFocus: s,
      onCloseAutoFocus: i,
      disableOutsidePointerEvents: c,
      onEntryFocus: u,
      onEscapeKeyDown: l,
      onPointerDownOutside: f,
      onFocusOutside: d,
      onInteractOutside: m,
      onDismiss: p,
      disableOutsideScroll: x,
      ...g
    } = e, v = rt(ye, n), b = vn(ye, n), A = xn(n), w = uc(n), E = $m(n), [C, P] = h.useState(null), D = h.useRef(null), M = ce(t, D, v.onContentChange), k = h.useRef(0), Y = h.useRef(""), z = h.useRef(0), Z = h.useRef(null), q = h.useRef("right"), T = h.useRef(0), R = x ? fr : h.Fragment, j = x ? { as: ih, allowPinchZoom: !0 } : void 0, O = (S) => {
      const I = Y.current + S, y = E().filter((J) => !J.disabled), G = document.activeElement, Q = y.find((J) => J.ref.current === G)?.textValue, W = y.map((J) => J.textValue), X = bh(W, I, Q), _ = y.find((J) => J.textValue === X)?.ref.current;
      (function J(oe) {
        Y.current = oe, window.clearTimeout(k.current), oe !== "" && (k.current = window.setTimeout(() => J(""), 1e3));
      })(I), _ && setTimeout(() => _.focus());
    };
    h.useEffect(() => () => window.clearTimeout(k.current), []), jo();
    const N = h.useCallback((S) => q.current === Z.current?.side && Ah(S, Z.current?.area), []);
    return /* @__PURE__ */ a.jsx(
      oh,
      {
        scope: n,
        searchRef: Y,
        onItemEnter: h.useCallback(
          (S) => {
            N(S) && S.preventDefault();
          },
          [N]
        ),
        onItemLeave: h.useCallback(
          (S) => {
            N(S) || (D.current?.focus(), P(null));
          },
          [N]
        ),
        onTriggerLeave: h.useCallback(
          (S) => {
            N(S) && S.preventDefault();
          },
          [N]
        ),
        pointerGraceTimerRef: z,
        onPointerGraceIntentChange: h.useCallback((S) => {
          Z.current = S;
        }, []),
        children: /* @__PURE__ */ a.jsx(R, { ...j, children: /* @__PURE__ */ a.jsx(
          lr,
          {
            asChild: !0,
            trapped: o,
            onMountAutoFocus: U(s, (S) => {
              S.preventDefault(), D.current?.focus({ preventScroll: !0 });
            }),
            onUnmountAutoFocus: i,
            children: /* @__PURE__ */ a.jsx(
              cr,
              {
                asChild: !0,
                disableOutsidePointerEvents: c,
                onEscapeKeyDown: l,
                onPointerDownOutside: f,
                onFocusOutside: d,
                onInteractOutside: m,
                onDismiss: p,
                children: /* @__PURE__ */ a.jsx(
                  Um,
                  {
                    asChild: !0,
                    ...w,
                    dir: b.dir,
                    orientation: "vertical",
                    loop: r,
                    currentTabStopId: C,
                    onCurrentTabStopIdChange: P,
                    onEntryFocus: U(u, (S) => {
                      b.isUsingKeyboardRef.current || S.preventDefault();
                    }),
                    preventScrollOnEntryFocus: !0,
                    children: /* @__PURE__ */ a.jsx(
                      ec,
                      {
                        role: "menu",
                        "aria-orientation": "vertical",
                        "data-state": Rc(v.open),
                        "data-radix-menu-content": "",
                        dir: b.dir,
                        ...A,
                        ...g,
                        ref: M,
                        style: { outline: "none", ...g.style },
                        onKeyDown: U(g.onKeyDown, (S) => {
                          const y = S.target.closest("[data-radix-menu-content]") === S.currentTarget, G = S.ctrlKey || S.altKey || S.metaKey, Q = S.key.length === 1;
                          y && (S.key === "Tab" && S.preventDefault(), !G && Q && O(S.key));
                          const W = D.current;
                          if (S.target !== W || !Xm.includes(S.key)) return;
                          S.preventDefault();
                          const _ = E().filter((J) => !J.disabled).map((J) => J.ref.current);
                          cc.includes(S.key) && _.reverse(), xh(_);
                        }),
                        onBlur: U(e.onBlur, (S) => {
                          S.currentTarget.contains(S.target) || (window.clearTimeout(k.current), Y.current = "");
                        }),
                        onPointerMove: U(
                          e.onPointerMove,
                          en((S) => {
                            const I = S.target, y = T.current !== S.clientX;
                            if (S.currentTarget.contains(I) && y) {
                              const G = S.clientX > T.current ? "right" : "left";
                              q.current = G, T.current = S.clientX;
                            }
                          })
                        )
                      }
                    )
                  }
                )
              }
            )
          }
        ) })
      }
    );
  }
);
pc.displayName = ye;
var ch = "MenuGroup", Zo = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, ...r } = e;
    return /* @__PURE__ */ a.jsx(re.div, { role: "group", ...r, ref: t });
  }
);
Zo.displayName = ch;
var lh = "MenuLabel", gc = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, ...r } = e;
    return /* @__PURE__ */ a.jsx(re.div, { ...r, ref: t });
  }
);
gc.displayName = lh;
var Vn = "MenuItem", la = "menu.itemSelect", br = h.forwardRef(
  (e, t) => {
    const { disabled: n = !1, onSelect: r, ...o } = e, s = h.useRef(null), i = vn(Vn, e.__scopeMenu), c = Lo(Vn, e.__scopeMenu), u = ce(t, s), l = h.useRef(!1), f = () => {
      const d = s.current;
      if (!n && d) {
        const m = new CustomEvent(la, { bubbles: !0, cancelable: !0 });
        d.addEventListener(la, (p) => r?.(p), { once: !0 }), Xa(d, m), m.defaultPrevented ? l.current = !1 : i.onClose();
      }
    };
    return /* @__PURE__ */ a.jsx(
      xc,
      {
        ...o,
        ref: u,
        disabled: n,
        onClick: U(e.onClick, f),
        onPointerDown: (d) => {
          e.onPointerDown?.(d), l.current = !0;
        },
        onPointerUp: U(e.onPointerUp, (d) => {
          l.current || d.currentTarget?.click();
        }),
        onKeyDown: U(e.onKeyDown, (d) => {
          const m = c.searchRef.current !== "";
          n || m && d.key === " " || so.includes(d.key) && (d.currentTarget.click(), d.preventDefault());
        })
      }
    );
  }
);
br.displayName = Vn;
var xc = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, disabled: r = !1, textValue: o, ...s } = e, i = Lo(Vn, n), c = uc(n), u = h.useRef(null), l = ce(t, u), [f, d] = h.useState(!1), [m, p] = h.useState("");
    return h.useEffect(() => {
      const x = u.current;
      x && p((x.textContent ?? "").trim());
    }, [s.children]), /* @__PURE__ */ a.jsx(
      $t.ItemSlot,
      {
        scope: n,
        disabled: r,
        textValue: o ?? m,
        children: /* @__PURE__ */ a.jsx(Vm, { asChild: !0, ...c, focusable: !r, children: /* @__PURE__ */ a.jsx(
          re.div,
          {
            role: "menuitem",
            "data-highlighted": f ? "" : void 0,
            "aria-disabled": r || void 0,
            "data-disabled": r ? "" : void 0,
            ...s,
            ref: l,
            onPointerMove: U(
              e.onPointerMove,
              en((x) => {
                r ? i.onItemLeave(x) : (i.onItemEnter(x), x.defaultPrevented || x.currentTarget.focus({ preventScroll: !0 }));
              })
            ),
            onPointerLeave: U(
              e.onPointerLeave,
              en((x) => i.onItemLeave(x))
            ),
            onFocus: U(e.onFocus, () => d(!0)),
            onBlur: U(e.onBlur, () => d(!1))
          }
        ) })
      }
    );
  }
), uh = "MenuCheckboxItem", vc = h.forwardRef(
  (e, t) => {
    const { checked: n = !1, onCheckedChange: r, ...o } = e;
    return /* @__PURE__ */ a.jsx(Ec, { scope: e.__scopeMenu, checked: n, children: /* @__PURE__ */ a.jsx(
      br,
      {
        role: "menuitemcheckbox",
        "aria-checked": qn(n) ? "mixed" : n,
        ...o,
        ref: t,
        "data-state": Uo(n),
        onSelect: U(
          o.onSelect,
          () => r?.(qn(n) ? !0 : !n),
          { checkForDefaultPrevented: !1 }
        )
      }
    ) });
  }
);
vc.displayName = uh;
var bc = "MenuRadioGroup", [dh, fh] = vt(
  bc,
  { value: void 0, onValueChange: () => {
  } }
), wc = h.forwardRef(
  (e, t) => {
    const { value: n, onValueChange: r, ...o } = e, s = ze(r);
    return /* @__PURE__ */ a.jsx(dh, { scope: e.__scopeMenu, value: n, onValueChange: s, children: /* @__PURE__ */ a.jsx(Zo, { ...o, ref: t }) });
  }
);
wc.displayName = bc;
var Ac = "MenuRadioItem", yc = h.forwardRef(
  (e, t) => {
    const { value: n, ...r } = e, o = fh(Ac, e.__scopeMenu), s = n === o.value;
    return /* @__PURE__ */ a.jsx(Ec, { scope: e.__scopeMenu, checked: s, children: /* @__PURE__ */ a.jsx(
      br,
      {
        role: "menuitemradio",
        "aria-checked": s,
        ...r,
        ref: t,
        "data-state": Uo(s),
        onSelect: U(
          r.onSelect,
          () => o.onValueChange?.(n),
          { checkForDefaultPrevented: !1 }
        )
      }
    ) });
  }
);
yc.displayName = Ac;
var Jo = "MenuItemIndicator", [Ec, mh] = vt(
  Jo,
  { checked: !1 }
), Mc = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, forceMount: r, ...o } = e, s = mh(Jo, n);
    return /* @__PURE__ */ a.jsx(
      Te,
      {
        present: r || qn(s.checked) || s.checked === !0,
        children: /* @__PURE__ */ a.jsx(
          re.span,
          {
            ...o,
            ref: t,
            "data-state": Uo(s.checked)
          }
        )
      }
    );
  }
);
Mc.displayName = Jo;
var hh = "MenuSeparator", jc = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, ...r } = e;
    return /* @__PURE__ */ a.jsx(
      re.div,
      {
        role: "separator",
        "aria-orientation": "horizontal",
        ...r,
        ref: t
      }
    );
  }
);
jc.displayName = hh;
var ph = "MenuArrow", Cc = h.forwardRef(
  (e, t) => {
    const { __scopeMenu: n, ...r } = e, o = xn(n);
    return /* @__PURE__ */ a.jsx(tc, { ...o, ...r, ref: t });
  }
);
Cc.displayName = ph;
var Wo = "MenuSub", [gh, Nc] = vt(Wo), Ic = (e) => {
  const { __scopeMenu: t, children: n, open: r = !1, onOpenChange: o } = e, s = rt(Wo, t), i = xn(t), [c, u] = h.useState(null), [l, f] = h.useState(null), d = ze(o);
  return h.useEffect(() => (s.open === !1 && d(!1), () => d(!1)), [s.open, d]), /* @__PURE__ */ a.jsx(Go, { ...i, children: /* @__PURE__ */ a.jsx(
    dc,
    {
      scope: t,
      open: r,
      onOpenChange: d,
      content: l,
      onContentChange: f,
      children: /* @__PURE__ */ a.jsx(
        gh,
        {
          scope: t,
          contentId: Fe(),
          triggerId: Fe(),
          trigger: c,
          onTriggerChange: u,
          children: n
        }
      )
    }
  ) });
};
Ic.displayName = Wo;
var Wt = "MenuSubTrigger", Sc = h.forwardRef(
  (e, t) => {
    const n = rt(Wt, e.__scopeMenu), r = vn(Wt, e.__scopeMenu), o = Nc(Wt, e.__scopeMenu), s = Lo(Wt, e.__scopeMenu), i = h.useRef(null), { pointerGraceTimerRef: c, onPointerGraceIntentChange: u } = s, l = { __scopeMenu: e.__scopeMenu }, f = h.useCallback(() => {
      i.current && window.clearTimeout(i.current), i.current = null;
    }, []);
    return h.useEffect(() => f, [f]), h.useEffect(() => {
      const d = c.current;
      return () => {
        window.clearTimeout(d), u(null);
      };
    }, [c, u]), /* @__PURE__ */ a.jsx(Ho, { asChild: !0, ...l, children: /* @__PURE__ */ a.jsx(
      xc,
      {
        id: o.triggerId,
        "aria-haspopup": "menu",
        "aria-expanded": n.open,
        "aria-controls": o.contentId,
        "data-state": Rc(n.open),
        ...e,
        ref: ir(t, o.onTriggerChange),
        onClick: (d) => {
          e.onClick?.(d), !(e.disabled || d.defaultPrevented) && (d.currentTarget.focus(), n.open || n.onOpenChange(!0));
        },
        onPointerMove: U(
          e.onPointerMove,
          en((d) => {
            s.onItemEnter(d), !d.defaultPrevented && !e.disabled && !n.open && !i.current && (s.onPointerGraceIntentChange(null), i.current = window.setTimeout(() => {
              n.onOpenChange(!0), f();
            }, 100));
          })
        ),
        onPointerLeave: U(
          e.onPointerLeave,
          en((d) => {
            f();
            const m = n.content?.getBoundingClientRect();
            if (m) {
              const p = n.content?.dataset.side, x = p === "right", g = x ? -5 : 5, v = m[x ? "left" : "right"], b = m[x ? "right" : "left"];
              s.onPointerGraceIntentChange({
                area: [
                  // Apply a bleed on clientX to ensure that our exit point is
                  // consistently within polygon bounds
                  { x: d.clientX + g, y: d.clientY },
                  { x: v, y: m.top },
                  { x: b, y: m.top },
                  { x: b, y: m.bottom },
                  { x: v, y: m.bottom }
                ],
                side: p
              }), window.clearTimeout(c.current), c.current = window.setTimeout(
                () => s.onPointerGraceIntentChange(null),
                300
              );
            } else {
              if (s.onTriggerLeave(d), d.defaultPrevented) return;
              s.onPointerGraceIntentChange(null);
            }
          })
        ),
        onKeyDown: U(e.onKeyDown, (d) => {
          const m = s.searchRef.current !== "";
          e.disabled || m && d.key === " " || Km[r.dir].includes(d.key) && (n.onOpenChange(!0), n.content?.focus(), d.preventDefault());
        })
      }
    ) });
  }
);
Sc.displayName = Wt;
var kc = "MenuSubContent", Dc = h.forwardRef(
  (e, t) => {
    const n = mc(ye, e.__scopeMenu), { forceMount: r = n.forceMount, ...o } = e, s = rt(ye, e.__scopeMenu), i = vn(ye, e.__scopeMenu), c = Nc(kc, e.__scopeMenu), u = h.useRef(null), l = ce(t, u);
    return /* @__PURE__ */ a.jsx($t.Provider, { scope: e.__scopeMenu, children: /* @__PURE__ */ a.jsx(Te, { present: r || s.open, children: /* @__PURE__ */ a.jsx($t.Slot, { scope: e.__scopeMenu, children: /* @__PURE__ */ a.jsx(
      Qo,
      {
        id: c.contentId,
        "aria-labelledby": c.triggerId,
        ...o,
        ref: l,
        align: "start",
        side: i.dir === "rtl" ? "left" : "right",
        disableOutsidePointerEvents: !1,
        disableOutsideScroll: !1,
        trapFocus: !1,
        onOpenAutoFocus: (f) => {
          i.isUsingKeyboardRef.current && u.current?.focus(), f.preventDefault();
        },
        onCloseAutoFocus: (f) => f.preventDefault(),
        onFocusOutside: U(e.onFocusOutside, (f) => {
          f.target !== c.trigger && s.onOpenChange(!1);
        }),
        onEscapeKeyDown: U(e.onEscapeKeyDown, (f) => {
          i.onClose(), f.preventDefault();
        }),
        onKeyDown: U(e.onKeyDown, (f) => {
          const d = f.currentTarget.contains(f.target), m = _m[i.dir].includes(f.key);
          d && m && (s.onOpenChange(!1), c.trigger?.focus(), f.preventDefault());
        })
      }
    ) }) }) });
  }
);
Dc.displayName = kc;
function Rc(e) {
  return e ? "open" : "closed";
}
function qn(e) {
  return e === "indeterminate";
}
function Uo(e) {
  return qn(e) ? "indeterminate" : e ? "checked" : "unchecked";
}
function xh(e) {
  const t = document.activeElement;
  for (const n of e)
    if (n === t || (n.focus(), document.activeElement !== t)) return;
}
function vh(e, t) {
  return e.map((n, r) => e[(t + r) % e.length]);
}
function bh(e, t, n) {
  const o = t.length > 1 && Array.from(t).every((l) => l === t[0]) ? t[0] : t, s = n ? e.indexOf(n) : -1;
  let i = vh(e, Math.max(s, 0));
  o.length === 1 && (i = i.filter((l) => l !== n));
  const u = i.find(
    (l) => l.toLowerCase().startsWith(o.toLowerCase())
  );
  return u !== n ? u : void 0;
}
function wh(e, t) {
  const { x: n, y: r } = e;
  let o = !1;
  for (let s = 0, i = t.length - 1; s < t.length; i = s++) {
    const c = t[s], u = t[i], l = c.x, f = c.y, d = u.x, m = u.y;
    f > r != m > r && n < (d - l) * (r - f) / (m - f) + l && (o = !o);
  }
  return o;
}
function Ah(e, t) {
  if (!t) return !1;
  const n = { x: e.clientX, y: e.clientY };
  return wh(n, t);
}
function en(e) {
  return (t) => t.pointerType === "mouse" ? e(t) : void 0;
}
var yh = fc, Eh = Ho, Mh = hc, jh = pc, Ch = Zo, Nh = gc, Ih = br, Sh = vc, kh = wc, Dh = yc, Rh = Mc, Ph = jc, Oh = Cc, zh = Ic, Bh = Sc, Th = Dc, wr = "DropdownMenu", [Gh] = xt(
  wr,
  [lc]
), me = lc(), [Yh, Pc] = Gh(wr), Oc = (e) => {
  const {
    __scopeDropdownMenu: t,
    children: n,
    dir: r,
    open: o,
    defaultOpen: s,
    onOpenChange: i,
    modal: c = !0
  } = e, u = me(t), l = h.useRef(null), [f, d] = fn({
    prop: o,
    defaultProp: s ?? !1,
    onChange: i,
    caller: wr
  });
  return /* @__PURE__ */ a.jsx(
    Yh,
    {
      scope: t,
      triggerId: Fe(),
      triggerRef: l,
      contentId: Fe(),
      open: f,
      onOpenChange: d,
      onOpenToggle: h.useCallback(() => d((m) => !m), [d]),
      modal: c,
      children: /* @__PURE__ */ a.jsx(yh, { ...u, open: f, onOpenChange: d, dir: r, modal: c, children: n })
    }
  );
};
Oc.displayName = wr;
var zc = "DropdownMenuTrigger", Bc = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, disabled: r = !1, ...o } = e, s = Pc(zc, n), i = me(n);
    return /* @__PURE__ */ a.jsx(Eh, { asChild: !0, ...i, children: /* @__PURE__ */ a.jsx(
      re.button,
      {
        type: "button",
        id: s.triggerId,
        "aria-haspopup": "menu",
        "aria-expanded": s.open,
        "aria-controls": s.open ? s.contentId : void 0,
        "data-state": s.open ? "open" : "closed",
        "data-disabled": r ? "" : void 0,
        disabled: r,
        ...o,
        ref: ir(t, s.triggerRef),
        onPointerDown: U(e.onPointerDown, (c) => {
          !r && c.button === 0 && c.ctrlKey === !1 && (s.onOpenToggle(), s.open || c.preventDefault());
        }),
        onKeyDown: U(e.onKeyDown, (c) => {
          r || (["Enter", " "].includes(c.key) && s.onOpenToggle(), c.key === "ArrowDown" && s.onOpenChange(!0), ["Enter", " ", "ArrowDown"].includes(c.key) && c.preventDefault());
        })
      }
    ) });
  }
);
Bc.displayName = zc;
var Hh = "DropdownMenuPortal", Tc = (e) => {
  const { __scopeDropdownMenu: t, ...n } = e, r = me(t);
  return /* @__PURE__ */ a.jsx(Mh, { ...r, ...n });
};
Tc.displayName = Hh;
var Gc = "DropdownMenuContent", Yc = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, ...r } = e, o = Pc(Gc, n), s = me(n), i = h.useRef(!1);
    return /* @__PURE__ */ a.jsx(
      jh,
      {
        id: o.contentId,
        "aria-labelledby": o.triggerId,
        ...s,
        ...r,
        ref: t,
        onCloseAutoFocus: U(e.onCloseAutoFocus, (c) => {
          i.current || o.triggerRef.current?.focus(), i.current = !1, c.preventDefault();
        }),
        onInteractOutside: U(e.onInteractOutside, (c) => {
          const u = c.detail.originalEvent, l = u.button === 0 && u.ctrlKey === !0, f = u.button === 2 || l;
          (!o.modal || f) && (i.current = !0);
        }),
        style: {
          ...e.style,
          "--radix-dropdown-menu-content-transform-origin": "var(--radix-popper-transform-origin)",
          "--radix-dropdown-menu-content-available-width": "var(--radix-popper-available-width)",
          "--radix-dropdown-menu-content-available-height": "var(--radix-popper-available-height)",
          "--radix-dropdown-menu-trigger-width": "var(--radix-popper-anchor-width)",
          "--radix-dropdown-menu-trigger-height": "var(--radix-popper-anchor-height)"
        }
      }
    );
  }
);
Yc.displayName = Gc;
var Fh = "DropdownMenuGroup", Hc = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
    return /* @__PURE__ */ a.jsx(Ch, { ...o, ...r, ref: t });
  }
);
Hc.displayName = Fh;
var Lh = "DropdownMenuLabel", Fc = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
    return /* @__PURE__ */ a.jsx(Nh, { ...o, ...r, ref: t });
  }
);
Fc.displayName = Lh;
var Qh = "DropdownMenuItem", Lc = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
    return /* @__PURE__ */ a.jsx(Ih, { ...o, ...r, ref: t });
  }
);
Lc.displayName = Qh;
var Zh = "DropdownMenuCheckboxItem", Qc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(Sh, { ...o, ...r, ref: t });
});
Qc.displayName = Zh;
var Jh = "DropdownMenuRadioGroup", Zc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(kh, { ...o, ...r, ref: t });
});
Zc.displayName = Jh;
var Wh = "DropdownMenuRadioItem", Jc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(Dh, { ...o, ...r, ref: t });
});
Jc.displayName = Wh;
var Uh = "DropdownMenuItemIndicator", Wc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(Rh, { ...o, ...r, ref: t });
});
Wc.displayName = Uh;
var Vh = "DropdownMenuSeparator", Uc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(Ph, { ...o, ...r, ref: t });
});
Uc.displayName = Vh;
var qh = "DropdownMenuArrow", Xh = h.forwardRef(
  (e, t) => {
    const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
    return /* @__PURE__ */ a.jsx(Oh, { ...o, ...r, ref: t });
  }
);
Xh.displayName = qh;
var Kh = (e) => {
  const { __scopeDropdownMenu: t, children: n, open: r, onOpenChange: o, defaultOpen: s } = e, i = me(t), [c, u] = fn({
    prop: r,
    defaultProp: s ?? !1,
    onChange: o,
    caller: "DropdownMenuSub"
  });
  return /* @__PURE__ */ a.jsx(zh, { ...i, open: c, onOpenChange: u, children: n });
}, _h = "DropdownMenuSubTrigger", Vc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(Bh, { ...o, ...r, ref: t });
});
Vc.displayName = _h;
var $h = "DropdownMenuSubContent", qc = h.forwardRef((e, t) => {
  const { __scopeDropdownMenu: n, ...r } = e, o = me(n);
  return /* @__PURE__ */ a.jsx(
    Th,
    {
      ...o,
      ...r,
      ref: t,
      style: {
        ...e.style,
        "--radix-dropdown-menu-content-transform-origin": "var(--radix-popper-transform-origin)",
        "--radix-dropdown-menu-content-available-width": "var(--radix-popper-available-width)",
        "--radix-dropdown-menu-content-available-height": "var(--radix-popper-available-height)",
        "--radix-dropdown-menu-trigger-width": "var(--radix-popper-anchor-width)",
        "--radix-dropdown-menu-trigger-height": "var(--radix-popper-anchor-height)"
      }
    }
  );
});
qc.displayName = $h;
var ep = Oc, tp = Bc, Xc = Tc, np = Yc, rp = Hc, op = Fc, sp = Lc, ap = Qc, ip = Zc, cp = Jc, Kc = Wc, lp = Uc, up = Kh, dp = Vc, fp = qc, mp = "Label", _c = h.forwardRef((e, t) => /* @__PURE__ */ a.jsx(
  re.label,
  {
    ...e,
    ref: t,
    onMouseDown: (n) => {
      n.target.closest("button, input, select, textarea") || (e.onMouseDown?.(n), !n.defaultPrevented && n.detail > 1 && n.preventDefault());
    }
  }
));
_c.displayName = mp;
var hp = _c, Ar = "Popover", [$c] = xt(Ar, [
  vr
]), bn = vr(), [pp, ot] = $c(Ar), el = (e) => {
  const {
    __scopePopover: t,
    children: n,
    open: r,
    defaultOpen: o,
    onOpenChange: s,
    modal: i = !1
  } = e, c = bn(t), u = h.useRef(null), [l, f] = h.useState(!1), [d, m] = fn({
    prop: r,
    defaultProp: o ?? !1,
    onChange: s,
    caller: Ar
  });
  return /* @__PURE__ */ a.jsx(Go, { ...c, children: /* @__PURE__ */ a.jsx(
    pp,
    {
      scope: t,
      contentId: Fe(),
      triggerRef: u,
      open: d,
      onOpenChange: m,
      onOpenToggle: h.useCallback(() => m((p) => !p), [m]),
      hasCustomAnchor: l,
      onCustomAnchorAdd: h.useCallback(() => f(!0), []),
      onCustomAnchorRemove: h.useCallback(() => f(!1), []),
      modal: i,
      children: n
    }
  ) });
};
el.displayName = Ar;
var tl = "PopoverAnchor", gp = h.forwardRef(
  (e, t) => {
    const { __scopePopover: n, ...r } = e, o = ot(tl, n), s = bn(n), { onCustomAnchorAdd: i, onCustomAnchorRemove: c } = o;
    return h.useEffect(() => (i(), () => c()), [i, c]), /* @__PURE__ */ a.jsx(Yo, { ...s, ...r, ref: t });
  }
);
gp.displayName = tl;
var nl = "PopoverTrigger", rl = h.forwardRef(
  (e, t) => {
    const { __scopePopover: n, ...r } = e, o = ot(nl, n), s = bn(n), i = ce(t, o.triggerRef), c = /* @__PURE__ */ a.jsx(
      re.button,
      {
        type: "button",
        "aria-haspopup": "dialog",
        "aria-expanded": o.open,
        "aria-controls": o.contentId,
        "data-state": cl(o.open),
        ...r,
        ref: i,
        onClick: U(e.onClick, o.onOpenToggle)
      }
    );
    return o.hasCustomAnchor ? c : /* @__PURE__ */ a.jsx(Yo, { asChild: !0, ...s, children: c });
  }
);
rl.displayName = nl;
var Vo = "PopoverPortal", [xp, vp] = $c(Vo, {
  forceMount: void 0
}), ol = (e) => {
  const { __scopePopover: t, forceMount: n, children: r, container: o } = e, s = ot(Vo, t);
  return /* @__PURE__ */ a.jsx(xp, { scope: t, forceMount: n, children: /* @__PURE__ */ a.jsx(Te, { present: n || s.open, children: /* @__PURE__ */ a.jsx(ur, { asChild: !0, container: o, children: r }) }) });
};
ol.displayName = Vo;
var Pt = "PopoverContent", sl = h.forwardRef(
  (e, t) => {
    const n = vp(Pt, e.__scopePopover), { forceMount: r = n.forceMount, ...o } = e, s = ot(Pt, e.__scopePopover);
    return /* @__PURE__ */ a.jsx(Te, { present: r || s.open, children: s.modal ? /* @__PURE__ */ a.jsx(wp, { ...o, ref: t }) : /* @__PURE__ */ a.jsx(Ap, { ...o, ref: t }) });
  }
);
sl.displayName = Pt;
var bp = /* @__PURE__ */ ft("PopoverContent.RemoveScroll"), wp = h.forwardRef(
  (e, t) => {
    const n = ot(Pt, e.__scopePopover), r = h.useRef(null), o = ce(t, r), s = h.useRef(!1);
    return h.useEffect(() => {
      const i = r.current;
      if (i) return Co(i);
    }, []), /* @__PURE__ */ a.jsx(fr, { as: bp, allowPinchZoom: !0, children: /* @__PURE__ */ a.jsx(
      al,
      {
        ...e,
        ref: o,
        trapFocus: n.open,
        disableOutsidePointerEvents: !0,
        onCloseAutoFocus: U(e.onCloseAutoFocus, (i) => {
          i.preventDefault(), s.current || n.triggerRef.current?.focus();
        }),
        onPointerDownOutside: U(
          e.onPointerDownOutside,
          (i) => {
            const c = i.detail.originalEvent, u = c.button === 0 && c.ctrlKey === !0, l = c.button === 2 || u;
            s.current = l;
          },
          { checkForDefaultPrevented: !1 }
        ),
        onFocusOutside: U(
          e.onFocusOutside,
          (i) => i.preventDefault(),
          { checkForDefaultPrevented: !1 }
        )
      }
    ) });
  }
), Ap = h.forwardRef(
  (e, t) => {
    const n = ot(Pt, e.__scopePopover), r = h.useRef(!1), o = h.useRef(!1);
    return /* @__PURE__ */ a.jsx(
      al,
      {
        ...e,
        ref: t,
        trapFocus: !1,
        disableOutsidePointerEvents: !1,
        onCloseAutoFocus: (s) => {
          e.onCloseAutoFocus?.(s), s.defaultPrevented || (r.current || n.triggerRef.current?.focus(), s.preventDefault()), r.current = !1, o.current = !1;
        },
        onInteractOutside: (s) => {
          e.onInteractOutside?.(s), s.defaultPrevented || (r.current = !0, s.detail.originalEvent.type === "pointerdown" && (o.current = !0));
          const i = s.target;
          n.triggerRef.current?.contains(i) && s.preventDefault(), s.detail.originalEvent.type === "focusin" && o.current && s.preventDefault();
        }
      }
    );
  }
), al = h.forwardRef(
  (e, t) => {
    const {
      __scopePopover: n,
      trapFocus: r,
      onOpenAutoFocus: o,
      onCloseAutoFocus: s,
      disableOutsidePointerEvents: i,
      onEscapeKeyDown: c,
      onPointerDownOutside: u,
      onFocusOutside: l,
      onInteractOutside: f,
      ...d
    } = e, m = ot(Pt, n), p = bn(n);
    return jo(), /* @__PURE__ */ a.jsx(
      lr,
      {
        asChild: !0,
        loop: !0,
        trapped: r,
        onMountAutoFocus: o,
        onUnmountAutoFocus: s,
        children: /* @__PURE__ */ a.jsx(
          cr,
          {
            asChild: !0,
            disableOutsidePointerEvents: i,
            onInteractOutside: f,
            onEscapeKeyDown: c,
            onPointerDownOutside: u,
            onFocusOutside: l,
            onDismiss: () => m.onOpenChange(!1),
            children: /* @__PURE__ */ a.jsx(
              ec,
              {
                "data-state": cl(m.open),
                role: "dialog",
                id: m.contentId,
                ...p,
                ...d,
                ref: t,
                style: {
                  ...d.style,
                  "--radix-popover-content-transform-origin": "var(--radix-popper-transform-origin)",
                  "--radix-popover-content-available-width": "var(--radix-popper-available-width)",
                  "--radix-popover-content-available-height": "var(--radix-popper-available-height)",
                  "--radix-popover-trigger-width": "var(--radix-popper-anchor-width)",
                  "--radix-popover-trigger-height": "var(--radix-popper-anchor-height)"
                }
              }
            )
          }
        )
      }
    );
  }
), il = "PopoverClose", yp = h.forwardRef(
  (e, t) => {
    const { __scopePopover: n, ...r } = e, o = ot(il, n);
    return /* @__PURE__ */ a.jsx(
      re.button,
      {
        type: "button",
        ...r,
        ref: t,
        onClick: U(e.onClick, () => o.onOpenChange(!1))
      }
    );
  }
);
yp.displayName = il;
var Ep = "PopoverArrow", Mp = h.forwardRef(
  (e, t) => {
    const { __scopePopover: n, ...r } = e, o = bn(n);
    return /* @__PURE__ */ a.jsx(tc, { ...o, ...r, ref: t });
  }
);
Mp.displayName = Ep;
function cl(e) {
  return e ? "open" : "closed";
}
var qo = el, Xo = rl, Ko = ol, _o = sl, jp = "Separator", ua = "horizontal", Cp = ["horizontal", "vertical"], ll = h.forwardRef((e, t) => {
  const { decorative: n, orientation: r = ua, ...o } = e, s = Np(r) ? r : ua, c = n ? { role: "none" } : { "aria-orientation": s === "vertical" ? s : void 0, role: "separator" };
  return /* @__PURE__ */ a.jsx(
    re.div,
    {
      "data-orientation": s,
      ...c,
      ...o,
      ref: t
    }
  );
});
ll.displayName = jp;
function Np(e) {
  return Cp.includes(e);
}
var Ip = ll;
const Sp = (e, t) => {
  const n = new Array(e.length + t.length);
  for (let r = 0; r < e.length; r++)
    n[r] = e[r];
  for (let r = 0; r < t.length; r++)
    n[e.length + r] = t[r];
  return n;
}, kp = (e, t) => ({
  classGroupId: e,
  validator: t
}), ul = (e = /* @__PURE__ */ new Map(), t = null, n) => ({
  nextPart: e,
  validators: t,
  classGroupId: n
}), Xn = "-", da = [], Dp = "arbitrary..", Rp = (e) => {
  const t = Op(e), {
    conflictingClassGroups: n,
    conflictingClassGroupModifiers: r
  } = e;
  return {
    getClassGroupId: (i) => {
      if (i.startsWith("[") && i.endsWith("]"))
        return Pp(i);
      const c = i.split(Xn), u = c[0] === "" && c.length > 1 ? 1 : 0;
      return dl(c, u, t);
    },
    getConflictingClassGroupIds: (i, c) => {
      if (c) {
        const u = r[i], l = n[i];
        return u ? l ? Sp(l, u) : u : l || da;
      }
      return n[i] || da;
    }
  };
}, dl = (e, t, n) => {
  if (e.length - t === 0)
    return n.classGroupId;
  const o = e[t], s = n.nextPart.get(o);
  if (s) {
    const l = dl(e, t + 1, s);
    if (l) return l;
  }
  const i = n.validators;
  if (i === null)
    return;
  const c = t === 0 ? e.join(Xn) : e.slice(t).join(Xn), u = i.length;
  for (let l = 0; l < u; l++) {
    const f = i[l];
    if (f.validator(c))
      return f.classGroupId;
  }
}, Pp = (e) => e.slice(1, -1).indexOf(":") === -1 ? void 0 : (() => {
  const t = e.slice(1, -1), n = t.indexOf(":"), r = t.slice(0, n);
  return r ? Dp + r : void 0;
})(), Op = (e) => {
  const {
    theme: t,
    classGroups: n
  } = e;
  return zp(n, t);
}, zp = (e, t) => {
  const n = ul();
  for (const r in e) {
    const o = e[r];
    $o(o, n, r, t);
  }
  return n;
}, $o = (e, t, n, r) => {
  const o = e.length;
  for (let s = 0; s < o; s++) {
    const i = e[s];
    Bp(i, t, n, r);
  }
}, Bp = (e, t, n, r) => {
  if (typeof e == "string") {
    Tp(e, t, n);
    return;
  }
  if (typeof e == "function") {
    Gp(e, t, n, r);
    return;
  }
  Yp(e, t, n, r);
}, Tp = (e, t, n) => {
  const r = e === "" ? t : fl(t, e);
  r.classGroupId = n;
}, Gp = (e, t, n, r) => {
  if (Hp(e)) {
    $o(e(r), t, n, r);
    return;
  }
  t.validators === null && (t.validators = []), t.validators.push(kp(n, e));
}, Yp = (e, t, n, r) => {
  const o = Object.entries(e), s = o.length;
  for (let i = 0; i < s; i++) {
    const [c, u] = o[i];
    $o(u, fl(t, c), n, r);
  }
}, fl = (e, t) => {
  let n = e;
  const r = t.split(Xn), o = r.length;
  for (let s = 0; s < o; s++) {
    const i = r[s];
    let c = n.nextPart.get(i);
    c || (c = ul(), n.nextPart.set(i, c)), n = c;
  }
  return n;
}, Hp = (e) => "isThemeGetter" in e && e.isThemeGetter === !0, Fp = (e) => {
  if (e < 1)
    return {
      get: () => {
      },
      set: () => {
      }
    };
  let t = 0, n = /* @__PURE__ */ Object.create(null), r = /* @__PURE__ */ Object.create(null);
  const o = (s, i) => {
    n[s] = i, t++, t > e && (t = 0, r = n, n = /* @__PURE__ */ Object.create(null));
  };
  return {
    get(s) {
      let i = n[s];
      if (i !== void 0)
        return i;
      if ((i = r[s]) !== void 0)
        return o(s, i), i;
    },
    set(s, i) {
      s in n ? n[s] = i : o(s, i);
    }
  };
}, ao = "!", fa = ":", Lp = [], ma = (e, t, n, r, o) => ({
  modifiers: e,
  hasImportantModifier: t,
  baseClassName: n,
  maybePostfixModifierPosition: r,
  isExternal: o
}), Qp = (e) => {
  const {
    prefix: t,
    experimentalParseClassName: n
  } = e;
  let r = (o) => {
    const s = [];
    let i = 0, c = 0, u = 0, l;
    const f = o.length;
    for (let g = 0; g < f; g++) {
      const v = o[g];
      if (i === 0 && c === 0) {
        if (v === fa) {
          s.push(o.slice(u, g)), u = g + 1;
          continue;
        }
        if (v === "/") {
          l = g;
          continue;
        }
      }
      v === "[" ? i++ : v === "]" ? i-- : v === "(" ? c++ : v === ")" && c--;
    }
    const d = s.length === 0 ? o : o.slice(u);
    let m = d, p = !1;
    d.endsWith(ao) ? (m = d.slice(0, -1), p = !0) : (
      /**
       * In Tailwind CSS v3 the important modifier was at the start of the base class name. This is still supported for legacy reasons.
       * @see https://github.com/dcastil/tailwind-merge/issues/513#issuecomment-2614029864
       */
      d.startsWith(ao) && (m = d.slice(1), p = !0)
    );
    const x = l && l > u ? l - u : void 0;
    return ma(s, p, m, x);
  };
  if (t) {
    const o = t + fa, s = r;
    r = (i) => i.startsWith(o) ? s(i.slice(o.length)) : ma(Lp, !1, i, void 0, !0);
  }
  if (n) {
    const o = r;
    r = (s) => n({
      className: s,
      parseClassName: o
    });
  }
  return r;
}, Zp = (e) => {
  const t = /* @__PURE__ */ new Map();
  return e.orderSensitiveModifiers.forEach((n, r) => {
    t.set(n, 1e6 + r);
  }), (n) => {
    const r = [];
    let o = [];
    for (let s = 0; s < n.length; s++) {
      const i = n[s], c = i[0] === "[", u = t.has(i);
      c || u ? (o.length > 0 && (o.sort(), r.push(...o), o = []), r.push(i)) : o.push(i);
    }
    return o.length > 0 && (o.sort(), r.push(...o)), r;
  };
}, Jp = (e) => ({
  cache: Fp(e.cacheSize),
  parseClassName: Qp(e),
  sortModifiers: Zp(e),
  ...Rp(e)
}), Wp = /\s+/, Up = (e, t) => {
  const {
    parseClassName: n,
    getClassGroupId: r,
    getConflictingClassGroupIds: o,
    sortModifiers: s
  } = t, i = [], c = e.trim().split(Wp);
  let u = "";
  for (let l = c.length - 1; l >= 0; l -= 1) {
    const f = c[l], {
      isExternal: d,
      modifiers: m,
      hasImportantModifier: p,
      baseClassName: x,
      maybePostfixModifierPosition: g
    } = n(f);
    if (d) {
      u = f + (u.length > 0 ? " " + u : u);
      continue;
    }
    let v = !!g, b = r(v ? x.substring(0, g) : x);
    if (!b) {
      if (!v) {
        u = f + (u.length > 0 ? " " + u : u);
        continue;
      }
      if (b = r(x), !b) {
        u = f + (u.length > 0 ? " " + u : u);
        continue;
      }
      v = !1;
    }
    const A = m.length === 0 ? "" : m.length === 1 ? m[0] : s(m).join(":"), w = p ? A + ao : A, E = w + b;
    if (i.indexOf(E) > -1)
      continue;
    i.push(E);
    const C = o(b, v);
    for (let P = 0; P < C.length; ++P) {
      const D = C[P];
      i.push(w + D);
    }
    u = f + (u.length > 0 ? " " + u : u);
  }
  return u;
}, Vp = (...e) => {
  let t = 0, n, r, o = "";
  for (; t < e.length; )
    (n = e[t++]) && (r = ml(n)) && (o && (o += " "), o += r);
  return o;
}, ml = (e) => {
  if (typeof e == "string")
    return e;
  let t, n = "";
  for (let r = 0; r < e.length; r++)
    e[r] && (t = ml(e[r])) && (n && (n += " "), n += t);
  return n;
}, qp = (e, ...t) => {
  let n, r, o, s;
  const i = (u) => {
    const l = t.reduce((f, d) => d(f), e());
    return n = Jp(l), r = n.cache.get, o = n.cache.set, s = c, c(u);
  }, c = (u) => {
    const l = r(u);
    if (l)
      return l;
    const f = Up(u, n);
    return o(u, f), f;
  };
  return s = i, (...u) => s(Vp(...u));
}, Xp = [], ie = (e) => {
  const t = (n) => n[e] || Xp;
  return t.isThemeGetter = !0, t;
}, hl = /^\[(?:(\w[\w-]*):)?(.+)\]$/i, pl = /^\((?:(\w[\w-]*):)?(.+)\)$/i, Kp = /^\d+(?:\.\d+)?\/\d+(?:\.\d+)?$/, _p = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/, $p = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/, eg = /^(rgba?|hsla?|hwb|(ok)?(lab|lch)|color-mix)\(.+\)$/, tg = /^(inset_)?-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)/, ng = /^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$/, Je = (e) => Kp.test(e), K = (e) => !!e && !Number.isNaN(Number(e)), We = (e) => !!e && Number.isInteger(Number(e)), Fr = (e) => e.endsWith("%") && K(e.slice(0, -1)), Ye = (e) => _p.test(e), gl = () => !0, rg = (e) => (
  // `colorFunctionRegex` check is necessary because color functions can have percentages in them which which would be incorrectly classified as lengths.
  // For example, `hsl(0 0% 0%)` would be classified as a length without this check.
  // I could also use lookbehind assertion in `lengthUnitRegex` but that isn't supported widely enough.
  $p.test(e) && !eg.test(e)
), es = () => !1, og = (e) => tg.test(e), sg = (e) => ng.test(e), ag = (e) => !F(e) && !L(e), ig = (e) => st(e, bl, es), F = (e) => hl.test(e), ct = (e) => st(e, wl, rg), ha = (e) => st(e, pg, K), cg = (e) => st(e, yl, gl), lg = (e) => st(e, Al, es), pa = (e) => st(e, xl, es), ug = (e) => st(e, vl, sg), Dn = (e) => st(e, El, og), L = (e) => pl.test(e), Lt = (e) => bt(e, wl), dg = (e) => bt(e, Al), ga = (e) => bt(e, xl), fg = (e) => bt(e, bl), mg = (e) => bt(e, vl), Rn = (e) => bt(e, El, !0), hg = (e) => bt(e, yl, !0), st = (e, t, n) => {
  const r = hl.exec(e);
  return r ? r[1] ? t(r[1]) : n(r[2]) : !1;
}, bt = (e, t, n = !1) => {
  const r = pl.exec(e);
  return r ? r[1] ? t(r[1]) : n : !1;
}, xl = (e) => e === "position" || e === "percentage", vl = (e) => e === "image" || e === "url", bl = (e) => e === "length" || e === "size" || e === "bg-size", wl = (e) => e === "length", pg = (e) => e === "number", Al = (e) => e === "family-name", yl = (e) => e === "number" || e === "weight", El = (e) => e === "shadow", gg = () => {
  const e = ie("color"), t = ie("font"), n = ie("text"), r = ie("font-weight"), o = ie("tracking"), s = ie("leading"), i = ie("breakpoint"), c = ie("container"), u = ie("spacing"), l = ie("radius"), f = ie("shadow"), d = ie("inset-shadow"), m = ie("text-shadow"), p = ie("drop-shadow"), x = ie("blur"), g = ie("perspective"), v = ie("aspect"), b = ie("ease"), A = ie("animate"), w = () => ["auto", "avoid", "all", "avoid-page", "page", "left", "right", "column"], E = () => [
    "center",
    "top",
    "bottom",
    "left",
    "right",
    "top-left",
    // Deprecated since Tailwind CSS v4.1.0, see https://github.com/tailwindlabs/tailwindcss/pull/17378
    "left-top",
    "top-right",
    // Deprecated since Tailwind CSS v4.1.0, see https://github.com/tailwindlabs/tailwindcss/pull/17378
    "right-top",
    "bottom-right",
    // Deprecated since Tailwind CSS v4.1.0, see https://github.com/tailwindlabs/tailwindcss/pull/17378
    "right-bottom",
    "bottom-left",
    // Deprecated since Tailwind CSS v4.1.0, see https://github.com/tailwindlabs/tailwindcss/pull/17378
    "left-bottom"
  ], C = () => [...E(), L, F], P = () => ["auto", "hidden", "clip", "visible", "scroll"], D = () => ["auto", "contain", "none"], M = () => [L, F, u], k = () => [Je, "full", "auto", ...M()], Y = () => [We, "none", "subgrid", L, F], z = () => ["auto", {
    span: ["full", We, L, F]
  }, We, L, F], Z = () => [We, "auto", L, F], q = () => ["auto", "min", "max", "fr", L, F], T = () => ["start", "end", "center", "between", "around", "evenly", "stretch", "baseline", "center-safe", "end-safe"], R = () => ["start", "end", "center", "stretch", "center-safe", "end-safe"], j = () => ["auto", ...M()], O = () => [Je, "auto", "full", "dvw", "dvh", "lvw", "lvh", "svw", "svh", "min", "max", "fit", ...M()], N = () => [Je, "screen", "full", "dvw", "lvw", "svw", "min", "max", "fit", ...M()], S = () => [Je, "screen", "full", "lh", "dvh", "lvh", "svh", "min", "max", "fit", ...M()], I = () => [e, L, F], y = () => [...E(), ga, pa, {
    position: [L, F]
  }], G = () => ["no-repeat", {
    repeat: ["", "x", "y", "space", "round"]
  }], Q = () => ["auto", "cover", "contain", fg, ig, {
    size: [L, F]
  }], W = () => [Fr, Lt, ct], X = () => [
    // Deprecated since Tailwind CSS v4.0.0
    "",
    "none",
    "full",
    l,
    L,
    F
  ], _ = () => ["", K, Lt, ct], J = () => ["solid", "dashed", "dotted", "double"], oe = () => ["normal", "multiply", "screen", "overlay", "darken", "lighten", "color-dodge", "color-burn", "hard-light", "soft-light", "difference", "exclusion", "hue", "saturation", "color", "luminosity"], ee = () => [K, Fr, ga, pa], ne = () => [
    // Deprecated since Tailwind CSS v4.0.0
    "",
    "none",
    x,
    L,
    F
  ], le = () => ["none", K, L, F], he = () => ["none", K, L, F], fe = () => [K, L, F], Yt = () => [Je, "full", ...M()];
  return {
    cacheSize: 500,
    theme: {
      animate: ["spin", "ping", "pulse", "bounce"],
      aspect: ["video"],
      blur: [Ye],
      breakpoint: [Ye],
      color: [gl],
      container: [Ye],
      "drop-shadow": [Ye],
      ease: ["in", "out", "in-out"],
      font: [ag],
      "font-weight": ["thin", "extralight", "light", "normal", "medium", "semibold", "bold", "extrabold", "black"],
      "inset-shadow": [Ye],
      leading: ["none", "tight", "snug", "normal", "relaxed", "loose"],
      perspective: ["dramatic", "near", "normal", "midrange", "distant", "none"],
      radius: [Ye],
      shadow: [Ye],
      spacing: ["px", K],
      text: [Ye],
      "text-shadow": [Ye],
      tracking: ["tighter", "tight", "normal", "wide", "wider", "widest"]
    },
    classGroups: {
      // --------------
      // --- Layout ---
      // --------------
      /**
       * Aspect Ratio
       * @see https://tailwindcss.com/docs/aspect-ratio
       */
      aspect: [{
        aspect: ["auto", "square", Je, F, L, v]
      }],
      /**
       * Container
       * @see https://tailwindcss.com/docs/container
       * @deprecated since Tailwind CSS v4.0.0
       */
      container: ["container"],
      /**
       * Columns
       * @see https://tailwindcss.com/docs/columns
       */
      columns: [{
        columns: [K, F, L, c]
      }],
      /**
       * Break After
       * @see https://tailwindcss.com/docs/break-after
       */
      "break-after": [{
        "break-after": w()
      }],
      /**
       * Break Before
       * @see https://tailwindcss.com/docs/break-before
       */
      "break-before": [{
        "break-before": w()
      }],
      /**
       * Break Inside
       * @see https://tailwindcss.com/docs/break-inside
       */
      "break-inside": [{
        "break-inside": ["auto", "avoid", "avoid-page", "avoid-column"]
      }],
      /**
       * Box Decoration Break
       * @see https://tailwindcss.com/docs/box-decoration-break
       */
      "box-decoration": [{
        "box-decoration": ["slice", "clone"]
      }],
      /**
       * Box Sizing
       * @see https://tailwindcss.com/docs/box-sizing
       */
      box: [{
        box: ["border", "content"]
      }],
      /**
       * Display
       * @see https://tailwindcss.com/docs/display
       */
      display: ["block", "inline-block", "inline", "flex", "inline-flex", "table", "inline-table", "table-caption", "table-cell", "table-column", "table-column-group", "table-footer-group", "table-header-group", "table-row-group", "table-row", "flow-root", "grid", "inline-grid", "contents", "list-item", "hidden"],
      /**
       * Screen Reader Only
       * @see https://tailwindcss.com/docs/display#screen-reader-only
       */
      sr: ["sr-only", "not-sr-only"],
      /**
       * Floats
       * @see https://tailwindcss.com/docs/float
       */
      float: [{
        float: ["right", "left", "none", "start", "end"]
      }],
      /**
       * Clear
       * @see https://tailwindcss.com/docs/clear
       */
      clear: [{
        clear: ["left", "right", "both", "none", "start", "end"]
      }],
      /**
       * Isolation
       * @see https://tailwindcss.com/docs/isolation
       */
      isolation: ["isolate", "isolation-auto"],
      /**
       * Object Fit
       * @see https://tailwindcss.com/docs/object-fit
       */
      "object-fit": [{
        object: ["contain", "cover", "fill", "none", "scale-down"]
      }],
      /**
       * Object Position
       * @see https://tailwindcss.com/docs/object-position
       */
      "object-position": [{
        object: C()
      }],
      /**
       * Overflow
       * @see https://tailwindcss.com/docs/overflow
       */
      overflow: [{
        overflow: P()
      }],
      /**
       * Overflow X
       * @see https://tailwindcss.com/docs/overflow
       */
      "overflow-x": [{
        "overflow-x": P()
      }],
      /**
       * Overflow Y
       * @see https://tailwindcss.com/docs/overflow
       */
      "overflow-y": [{
        "overflow-y": P()
      }],
      /**
       * Overscroll Behavior
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      overscroll: [{
        overscroll: D()
      }],
      /**
       * Overscroll Behavior X
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      "overscroll-x": [{
        "overscroll-x": D()
      }],
      /**
       * Overscroll Behavior Y
       * @see https://tailwindcss.com/docs/overscroll-behavior
       */
      "overscroll-y": [{
        "overscroll-y": D()
      }],
      /**
       * Position
       * @see https://tailwindcss.com/docs/position
       */
      position: ["static", "fixed", "absolute", "relative", "sticky"],
      /**
       * Inset
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      inset: [{
        inset: k()
      }],
      /**
       * Inset Inline
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-x": [{
        "inset-x": k()
      }],
      /**
       * Inset Block
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-y": [{
        "inset-y": k()
      }],
      /**
       * Inset Inline Start
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       * @todo class group will be renamed to `inset-s` in next major release
       */
      start: [{
        "inset-s": k(),
        /**
         * @deprecated since Tailwind CSS v4.2.0 in favor of `inset-s-*` utilities.
         * @see https://github.com/tailwindlabs/tailwindcss/pull/19613
         */
        start: k()
      }],
      /**
       * Inset Inline End
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       * @todo class group will be renamed to `inset-e` in next major release
       */
      end: [{
        "inset-e": k(),
        /**
         * @deprecated since Tailwind CSS v4.2.0 in favor of `inset-e-*` utilities.
         * @see https://github.com/tailwindlabs/tailwindcss/pull/19613
         */
        end: k()
      }],
      /**
       * Inset Block Start
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-bs": [{
        "inset-bs": k()
      }],
      /**
       * Inset Block End
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      "inset-be": [{
        "inset-be": k()
      }],
      /**
       * Top
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      top: [{
        top: k()
      }],
      /**
       * Right
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      right: [{
        right: k()
      }],
      /**
       * Bottom
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      bottom: [{
        bottom: k()
      }],
      /**
       * Left
       * @see https://tailwindcss.com/docs/top-right-bottom-left
       */
      left: [{
        left: k()
      }],
      /**
       * Visibility
       * @see https://tailwindcss.com/docs/visibility
       */
      visibility: ["visible", "invisible", "collapse"],
      /**
       * Z-Index
       * @see https://tailwindcss.com/docs/z-index
       */
      z: [{
        z: [We, "auto", L, F]
      }],
      // ------------------------
      // --- Flexbox and Grid ---
      // ------------------------
      /**
       * Flex Basis
       * @see https://tailwindcss.com/docs/flex-basis
       */
      basis: [{
        basis: [Je, "full", "auto", c, ...M()]
      }],
      /**
       * Flex Direction
       * @see https://tailwindcss.com/docs/flex-direction
       */
      "flex-direction": [{
        flex: ["row", "row-reverse", "col", "col-reverse"]
      }],
      /**
       * Flex Wrap
       * @see https://tailwindcss.com/docs/flex-wrap
       */
      "flex-wrap": [{
        flex: ["nowrap", "wrap", "wrap-reverse"]
      }],
      /**
       * Flex
       * @see https://tailwindcss.com/docs/flex
       */
      flex: [{
        flex: [K, Je, "auto", "initial", "none", F]
      }],
      /**
       * Flex Grow
       * @see https://tailwindcss.com/docs/flex-grow
       */
      grow: [{
        grow: ["", K, L, F]
      }],
      /**
       * Flex Shrink
       * @see https://tailwindcss.com/docs/flex-shrink
       */
      shrink: [{
        shrink: ["", K, L, F]
      }],
      /**
       * Order
       * @see https://tailwindcss.com/docs/order
       */
      order: [{
        order: [We, "first", "last", "none", L, F]
      }],
      /**
       * Grid Template Columns
       * @see https://tailwindcss.com/docs/grid-template-columns
       */
      "grid-cols": [{
        "grid-cols": Y()
      }],
      /**
       * Grid Column Start / End
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-start-end": [{
        col: z()
      }],
      /**
       * Grid Column Start
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-start": [{
        "col-start": Z()
      }],
      /**
       * Grid Column End
       * @see https://tailwindcss.com/docs/grid-column
       */
      "col-end": [{
        "col-end": Z()
      }],
      /**
       * Grid Template Rows
       * @see https://tailwindcss.com/docs/grid-template-rows
       */
      "grid-rows": [{
        "grid-rows": Y()
      }],
      /**
       * Grid Row Start / End
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-start-end": [{
        row: z()
      }],
      /**
       * Grid Row Start
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-start": [{
        "row-start": Z()
      }],
      /**
       * Grid Row End
       * @see https://tailwindcss.com/docs/grid-row
       */
      "row-end": [{
        "row-end": Z()
      }],
      /**
       * Grid Auto Flow
       * @see https://tailwindcss.com/docs/grid-auto-flow
       */
      "grid-flow": [{
        "grid-flow": ["row", "col", "dense", "row-dense", "col-dense"]
      }],
      /**
       * Grid Auto Columns
       * @see https://tailwindcss.com/docs/grid-auto-columns
       */
      "auto-cols": [{
        "auto-cols": q()
      }],
      /**
       * Grid Auto Rows
       * @see https://tailwindcss.com/docs/grid-auto-rows
       */
      "auto-rows": [{
        "auto-rows": q()
      }],
      /**
       * Gap
       * @see https://tailwindcss.com/docs/gap
       */
      gap: [{
        gap: M()
      }],
      /**
       * Gap X
       * @see https://tailwindcss.com/docs/gap
       */
      "gap-x": [{
        "gap-x": M()
      }],
      /**
       * Gap Y
       * @see https://tailwindcss.com/docs/gap
       */
      "gap-y": [{
        "gap-y": M()
      }],
      /**
       * Justify Content
       * @see https://tailwindcss.com/docs/justify-content
       */
      "justify-content": [{
        justify: [...T(), "normal"]
      }],
      /**
       * Justify Items
       * @see https://tailwindcss.com/docs/justify-items
       */
      "justify-items": [{
        "justify-items": [...R(), "normal"]
      }],
      /**
       * Justify Self
       * @see https://tailwindcss.com/docs/justify-self
       */
      "justify-self": [{
        "justify-self": ["auto", ...R()]
      }],
      /**
       * Align Content
       * @see https://tailwindcss.com/docs/align-content
       */
      "align-content": [{
        content: ["normal", ...T()]
      }],
      /**
       * Align Items
       * @see https://tailwindcss.com/docs/align-items
       */
      "align-items": [{
        items: [...R(), {
          baseline: ["", "last"]
        }]
      }],
      /**
       * Align Self
       * @see https://tailwindcss.com/docs/align-self
       */
      "align-self": [{
        self: ["auto", ...R(), {
          baseline: ["", "last"]
        }]
      }],
      /**
       * Place Content
       * @see https://tailwindcss.com/docs/place-content
       */
      "place-content": [{
        "place-content": T()
      }],
      /**
       * Place Items
       * @see https://tailwindcss.com/docs/place-items
       */
      "place-items": [{
        "place-items": [...R(), "baseline"]
      }],
      /**
       * Place Self
       * @see https://tailwindcss.com/docs/place-self
       */
      "place-self": [{
        "place-self": ["auto", ...R()]
      }],
      // Spacing
      /**
       * Padding
       * @see https://tailwindcss.com/docs/padding
       */
      p: [{
        p: M()
      }],
      /**
       * Padding Inline
       * @see https://tailwindcss.com/docs/padding
       */
      px: [{
        px: M()
      }],
      /**
       * Padding Block
       * @see https://tailwindcss.com/docs/padding
       */
      py: [{
        py: M()
      }],
      /**
       * Padding Inline Start
       * @see https://tailwindcss.com/docs/padding
       */
      ps: [{
        ps: M()
      }],
      /**
       * Padding Inline End
       * @see https://tailwindcss.com/docs/padding
       */
      pe: [{
        pe: M()
      }],
      /**
       * Padding Block Start
       * @see https://tailwindcss.com/docs/padding
       */
      pbs: [{
        pbs: M()
      }],
      /**
       * Padding Block End
       * @see https://tailwindcss.com/docs/padding
       */
      pbe: [{
        pbe: M()
      }],
      /**
       * Padding Top
       * @see https://tailwindcss.com/docs/padding
       */
      pt: [{
        pt: M()
      }],
      /**
       * Padding Right
       * @see https://tailwindcss.com/docs/padding
       */
      pr: [{
        pr: M()
      }],
      /**
       * Padding Bottom
       * @see https://tailwindcss.com/docs/padding
       */
      pb: [{
        pb: M()
      }],
      /**
       * Padding Left
       * @see https://tailwindcss.com/docs/padding
       */
      pl: [{
        pl: M()
      }],
      /**
       * Margin
       * @see https://tailwindcss.com/docs/margin
       */
      m: [{
        m: j()
      }],
      /**
       * Margin Inline
       * @see https://tailwindcss.com/docs/margin
       */
      mx: [{
        mx: j()
      }],
      /**
       * Margin Block
       * @see https://tailwindcss.com/docs/margin
       */
      my: [{
        my: j()
      }],
      /**
       * Margin Inline Start
       * @see https://tailwindcss.com/docs/margin
       */
      ms: [{
        ms: j()
      }],
      /**
       * Margin Inline End
       * @see https://tailwindcss.com/docs/margin
       */
      me: [{
        me: j()
      }],
      /**
       * Margin Block Start
       * @see https://tailwindcss.com/docs/margin
       */
      mbs: [{
        mbs: j()
      }],
      /**
       * Margin Block End
       * @see https://tailwindcss.com/docs/margin
       */
      mbe: [{
        mbe: j()
      }],
      /**
       * Margin Top
       * @see https://tailwindcss.com/docs/margin
       */
      mt: [{
        mt: j()
      }],
      /**
       * Margin Right
       * @see https://tailwindcss.com/docs/margin
       */
      mr: [{
        mr: j()
      }],
      /**
       * Margin Bottom
       * @see https://tailwindcss.com/docs/margin
       */
      mb: [{
        mb: j()
      }],
      /**
       * Margin Left
       * @see https://tailwindcss.com/docs/margin
       */
      ml: [{
        ml: j()
      }],
      /**
       * Space Between X
       * @see https://tailwindcss.com/docs/margin#adding-space-between-children
       */
      "space-x": [{
        "space-x": M()
      }],
      /**
       * Space Between X Reverse
       * @see https://tailwindcss.com/docs/margin#adding-space-between-children
       */
      "space-x-reverse": ["space-x-reverse"],
      /**
       * Space Between Y
       * @see https://tailwindcss.com/docs/margin#adding-space-between-children
       */
      "space-y": [{
        "space-y": M()
      }],
      /**
       * Space Between Y Reverse
       * @see https://tailwindcss.com/docs/margin#adding-space-between-children
       */
      "space-y-reverse": ["space-y-reverse"],
      // --------------
      // --- Sizing ---
      // --------------
      /**
       * Size
       * @see https://tailwindcss.com/docs/width#setting-both-width-and-height
       */
      size: [{
        size: O()
      }],
      /**
       * Inline Size
       * @see https://tailwindcss.com/docs/width
       */
      "inline-size": [{
        inline: ["auto", ...N()]
      }],
      /**
       * Min-Inline Size
       * @see https://tailwindcss.com/docs/min-width
       */
      "min-inline-size": [{
        "min-inline": ["auto", ...N()]
      }],
      /**
       * Max-Inline Size
       * @see https://tailwindcss.com/docs/max-width
       */
      "max-inline-size": [{
        "max-inline": ["none", ...N()]
      }],
      /**
       * Block Size
       * @see https://tailwindcss.com/docs/height
       */
      "block-size": [{
        block: ["auto", ...S()]
      }],
      /**
       * Min-Block Size
       * @see https://tailwindcss.com/docs/min-height
       */
      "min-block-size": [{
        "min-block": ["auto", ...S()]
      }],
      /**
       * Max-Block Size
       * @see https://tailwindcss.com/docs/max-height
       */
      "max-block-size": [{
        "max-block": ["none", ...S()]
      }],
      /**
       * Width
       * @see https://tailwindcss.com/docs/width
       */
      w: [{
        w: [c, "screen", ...O()]
      }],
      /**
       * Min-Width
       * @see https://tailwindcss.com/docs/min-width
       */
      "min-w": [{
        "min-w": [
          c,
          "screen",
          /** Deprecated. @see https://github.com/tailwindlabs/tailwindcss.com/issues/2027#issuecomment-2620152757 */
          "none",
          ...O()
        ]
      }],
      /**
       * Max-Width
       * @see https://tailwindcss.com/docs/max-width
       */
      "max-w": [{
        "max-w": [
          c,
          "screen",
          "none",
          /** Deprecated since Tailwind CSS v4.0.0. @see https://github.com/tailwindlabs/tailwindcss.com/issues/2027#issuecomment-2620152757 */
          "prose",
          /** Deprecated since Tailwind CSS v4.0.0. @see https://github.com/tailwindlabs/tailwindcss.com/issues/2027#issuecomment-2620152757 */
          {
            screen: [i]
          },
          ...O()
        ]
      }],
      /**
       * Height
       * @see https://tailwindcss.com/docs/height
       */
      h: [{
        h: ["screen", "lh", ...O()]
      }],
      /**
       * Min-Height
       * @see https://tailwindcss.com/docs/min-height
       */
      "min-h": [{
        "min-h": ["screen", "lh", "none", ...O()]
      }],
      /**
       * Max-Height
       * @see https://tailwindcss.com/docs/max-height
       */
      "max-h": [{
        "max-h": ["screen", "lh", ...O()]
      }],
      // ------------------
      // --- Typography ---
      // ------------------
      /**
       * Font Size
       * @see https://tailwindcss.com/docs/font-size
       */
      "font-size": [{
        text: ["base", n, Lt, ct]
      }],
      /**
       * Font Smoothing
       * @see https://tailwindcss.com/docs/font-smoothing
       */
      "font-smoothing": ["antialiased", "subpixel-antialiased"],
      /**
       * Font Style
       * @see https://tailwindcss.com/docs/font-style
       */
      "font-style": ["italic", "not-italic"],
      /**
       * Font Weight
       * @see https://tailwindcss.com/docs/font-weight
       */
      "font-weight": [{
        font: [r, hg, cg]
      }],
      /**
       * Font Stretch
       * @see https://tailwindcss.com/docs/font-stretch
       */
      "font-stretch": [{
        "font-stretch": ["ultra-condensed", "extra-condensed", "condensed", "semi-condensed", "normal", "semi-expanded", "expanded", "extra-expanded", "ultra-expanded", Fr, F]
      }],
      /**
       * Font Family
       * @see https://tailwindcss.com/docs/font-family
       */
      "font-family": [{
        font: [dg, lg, t]
      }],
      /**
       * Font Feature Settings
       * @see https://tailwindcss.com/docs/font-feature-settings
       */
      "font-features": [{
        "font-features": [F]
      }],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-normal": ["normal-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-ordinal": ["ordinal"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-slashed-zero": ["slashed-zero"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-figure": ["lining-nums", "oldstyle-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-spacing": ["proportional-nums", "tabular-nums"],
      /**
       * Font Variant Numeric
       * @see https://tailwindcss.com/docs/font-variant-numeric
       */
      "fvn-fraction": ["diagonal-fractions", "stacked-fractions"],
      /**
       * Letter Spacing
       * @see https://tailwindcss.com/docs/letter-spacing
       */
      tracking: [{
        tracking: [o, L, F]
      }],
      /**
       * Line Clamp
       * @see https://tailwindcss.com/docs/line-clamp
       */
      "line-clamp": [{
        "line-clamp": [K, "none", L, ha]
      }],
      /**
       * Line Height
       * @see https://tailwindcss.com/docs/line-height
       */
      leading: [{
        leading: [
          /** Deprecated since Tailwind CSS v4.0.0. @see https://github.com/tailwindlabs/tailwindcss.com/issues/2027#issuecomment-2620152757 */
          s,
          ...M()
        ]
      }],
      /**
       * List Style Image
       * @see https://tailwindcss.com/docs/list-style-image
       */
      "list-image": [{
        "list-image": ["none", L, F]
      }],
      /**
       * List Style Position
       * @see https://tailwindcss.com/docs/list-style-position
       */
      "list-style-position": [{
        list: ["inside", "outside"]
      }],
      /**
       * List Style Type
       * @see https://tailwindcss.com/docs/list-style-type
       */
      "list-style-type": [{
        list: ["disc", "decimal", "none", L, F]
      }],
      /**
       * Text Alignment
       * @see https://tailwindcss.com/docs/text-align
       */
      "text-alignment": [{
        text: ["left", "center", "right", "justify", "start", "end"]
      }],
      /**
       * Placeholder Color
       * @deprecated since Tailwind CSS v3.0.0
       * @see https://v3.tailwindcss.com/docs/placeholder-color
       */
      "placeholder-color": [{
        placeholder: I()
      }],
      /**
       * Text Color
       * @see https://tailwindcss.com/docs/text-color
       */
      "text-color": [{
        text: I()
      }],
      /**
       * Text Decoration
       * @see https://tailwindcss.com/docs/text-decoration
       */
      "text-decoration": ["underline", "overline", "line-through", "no-underline"],
      /**
       * Text Decoration Style
       * @see https://tailwindcss.com/docs/text-decoration-style
       */
      "text-decoration-style": [{
        decoration: [...J(), "wavy"]
      }],
      /**
       * Text Decoration Thickness
       * @see https://tailwindcss.com/docs/text-decoration-thickness
       */
      "text-decoration-thickness": [{
        decoration: [K, "from-font", "auto", L, ct]
      }],
      /**
       * Text Decoration Color
       * @see https://tailwindcss.com/docs/text-decoration-color
       */
      "text-decoration-color": [{
        decoration: I()
      }],
      /**
       * Text Underline Offset
       * @see https://tailwindcss.com/docs/text-underline-offset
       */
      "underline-offset": [{
        "underline-offset": [K, "auto", L, F]
      }],
      /**
       * Text Transform
       * @see https://tailwindcss.com/docs/text-transform
       */
      "text-transform": ["uppercase", "lowercase", "capitalize", "normal-case"],
      /**
       * Text Overflow
       * @see https://tailwindcss.com/docs/text-overflow
       */
      "text-overflow": ["truncate", "text-ellipsis", "text-clip"],
      /**
       * Text Wrap
       * @see https://tailwindcss.com/docs/text-wrap
       */
      "text-wrap": [{
        text: ["wrap", "nowrap", "balance", "pretty"]
      }],
      /**
       * Text Indent
       * @see https://tailwindcss.com/docs/text-indent
       */
      indent: [{
        indent: M()
      }],
      /**
       * Vertical Alignment
       * @see https://tailwindcss.com/docs/vertical-align
       */
      "vertical-align": [{
        align: ["baseline", "top", "middle", "bottom", "text-top", "text-bottom", "sub", "super", L, F]
      }],
      /**
       * Whitespace
       * @see https://tailwindcss.com/docs/whitespace
       */
      whitespace: [{
        whitespace: ["normal", "nowrap", "pre", "pre-line", "pre-wrap", "break-spaces"]
      }],
      /**
       * Word Break
       * @see https://tailwindcss.com/docs/word-break
       */
      break: [{
        break: ["normal", "words", "all", "keep"]
      }],
      /**
       * Overflow Wrap
       * @see https://tailwindcss.com/docs/overflow-wrap
       */
      wrap: [{
        wrap: ["break-word", "anywhere", "normal"]
      }],
      /**
       * Hyphens
       * @see https://tailwindcss.com/docs/hyphens
       */
      hyphens: [{
        hyphens: ["none", "manual", "auto"]
      }],
      /**
       * Content
       * @see https://tailwindcss.com/docs/content
       */
      content: [{
        content: ["none", L, F]
      }],
      // -------------------
      // --- Backgrounds ---
      // -------------------
      /**
       * Background Attachment
       * @see https://tailwindcss.com/docs/background-attachment
       */
      "bg-attachment": [{
        bg: ["fixed", "local", "scroll"]
      }],
      /**
       * Background Clip
       * @see https://tailwindcss.com/docs/background-clip
       */
      "bg-clip": [{
        "bg-clip": ["border", "padding", "content", "text"]
      }],
      /**
       * Background Origin
       * @see https://tailwindcss.com/docs/background-origin
       */
      "bg-origin": [{
        "bg-origin": ["border", "padding", "content"]
      }],
      /**
       * Background Position
       * @see https://tailwindcss.com/docs/background-position
       */
      "bg-position": [{
        bg: y()
      }],
      /**
       * Background Repeat
       * @see https://tailwindcss.com/docs/background-repeat
       */
      "bg-repeat": [{
        bg: G()
      }],
      /**
       * Background Size
       * @see https://tailwindcss.com/docs/background-size
       */
      "bg-size": [{
        bg: Q()
      }],
      /**
       * Background Image
       * @see https://tailwindcss.com/docs/background-image
       */
      "bg-image": [{
        bg: ["none", {
          linear: [{
            to: ["t", "tr", "r", "br", "b", "bl", "l", "tl"]
          }, We, L, F],
          radial: ["", L, F],
          conic: [We, L, F]
        }, mg, ug]
      }],
      /**
       * Background Color
       * @see https://tailwindcss.com/docs/background-color
       */
      "bg-color": [{
        bg: I()
      }],
      /**
       * Gradient Color Stops From Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-from-pos": [{
        from: W()
      }],
      /**
       * Gradient Color Stops Via Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-via-pos": [{
        via: W()
      }],
      /**
       * Gradient Color Stops To Position
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-to-pos": [{
        to: W()
      }],
      /**
       * Gradient Color Stops From
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-from": [{
        from: I()
      }],
      /**
       * Gradient Color Stops Via
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-via": [{
        via: I()
      }],
      /**
       * Gradient Color Stops To
       * @see https://tailwindcss.com/docs/gradient-color-stops
       */
      "gradient-to": [{
        to: I()
      }],
      // ---------------
      // --- Borders ---
      // ---------------
      /**
       * Border Radius
       * @see https://tailwindcss.com/docs/border-radius
       */
      rounded: [{
        rounded: X()
      }],
      /**
       * Border Radius Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-s": [{
        "rounded-s": X()
      }],
      /**
       * Border Radius End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-e": [{
        "rounded-e": X()
      }],
      /**
       * Border Radius Top
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-t": [{
        "rounded-t": X()
      }],
      /**
       * Border Radius Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-r": [{
        "rounded-r": X()
      }],
      /**
       * Border Radius Bottom
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-b": [{
        "rounded-b": X()
      }],
      /**
       * Border Radius Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-l": [{
        "rounded-l": X()
      }],
      /**
       * Border Radius Start Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-ss": [{
        "rounded-ss": X()
      }],
      /**
       * Border Radius Start End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-se": [{
        "rounded-se": X()
      }],
      /**
       * Border Radius End End
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-ee": [{
        "rounded-ee": X()
      }],
      /**
       * Border Radius End Start
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-es": [{
        "rounded-es": X()
      }],
      /**
       * Border Radius Top Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-tl": [{
        "rounded-tl": X()
      }],
      /**
       * Border Radius Top Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-tr": [{
        "rounded-tr": X()
      }],
      /**
       * Border Radius Bottom Right
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-br": [{
        "rounded-br": X()
      }],
      /**
       * Border Radius Bottom Left
       * @see https://tailwindcss.com/docs/border-radius
       */
      "rounded-bl": [{
        "rounded-bl": X()
      }],
      /**
       * Border Width
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w": [{
        border: _()
      }],
      /**
       * Border Width Inline
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-x": [{
        "border-x": _()
      }],
      /**
       * Border Width Block
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-y": [{
        "border-y": _()
      }],
      /**
       * Border Width Inline Start
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-s": [{
        "border-s": _()
      }],
      /**
       * Border Width Inline End
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-e": [{
        "border-e": _()
      }],
      /**
       * Border Width Block Start
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-bs": [{
        "border-bs": _()
      }],
      /**
       * Border Width Block End
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-be": [{
        "border-be": _()
      }],
      /**
       * Border Width Top
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-t": [{
        "border-t": _()
      }],
      /**
       * Border Width Right
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-r": [{
        "border-r": _()
      }],
      /**
       * Border Width Bottom
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-b": [{
        "border-b": _()
      }],
      /**
       * Border Width Left
       * @see https://tailwindcss.com/docs/border-width
       */
      "border-w-l": [{
        "border-l": _()
      }],
      /**
       * Divide Width X
       * @see https://tailwindcss.com/docs/border-width#between-children
       */
      "divide-x": [{
        "divide-x": _()
      }],
      /**
       * Divide Width X Reverse
       * @see https://tailwindcss.com/docs/border-width#between-children
       */
      "divide-x-reverse": ["divide-x-reverse"],
      /**
       * Divide Width Y
       * @see https://tailwindcss.com/docs/border-width#between-children
       */
      "divide-y": [{
        "divide-y": _()
      }],
      /**
       * Divide Width Y Reverse
       * @see https://tailwindcss.com/docs/border-width#between-children
       */
      "divide-y-reverse": ["divide-y-reverse"],
      /**
       * Border Style
       * @see https://tailwindcss.com/docs/border-style
       */
      "border-style": [{
        border: [...J(), "hidden", "none"]
      }],
      /**
       * Divide Style
       * @see https://tailwindcss.com/docs/border-style#setting-the-divider-style
       */
      "divide-style": [{
        divide: [...J(), "hidden", "none"]
      }],
      /**
       * Border Color
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color": [{
        border: I()
      }],
      /**
       * Border Color Inline
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-x": [{
        "border-x": I()
      }],
      /**
       * Border Color Block
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-y": [{
        "border-y": I()
      }],
      /**
       * Border Color Inline Start
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-s": [{
        "border-s": I()
      }],
      /**
       * Border Color Inline End
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-e": [{
        "border-e": I()
      }],
      /**
       * Border Color Block Start
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-bs": [{
        "border-bs": I()
      }],
      /**
       * Border Color Block End
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-be": [{
        "border-be": I()
      }],
      /**
       * Border Color Top
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-t": [{
        "border-t": I()
      }],
      /**
       * Border Color Right
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-r": [{
        "border-r": I()
      }],
      /**
       * Border Color Bottom
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-b": [{
        "border-b": I()
      }],
      /**
       * Border Color Left
       * @see https://tailwindcss.com/docs/border-color
       */
      "border-color-l": [{
        "border-l": I()
      }],
      /**
       * Divide Color
       * @see https://tailwindcss.com/docs/divide-color
       */
      "divide-color": [{
        divide: I()
      }],
      /**
       * Outline Style
       * @see https://tailwindcss.com/docs/outline-style
       */
      "outline-style": [{
        outline: [...J(), "none", "hidden"]
      }],
      /**
       * Outline Offset
       * @see https://tailwindcss.com/docs/outline-offset
       */
      "outline-offset": [{
        "outline-offset": [K, L, F]
      }],
      /**
       * Outline Width
       * @see https://tailwindcss.com/docs/outline-width
       */
      "outline-w": [{
        outline: ["", K, Lt, ct]
      }],
      /**
       * Outline Color
       * @see https://tailwindcss.com/docs/outline-color
       */
      "outline-color": [{
        outline: I()
      }],
      // ---------------
      // --- Effects ---
      // ---------------
      /**
       * Box Shadow
       * @see https://tailwindcss.com/docs/box-shadow
       */
      shadow: [{
        shadow: [
          // Deprecated since Tailwind CSS v4.0.0
          "",
          "none",
          f,
          Rn,
          Dn
        ]
      }],
      /**
       * Box Shadow Color
       * @see https://tailwindcss.com/docs/box-shadow#setting-the-shadow-color
       */
      "shadow-color": [{
        shadow: I()
      }],
      /**
       * Inset Box Shadow
       * @see https://tailwindcss.com/docs/box-shadow#adding-an-inset-shadow
       */
      "inset-shadow": [{
        "inset-shadow": ["none", d, Rn, Dn]
      }],
      /**
       * Inset Box Shadow Color
       * @see https://tailwindcss.com/docs/box-shadow#setting-the-inset-shadow-color
       */
      "inset-shadow-color": [{
        "inset-shadow": I()
      }],
      /**
       * Ring Width
       * @see https://tailwindcss.com/docs/box-shadow#adding-a-ring
       */
      "ring-w": [{
        ring: _()
      }],
      /**
       * Ring Width Inset
       * @see https://v3.tailwindcss.com/docs/ring-width#inset-rings
       * @deprecated since Tailwind CSS v4.0.0
       * @see https://github.com/tailwindlabs/tailwindcss/blob/v4.0.0/packages/tailwindcss/src/utilities.ts#L4158
       */
      "ring-w-inset": ["ring-inset"],
      /**
       * Ring Color
       * @see https://tailwindcss.com/docs/box-shadow#setting-the-ring-color
       */
      "ring-color": [{
        ring: I()
      }],
      /**
       * Ring Offset Width
       * @see https://v3.tailwindcss.com/docs/ring-offset-width
       * @deprecated since Tailwind CSS v4.0.0
       * @see https://github.com/tailwindlabs/tailwindcss/blob/v4.0.0/packages/tailwindcss/src/utilities.ts#L4158
       */
      "ring-offset-w": [{
        "ring-offset": [K, ct]
      }],
      /**
       * Ring Offset Color
       * @see https://v3.tailwindcss.com/docs/ring-offset-color
       * @deprecated since Tailwind CSS v4.0.0
       * @see https://github.com/tailwindlabs/tailwindcss/blob/v4.0.0/packages/tailwindcss/src/utilities.ts#L4158
       */
      "ring-offset-color": [{
        "ring-offset": I()
      }],
      /**
       * Inset Ring Width
       * @see https://tailwindcss.com/docs/box-shadow#adding-an-inset-ring
       */
      "inset-ring-w": [{
        "inset-ring": _()
      }],
      /**
       * Inset Ring Color
       * @see https://tailwindcss.com/docs/box-shadow#setting-the-inset-ring-color
       */
      "inset-ring-color": [{
        "inset-ring": I()
      }],
      /**
       * Text Shadow
       * @see https://tailwindcss.com/docs/text-shadow
       */
      "text-shadow": [{
        "text-shadow": ["none", m, Rn, Dn]
      }],
      /**
       * Text Shadow Color
       * @see https://tailwindcss.com/docs/text-shadow#setting-the-shadow-color
       */
      "text-shadow-color": [{
        "text-shadow": I()
      }],
      /**
       * Opacity
       * @see https://tailwindcss.com/docs/opacity
       */
      opacity: [{
        opacity: [K, L, F]
      }],
      /**
       * Mix Blend Mode
       * @see https://tailwindcss.com/docs/mix-blend-mode
       */
      "mix-blend": [{
        "mix-blend": [...oe(), "plus-darker", "plus-lighter"]
      }],
      /**
       * Background Blend Mode
       * @see https://tailwindcss.com/docs/background-blend-mode
       */
      "bg-blend": [{
        "bg-blend": oe()
      }],
      /**
       * Mask Clip
       * @see https://tailwindcss.com/docs/mask-clip
       */
      "mask-clip": [{
        "mask-clip": ["border", "padding", "content", "fill", "stroke", "view"]
      }, "mask-no-clip"],
      /**
       * Mask Composite
       * @see https://tailwindcss.com/docs/mask-composite
       */
      "mask-composite": [{
        mask: ["add", "subtract", "intersect", "exclude"]
      }],
      /**
       * Mask Image
       * @see https://tailwindcss.com/docs/mask-image
       */
      "mask-image-linear-pos": [{
        "mask-linear": [K]
      }],
      "mask-image-linear-from-pos": [{
        "mask-linear-from": ee()
      }],
      "mask-image-linear-to-pos": [{
        "mask-linear-to": ee()
      }],
      "mask-image-linear-from-color": [{
        "mask-linear-from": I()
      }],
      "mask-image-linear-to-color": [{
        "mask-linear-to": I()
      }],
      "mask-image-t-from-pos": [{
        "mask-t-from": ee()
      }],
      "mask-image-t-to-pos": [{
        "mask-t-to": ee()
      }],
      "mask-image-t-from-color": [{
        "mask-t-from": I()
      }],
      "mask-image-t-to-color": [{
        "mask-t-to": I()
      }],
      "mask-image-r-from-pos": [{
        "mask-r-from": ee()
      }],
      "mask-image-r-to-pos": [{
        "mask-r-to": ee()
      }],
      "mask-image-r-from-color": [{
        "mask-r-from": I()
      }],
      "mask-image-r-to-color": [{
        "mask-r-to": I()
      }],
      "mask-image-b-from-pos": [{
        "mask-b-from": ee()
      }],
      "mask-image-b-to-pos": [{
        "mask-b-to": ee()
      }],
      "mask-image-b-from-color": [{
        "mask-b-from": I()
      }],
      "mask-image-b-to-color": [{
        "mask-b-to": I()
      }],
      "mask-image-l-from-pos": [{
        "mask-l-from": ee()
      }],
      "mask-image-l-to-pos": [{
        "mask-l-to": ee()
      }],
      "mask-image-l-from-color": [{
        "mask-l-from": I()
      }],
      "mask-image-l-to-color": [{
        "mask-l-to": I()
      }],
      "mask-image-x-from-pos": [{
        "mask-x-from": ee()
      }],
      "mask-image-x-to-pos": [{
        "mask-x-to": ee()
      }],
      "mask-image-x-from-color": [{
        "mask-x-from": I()
      }],
      "mask-image-x-to-color": [{
        "mask-x-to": I()
      }],
      "mask-image-y-from-pos": [{
        "mask-y-from": ee()
      }],
      "mask-image-y-to-pos": [{
        "mask-y-to": ee()
      }],
      "mask-image-y-from-color": [{
        "mask-y-from": I()
      }],
      "mask-image-y-to-color": [{
        "mask-y-to": I()
      }],
      "mask-image-radial": [{
        "mask-radial": [L, F]
      }],
      "mask-image-radial-from-pos": [{
        "mask-radial-from": ee()
      }],
      "mask-image-radial-to-pos": [{
        "mask-radial-to": ee()
      }],
      "mask-image-radial-from-color": [{
        "mask-radial-from": I()
      }],
      "mask-image-radial-to-color": [{
        "mask-radial-to": I()
      }],
      "mask-image-radial-shape": [{
        "mask-radial": ["circle", "ellipse"]
      }],
      "mask-image-radial-size": [{
        "mask-radial": [{
          closest: ["side", "corner"],
          farthest: ["side", "corner"]
        }]
      }],
      "mask-image-radial-pos": [{
        "mask-radial-at": E()
      }],
      "mask-image-conic-pos": [{
        "mask-conic": [K]
      }],
      "mask-image-conic-from-pos": [{
        "mask-conic-from": ee()
      }],
      "mask-image-conic-to-pos": [{
        "mask-conic-to": ee()
      }],
      "mask-image-conic-from-color": [{
        "mask-conic-from": I()
      }],
      "mask-image-conic-to-color": [{
        "mask-conic-to": I()
      }],
      /**
       * Mask Mode
       * @see https://tailwindcss.com/docs/mask-mode
       */
      "mask-mode": [{
        mask: ["alpha", "luminance", "match"]
      }],
      /**
       * Mask Origin
       * @see https://tailwindcss.com/docs/mask-origin
       */
      "mask-origin": [{
        "mask-origin": ["border", "padding", "content", "fill", "stroke", "view"]
      }],
      /**
       * Mask Position
       * @see https://tailwindcss.com/docs/mask-position
       */
      "mask-position": [{
        mask: y()
      }],
      /**
       * Mask Repeat
       * @see https://tailwindcss.com/docs/mask-repeat
       */
      "mask-repeat": [{
        mask: G()
      }],
      /**
       * Mask Size
       * @see https://tailwindcss.com/docs/mask-size
       */
      "mask-size": [{
        mask: Q()
      }],
      /**
       * Mask Type
       * @see https://tailwindcss.com/docs/mask-type
       */
      "mask-type": [{
        "mask-type": ["alpha", "luminance"]
      }],
      /**
       * Mask Image
       * @see https://tailwindcss.com/docs/mask-image
       */
      "mask-image": [{
        mask: ["none", L, F]
      }],
      // ---------------
      // --- Filters ---
      // ---------------
      /**
       * Filter
       * @see https://tailwindcss.com/docs/filter
       */
      filter: [{
        filter: [
          // Deprecated since Tailwind CSS v3.0.0
          "",
          "none",
          L,
          F
        ]
      }],
      /**
       * Blur
       * @see https://tailwindcss.com/docs/blur
       */
      blur: [{
        blur: ne()
      }],
      /**
       * Brightness
       * @see https://tailwindcss.com/docs/brightness
       */
      brightness: [{
        brightness: [K, L, F]
      }],
      /**
       * Contrast
       * @see https://tailwindcss.com/docs/contrast
       */
      contrast: [{
        contrast: [K, L, F]
      }],
      /**
       * Drop Shadow
       * @see https://tailwindcss.com/docs/drop-shadow
       */
      "drop-shadow": [{
        "drop-shadow": [
          // Deprecated since Tailwind CSS v4.0.0
          "",
          "none",
          p,
          Rn,
          Dn
        ]
      }],
      /**
       * Drop Shadow Color
       * @see https://tailwindcss.com/docs/filter-drop-shadow#setting-the-shadow-color
       */
      "drop-shadow-color": [{
        "drop-shadow": I()
      }],
      /**
       * Grayscale
       * @see https://tailwindcss.com/docs/grayscale
       */
      grayscale: [{
        grayscale: ["", K, L, F]
      }],
      /**
       * Hue Rotate
       * @see https://tailwindcss.com/docs/hue-rotate
       */
      "hue-rotate": [{
        "hue-rotate": [K, L, F]
      }],
      /**
       * Invert
       * @see https://tailwindcss.com/docs/invert
       */
      invert: [{
        invert: ["", K, L, F]
      }],
      /**
       * Saturate
       * @see https://tailwindcss.com/docs/saturate
       */
      saturate: [{
        saturate: [K, L, F]
      }],
      /**
       * Sepia
       * @see https://tailwindcss.com/docs/sepia
       */
      sepia: [{
        sepia: ["", K, L, F]
      }],
      /**
       * Backdrop Filter
       * @see https://tailwindcss.com/docs/backdrop-filter
       */
      "backdrop-filter": [{
        "backdrop-filter": [
          // Deprecated since Tailwind CSS v3.0.0
          "",
          "none",
          L,
          F
        ]
      }],
      /**
       * Backdrop Blur
       * @see https://tailwindcss.com/docs/backdrop-blur
       */
      "backdrop-blur": [{
        "backdrop-blur": ne()
      }],
      /**
       * Backdrop Brightness
       * @see https://tailwindcss.com/docs/backdrop-brightness
       */
      "backdrop-brightness": [{
        "backdrop-brightness": [K, L, F]
      }],
      /**
       * Backdrop Contrast
       * @see https://tailwindcss.com/docs/backdrop-contrast
       */
      "backdrop-contrast": [{
        "backdrop-contrast": [K, L, F]
      }],
      /**
       * Backdrop Grayscale
       * @see https://tailwindcss.com/docs/backdrop-grayscale
       */
      "backdrop-grayscale": [{
        "backdrop-grayscale": ["", K, L, F]
      }],
      /**
       * Backdrop Hue Rotate
       * @see https://tailwindcss.com/docs/backdrop-hue-rotate
       */
      "backdrop-hue-rotate": [{
        "backdrop-hue-rotate": [K, L, F]
      }],
      /**
       * Backdrop Invert
       * @see https://tailwindcss.com/docs/backdrop-invert
       */
      "backdrop-invert": [{
        "backdrop-invert": ["", K, L, F]
      }],
      /**
       * Backdrop Opacity
       * @see https://tailwindcss.com/docs/backdrop-opacity
       */
      "backdrop-opacity": [{
        "backdrop-opacity": [K, L, F]
      }],
      /**
       * Backdrop Saturate
       * @see https://tailwindcss.com/docs/backdrop-saturate
       */
      "backdrop-saturate": [{
        "backdrop-saturate": [K, L, F]
      }],
      /**
       * Backdrop Sepia
       * @see https://tailwindcss.com/docs/backdrop-sepia
       */
      "backdrop-sepia": [{
        "backdrop-sepia": ["", K, L, F]
      }],
      // --------------
      // --- Tables ---
      // --------------
      /**
       * Border Collapse
       * @see https://tailwindcss.com/docs/border-collapse
       */
      "border-collapse": [{
        border: ["collapse", "separate"]
      }],
      /**
       * Border Spacing
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing": [{
        "border-spacing": M()
      }],
      /**
       * Border Spacing X
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing-x": [{
        "border-spacing-x": M()
      }],
      /**
       * Border Spacing Y
       * @see https://tailwindcss.com/docs/border-spacing
       */
      "border-spacing-y": [{
        "border-spacing-y": M()
      }],
      /**
       * Table Layout
       * @see https://tailwindcss.com/docs/table-layout
       */
      "table-layout": [{
        table: ["auto", "fixed"]
      }],
      /**
       * Caption Side
       * @see https://tailwindcss.com/docs/caption-side
       */
      caption: [{
        caption: ["top", "bottom"]
      }],
      // ---------------------------------
      // --- Transitions and Animation ---
      // ---------------------------------
      /**
       * Transition Property
       * @see https://tailwindcss.com/docs/transition-property
       */
      transition: [{
        transition: ["", "all", "colors", "opacity", "shadow", "transform", "none", L, F]
      }],
      /**
       * Transition Behavior
       * @see https://tailwindcss.com/docs/transition-behavior
       */
      "transition-behavior": [{
        transition: ["normal", "discrete"]
      }],
      /**
       * Transition Duration
       * @see https://tailwindcss.com/docs/transition-duration
       */
      duration: [{
        duration: [K, "initial", L, F]
      }],
      /**
       * Transition Timing Function
       * @see https://tailwindcss.com/docs/transition-timing-function
       */
      ease: [{
        ease: ["linear", "initial", b, L, F]
      }],
      /**
       * Transition Delay
       * @see https://tailwindcss.com/docs/transition-delay
       */
      delay: [{
        delay: [K, L, F]
      }],
      /**
       * Animation
       * @see https://tailwindcss.com/docs/animation
       */
      animate: [{
        animate: ["none", A, L, F]
      }],
      // ------------------
      // --- Transforms ---
      // ------------------
      /**
       * Backface Visibility
       * @see https://tailwindcss.com/docs/backface-visibility
       */
      backface: [{
        backface: ["hidden", "visible"]
      }],
      /**
       * Perspective
       * @see https://tailwindcss.com/docs/perspective
       */
      perspective: [{
        perspective: [g, L, F]
      }],
      /**
       * Perspective Origin
       * @see https://tailwindcss.com/docs/perspective-origin
       */
      "perspective-origin": [{
        "perspective-origin": C()
      }],
      /**
       * Rotate
       * @see https://tailwindcss.com/docs/rotate
       */
      rotate: [{
        rotate: le()
      }],
      /**
       * Rotate X
       * @see https://tailwindcss.com/docs/rotate
       */
      "rotate-x": [{
        "rotate-x": le()
      }],
      /**
       * Rotate Y
       * @see https://tailwindcss.com/docs/rotate
       */
      "rotate-y": [{
        "rotate-y": le()
      }],
      /**
       * Rotate Z
       * @see https://tailwindcss.com/docs/rotate
       */
      "rotate-z": [{
        "rotate-z": le()
      }],
      /**
       * Scale
       * @see https://tailwindcss.com/docs/scale
       */
      scale: [{
        scale: he()
      }],
      /**
       * Scale X
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-x": [{
        "scale-x": he()
      }],
      /**
       * Scale Y
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-y": [{
        "scale-y": he()
      }],
      /**
       * Scale Z
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-z": [{
        "scale-z": he()
      }],
      /**
       * Scale 3D
       * @see https://tailwindcss.com/docs/scale
       */
      "scale-3d": ["scale-3d"],
      /**
       * Skew
       * @see https://tailwindcss.com/docs/skew
       */
      skew: [{
        skew: fe()
      }],
      /**
       * Skew X
       * @see https://tailwindcss.com/docs/skew
       */
      "skew-x": [{
        "skew-x": fe()
      }],
      /**
       * Skew Y
       * @see https://tailwindcss.com/docs/skew
       */
      "skew-y": [{
        "skew-y": fe()
      }],
      /**
       * Transform
       * @see https://tailwindcss.com/docs/transform
       */
      transform: [{
        transform: [L, F, "", "none", "gpu", "cpu"]
      }],
      /**
       * Transform Origin
       * @see https://tailwindcss.com/docs/transform-origin
       */
      "transform-origin": [{
        origin: C()
      }],
      /**
       * Transform Style
       * @see https://tailwindcss.com/docs/transform-style
       */
      "transform-style": [{
        transform: ["3d", "flat"]
      }],
      /**
       * Translate
       * @see https://tailwindcss.com/docs/translate
       */
      translate: [{
        translate: Yt()
      }],
      /**
       * Translate X
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-x": [{
        "translate-x": Yt()
      }],
      /**
       * Translate Y
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-y": [{
        "translate-y": Yt()
      }],
      /**
       * Translate Z
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-z": [{
        "translate-z": Yt()
      }],
      /**
       * Translate None
       * @see https://tailwindcss.com/docs/translate
       */
      "translate-none": ["translate-none"],
      // ---------------------
      // --- Interactivity ---
      // ---------------------
      /**
       * Accent Color
       * @see https://tailwindcss.com/docs/accent-color
       */
      accent: [{
        accent: I()
      }],
      /**
       * Appearance
       * @see https://tailwindcss.com/docs/appearance
       */
      appearance: [{
        appearance: ["none", "auto"]
      }],
      /**
       * Caret Color
       * @see https://tailwindcss.com/docs/just-in-time-mode#caret-color-utilities
       */
      "caret-color": [{
        caret: I()
      }],
      /**
       * Color Scheme
       * @see https://tailwindcss.com/docs/color-scheme
       */
      "color-scheme": [{
        scheme: ["normal", "dark", "light", "light-dark", "only-dark", "only-light"]
      }],
      /**
       * Cursor
       * @see https://tailwindcss.com/docs/cursor
       */
      cursor: [{
        cursor: ["auto", "default", "pointer", "wait", "text", "move", "help", "not-allowed", "none", "context-menu", "progress", "cell", "crosshair", "vertical-text", "alias", "copy", "no-drop", "grab", "grabbing", "all-scroll", "col-resize", "row-resize", "n-resize", "e-resize", "s-resize", "w-resize", "ne-resize", "nw-resize", "se-resize", "sw-resize", "ew-resize", "ns-resize", "nesw-resize", "nwse-resize", "zoom-in", "zoom-out", L, F]
      }],
      /**
       * Field Sizing
       * @see https://tailwindcss.com/docs/field-sizing
       */
      "field-sizing": [{
        "field-sizing": ["fixed", "content"]
      }],
      /**
       * Pointer Events
       * @see https://tailwindcss.com/docs/pointer-events
       */
      "pointer-events": [{
        "pointer-events": ["auto", "none"]
      }],
      /**
       * Resize
       * @see https://tailwindcss.com/docs/resize
       */
      resize: [{
        resize: ["none", "", "y", "x"]
      }],
      /**
       * Scroll Behavior
       * @see https://tailwindcss.com/docs/scroll-behavior
       */
      "scroll-behavior": [{
        scroll: ["auto", "smooth"]
      }],
      /**
       * Scroll Margin
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-m": [{
        "scroll-m": M()
      }],
      /**
       * Scroll Margin Inline
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mx": [{
        "scroll-mx": M()
      }],
      /**
       * Scroll Margin Block
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-my": [{
        "scroll-my": M()
      }],
      /**
       * Scroll Margin Inline Start
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-ms": [{
        "scroll-ms": M()
      }],
      /**
       * Scroll Margin Inline End
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-me": [{
        "scroll-me": M()
      }],
      /**
       * Scroll Margin Block Start
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mbs": [{
        "scroll-mbs": M()
      }],
      /**
       * Scroll Margin Block End
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mbe": [{
        "scroll-mbe": M()
      }],
      /**
       * Scroll Margin Top
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mt": [{
        "scroll-mt": M()
      }],
      /**
       * Scroll Margin Right
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mr": [{
        "scroll-mr": M()
      }],
      /**
       * Scroll Margin Bottom
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-mb": [{
        "scroll-mb": M()
      }],
      /**
       * Scroll Margin Left
       * @see https://tailwindcss.com/docs/scroll-margin
       */
      "scroll-ml": [{
        "scroll-ml": M()
      }],
      /**
       * Scroll Padding
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-p": [{
        "scroll-p": M()
      }],
      /**
       * Scroll Padding Inline
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-px": [{
        "scroll-px": M()
      }],
      /**
       * Scroll Padding Block
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-py": [{
        "scroll-py": M()
      }],
      /**
       * Scroll Padding Inline Start
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-ps": [{
        "scroll-ps": M()
      }],
      /**
       * Scroll Padding Inline End
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pe": [{
        "scroll-pe": M()
      }],
      /**
       * Scroll Padding Block Start
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pbs": [{
        "scroll-pbs": M()
      }],
      /**
       * Scroll Padding Block End
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pbe": [{
        "scroll-pbe": M()
      }],
      /**
       * Scroll Padding Top
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pt": [{
        "scroll-pt": M()
      }],
      /**
       * Scroll Padding Right
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pr": [{
        "scroll-pr": M()
      }],
      /**
       * Scroll Padding Bottom
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pb": [{
        "scroll-pb": M()
      }],
      /**
       * Scroll Padding Left
       * @see https://tailwindcss.com/docs/scroll-padding
       */
      "scroll-pl": [{
        "scroll-pl": M()
      }],
      /**
       * Scroll Snap Align
       * @see https://tailwindcss.com/docs/scroll-snap-align
       */
      "snap-align": [{
        snap: ["start", "end", "center", "align-none"]
      }],
      /**
       * Scroll Snap Stop
       * @see https://tailwindcss.com/docs/scroll-snap-stop
       */
      "snap-stop": [{
        snap: ["normal", "always"]
      }],
      /**
       * Scroll Snap Type
       * @see https://tailwindcss.com/docs/scroll-snap-type
       */
      "snap-type": [{
        snap: ["none", "x", "y", "both"]
      }],
      /**
       * Scroll Snap Type Strictness
       * @see https://tailwindcss.com/docs/scroll-snap-type
       */
      "snap-strictness": [{
        snap: ["mandatory", "proximity"]
      }],
      /**
       * Touch Action
       * @see https://tailwindcss.com/docs/touch-action
       */
      touch: [{
        touch: ["auto", "none", "manipulation"]
      }],
      /**
       * Touch Action X
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-x": [{
        "touch-pan": ["x", "left", "right"]
      }],
      /**
       * Touch Action Y
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-y": [{
        "touch-pan": ["y", "up", "down"]
      }],
      /**
       * Touch Action Pinch Zoom
       * @see https://tailwindcss.com/docs/touch-action
       */
      "touch-pz": ["touch-pinch-zoom"],
      /**
       * User Select
       * @see https://tailwindcss.com/docs/user-select
       */
      select: [{
        select: ["none", "text", "all", "auto"]
      }],
      /**
       * Will Change
       * @see https://tailwindcss.com/docs/will-change
       */
      "will-change": [{
        "will-change": ["auto", "scroll", "contents", "transform", L, F]
      }],
      // -----------
      // --- SVG ---
      // -----------
      /**
       * Fill
       * @see https://tailwindcss.com/docs/fill
       */
      fill: [{
        fill: ["none", ...I()]
      }],
      /**
       * Stroke Width
       * @see https://tailwindcss.com/docs/stroke-width
       */
      "stroke-w": [{
        stroke: [K, Lt, ct, ha]
      }],
      /**
       * Stroke
       * @see https://tailwindcss.com/docs/stroke
       */
      stroke: [{
        stroke: ["none", ...I()]
      }],
      // ---------------------
      // --- Accessibility ---
      // ---------------------
      /**
       * Forced Color Adjust
       * @see https://tailwindcss.com/docs/forced-color-adjust
       */
      "forced-color-adjust": [{
        "forced-color-adjust": ["auto", "none"]
      }]
    },
    conflictingClassGroups: {
      overflow: ["overflow-x", "overflow-y"],
      overscroll: ["overscroll-x", "overscroll-y"],
      inset: ["inset-x", "inset-y", "inset-bs", "inset-be", "start", "end", "top", "right", "bottom", "left"],
      "inset-x": ["right", "left"],
      "inset-y": ["top", "bottom"],
      flex: ["basis", "grow", "shrink"],
      gap: ["gap-x", "gap-y"],
      p: ["px", "py", "ps", "pe", "pbs", "pbe", "pt", "pr", "pb", "pl"],
      px: ["pr", "pl"],
      py: ["pt", "pb"],
      m: ["mx", "my", "ms", "me", "mbs", "mbe", "mt", "mr", "mb", "ml"],
      mx: ["mr", "ml"],
      my: ["mt", "mb"],
      size: ["w", "h"],
      "font-size": ["leading"],
      "fvn-normal": ["fvn-ordinal", "fvn-slashed-zero", "fvn-figure", "fvn-spacing", "fvn-fraction"],
      "fvn-ordinal": ["fvn-normal"],
      "fvn-slashed-zero": ["fvn-normal"],
      "fvn-figure": ["fvn-normal"],
      "fvn-spacing": ["fvn-normal"],
      "fvn-fraction": ["fvn-normal"],
      "line-clamp": ["display", "overflow"],
      rounded: ["rounded-s", "rounded-e", "rounded-t", "rounded-r", "rounded-b", "rounded-l", "rounded-ss", "rounded-se", "rounded-ee", "rounded-es", "rounded-tl", "rounded-tr", "rounded-br", "rounded-bl"],
      "rounded-s": ["rounded-ss", "rounded-es"],
      "rounded-e": ["rounded-se", "rounded-ee"],
      "rounded-t": ["rounded-tl", "rounded-tr"],
      "rounded-r": ["rounded-tr", "rounded-br"],
      "rounded-b": ["rounded-br", "rounded-bl"],
      "rounded-l": ["rounded-tl", "rounded-bl"],
      "border-spacing": ["border-spacing-x", "border-spacing-y"],
      "border-w": ["border-w-x", "border-w-y", "border-w-s", "border-w-e", "border-w-bs", "border-w-be", "border-w-t", "border-w-r", "border-w-b", "border-w-l"],
      "border-w-x": ["border-w-r", "border-w-l"],
      "border-w-y": ["border-w-t", "border-w-b"],
      "border-color": ["border-color-x", "border-color-y", "border-color-s", "border-color-e", "border-color-bs", "border-color-be", "border-color-t", "border-color-r", "border-color-b", "border-color-l"],
      "border-color-x": ["border-color-r", "border-color-l"],
      "border-color-y": ["border-color-t", "border-color-b"],
      translate: ["translate-x", "translate-y", "translate-none"],
      "translate-none": ["translate", "translate-x", "translate-y", "translate-z"],
      "scroll-m": ["scroll-mx", "scroll-my", "scroll-ms", "scroll-me", "scroll-mbs", "scroll-mbe", "scroll-mt", "scroll-mr", "scroll-mb", "scroll-ml"],
      "scroll-mx": ["scroll-mr", "scroll-ml"],
      "scroll-my": ["scroll-mt", "scroll-mb"],
      "scroll-p": ["scroll-px", "scroll-py", "scroll-ps", "scroll-pe", "scroll-pbs", "scroll-pbe", "scroll-pt", "scroll-pr", "scroll-pb", "scroll-pl"],
      "scroll-px": ["scroll-pr", "scroll-pl"],
      "scroll-py": ["scroll-pt", "scroll-pb"],
      touch: ["touch-x", "touch-y", "touch-pz"],
      "touch-x": ["touch"],
      "touch-y": ["touch"],
      "touch-pz": ["touch"]
    },
    conflictingClassGroupModifiers: {
      "font-size": ["leading"]
    },
    orderSensitiveModifiers: ["*", "**", "after", "backdrop", "before", "details-content", "file", "first-letter", "first-line", "marker", "placeholder", "selection"]
  };
}, xg = /* @__PURE__ */ qp(gg);
function H(...e) {
  return xg(qa(e));
}
const vg = Vu(
  "inline-flex shrink-0 cursor-pointer items-center justify-center gap-2 rounded-md text-sm font-medium whitespace-nowrap transition-all outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        default: "bg-primary text-primary-foreground hover:bg-primary/90",
        destructive: "bg-destructive text-white hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:bg-destructive/60 dark:focus-visible:ring-destructive/40",
        outline: "border bg-background shadow-xs hover:bg-accent hover:text-accent-foreground dark:border-input dark:bg-input/30 dark:hover:bg-input/50",
        secondary: "bg-secondary text-secondary-foreground hover:bg-secondary/80",
        ghost: "hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50",
        link: "text-primary underline-offset-4 hover:underline"
      },
      size: {
        default: "h-9 px-4 py-2 has-[>svg]:px-3",
        xs: "h-6 gap-1 rounded-md px-2 text-xs has-[>svg]:px-1.5 [&_svg:not([class*='size-'])]:size-3",
        sm: "h-8 gap-1.5 rounded-md px-3 has-[>svg]:px-2.5",
        lg: "h-10 rounded-md px-6 has-[>svg]:px-4",
        icon: "size-9",
        "icon-xs": "size-6 rounded-md [&_svg:not([class*='size-'])]:size-3",
        "icon-sm": "size-8",
        "icon-lg": "size-10"
      }
    },
    defaultVariants: {
      variant: "default",
      size: "default"
    }
  }
);
function te({
  className: e,
  variant: t = "default",
  size: n = "default",
  asChild: r = !1,
  ...o
}) {
  const s = r ? qu : "button";
  return /* @__PURE__ */ a.jsx(
    s,
    {
      "data-slot": "button",
      "data-variant": t,
      "data-size": n,
      className: H(vg({ variant: t, size: n, className: e })),
      ...o
    }
  );
}
function Ml({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card",
      className: H(
        "flex flex-col gap-6 rounded-xl border bg-card py-6 text-card-foreground shadow-sm",
        e
      ),
      ...t
    }
  );
}
function jl({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-header",
      className: H(
        "@container/card-header grid auto-rows-min grid-rows-[auto_auto] items-start gap-2 px-6 has-data-[slot=card-action]:grid-cols-[1fr_auto] [.border-b]:pb-6",
        e
      ),
      ...t
    }
  );
}
function Cl({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-title",
      className: H("leading-none font-semibold", e),
      ...t
    }
  );
}
function bg({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-description",
      className: H("text-sm text-muted-foreground", e),
      ...t
    }
  );
}
function ew({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-action",
      className: H(
        "col-start-2 row-span-2 row-start-1 self-start justify-self-end",
        e
      ),
      ...t
    }
  );
}
function Nl({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-content",
      className: H("px-6", e),
      ...t
    }
  );
}
function tw({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "card-footer",
      className: H("flex items-center px-6 [.border-t]:pt-6", e),
      ...t
    }
  );
}
function ut({ className: e, type: t, ...n }) {
  return /* @__PURE__ */ a.jsx(
    "input",
    {
      type: t,
      "data-slot": "input",
      className: H(
        "h-9 w-full min-w-0 rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm dark:bg-input/30",
        "focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50",
        "aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40",
        e
      ),
      ...n
    }
  );
}
function Nt({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    hp,
    {
      "data-slot": "label",
      className: H(
        "flex items-center gap-2 text-sm leading-none font-medium select-none group-data-[disabled=true]:pointer-events-none group-data-[disabled=true]:opacity-50 peer-disabled:cursor-not-allowed peer-disabled:opacity-50",
        e
      ),
      ...t
    }
  );
}
function wg({
  className: e,
  orientation: t = "horizontal",
  decorative: n = !0,
  ...r
}) {
  return /* @__PURE__ */ a.jsx(
    Ip,
    {
      "data-slot": "separator",
      decorative: n,
      orientation: t,
      className: H(
        "shrink-0 bg-border data-[orientation=horizontal]:h-px data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:w-px",
        e
      ),
      ...r
    }
  );
}
const Il = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim();
const Ag = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase();
const yg = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
);
const xa = (e) => {
  const t = yg(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
};
var Lr = {
  xmlns: "http://www.w3.org/2000/svg",
  width: 24,
  height: 24,
  viewBox: "0 0 24 24",
  fill: "none",
  stroke: "currentColor",
  strokeWidth: 2,
  strokeLinecap: "round",
  strokeLinejoin: "round"
};
const Eg = (e) => {
  for (const t in e)
    if (t.startsWith("aria-") || t === "role" || t === "title")
      return !0;
  return !1;
}, Mg = zt({}), jg = () => gt(Mg), Cg = ar(
  ({ color: e, size: t, strokeWidth: n, absoluteStrokeWidth: r, className: o = "", children: s, iconNode: i, ...c }, u) => {
    const {
      size: l = 24,
      strokeWidth: f = 2,
      absoluteStrokeWidth: d = !1,
      color: m = "currentColor",
      className: p = ""
    } = jg() ?? {}, x = r ?? d ? Number(n ?? f) * 24 / Number(t ?? l) : n ?? f;
    return _r(
      "svg",
      {
        ref: u,
        ...Lr,
        width: t ?? l ?? Lr.width,
        height: t ?? l ?? Lr.height,
        stroke: e ?? m,
        strokeWidth: x,
        className: Il("lucide", p, o),
        ...!s && !Eg(c) && { "aria-hidden": "true" },
        ...c
      },
      [
        ...i.map(([g, v]) => _r(g, v)),
        ...Array.isArray(s) ? s : [s]
      ]
    );
  }
);
const se = (e, t) => {
  const n = ar(
    ({ className: r, ...o }, s) => _r(Cg, {
      ref: s,
      iconNode: t,
      className: Il(
        `lucide-${Ag(xa(e))}`,
        `lucide-${e}`,
        r
      ),
      ...o
    })
  );
  return n.displayName = xa(e), n;
};
const Ng = [
  ["path", { d: "M10.268 21a2 2 0 0 0 3.464 0", key: "vwvbt9" }],
  [
    "path",
    {
      d: "M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326",
      key: "11g9vi"
    }
  ]
], Ig = se("bell", Ng);
const Sg = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], ts = se("check", Sg);
const kg = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], ns = se("chevron-down", kg);
const Dg = [["path", { d: "m15 18-6-6 6-6", key: "1wnfg3" }]], Rg = se("chevron-left", Dg);
const Pg = [["path", { d: "m9 18 6-6-6-6", key: "mthhwq" }]], It = se("chevron-right", Pg);
const Og = [["path", { d: "m18 15-6-6-6 6", key: "153udz" }]], io = se("chevron-up", Og);
const zg = [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["line", { x1: "12", x2: "12", y1: "8", y2: "12", key: "1pkeuh" }],
  ["line", { x1: "12", x2: "12.01", y1: "16", y2: "16", key: "4dfq90" }]
], Sl = se("circle-alert", zg);
const Bg = [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["path", { d: "m9 12 2 2 4-4", key: "dzmm74" }]
], Tg = se("circle-check", Bg);
const Gg = [["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }]], Yg = se("circle", Gg);
const Hg = [
  ["rect", { width: "8", height: "4", x: "8", y: "2", rx: "1", ry: "1", key: "tgr4d6" }],
  [
    "path",
    {
      d: "M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2",
      key: "116196"
    }
  ],
  ["path", { d: "M12 11h4", key: "1jrz19" }],
  ["path", { d: "M12 16h4", key: "n85exb" }],
  ["path", { d: "M8 11h.01", key: "1dfujw" }],
  ["path", { d: "M8 16h.01", key: "18s6g9" }]
], Fg = se("clipboard-list", Hg);
const Lg = [
  ["path", { d: "m15 15 6 6", key: "1s409w" }],
  ["path", { d: "m15 9 6-6", key: "ko1vev" }],
  ["path", { d: "M21 16v5h-5", key: "1ck2sf" }],
  ["path", { d: "M21 8V3h-5", key: "1qoq8a" }],
  ["path", { d: "M3 16v5h5", key: "1t08am" }],
  ["path", { d: "m3 21 6-6", key: "wwnumi" }],
  ["path", { d: "M3 8V3h5", key: "1ln10m" }],
  ["path", { d: "M9 9 3 3", key: "v551iv" }]
], va = se("expand", Lg);
const Qg = [
  ["circle", { cx: "12", cy: "12", r: "10", key: "1mglay" }],
  ["path", { d: "M12 16v-4", key: "1dtifu" }],
  ["path", { d: "M12 8h.01", key: "e9boi3" }]
], Zg = se("info", Qg);
const Jg = [["path", { d: "M21 12a9 9 0 1 1-6.219-8.56", key: "13zald" }]], kl = se("loader-circle", Jg);
const Wg = [
  ["rect", { width: "20", height: "14", x: "2", y: "3", rx: "2", key: "48i651" }],
  ["line", { x1: "8", x2: "16", y1: "21", y2: "21", key: "1svkeh" }],
  ["line", { x1: "12", x2: "12", y1: "17", y2: "21", key: "vw1qmm" }]
], co = se("monitor", Wg);
const Ug = [
  [
    "path",
    {
      d: "M20.985 12.486a9 9 0 1 1-9.473-9.472c.405-.022.617.46.402.803a6 6 0 0 0 8.268 8.268c.344-.215.825-.004.803.401",
      key: "kfwtm"
    }
  ]
], Kn = se("moon", Ug);
const Vg = [
  ["rect", { width: "18", height: "18", x: "3", y: "3", rx: "2", key: "afitv7" }],
  ["path", { d: "M15 3v18", key: "14nvp0" }]
], ba = se("panel-right", Vg);
const qg = [
  [
    "path",
    {
      d: "M15.2 3a2 2 0 0 1 1.4.6l3.8 3.8a2 2 0 0 1 .6 1.4V19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z",
      key: "1c8476"
    }
  ],
  ["path", { d: "M17 21v-7a1 1 0 0 0-1-1H8a1 1 0 0 0-1 1v7", key: "1ydtos" }],
  ["path", { d: "M7 3v4a1 1 0 0 0 1 1h7", key: "t51u73" }]
], Xg = se("save", qg);
const Kg = [
  ["path", { d: "m21 21-4.34-4.34", key: "14j7rj" }],
  ["circle", { cx: "11", cy: "11", r: "8", key: "4ej97u" }]
], _g = se("search", Kg);
const $g = [
  ["path", { d: "M10 5H3", key: "1qgfaw" }],
  ["path", { d: "M12 19H3", key: "yhmn1j" }],
  ["path", { d: "M14 3v4", key: "1sua03" }],
  ["path", { d: "M16 17v4", key: "1q0r14" }],
  ["path", { d: "M21 12h-9", key: "1o4lsq" }],
  ["path", { d: "M21 19h-5", key: "1rlt1p" }],
  ["path", { d: "M21 5h-7", key: "1oszz2" }],
  ["path", { d: "M8 10v4", key: "tgpxqk" }],
  ["path", { d: "M8 12H3", key: "a7s4jb" }]
], ex = se("sliders-horizontal", $g);
const tx = [
  ["circle", { cx: "12", cy: "12", r: "4", key: "4exip2" }],
  ["path", { d: "M12 2v2", key: "tus03m" }],
  ["path", { d: "M12 20v2", key: "1lh1kg" }],
  ["path", { d: "m4.93 4.93 1.41 1.41", key: "149t6j" }],
  ["path", { d: "m17.66 17.66 1.41 1.41", key: "ptbguv" }],
  ["path", { d: "M2 12h2", key: "1t8f8n" }],
  ["path", { d: "M20 12h2", key: "1q8mjw" }],
  ["path", { d: "m6.34 17.66-1.41 1.41", key: "1m8zz5" }],
  ["path", { d: "m19.07 4.93-1.41 1.41", key: "1shlcs" }]
], _n = se("sun", tx);
const nx = [
  [
    "path",
    {
      d: "m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3",
      key: "wmoenq"
    }
  ],
  ["path", { d: "M12 9v4", key: "juzpu7" }],
  ["path", { d: "M12 17h.01", key: "p32p05" }]
], rx = se("triangle-alert", nx);
const ox = [
  ["path", { d: "M18 6 6 18", key: "1bl5f8" }],
  ["path", { d: "m6 6 12 12", key: "d8bk6v" }]
], wn = se("x", ox);
function rs({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Ci, { "data-slot": "dialog", ...e });
}
function nw({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Ni, { "data-slot": "dialog-trigger", ...e });
}
function sx({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Ii, { "data-slot": "dialog-portal", ...e });
}
function rw({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(mn, { "data-slot": "dialog-close", ...e });
}
function ax({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Si,
    {
      "data-slot": "dialog-overlay",
      className: H(
        "fixed inset-0 z-50 bg-black/50 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0",
        e
      ),
      ...t
    }
  );
}
function os({
  className: e,
  children: t,
  showCloseButton: n = !0,
  ...r
}) {
  return /* @__PURE__ */ a.jsxs(sx, { "data-slot": "dialog-portal", children: [
    /* @__PURE__ */ a.jsx(ax, {}),
    /* @__PURE__ */ a.jsxs(
      ki,
      {
        "data-slot": "dialog-content",
        className: H(
          "fixed top-[50%] left-[50%] z-50 grid max-h-[calc(100vh-1rem)] w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border bg-background p-6 shadow-lg duration-200 outline-none data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 sm:max-w-lg",
          e
        ),
        ...r,
        children: [
          t,
          n && /* @__PURE__ */ a.jsxs(
            mn,
            {
              "data-slot": "dialog-close",
              className: "absolute top-4 right-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
              children: [
                /* @__PURE__ */ a.jsx(wn, {}),
                /* @__PURE__ */ a.jsx("span", { className: "sr-only", children: "Close" })
              ]
            }
          )
        ]
      }
    )
  ] });
}
function ss({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "dialog-header",
      className: H("flex flex-col gap-2 text-center sm:text-left", e),
      ...t
    }
  );
}
function as({
  className: e,
  showCloseButton: t = !1,
  children: n,
  ...r
}) {
  return /* @__PURE__ */ a.jsxs(
    "div",
    {
      "data-slot": "dialog-footer",
      className: H(
        "shrink-0 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end",
        e
      ),
      ...r,
      children: [
        n,
        t && /* @__PURE__ */ a.jsx(mn, { asChild: !0, children: /* @__PURE__ */ a.jsx(te, { variant: "outline", children: "Close" }) })
      ]
    }
  );
}
function is({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Di,
    {
      "data-slot": "dialog-title",
      className: H("text-lg leading-none font-semibold", e),
      ...t
    }
  );
}
function cs({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Ri,
    {
      "data-slot": "dialog-description",
      className: H("text-sm text-muted-foreground", e),
      ...t
    }
  );
}
const $n = "auth.user";
function Dl() {
  try {
    const e = localStorage.getItem($n);
    return e ? JSON.parse(e) : null;
  } catch {
    return localStorage.removeItem($n), null;
  }
}
function ls() {
  localStorage.removeItem($n);
}
function Rl(e) {
  localStorage.setItem($n, JSON.stringify(e));
}
let Pn = null, On = null;
const ix = 429, cx = 1, lo = "auth:session-expired";
function lx(e) {
  if (typeof e == "string")
    return e;
  if (e && typeof e == "object") {
    const t = e, n = t.message ?? t.detail ?? t.error;
    if (typeof n == "string")
      return n;
  }
  return "";
}
async function ux(e) {
  const t = e.clone(), n = t.headers.get("content-type") || "";
  try {
    if (n.includes("application/json")) {
      const r = await t.json();
      return lx(r);
    }
    return (await t.text()).trim();
  } catch {
    return "";
  }
}
function dx(e) {
  if (!e)
    return null;
  const t = Number(e);
  if (Number.isFinite(t) && t >= 0)
    return t * 1e3;
  const n = Date.parse(e);
  return Number.isNaN(n) ? null : Math.max(n - Date.now(), 0);
}
function fx(e) {
  return new Promise((t) => {
    window.setTimeout(t, e);
  });
}
async function An(e, t, n = cx) {
  const r = await fetch(e, {
    ...t,
    cache: t?.cache ?? "no-store"
  });
  if (r.status !== ix || n <= 0)
    return r;
  const o = dx(r.headers.get("retry-after"));
  return o === null ? r : (await fx(o), An(e, t, n - 1));
}
async function mx(e) {
  if (e.status !== 401)
    return !1;
  const t = (await ux(e)).toLowerCase();
  return t.includes("token") && t.includes("expired");
}
function hx() {
  const e = Dl();
  if (!e)
    return null;
  const t = e.username ?? e.email ?? e.sub;
  return typeof t == "string" && t.trim() ? t.trim() : null;
}
async function px() {
  return Pn || (Pn = (async () => {
    const e = hx();
    return e ? !!(await An(nt("refresh"), {
      method: "POST",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
        username: e
      },
      credentials: "include"
    })).ok : !1;
  })().finally(() => {
    Pn = null;
  })), Pn;
}
async function gx() {
  return On || (On = (async () => {
    try {
      await An(nt("logout"), {
        method: "GET",
        headers: {
          Accept: "application/json"
        },
        credentials: "include"
      });
    } catch {
    } finally {
      window.dispatchEvent(new Event(lo));
    }
  })().finally(() => {
    On = null;
  })), On;
}
async function at(e, t = {}) {
  const { skipRefresh: n = !1, ...r } = t, o = await An(e, r);
  return n || !await mx(o) ? o : await px() ? at(e, { ...r, skipRefresh: !0 }) : (await gx(), o);
}
const xx = {
  error: {
    container: "border-destructive/35 bg-destructive/10 text-destructive dark:border-destructive/30 dark:bg-destructive/12",
    icon: Sl
  },
  success: {
    container: "border-emerald-500/35 bg-emerald-500/10 text-emerald-700 dark:border-emerald-400/30 dark:bg-emerald-400/12 dark:text-emerald-300",
    icon: Tg
  },
  info: {
    container: "border-sky-500/35 bg-sky-500/10 text-sky-700 dark:border-sky-400/30 dark:bg-sky-400/12 dark:text-sky-300",
    icon: Zg
  }
};
function tn({
  children: e,
  className: t,
  variant: n = "info"
}) {
  const { container: r, icon: o } = xx[n];
  return /* @__PURE__ */ a.jsxs(
    "div",
    {
      role: n === "error" ? "alert" : "status",
      className: H(
        "flex items-start gap-3 rounded-xl border px-3 py-2 text-sm",
        r,
        t
      ),
      children: [
        /* @__PURE__ */ a.jsx(o, { className: "mt-0.5 size-4 shrink-0" }),
        /* @__PURE__ */ a.jsx("div", { children: e })
      ]
    }
  );
}
function vx({ open: e, handleClose: t }) {
  const [n, r] = h.useState(!1), [o, s] = h.useState(""), i = () => {
    s(""), t();
  }, c = (l) => {
    l || i();
  }, u = async (l) => {
    s("");
    const d = new FormData(l).get("username");
    if (!d || typeof d != "string") {
      s("Username is required.");
      return;
    }
    r(!0);
    try {
      const m = new URLSearchParams();
      m.append("username", d);
      const p = await at(nt("resetPassword"), {
        method: "POST",
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          Accept: "application/json"
        },
        credentials: "include",
        body: m.toString()
      });
      if (!p.ok)
        throw new Error(`Reset password failed: ${p.status} ${p.statusText}`);
      i();
    } catch (m) {
      s(
        m instanceof Error ? m.message : "Failed to request password reset. Please try again."
      );
    } finally {
      r(!1);
    }
  };
  return /* @__PURE__ */ a.jsx(rs, { open: e, onOpenChange: c, children: /* @__PURE__ */ a.jsx(os, { className: "flex max-h-[92vh] w-[min(97vw,38rem)] max-w-none flex-col overflow-hidden border-border/70 bg-card/95 p-0", children: /* @__PURE__ */ a.jsxs(
    "form",
    {
      className: "flex min-h-0 flex-1 flex-col",
      onSubmit: (l) => {
        l.preventDefault(), l.stopPropagation(), u(l.currentTarget);
      },
      children: [
        /* @__PURE__ */ a.jsxs(ss, { className: "border-b border-border/70 px-6 py-4 text-left", children: [
          /* @__PURE__ */ a.jsx(is, { children: "Reset password" }),
          /* @__PURE__ */ a.jsx(cs, { children: "Enter your account's username, and we'll send you a link to reset your password." })
        ] }),
        /* @__PURE__ */ a.jsx("div", { className: "min-h-0 flex-1 overflow-y-auto px-6 py-5", children: /* @__PURE__ */ a.jsxs("div", { className: "space-y-4", children: [
          o && /* @__PURE__ */ a.jsx(tn, { variant: "error", children: o }),
          /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
            /* @__PURE__ */ a.jsx(Nt, { htmlFor: "forgot-password-username", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
              /* @__PURE__ */ a.jsx("span", { children: "Username" }),
              /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
            ] }) }),
            /* @__PURE__ */ a.jsx(
              ut,
              {
                autoFocus: !0,
                required: !0,
                id: "forgot-password-username",
                name: "username",
                placeholder: "username",
                type: "text",
                disabled: n
              }
            )
          ] })
        ] }) }),
        /* @__PURE__ */ a.jsxs(as, { className: "gap-2 border-t border-border/70 px-6 py-4 sm:justify-between", children: [
          /* @__PURE__ */ a.jsx(
            te,
            {
              type: "button",
              variant: "outline",
              onClick: i,
              disabled: n,
              children: "Cancel"
            }
          ),
          /* @__PURE__ */ a.jsx(te, { type: "submit", disabled: n, children: n ? "Sending..." : "Continue" })
        ] })
      ]
    }
  ) }) });
}
const Pl = "app-color-mode", Ol = h.createContext({
  mode: "system",
  resolvedMode: "light",
  systemMode: "light",
  setMode: () => {
  }
});
function bx() {
  if (typeof window > "u")
    return "system";
  const e = window.localStorage.getItem(Pl);
  return e === "light" || e === "dark" || e === "system" ? e : "system";
}
function wx() {
  return typeof window > "u" ? "light" : window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
}
function Ax(e) {
  if (typeof document > "u")
    return;
  const t = document.documentElement;
  t.classList.toggle("dark", e === "dark"), t.dataset.theme = e, t.style.colorScheme = e;
}
function zl() {
  return h.useContext(Ol);
}
function Bl({ children: e, disableCustomTheme: t }) {
  const [n, r] = h.useState(() => bx()), [o, s] = h.useState(() => wx()), i = n === "system" ? o : n;
  h.useEffect(() => {
    if (typeof window > "u")
      return;
    const u = window.matchMedia("(prefers-color-scheme: dark)"), l = (f) => {
      s(f.matches ? "dark" : "light");
    };
    return s(u.matches ? "dark" : "light"), u.addEventListener("change", l), () => {
      u.removeEventListener("change", l);
    };
  }, []), h.useEffect(() => {
    typeof window > "u" || window.localStorage.setItem(Pl, n);
  }, [n]), h.useEffect(() => {
    t || Ax(i);
  }, [t, i]);
  const c = h.useMemo(() => ({
    mode: n,
    resolvedMode: i,
    systemMode: o,
    setMode: r
  }), [n, i, o]);
  return /* @__PURE__ */ a.jsx(Ol.Provider, { value: c, children: e });
}
function us({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(ep, { "data-slot": "dropdown-menu", ...e });
}
function ow({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Xc, { "data-slot": "dropdown-menu-portal", ...e });
}
function ds({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(
    tp,
    {
      "data-slot": "dropdown-menu-trigger",
      ...e
    }
  );
}
function fs({
  className: e,
  sideOffset: t = 4,
  ...n
}) {
  return /* @__PURE__ */ a.jsx(Xc, { children: /* @__PURE__ */ a.jsx(
    np,
    {
      "data-slot": "dropdown-menu-content",
      sideOffset: t,
      className: H(
        "z-50 max-h-(--radix-dropdown-menu-content-available-height) min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-md border bg-popover p-1 text-popover-foreground shadow-md data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
        e
      ),
      ...n
    }
  ) });
}
function sw({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(rp, { "data-slot": "dropdown-menu-group", ...e });
}
function aw({
  className: e,
  inset: t,
  variant: n = "default",
  ...r
}) {
  return /* @__PURE__ */ a.jsx(
    sp,
    {
      "data-slot": "dropdown-menu-item",
      "data-inset": t,
      "data-variant": n,
      className: H(
        "relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[inset]:pl-8 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive dark:data-[variant=destructive]:focus:bg-destructive/20 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground data-[variant=destructive]:*:[svg]:text-destructive!",
        e
      ),
      ...r
    }
  );
}
function iw({
  className: e,
  children: t,
  checked: n,
  ...r
}) {
  return /* @__PURE__ */ a.jsxs(
    ap,
    {
      "data-slot": "dropdown-menu-checkbox-item",
      className: H(
        "relative flex cursor-default items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        e
      ),
      checked: n,
      ...r,
      children: [
        /* @__PURE__ */ a.jsx("span", { className: "pointer-events-none absolute left-2 flex size-3.5 items-center justify-center", children: /* @__PURE__ */ a.jsx(Kc, { children: /* @__PURE__ */ a.jsx(ts, { className: "size-4" }) }) }),
        t
      ]
    }
  );
}
function Tl({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(
    ip,
    {
      "data-slot": "dropdown-menu-radio-group",
      ...e
    }
  );
}
function St({
  className: e,
  children: t,
  ...n
}) {
  return /* @__PURE__ */ a.jsxs(
    cp,
    {
      "data-slot": "dropdown-menu-radio-item",
      className: H(
        "relative flex cursor-default items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        e
      ),
      ...n,
      children: [
        /* @__PURE__ */ a.jsx("span", { className: "pointer-events-none absolute left-2 flex size-3.5 items-center justify-center", children: /* @__PURE__ */ a.jsx(Kc, { children: /* @__PURE__ */ a.jsx(Yg, { className: "size-2 fill-current" }) }) }),
        t
      ]
    }
  );
}
function yx({
  className: e,
  inset: t,
  ...n
}) {
  return /* @__PURE__ */ a.jsx(
    op,
    {
      "data-slot": "dropdown-menu-label",
      "data-inset": t,
      className: H(
        "px-2 py-1.5 text-sm font-medium data-[inset]:pl-8",
        e
      ),
      ...n
    }
  );
}
function Ex({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    lp,
    {
      "data-slot": "dropdown-menu-separator",
      className: H("-mx-1 my-1 h-px bg-border", e),
      ...t
    }
  );
}
function cw({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    "span",
    {
      "data-slot": "dropdown-menu-shortcut",
      className: H(
        "ml-auto text-xs tracking-widest text-muted-foreground",
        e
      ),
      ...t
    }
  );
}
function lw({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(up, { "data-slot": "dropdown-menu-sub", ...e });
}
function uw({
  className: e,
  inset: t,
  children: n,
  ...r
}) {
  return /* @__PURE__ */ a.jsxs(
    dp,
    {
      "data-slot": "dropdown-menu-sub-trigger",
      "data-inset": t,
      className: H(
        "flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground data-[inset]:pl-8 data-[state=open]:bg-accent data-[state=open]:text-accent-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 [&_svg:not([class*='text-'])]:text-muted-foreground",
        e
      ),
      ...r,
      children: [
        n,
        /* @__PURE__ */ a.jsx(It, { className: "ml-auto size-4" })
      ]
    }
  );
}
function dw({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    fp,
    {
      "data-slot": "dropdown-menu-sub-content",
      className: H(
        "z-50 min-w-[8rem] origin-(--radix-dropdown-menu-content-transform-origin) overflow-hidden rounded-md border bg-popover p-1 text-popover-foreground shadow-lg data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
        e
      ),
      ...t
    }
  );
}
function Gl(e) {
  const { mode: t, setMode: n } = zl(), { className: r, ...o } = e, s = {
    system: "System",
    light: "Light",
    dark: "Dark"
  }, i = {
    system: /* @__PURE__ */ a.jsx(co, { className: "size-4" }),
    light: /* @__PURE__ */ a.jsx(_n, { className: "size-4" }),
    dark: /* @__PURE__ */ a.jsx(Kn, { className: "size-4" })
  };
  return /* @__PURE__ */ a.jsxs(us, { children: [
    /* @__PURE__ */ a.jsx(ds, { asChild: !0, children: /* @__PURE__ */ a.jsxs(
      te,
      {
        "data-screenshot": "toggle-mode",
        variant: "outline",
        size: "sm",
        className: H(
          "rounded-full border-border/70 bg-background/85 px-3 shadow-sm backdrop-blur",
          r
        ),
        ...o,
        children: [
          i[t],
          /* @__PURE__ */ a.jsx("span", { className: "hidden sm:inline", children: s[t] })
        ]
      }
    ) }),
    /* @__PURE__ */ a.jsx(fs, { align: "end", className: "w-36", children: /* @__PURE__ */ a.jsxs(
      Tl,
      {
        value: t,
        onValueChange: (c) => n(c),
        children: [
          /* @__PURE__ */ a.jsxs(St, { value: "system", children: [
            /* @__PURE__ */ a.jsx(co, { className: "mr-2 size-4" }),
            "System"
          ] }),
          /* @__PURE__ */ a.jsxs(St, { value: "light", children: [
            /* @__PURE__ */ a.jsx(_n, { className: "mr-2 size-4" }),
            "Light"
          ] }),
          /* @__PURE__ */ a.jsxs(St, { value: "dark", children: [
            /* @__PURE__ */ a.jsx(Kn, { className: "mr-2 size-4" }),
            "Dark"
          ] })
        ]
      }
    ) })
  ] });
}
const Mx = "data:image/png;base64,";
function Yl() {
  return /* @__PURE__ */ a.jsx("img", { src: Mx, width: "150", alt: "Sitemark" });
}
var Yn = { exports: {} }, jx = Yn.exports, wa;
function Cx() {
  return wa || (wa = 1, (function(e, t) {
    (function(n, r) {
      e.exports = r();
    })(jx, (function() {
      var n = 1e3, r = 6e4, o = 36e5, s = "millisecond", i = "second", c = "minute", u = "hour", l = "day", f = "week", d = "month", m = "quarter", p = "year", x = "date", g = "Invalid Date", v = /^(\d{4})[-/]?(\d{1,2})?[-/]?(\d{0,2})[Tt\s]*(\d{1,2})?:?(\d{1,2})?:?(\d{1,2})?[.:]?(\d+)?$/, b = /\[([^\]]+)]|Y{1,4}|M{1,4}|D{1,2}|d{1,4}|H{1,2}|h{1,2}|a|A|m{1,2}|s{1,2}|Z{1,2}|SSS/g, A = { name: "en", weekdays: "Sunday_Monday_Tuesday_Wednesday_Thursday_Friday_Saturday".split("_"), months: "January_February_March_April_May_June_July_August_September_October_November_December".split("_"), ordinal: function(T) {
        var R = ["th", "st", "nd", "rd"], j = T % 100;
        return "[" + T + (R[(j - 20) % 10] || R[j] || R[0]) + "]";
      } }, w = function(T, R, j) {
        var O = String(T);
        return !O || O.length >= R ? T : "" + Array(R + 1 - O.length).join(j) + T;
      }, E = { s: w, z: function(T) {
        var R = -T.utcOffset(), j = Math.abs(R), O = Math.floor(j / 60), N = j % 60;
        return (R <= 0 ? "+" : "-") + w(O, 2, "0") + ":" + w(N, 2, "0");
      }, m: function T(R, j) {
        if (R.date() < j.date()) return -T(j, R);
        var O = 12 * (j.year() - R.year()) + (j.month() - R.month()), N = R.clone().add(O, d), S = j - N < 0, I = R.clone().add(O + (S ? -1 : 1), d);
        return +(-(O + (j - N) / (S ? N - I : I - N)) || 0);
      }, a: function(T) {
        return T < 0 ? Math.ceil(T) || 0 : Math.floor(T);
      }, p: function(T) {
        return { M: d, y: p, w: f, d: l, D: x, h: u, m: c, s: i, ms: s, Q: m }[T] || String(T || "").toLowerCase().replace(/s$/, "");
      }, u: function(T) {
        return T === void 0;
      } }, C = "en", P = {};
      P[C] = A;
      var D = "$isDayjsObject", M = function(T) {
        return T instanceof Z || !(!T || !T[D]);
      }, k = function T(R, j, O) {
        var N;
        if (!R) return C;
        if (typeof R == "string") {
          var S = R.toLowerCase();
          P[S] && (N = S), j && (P[S] = j, N = S);
          var I = R.split("-");
          if (!N && I.length > 1) return T(I[0]);
        } else {
          var y = R.name;
          P[y] = R, N = y;
        }
        return !O && N && (C = N), N || !O && C;
      }, Y = function(T, R) {
        if (M(T)) return T.clone();
        var j = typeof R == "object" ? R : {};
        return j.date = T, j.args = arguments, new Z(j);
      }, z = E;
      z.l = k, z.i = M, z.w = function(T, R) {
        return Y(T, { locale: R.$L, utc: R.$u, x: R.$x, $offset: R.$offset });
      };
      var Z = (function() {
        function T(j) {
          this.$L = k(j.locale, null, !0), this.parse(j), this.$x = this.$x || j.x || {}, this[D] = !0;
        }
        var R = T.prototype;
        return R.parse = function(j) {
          this.$d = (function(O) {
            var N = O.date, S = O.utc;
            if (N === null) return /* @__PURE__ */ new Date(NaN);
            if (z.u(N)) return /* @__PURE__ */ new Date();
            if (N instanceof Date) return new Date(N);
            if (typeof N == "string" && !/Z$/i.test(N)) {
              var I = N.match(v);
              if (I) {
                var y = I[2] - 1 || 0, G = (I[7] || "0").substring(0, 3);
                return S ? new Date(Date.UTC(I[1], y, I[3] || 1, I[4] || 0, I[5] || 0, I[6] || 0, G)) : new Date(I[1], y, I[3] || 1, I[4] || 0, I[5] || 0, I[6] || 0, G);
              }
            }
            return new Date(N);
          })(j), this.init();
        }, R.init = function() {
          var j = this.$d;
          this.$y = j.getFullYear(), this.$M = j.getMonth(), this.$D = j.getDate(), this.$W = j.getDay(), this.$H = j.getHours(), this.$m = j.getMinutes(), this.$s = j.getSeconds(), this.$ms = j.getMilliseconds();
        }, R.$utils = function() {
          return z;
        }, R.isValid = function() {
          return this.$d.toString() !== g;
        }, R.isSame = function(j, O) {
          var N = Y(j);
          return this.startOf(O) <= N && N <= this.endOf(O);
        }, R.isAfter = function(j, O) {
          return Y(j) < this.startOf(O);
        }, R.isBefore = function(j, O) {
          return this.endOf(O) < Y(j);
        }, R.$g = function(j, O, N) {
          return z.u(j) ? this[O] : this.set(N, j);
        }, R.unix = function() {
          return Math.floor(this.valueOf() / 1e3);
        }, R.valueOf = function() {
          return this.$d.getTime();
        }, R.startOf = function(j, O) {
          var N = this, S = !!z.u(O) || O, I = z.p(j), y = function(ee, ne) {
            var le = z.w(N.$u ? Date.UTC(N.$y, ne, ee) : new Date(N.$y, ne, ee), N);
            return S ? le : le.endOf(l);
          }, G = function(ee, ne) {
            return z.w(N.toDate()[ee].apply(N.toDate("s"), (S ? [0, 0, 0, 0] : [23, 59, 59, 999]).slice(ne)), N);
          }, Q = this.$W, W = this.$M, X = this.$D, _ = "set" + (this.$u ? "UTC" : "");
          switch (I) {
            case p:
              return S ? y(1, 0) : y(31, 11);
            case d:
              return S ? y(1, W) : y(0, W + 1);
            case f:
              var J = this.$locale().weekStart || 0, oe = (Q < J ? Q + 7 : Q) - J;
              return y(S ? X - oe : X + (6 - oe), W);
            case l:
            case x:
              return G(_ + "Hours", 0);
            case u:
              return G(_ + "Minutes", 1);
            case c:
              return G(_ + "Seconds", 2);
            case i:
              return G(_ + "Milliseconds", 3);
            default:
              return this.clone();
          }
        }, R.endOf = function(j) {
          return this.startOf(j, !1);
        }, R.$set = function(j, O) {
          var N, S = z.p(j), I = "set" + (this.$u ? "UTC" : ""), y = (N = {}, N[l] = I + "Date", N[x] = I + "Date", N[d] = I + "Month", N[p] = I + "FullYear", N[u] = I + "Hours", N[c] = I + "Minutes", N[i] = I + "Seconds", N[s] = I + "Milliseconds", N)[S], G = S === l ? this.$D + (O - this.$W) : O;
          if (S === d || S === p) {
            var Q = this.clone().set(x, 1);
            Q.$d[y](G), Q.init(), this.$d = Q.set(x, Math.min(this.$D, Q.daysInMonth())).$d;
          } else y && this.$d[y](G);
          return this.init(), this;
        }, R.set = function(j, O) {
          return this.clone().$set(j, O);
        }, R.get = function(j) {
          return this[z.p(j)]();
        }, R.add = function(j, O) {
          var N, S = this;
          j = Number(j);
          var I = z.p(O), y = function(W) {
            var X = Y(S);
            return z.w(X.date(X.date() + Math.round(W * j)), S);
          };
          if (I === d) return this.set(d, this.$M + j);
          if (I === p) return this.set(p, this.$y + j);
          if (I === l) return y(1);
          if (I === f) return y(7);
          var G = (N = {}, N[c] = r, N[u] = o, N[i] = n, N)[I] || 1, Q = this.$d.getTime() + j * G;
          return z.w(Q, this);
        }, R.subtract = function(j, O) {
          return this.add(-1 * j, O);
        }, R.format = function(j) {
          var O = this, N = this.$locale();
          if (!this.isValid()) return N.invalidDate || g;
          var S = j || "YYYY-MM-DDTHH:mm:ssZ", I = z.z(this), y = this.$H, G = this.$m, Q = this.$M, W = N.weekdays, X = N.months, _ = N.meridiem, J = function(ne, le, he, fe) {
            return ne && (ne[le] || ne(O, S)) || he[le].slice(0, fe);
          }, oe = function(ne) {
            return z.s(y % 12 || 12, ne, "0");
          }, ee = _ || function(ne, le, he) {
            var fe = ne < 12 ? "AM" : "PM";
            return he ? fe.toLowerCase() : fe;
          };
          return S.replace(b, (function(ne, le) {
            return le || (function(he) {
              switch (he) {
                case "YY":
                  return String(O.$y).slice(-2);
                case "YYYY":
                  return z.s(O.$y, 4, "0");
                case "M":
                  return Q + 1;
                case "MM":
                  return z.s(Q + 1, 2, "0");
                case "MMM":
                  return J(N.monthsShort, Q, X, 3);
                case "MMMM":
                  return J(X, Q);
                case "D":
                  return O.$D;
                case "DD":
                  return z.s(O.$D, 2, "0");
                case "d":
                  return String(O.$W);
                case "dd":
                  return J(N.weekdaysMin, O.$W, W, 2);
                case "ddd":
                  return J(N.weekdaysShort, O.$W, W, 3);
                case "dddd":
                  return W[O.$W];
                case "H":
                  return String(y);
                case "HH":
                  return z.s(y, 2, "0");
                case "h":
                  return oe(1);
                case "hh":
                  return oe(2);
                case "a":
                  return ee(y, G, !0);
                case "A":
                  return ee(y, G, !1);
                case "m":
                  return String(G);
                case "mm":
                  return z.s(G, 2, "0");
                case "s":
                  return String(O.$s);
                case "ss":
                  return z.s(O.$s, 2, "0");
                case "SSS":
                  return z.s(O.$ms, 3, "0");
                case "Z":
                  return I;
              }
              return null;
            })(ne) || I.replace(":", "");
          }));
        }, R.utcOffset = function() {
          return 15 * -Math.round(this.$d.getTimezoneOffset() / 15);
        }, R.diff = function(j, O, N) {
          var S, I = this, y = z.p(O), G = Y(j), Q = (G.utcOffset() - this.utcOffset()) * r, W = this - G, X = function() {
            return z.m(I, G);
          };
          switch (y) {
            case p:
              S = X() / 12;
              break;
            case d:
              S = X();
              break;
            case m:
              S = X() / 3;
              break;
            case f:
              S = (W - Q) / 6048e5;
              break;
            case l:
              S = (W - Q) / 864e5;
              break;
            case u:
              S = W / o;
              break;
            case c:
              S = W / r;
              break;
            case i:
              S = W / n;
              break;
            default:
              S = W;
          }
          return N ? S : z.a(S);
        }, R.daysInMonth = function() {
          return this.endOf(d).$D;
        }, R.$locale = function() {
          return P[this.$L];
        }, R.locale = function(j, O) {
          if (!j) return this.$L;
          var N = this.clone(), S = k(j, O, !0);
          return S && (N.$L = S), N;
        }, R.clone = function() {
          return z.w(this.$d, this);
        }, R.toDate = function() {
          return new Date(this.valueOf());
        }, R.toJSON = function() {
          return this.isValid() ? this.toISOString() : null;
        }, R.toISOString = function() {
          return this.$d.toISOString();
        }, R.toString = function() {
          return this.$d.toUTCString();
        }, T;
      })(), q = Z.prototype;
      return Y.prototype = q, [["$ms", s], ["$s", i], ["$m", c], ["$H", u], ["$W", l], ["$M", d], ["$y", p], ["$D", x]].forEach((function(T) {
        q[T[1]] = function(R) {
          return this.$g(R, T[0], T[1]);
        };
      })), Y.extend = function(T, R) {
        return T.$i || (T(R, Z, Y), T.$i = !0), Y;
      }, Y.locale = k, Y.isDayjs = M, Y.unix = function(T) {
        return Y(1e3 * T);
      }, Y.en = P[C], Y.Ls = P, Y.p = {}, Y;
    }));
  })(Yn)), Yn.exports;
}
var Nx = Cx();
const nn = /* @__PURE__ */ Zu(Nx), Hl = "YYYY-MM-DDTHH:mm:ss.SSSZ", Ix = "YYYY-MM-DDTHH:mm", Aa = "HH:mm";
function Sx(e) {
  const t = e.getFullYear(), n = String(e.getMonth() + 1).padStart(2, "0"), r = String(e.getDate()).padStart(2, "0"), o = String(e.getHours()).padStart(2, "0"), s = String(e.getMinutes()).padStart(2, "0"), i = String(e.getSeconds()).padStart(2, "0"), c = String(e.getMilliseconds()).padStart(3, "0"), u = -e.getTimezoneOffset(), l = u >= 0 ? "+" : "-", f = Math.abs(u), d = String(Math.floor(f / 60)).padStart(2, "0"), m = String(f % 60).padStart(2, "0");
  return `${t}-${n}-${r}T${o}:${s}:${i}.${c}${l}${d}:${m}`;
}
function kx(e) {
  const t = new Date(e);
  return Number.isNaN(t.getTime()) ? void 0 : t;
}
function Dx(e) {
  const t = e.trim(), n = t.match(
    /^(\d{1,2}):(\d{2})(?::(\d{2}))?\s*([AaPp][Mm])$/
  );
  if (n) {
    const [, u, l, f, d] = n, m = Number(u), p = Number(l), x = f ? Number(f) : 0;
    if (m < 1 || m > 12 || p < 0 || p > 59 || x < 0 || x > 59)
      return;
    const g = d.toLowerCase() === "pm" ? m % 12 + 12 : m % 12;
    return `${String(g).padStart(2, "0")}:${l}`;
  }
  const r = t.match(/^(\d{1,2}):(\d{2})(?::\d{2})?$/);
  if (!r)
    return;
  const [, o, s] = r, i = Number(o), c = Number(s);
  if (!(i < 0 || i > 23 || c < 0 || c > 59))
    return `${String(i).padStart(2, "0")}:${s}`;
}
function fw(e) {
  return e ? new Date(e).toLocaleDateString() : "-";
}
function mw(e) {
  if (!e)
    return "-";
  const t = Intl.DateTimeFormat().resolvedOptions().timeZone;
  return new Date(e).toLocaleString(void 0, { timeZone: t });
}
function hw(e) {
  const t = e?.trim();
  if (!t)
    return;
  const n = nn(t);
  if (n.isValid())
    return n.format(Hl);
  const r = kx(t);
  return r ? Sx(r) : void 0;
}
function pw(e, t) {
  const n = e?.trim();
  if (!n)
    return;
  const r = t === "start" ? nn(n).startOf("day") : nn(n).endOf("day");
  return r.isValid() ? r.format(Hl) : void 0;
}
function gw(e) {
  const t = e?.trim();
  if (!t)
    return "";
  const n = nn(t);
  return n.isValid() ? n.format(Ix) : "";
}
function xw(e) {
  const t = e?.trim();
  if (!t)
    return;
  const n = nn(t, Aa);
  return n.isValid() ? n.format(Aa) : Dx(t);
}
function vw(e, t = 2, n = 8) {
  const r = e ?? "", o = r.split(`
`).length, s = Math.ceil(r.length / 70);
  return Math.min(n, Math.max(t, o, s));
}
function Rx(e) {
  if (!e || typeof e != "object")
    return null;
  const t = e, n = t.name, r = t.username, o = t.sub, s = t.account_type;
  return typeof n != "string" || typeof r != "string" || typeof o != "string" || typeof s != "string" ? null : {
    ...t,
    name: n,
    username: r,
    sub: o,
    account_type: s
  };
}
function Px(e) {
  if (!e || typeof e != "object")
    return e;
  const t = e;
  if (t.user)
    return t.user;
  if (t.data && typeof t.data == "object") {
    const n = t.data;
    if (n.user)
      return n.user;
  }
  return e;
}
async function Ox(e) {
  const t = new URLSearchParams();
  t.append("grant_type", "password"), t.append("username", e.username), t.append("password", e.password);
  const n = await An(nt("login"), {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
      Accept: "application/json"
    },
    credentials: "include",
    body: t.toString()
  });
  if (!n.ok)
    throw new Error(
      `Login failed: ${n.status} ${n.statusText}`
    );
  return ms();
}
async function zx(e) {
  const t = new URLSearchParams();
  t.append("credential", e);
  const n = await at(
    `${nt("authenticateGoogle")}?${t.toString()}`,
    {
      method: "GET",
      headers: {
        Accept: "application/json"
      },
      credentials: "include"
    }
  );
  if (!n.ok)
    throw new Error(
      `Google login failed: ${n.status} ${n.statusText}`
    );
  return ms();
}
async function Bx() {
  const e = await at(nt("logout"), {
    method: "GET",
    headers: {
      Accept: "application/json"
    },
    credentials: "include"
  });
  if (!e.ok)
    throw new Error(
      `Logout failed: ${e.status} ${e.statusText}`
    );
  ls();
}
async function ms() {
  const e = await at(nt("currentUser"), {
    method: "GET",
    headers: {
      Accept: "application/json"
    },
    credentials: "include"
  });
  if (!e.ok)
    throw new Error(
      `Failed to fetch current user: ${e.status} ${e.statusText}`
    );
  const t = await e.json(), n = Rx(Px(t));
  if (!n)
    throw new Error("Current user response did not contain a valid user");
  return Rl(n), n;
}
async function Tx(e) {
  const t = new URLSearchParams();
  t.append("token", e.token), t.append("username", e.username), t.append("password", e.password);
  const n = await at(
    nt("resetPasswordAuthorised"),
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
        Accept: "application/json"
      },
      credentials: "include",
      body: t.toString()
    }
  );
  if (!n.ok)
    throw new Error(
      `Password reset failed: ${n.status} ${n.statusText}`
    );
}
const Fl = zt(void 0);
function ya() {
  localStorage.clear(), ls();
}
function bw({ children: e }) {
  const [t, n] = de(null), [r, o] = de(!0);
  ae(() => {
    let u = !0;
    return (async () => {
      const f = Dl();
      f && n(f);
      try {
        const d = await ms();
        if (!u)
          return;
        n(d);
      } catch {
        if (!u)
          return;
        n(null), ls();
      } finally {
        u && o(!1);
      }
    })(), () => {
      u = !1;
    };
  }, []), ae(() => {
    const u = () => {
      ya(), n(null);
    };
    return window.addEventListener(lo, u), () => {
      window.removeEventListener(
        lo,
        u
      );
    };
  }, []);
  const s = async (u) => {
    try {
      const l = await Ox(u);
      n(l);
    } catch (l) {
      throw console.error("Login failed:", l), l;
    }
  }, i = (u) => {
    Rl(u), n(u);
  }, c = async () => {
    try {
      await Bx();
    } catch (u) {
      console.error("Logout API failed:", u);
    } finally {
      ya(), n(null);
    }
  };
  return /* @__PURE__ */ a.jsx(
    Fl.Provider,
    {
      value: {
        user: t,
        isLoading: r,
        isAuthenticated: !!t,
        login: s,
        setAuthenticatedUser: i,
        logout: c
      },
      children: e
    }
  );
}
function Ll() {
  const e = gt(Fl);
  if (e === void 0)
    throw new Error("useAuth must be used within an AuthProvider");
  return e;
}
const Gx = {};
function ww(e) {
  const { login: t, setAuthenticatedUser: n } = Ll(), [r, o] = h.useState(""), [s, i] = h.useState(""), [c, u] = h.useState(""), [l, f] = h.useState(""), [d, m] = h.useState(""), [p, x] = h.useState(""), [g, v] = h.useState(!1), [b, A] = h.useState(!1);
  h.useEffect(() => {
    e.authNotice && x(e.authNotice);
  }, [e.authNotice]), h.useEffect(() => {
    const M = e.googleClientId || (typeof import.meta < "u" ? Gx?.VITE_GOOGLE_CLIENT_ID : void 0) || "";
    if (!M)
      return;
    const k = document.createElement("script");
    k.src = "https://accounts.google.com/gsi/client", k.async = !0, k.defer = !0, document.head.appendChild(k), k.onload = () => {
      window.google?.accounts?.id && (window.google.accounts.id.initialize({
        client_id: M,
        callback: C,
        use_fedcm_for_prompt: !0,
        auto_select: !1,
        context: "signin"
      }), window.google.accounts.id.renderButton(
        document.getElementById("google-btn"),
        {
          theme: "outline",
          size: "large",
          text: "continue_with"
        }
      ), window.google.accounts.id.prompt((Y) => {
        (Y.isNotDisplayed() || Y.isSkippedMoment()) && console.log("Google prompt status:", Y.getNotDisplayedReason());
      }));
    };
  }, [e.googleClientId]);
  const w = (M) => {
    M.preventDefault(), M.stopPropagation(), A(!0);
  }, E = () => {
    A(!1);
  }, C = async (M) => {
    v(!0), m(""), x("");
    try {
      const k = await zx(M.credential);
      n(k);
    } catch (k) {
      m(
        k instanceof Error ? k.message : "Google sign-in failed. Please try again."
      );
    } finally {
      v(!1);
    }
  }, P = async (M) => {
    if (M.preventDefault(), !!D()) {
      v(!0), m("");
      try {
        await t({ username: r.trim(), password: s });
      } catch (k) {
        const Y = k instanceof Error ? k.message : "";
        if (Y.includes("401") || Y.includes("400")) {
          m("Invalid username or password.");
          return;
        }
        m(
          k instanceof Error ? k.message : "Login failed. Please try again."
        );
      } finally {
        v(!1);
      }
    }
  }, D = () => {
    let M = !0;
    return r.trim() ? u("") : (u("Please enter your username."), M = !1), s ? f("") : (f("Password is required."), M = !1), M;
  };
  return /* @__PURE__ */ a.jsx(Bl, { ...e, children: /* @__PURE__ */ a.jsxs("div", { className: "relative flex min-h-screen flex-col overflow-hidden px-4 py-6 sm:px-8", children: [
    /* @__PURE__ */ a.jsx("div", { className: "pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top,_rgba(56,189,248,0.28),transparent_32%),linear-gradient(180deg,rgba(248,250,252,0.96),rgba(255,255,255,1))] dark:bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.2),transparent_28%),linear-gradient(180deg,rgba(7,14,26,0.98),rgba(16,24,40,1))]" }),
    /* @__PURE__ */ a.jsx("div", { className: "pointer-events-none absolute inset-x-0 top-[-10rem] -z-10 mx-auto h-72 w-72 rounded-full bg-sky-300/30 blur-3xl dark:bg-sky-500/10" }),
    /* @__PURE__ */ a.jsx(Gl, { className: "fixed right-4 top-4 z-20 sm:right-6 sm:top-6" }),
    /* @__PURE__ */ a.jsx("div", { className: "mx-auto flex w-full max-w-md flex-1 items-center", children: /* @__PURE__ */ a.jsxs(Ml, { className: "w-full border-border/70 bg-card/90 py-0 shadow-[0_24px_70px_-35px_rgba(0,0,0,0.55)] backdrop-blur", children: [
      /* @__PURE__ */ a.jsxs(jl, { className: "gap-5 px-6 pt-6 sm:px-8 sm:pt-8", children: [
        /* @__PURE__ */ a.jsx("div", { className: "flex justify-center sm:justify-start", children: /* @__PURE__ */ a.jsx(Yl, {}) }),
        /* @__PURE__ */ a.jsx("div", { className: "space-y-2", children: /* @__PURE__ */ a.jsx(Cl, { className: "text-3xl tracking-tight", children: "Sign in" }) })
      ] }),
      /* @__PURE__ */ a.jsx(Nl, { className: "space-y-5 px-6 pb-6 sm:px-8 sm:pb-8", children: /* @__PURE__ */ a.jsxs(
        "form",
        {
          className: "space-y-4",
          onSubmit: P,
          noValidate: !0,
          children: [
            d && /* @__PURE__ */ a.jsx(tn, { variant: "error", children: d }),
            p && /* @__PURE__ */ a.jsx(tn, { variant: "success", children: p }),
            /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ a.jsx(Nt, { htmlFor: "username", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                /* @__PURE__ */ a.jsx("span", { children: "Username" }),
                /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
              ] }) }),
              /* @__PURE__ */ a.jsx(
                ut,
                {
                  "aria-invalid": !!c,
                  id: "username",
                  type: "text",
                  name: "username",
                  placeholder: "username",
                  autoComplete: "username",
                  autoFocus: !0,
                  required: !0,
                  value: r,
                  onChange: (M) => {
                    o(M.target.value), c && u("");
                  },
                  disabled: g
                }
              ),
              c && /* @__PURE__ */ a.jsx("p", { className: "text-sm text-destructive", children: c })
            ] }),
            /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ a.jsx(Nt, { htmlFor: "password", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                /* @__PURE__ */ a.jsx("span", { children: "Password" }),
                /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
              ] }) }),
              /* @__PURE__ */ a.jsx(
                ut,
                {
                  "aria-invalid": !!l,
                  name: "password",
                  placeholder: "••••••",
                  type: "password",
                  id: "password",
                  autoComplete: "current-password",
                  required: !0,
                  value: s,
                  onChange: (M) => {
                    i(M.target.value), l && f("");
                  },
                  disabled: g
                }
              ),
              l && /* @__PURE__ */ a.jsx("p", { className: "text-sm text-destructive", children: l })
            ] }),
            /* @__PURE__ */ a.jsx(te, { type: "submit", className: "w-full", disabled: g, children: g ? "Signing in..." : "Sign in" }),
            /* @__PURE__ */ a.jsx(
              "button",
              {
                type: "button",
                onClick: w,
                className: "mx-auto block text-sm font-medium text-primary transition hover:text-primary/80",
                children: "Forgot your password?"
              }
            ),
            e.googleClientId && /* @__PURE__ */ a.jsxs(a.Fragment, { children: [
              /* @__PURE__ */ a.jsxs("div", { className: "relative py-1", children: [
                /* @__PURE__ */ a.jsx(wg, {}),
                /* @__PURE__ */ a.jsx("span", { className: "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-card px-2 text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground", children: "or continue with" })
              ] }),
              /* @__PURE__ */ a.jsx("div", { className: "rounded-2xl border border-dashed border-border/70 bg-muted/35 p-4", children: /* @__PURE__ */ a.jsx(
                "div",
                {
                  id: "google-btn",
                  className: "flex min-h-10 justify-center"
                }
              ) })
            ] })
          ]
        }
      ) })
    ] }) }),
    /* @__PURE__ */ a.jsx(
      vx,
      {
        open: b,
        handleClose: E
      }
    ),
    /* @__PURE__ */ a.jsxs("div", { className: "space-y-3 pt-6 text-center", children: [
      /* @__PURE__ */ a.jsx(
        "a",
        {
          href: "https://docs.google.com/document/d/1vueS_dzdvDkBex5BLe_F03GEFLzjtoiFbNi0VJa2_gE/edit?tab=t.0",
          target: "_blank",
          rel: "noopener noreferrer",
          className: "inline-flex text-sm font-medium text-muted-foreground transition hover:text-foreground",
          children: "Hướng dẫn sử dụng / User Guide"
        }
      ),
      /* @__PURE__ */ a.jsx("p", { className: "text-sm text-muted-foreground", children: "© 2026 Springboard English" })
    ] })
  ] }) });
}
function Aw(e) {
  const [t, n] = h.useState(""), [r, o] = h.useState(""), [s, i] = h.useState(""), [c, u] = h.useState(""), [l, f] = h.useState(""), [d, m] = h.useState(!1);
  h.useEffect(() => {
    const g = new URLSearchParams(window.location.search).get("token") || "";
    n(g);
  }, []);
  const p = async (x) => {
    if (x.preventDefault(), f(""), !t.trim()) {
      f("Reset token is required.");
      return;
    }
    if (!r.trim()) {
      f("Username is required.");
      return;
    }
    if (!s) {
      f("New password is required.");
      return;
    }
    if (!c) {
      f("Please confirm your new password.");
      return;
    }
    if (s !== c) {
      f("Passwords do not match.");
      return;
    }
    m(!0);
    try {
      await Tx({
        token: t.trim(),
        username: r.trim(),
        password: s
      }), e.onNavigateToSignIn?.("Password reset successful. You can now sign in."), !e.onNavigateToSignIn && e.signInPath && window.location.assign(e.signInPath);
    } catch (g) {
      f(
        g instanceof Error ? g.message : "Failed to reset password. Please try again."
      );
    } finally {
      m(!1);
    }
  };
  return /* @__PURE__ */ a.jsx(Bl, { ...e, children: /* @__PURE__ */ a.jsxs("div", { className: "relative flex min-h-screen flex-col overflow-hidden px-4 py-6 sm:px-8", children: [
    /* @__PURE__ */ a.jsx("div", { className: "pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top,_rgba(34,197,94,0.18),transparent_30%),linear-gradient(180deg,rgba(248,250,252,0.96),rgba(255,255,255,1))] dark:bg-[radial-gradient(circle_at_top,_rgba(34,197,94,0.16),transparent_24%),linear-gradient(180deg,rgba(7,14,26,0.98),rgba(16,24,40,1))]" }),
    /* @__PURE__ */ a.jsx("div", { className: "pointer-events-none absolute bottom-[-8rem] right-[-3rem] -z-10 h-72 w-72 rounded-full bg-emerald-300/20 blur-3xl dark:bg-emerald-500/10" }),
    /* @__PURE__ */ a.jsx(Gl, { className: "fixed right-4 top-4 z-20 sm:right-6 sm:top-6" }),
    /* @__PURE__ */ a.jsx("div", { className: "mx-auto flex w-full max-w-md flex-1 items-center", children: /* @__PURE__ */ a.jsxs(Ml, { className: "w-full border-border/70 bg-card/90 py-0 shadow-[0_24px_70px_-35px_rgba(0,0,0,0.55)] backdrop-blur", children: [
      /* @__PURE__ */ a.jsxs(jl, { className: "gap-5 px-6 pt-6 sm:px-8 sm:pt-8", children: [
        /* @__PURE__ */ a.jsx("div", { className: "flex justify-center sm:justify-start", children: /* @__PURE__ */ a.jsx(Yl, {}) }),
        /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
          /* @__PURE__ */ a.jsx(Cl, { className: "text-3xl tracking-tight", children: "Reset password" }),
          /* @__PURE__ */ a.jsx(bg, { className: "text-sm leading-6", children: "Set a new password for your account using the reset link you received." })
        ] })
      ] }),
      /* @__PURE__ */ a.jsx(Nl, { className: "px-6 pb-6 sm:px-8 sm:pb-8", children: /* @__PURE__ */ a.jsxs(
        "form",
        {
          className: "space-y-4",
          onSubmit: p,
          noValidate: !0,
          children: [
            l && /* @__PURE__ */ a.jsx(tn, { variant: "error", children: l }),
            /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ a.jsx(Nt, { htmlFor: "username", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                /* @__PURE__ */ a.jsx("span", { children: "Username" }),
                /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
              ] }) }),
              /* @__PURE__ */ a.jsx(
                ut,
                {
                  id: "username",
                  name: "username",
                  type: "text",
                  autoComplete: "username",
                  required: !0,
                  value: r,
                  onChange: (x) => o(x.target.value),
                  disabled: d
                }
              )
            ] }),
            /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ a.jsx(Nt, { htmlFor: "new-password", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                /* @__PURE__ */ a.jsx("span", { children: "New password" }),
                /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
              ] }) }),
              /* @__PURE__ */ a.jsx(
                ut,
                {
                  id: "new-password",
                  name: "new-password",
                  type: "password",
                  autoComplete: "new-password",
                  required: !0,
                  value: s,
                  onChange: (x) => i(x.target.value),
                  disabled: d
                }
              )
            ] }),
            /* @__PURE__ */ a.jsxs("div", { className: "space-y-2", children: [
              /* @__PURE__ */ a.jsx(Nt, { htmlFor: "confirm-password", children: /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
                /* @__PURE__ */ a.jsx("span", { children: "Confirm new password" }),
                /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
              ] }) }),
              /* @__PURE__ */ a.jsx(
                ut,
                {
                  id: "confirm-password",
                  name: "confirm-password",
                  type: "password",
                  autoComplete: "new-password",
                  required: !0,
                  value: c,
                  onChange: (x) => u(x.target.value),
                  disabled: d
                }
              )
            ] }),
            /* @__PURE__ */ a.jsx(te, { type: "submit", className: "w-full", disabled: d, children: d ? "Resetting password..." : "Reset password" }),
            /* @__PURE__ */ a.jsx(
              te,
              {
                type: "button",
                className: "w-full",
                variant: "outline",
                onClick: () => {
                  e.onNavigateToSignIn?.(), !e.onNavigateToSignIn && e.signInPath && window.location.assign(e.signInPath);
                },
                disabled: d,
                children: "Back to sign in"
              }
            )
          ]
        }
      ) })
    ] }) })
  ] }) });
}
function Yx(e, t) {
  if (e.length !== t.length)
    return !1;
  for (let n = 0; n < e.length; n += 1)
    if (e[n]?.key !== t[n]?.key || e[n]?.value !== t[n]?.value)
      return !1;
  return !0;
}
function Hx(e, t) {
  return e ? e.count === t.count && e.saving === t.saving && e.disabled === t.disabled && e.keyLabel === t.keyLabel && e.valueLabel === t.valueLabel && e.onSave === t.onSave && e.onCancel === t.onCancel && Yx(e.items, t.items) : !1;
}
const Ql = zt(
  void 0
);
function yw({ children: e }) {
  const [t, n] = de(null), [r, o] = de(
    {}
  ), [s, i] = de({}), c = Re(
    (A, w) => {
      const E = A.trim();
      E && (i((C) => {
        const P = w.count;
        if (P <= 0) {
          if (!(E in C))
            return C;
          const D = { ...C };
          return delete D[E], D;
        }
        return C[E] === P ? C : {
          ...C,
          [E]: P
        };
      }), o((C) => {
        const P = C[E];
        return Hx(P, w) ? C : {
          ...C,
          [E]: w
        };
      }));
    },
    []
  ), u = Re((A) => {
    const w = A.trim();
    w && o((E) => {
      if (!(w in E))
        return E;
      const C = { ...E };
      return delete C[w], C;
    });
  }, []), l = ue(() => Object.entries(r).flatMap(
    ([A, w]) => w.items.map((E) => ({
      section: A,
      key: E.key,
      value: E.value
    }))
  ), [r]), f = ue(
    () => Object.values(r).reduce(
      (A, w) => A + w.count,
      0
    ),
    [r]
  ), d = ue(() => {
    const A = Object.entries(r).reduce(
      (w, [E, C]) => (w[E] = C.count, w),
      {}
    );
    return {
      ...s,
      ...A
    };
  }, [s, r]), m = ue(
    () => Object.entries(r).find(([, A]) => A.count > 0)?.[0] ?? null,
    [r]
  ), p = t && r[t] ? t : m, x = p ? r[p] ?? null : null, g = Re(() => {
    if (x?.onSave)
      return x.onSave();
  }, [x]), v = Re(() => {
    x?.onCancel?.();
  }, [x]), b = ue(
    () => ({
      globalPendingCount: f,
      globalPendingItems: l,
      sectionPendingCounts: d,
      activeSection: p,
      activeSectionPendingCount: x?.count ?? 0,
      activeSectionSaving: x?.saving ?? !1,
      activeSectionDisabled: x?.disabled ?? !1,
      activeSectionKeyLabel: x?.keyLabel ?? "Name",
      activeSectionValueLabel: x?.valueLabel ?? "Changed",
      setActiveSection: n,
      registerSection: c,
      unregisterSection: u,
      handleSaveActiveSection: g,
      handleCancelActiveSection: v
    }),
    [
      p,
      x,
      f,
      l,
      d,
      v,
      g,
      c,
      u
    ]
  );
  return /* @__PURE__ */ a.jsx(Ql.Provider, { value: b, children: e });
}
function Ew() {
  const e = gt(Ql);
  if (!e)
    throw new Error("usePendingChanges must be used within a PendingChangesProvider");
  return e;
}
function Mw(e) {
  const { mode: t, systemMode: n, setMode: r } = zl(), { className: o, ...s } = e, c = (n || t) === "light" ? /* @__PURE__ */ a.jsx(_n, { className: "size-4" }) : /* @__PURE__ */ a.jsx(Kn, { className: "size-4" });
  return /* @__PURE__ */ a.jsxs(us, { children: [
    /* @__PURE__ */ a.jsx(ds, { asChild: !0, children: /* @__PURE__ */ a.jsx(
      te,
      {
        "data-screenshot": "toggle-mode",
        variant: "ghost",
        size: "icon",
        className: H("rounded-full", o),
        "aria-label": "Toggle color mode",
        ...s,
        children: c
      }
    ) }),
    /* @__PURE__ */ a.jsxs(fs, { align: "end", className: "w-44", children: [
      /* @__PURE__ */ a.jsx(yx, { children: "Appearance" }),
      /* @__PURE__ */ a.jsx(Ex, {}),
      /* @__PURE__ */ a.jsxs(
        Tl,
        {
          value: t,
          onValueChange: (u) => r(u),
          children: [
            /* @__PURE__ */ a.jsxs(St, { value: "system", children: [
              /* @__PURE__ */ a.jsx(co, { className: "mr-2 size-4" }),
              "System"
            ] }),
            /* @__PURE__ */ a.jsxs(St, { value: "light", children: [
              /* @__PURE__ */ a.jsx(_n, { className: "mr-2 size-4" }),
              "Light"
            ] }),
            /* @__PURE__ */ a.jsxs(St, { value: "dark", children: [
              /* @__PURE__ */ a.jsx(Kn, { className: "mr-2 size-4" }),
              "Dark"
            ] })
          ]
        }
      )
    ] })
  ] });
}
function jw() {
  return /* @__PURE__ */ a.jsx("div", { className: "flex min-h-[240px] w-full items-center justify-center", children: /* @__PURE__ */ a.jsx(kl, { className: "size-7 animate-spin text-muted-foreground" }) });
}
function Qr(e) {
  return Array.isArray(e) ? e.filter(
    (t) => typeof t == "string" && t.trim().length > 0
  ) : typeof e == "string" ? e.split(/\s+/).map((t) => t.trim()).filter(Boolean) : [];
}
function Zl(e) {
  return !!e && typeof e == "object";
}
function hs(e) {
  if (Zl(e)) {
    const t = Qr(e.roles);
    return t.length > 0 ? t : Qr(e.scope);
  }
  return Qr(e);
}
function Cw(e, t) {
  return hs(e).includes(t);
}
function Fx(e, t) {
  const n = hs(e);
  return t.some((r) => n.includes(r));
}
function Nw(e) {
  return !Zl(e) || e.account_type !== "employee" ? !1 : Fx(e, [
    "classes_get",
    "sessions_get",
    "schedules_get",
    "tests_get",
    "classes_feedbacks_get"
  ]);
}
const Lx = ["payrolls_get", "tasks_get", "projects_get"];
function Iw({
  value: e,
  onValueChange: t,
  additionalOptions: n,
  placeholder: r = "Select scopes"
}) {
  const o = ue(() => hs(e), [e]), s = ue(() => new Set(o), [o]), i = ue(() => {
    const u = [...Lx, ...n ?? [], ...o];
    return Array.from(new Set(u.filter(Boolean)));
  }, [n, o]), c = o.length > 0 ? o.join(" ") : r;
  return /* @__PURE__ */ a.jsx("div", { className: "w-full min-w-0", children: /* @__PURE__ */ a.jsxs(us, { children: [
    /* @__PURE__ */ a.jsx(ds, { asChild: !0, children: /* @__PURE__ */ a.jsxs(
      "button",
      {
        type: "button",
        className: "grid h-10 w-full min-w-0 max-w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-2 overflow-hidden rounded-md border border-input bg-background px-3 text-sm font-normal",
        children: [
          /* @__PURE__ */ a.jsx("span", { className: "block min-w-0 max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-left", children: c }),
          /* @__PURE__ */ a.jsx(ns, { className: "size-4 shrink-0 opacity-60" })
        ]
      }
    ) }),
    /* @__PURE__ */ a.jsx(
      fs,
      {
        align: "start",
        className: "w-[--radix-dropdown-menu-trigger-width] min-w-[260px] p-1",
        children: /* @__PURE__ */ a.jsx("div", { className: "max-h-64 overflow-auto", children: i.map((u) => {
          const l = s.has(u);
          return /* @__PURE__ */ a.jsxs(
            "button",
            {
              type: "button",
              className: "flex w-full items-center justify-between rounded-sm px-2 py-1.5 text-left text-sm hover:bg-muted",
              onClick: () => {
                const f = l ? o.filter((d) => d !== u) : [...o, u];
                t(f.join(" "));
              },
              children: [
                /* @__PURE__ */ a.jsx("span", { children: u }),
                /* @__PURE__ */ a.jsx("span", { className: "ml-2 inline-flex size-4 items-center justify-center", children: l ? /* @__PURE__ */ a.jsx(ts, { className: "size-4" }) : null })
              ]
            },
            u
          );
        }) })
      }
    )
  ] }) });
}
function Jl({
  id: e,
  value: t,
  options: n,
  placeholder: r = "Select option",
  searchPlaceholder: o = "Search...",
  emptyMessage: s = "No options found.",
  disabled: i = !1,
  className: c,
  contentClassName: u,
  onValueChange: l
}) {
  const [f, d] = de(!1), [m, p] = de(""), x = String(t ?? ""), g = ue(
    () => n.find((A) => A.value === x),
    [x, n]
  ), v = ue(() => {
    const A = m.trim().toLowerCase();
    return A ? n.filter((w) => `${w.label} ${w.keywords ?? ""} ${w.value}`.toLowerCase().includes(A)) : n;
  }, [n, m]), b = (A) => {
    l(A), d(!1), p("");
  };
  return /* @__PURE__ */ a.jsxs(qo, { open: f, onOpenChange: d, children: [
    /* @__PURE__ */ a.jsx(Xo, { asChild: !0, children: /* @__PURE__ */ a.jsxs(
      "button",
      {
        id: e,
        type: "button",
        disabled: i,
        className: H(
          "flex h-10 w-full min-w-0 cursor-pointer items-center justify-between rounded-md border border-input bg-background px-3 text-sm shadow-xs outline-none transition",
          "focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50",
          "disabled:cursor-not-allowed disabled:opacity-50",
          c
        ),
        onClick: (A) => A.stopPropagation(),
        children: [
          /* @__PURE__ */ a.jsx("span", { className: H("min-w-0 flex-1 truncate text-left", !g && "text-muted-foreground"), children: g?.label ?? r }),
          /* @__PURE__ */ a.jsx(ns, { className: "ml-2 size-4 shrink-0 text-muted-foreground" })
        ]
      }
    ) }),
    /* @__PURE__ */ a.jsx(Ko, { children: /* @__PURE__ */ a.jsxs(
      _o,
      {
        sideOffset: 4,
        align: "start",
        className: H(
          "z-50 w-(--radix-popover-trigger-width) min-w-[12rem] max-h-[min(18rem,var(--radix-popover-content-available-height))] overflow-hidden rounded-md border border-border bg-popover p-2 text-popover-foreground shadow-md",
          "data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
          "data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95",
          "data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
          u
        ),
        onOpenAutoFocus: (A) => A.preventDefault(),
        onPointerDownOutside: () => p(""),
        onEscapeKeyDown: () => p(""),
        onClick: (A) => A.stopPropagation(),
        children: [
          /* @__PURE__ */ a.jsxs("div", { className: "relative mb-2", children: [
            /* @__PURE__ */ a.jsx(_g, { className: "pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-muted-foreground" }),
            /* @__PURE__ */ a.jsx(
              "input",
              {
                value: m,
                onChange: (A) => p(A.target.value),
                placeholder: o,
                className: H(
                  "h-8 w-full rounded-md border border-input bg-background pr-2 pl-8 text-sm shadow-xs outline-none transition",
                  "focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50"
                ),
                autoFocus: !0
              }
            )
          ] }),
          /* @__PURE__ */ a.jsx(
            "div",
            {
              className: "max-h-56 overflow-y-auto overscroll-contain pr-1 [scrollbar-gutter:stable] touch-pan-y",
              style: { WebkitOverflowScrolling: "touch" },
              onWheelCapture: (A) => A.stopPropagation(),
              children: v.length === 0 ? /* @__PURE__ */ a.jsx("div", { className: "px-2 py-1.5 text-sm text-muted-foreground", children: s }) : v.map((A) => {
                const w = A.value === x;
                return /* @__PURE__ */ a.jsxs(
                  "button",
                  {
                    type: "button",
                    onClick: () => b(A.value),
                    disabled: A.disabled,
                    className: H(
                      "flex w-full min-w-0 cursor-pointer items-center justify-between rounded-sm px-2 py-1.5 text-left text-sm outline-none transition",
                      "hover:bg-accent hover:text-accent-foreground focus-visible:bg-accent focus-visible:text-accent-foreground",
                      "disabled:pointer-events-none disabled:opacity-50"
                    ),
                    children: [
                      /* @__PURE__ */ a.jsx("span", { className: "min-w-0 flex-1 truncate", children: A.label }),
                      w ? /* @__PURE__ */ a.jsx(ts, { className: "ml-2 size-4 shrink-0" }) : null
                    ]
                  },
                  A.value
                );
              })
            }
          )
        ]
      }
    ) })
  ] });
}
function Qx({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "textarea",
    {
      "data-slot": "textarea",
      className: H(
        "flex min-h-16 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-xs transition-[color,box-shadow] outline-none placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-input/30",
        e
      ),
      ...t
    }
  );
}
function ps({ className: e, containerClassName: t, ...n }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "table-container",
      className: H("relative w-full overflow-x-auto", t),
      children: /* @__PURE__ */ a.jsx(
        "table",
        {
          "data-slot": "table",
          className: H("w-full caption-bottom text-sm", e),
          ...n
        }
      )
    }
  );
}
function Zx({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "thead",
    {
      "data-slot": "table-header",
      className: H("[&_tr]:border-b", e),
      ...t
    }
  );
}
function gs({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "tbody",
    {
      "data-slot": "table-body",
      className: H("[&_tr:last-child]:border-0", e),
      ...t
    }
  );
}
function Sw({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "tfoot",
    {
      "data-slot": "table-footer",
      className: H(
        "bg-muted/50 border-t font-medium [&>tr]:last:border-b-0",
        e
      ),
      ...t
    }
  );
}
function Mt({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "tr",
    {
      "data-slot": "table-row",
      className: H(
        "border-b transition-colors hover:bg-muted/40 data-[state=selected]:bg-muted",
        e
      ),
      ...t
    }
  );
}
function Jx({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "th",
    {
      "data-slot": "table-head",
      className: H(
        "h-11 px-4 text-left align-middle text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground",
        e
      ),
      ...t
    }
  );
}
function dt({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "td",
    {
      "data-slot": "table-cell",
      className: H("p-4 align-middle", e),
      ...t
    }
  );
}
function kw({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    "caption",
    {
      "data-slot": "table-caption",
      className: H("mt-4 text-sm text-muted-foreground", e),
      ...t
    }
  );
}
function Wx({ label: e, required: t = !1, helperText: n, children: r, className: o, align: s = "center" }) {
  return /* @__PURE__ */ a.jsxs(Mt, { className: H("hover:bg-transparent", o), children: [
    /* @__PURE__ */ a.jsx(
      dt,
      {
        className: H(
          "w-[220px] px-4 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground",
          s === "center" ? "align-middle" : "align-top"
        ),
        children: /* @__PURE__ */ a.jsxs("div", { className: "space-y-1", children: [
          /* @__PURE__ */ a.jsxs("div", { className: "inline-flex items-center gap-1", children: [
            /* @__PURE__ */ a.jsx("span", { children: e }),
            t && /* @__PURE__ */ a.jsx("span", { className: "text-destructive", "aria-hidden": "true", children: "*" })
          ] }),
          n && /* @__PURE__ */ a.jsx("p", { className: "text-xs normal-case leading-5 text-muted-foreground", children: n })
        ] })
      }
    ),
    /* @__PURE__ */ a.jsx(dt, { className: H("min-w-0 px-4 py-3 text-sm", s === "center" ? "align-middle" : "align-top"), children: r })
  ] });
}
function Ux({
  open: e,
  saving: t = !1,
  title: n,
  description: r,
  error: o,
  submitLabel: s = "Create",
  submitDisabled: i = !1,
  onClose: c,
  onSubmit: u,
  children: l
}) {
  return /* @__PURE__ */ a.jsx(rs, { open: e, onOpenChange: (f) => !f && !t && c(), children: /* @__PURE__ */ a.jsxs(
    os,
    {
      showCloseButton: !t,
      className: "!fixed !top-1/2 !left-1/2 !-translate-x-1/2 !-translate-y-1/2 !w-[80vw] max-h-[92vh] !max-w-none flex flex-col overflow-hidden border-0 bg-card p-0 shadow-none",
      children: [
        /* @__PURE__ */ a.jsxs(ss, { className: "border-b border-border/60 px-6 py-4 text-left", children: [
          /* @__PURE__ */ a.jsx(is, { children: n }),
          r && /* @__PURE__ */ a.jsx(cs, { children: r })
        ] }),
        /* @__PURE__ */ a.jsx("div", { className: "min-h-0 flex-1 overflow-y-auto px-6 py-5", children: /* @__PURE__ */ a.jsxs("div", { className: "flex min-h-full flex-col justify-center gap-4", children: [
          o && /* @__PURE__ */ a.jsx(tn, { variant: "error", children: o }),
          /* @__PURE__ */ a.jsx("div", { className: "overflow-hidden rounded-2xl bg-transparent", children: /* @__PURE__ */ a.jsx(ps, { className: "table-fixed", containerClassName: "overflow-x-hidden", children: /* @__PURE__ */ a.jsx(gs, { className: "[&_tr]:border-b-0", children: l }) }) })
        ] }) }),
        /* @__PURE__ */ a.jsxs(as, { className: "border-t border-border/60 px-6 py-4", showCloseButton: !1, children: [
          /* @__PURE__ */ a.jsx(te, { type: "button", variant: "outline", onClick: c, disabled: t, children: "Cancel" }),
          /* @__PURE__ */ a.jsx(te, { type: "button", onClick: () => {
            u();
          }, disabled: t || i, children: s })
        ] })
      ]
    }
  ) });
}
function Vx(e, t) {
  return e === "boolean" ? t === !0 : t == null ? "" : String(t);
}
function qx(e) {
  return e.includes(`
`) || e.length > 80;
}
function Xx(e, t) {
  const n = e.split(`
`).length, r = Math.ceil(e.length / 90);
  return Math.min(12, Math.max(t ?? 3, n, r));
}
function Kx(e) {
  return e === "number" ? "number" : e === "date" ? "date" : "text";
}
function Dw({
  open: e,
  title: t,
  fields: n,
  initialValues: r,
  error: o,
  saving: s = !1,
  onClose: i,
  onSubmit: c
}) {
  const [u, l] = de([]);
  ae(() => {
    e && l(n.map((m) => {
      const p = Vx(m.type, r[m.key]);
      return {
        ...m,
        value: p,
        initialValue: p
      };
    }));
  }, [n, r, e]);
  const f = (m, p) => {
    l((x) => x.map((g) => g.key === m ? { ...g, value: p } : g));
  }, d = async () => {
    const m = u.reduce((p, x) => {
      if (x.value === x.initialValue || x.readOnly)
        return p;
      if (x.type === "boolean")
        return p[x.key] = !!x.value, p;
      const g = typeof x.value == "string" ? x.value.trim() : "";
      return g ? x.type === "number" ? (p[x.key] = Number(g), p) : (p[x.key] = g, p) : (p[x.key] = null, p);
    }, {});
    await c(m);
  };
  return /* @__PURE__ */ a.jsx(
    Ux,
    {
      open: e,
      saving: s,
      title: t,
      description: "Only modified fields will be included in the update request.",
      error: o,
      submitLabel: "Save",
      onClose: i,
      onSubmit: d,
      children: u.map((m) => {
        const p = typeof m.value == "string" ? m.value : "", x = m.type === "text" && (m.multiline || qx(p));
        return /* @__PURE__ */ a.jsx(
          Wx,
          {
            label: m.label,
            helperText: m.helperText,
            align: x ? "start" : "center",
            children: m.type === "boolean" ? /* @__PURE__ */ a.jsxs("label", { className: "inline-flex items-center gap-3 text-sm", children: [
              /* @__PURE__ */ a.jsx(
                "input",
                {
                  type: "checkbox",
                  checked: !!m.value,
                  disabled: m.readOnly,
                  onChange: (g) => f(m.key, g.target.checked),
                  className: "size-4 rounded border-input accent-primary"
                }
              ),
              /* @__PURE__ */ a.jsx("span", { children: m.value ? "Enabled" : "Disabled" })
            ] }) : m.type === "select" ? /* @__PURE__ */ a.jsx(
              Jl,
              {
                value: p,
                onValueChange: (g) => f(m.key, g),
                disabled: m.readOnly,
                options: [
                  { value: "", label: `-- ${m.label} --` },
                  ...(m.options ?? []).map((g) => ({ value: g.value, label: g.label }))
                ],
                placeholder: `-- ${m.label} --`,
                searchPlaceholder: `Search ${m.label.toLowerCase()}...`
              }
            ) : x ? /* @__PURE__ */ a.jsx(
              Qx,
              {
                value: p,
                onChange: (g) => f(m.key, g.target.value),
                readOnly: m.readOnly,
                disabled: m.readOnly,
                rows: Xx(p, m.minRows),
                placeholder: m.label,
                className: "resize-y"
              }
            ) : /* @__PURE__ */ a.jsx(
              ut,
              {
                value: p,
                type: Kx(m.type),
                onChange: (g) => f(m.key, g.target.value),
                readOnly: m.readOnly,
                disabled: m.readOnly,
                placeholder: m.label
              }
            )
          },
          m.key
        );
      })
    }
  );
}
var xs = En(), V = (e) => yn(e, xs), vs = En();
V.write = (e) => yn(e, vs);
var yr = En();
V.onStart = (e) => yn(e, yr);
var bs = En();
V.onFrame = (e) => yn(e, bs);
var ws = En();
V.onFinish = (e) => yn(e, ws);
var kt = [];
V.setTimeout = (e, t) => {
  const n = V.now() + t, r = () => {
    const s = kt.findIndex((i) => i.cancel == r);
    ~s && kt.splice(s, 1), Ke -= ~s ? 1 : 0;
  }, o = { time: n, handler: e, cancel: r };
  return kt.splice(Wl(n), 0, o), Ke += 1, Ul(), o;
};
var Wl = (e) => ~(~kt.findIndex((t) => t.time > e) || ~kt.length);
V.cancel = (e) => {
  yr.delete(e), bs.delete(e), ws.delete(e), xs.delete(e), vs.delete(e);
};
V.sync = (e) => {
  uo = !0, V.batchedUpdates(e), uo = !1;
};
V.throttle = (e) => {
  let t;
  function n() {
    try {
      e(...t);
    } finally {
      t = null;
    }
  }
  function r(...o) {
    t = o, V.onStart(n);
  }
  return r.handler = e, r.cancel = () => {
    yr.delete(n), t = null;
  }, r;
};
var As = typeof window < "u" ? window.requestAnimationFrame : (
  // eslint-disable-next-line @typescript-eslint/no-empty-function
  (() => {
  })
);
V.use = (e) => As = e;
V.now = typeof performance < "u" ? () => performance.now() : Date.now;
V.batchedUpdates = (e) => e();
V.catch = console.error;
V.frameLoop = "always";
V.advance = () => {
  V.frameLoop !== "demand" ? console.warn(
    "Cannot call the manual advancement of rafz whilst frameLoop is not set as demand"
  ) : ql();
};
var Xe = -1, Ke = 0, uo = !1;
function yn(e, t) {
  uo ? (t.delete(e), e(0)) : (t.add(e), Ul());
}
function Ul() {
  Xe < 0 && (Xe = 0, V.frameLoop !== "demand" && As(Vl));
}
function _x() {
  Xe = -1;
}
function Vl() {
  ~Xe && (As(Vl), V.batchedUpdates(ql));
}
function ql() {
  const e = Xe;
  Xe = V.now();
  const t = Wl(Xe);
  if (t && (Xl(kt.splice(0, t), (n) => n.handler()), Ke -= t), !Ke) {
    _x();
    return;
  }
  yr.flush(), xs.flush(e ? Math.min(64, Xe - e) : 16.667), bs.flush(), vs.flush(), ws.flush();
}
function En() {
  let e = /* @__PURE__ */ new Set(), t = e;
  return {
    add(n) {
      Ke += t == e && !e.has(n) ? 1 : 0, e.add(n);
    },
    delete(n) {
      return Ke -= t == e && e.has(n) ? 1 : 0, e.delete(n);
    },
    flush(n) {
      t.size && (e = /* @__PURE__ */ new Set(), Ke -= t.size, Xl(t, (r) => r(n) && e.add(r)), Ke += e.size, t = e);
    }
  };
}
function Xl(e, t) {
  e.forEach((n) => {
    try {
      t(n);
    } catch (r) {
      V.catch(r);
    }
  });
}
var $x = Object.defineProperty, ev = (e, t) => {
  for (var n in t)
    $x(e, n, { get: t[n], enumerable: !0 });
}, Ie = {};
ev(Ie, {
  assign: () => nv,
  colors: () => _e,
  createStringInterpolator: () => Es,
  skipAnimation: () => _l,
  to: () => Kl,
  willAdvance: () => Ms
});
function fo() {
}
var tv = (e, t, n) => Object.defineProperty(e, t, { value: n, writable: !0, configurable: !0 }), B = {
  arr: Array.isArray,
  obj: (e) => !!e && e.constructor.name === "Object",
  fun: (e) => typeof e == "function",
  str: (e) => typeof e == "string",
  num: (e) => typeof e == "number",
  und: (e) => e === void 0
};
function He(e, t) {
  if (B.arr(e)) {
    if (!B.arr(t) || e.length !== t.length) return !1;
    for (let n = 0; n < e.length; n++)
      if (e[n] !== t[n]) return !1;
    return !0;
  }
  return e === t;
}
var $ = (e, t) => e.forEach(t);
function Be(e, t, n) {
  if (B.arr(e)) {
    for (let r = 0; r < e.length; r++)
      t.call(n, e[r], `${r}`);
    return;
  }
  for (const r in e)
    e.hasOwnProperty(r) && t.call(n, e[r], r);
}
var ve = (e) => B.und(e) ? [] : B.arr(e) ? e : [e];
function Vt(e, t) {
  if (e.size) {
    const n = Array.from(e);
    e.clear(), $(n, t);
  }
}
var Ut = (e, ...t) => Vt(e, (n) => n(...t)), ys = () => typeof window > "u" || !window.navigator || /ServerSideRendering|^Deno\//.test(window.navigator.userAgent), Es, Kl, _e = null, _l = !1, Ms = fo, nv = (e) => {
  e.to && (Kl = e.to), e.now && (V.now = e.now), e.colors !== void 0 && (_e = e.colors), e.skipAnimation != null && (_l = e.skipAnimation), e.createStringInterpolator && (Es = e.createStringInterpolator), e.requestAnimationFrame && V.use(e.requestAnimationFrame), e.batchedUpdates && (V.batchedUpdates = e.batchedUpdates), e.willAdvance && (Ms = e.willAdvance), e.frameLoop && (V.frameLoop = e.frameLoop);
}, qt = /* @__PURE__ */ new Set(), Ae = [], Zr = [], er = 0, Er = {
  get idle() {
    return !qt.size && !Ae.length;
  },
  /** Advance the given animation on every frame until idle. */
  start(e) {
    er > e.priority ? (qt.add(e), V.onStart(rv)) : ($l(e), V(mo));
  },
  /** Advance all animations by the given time. */
  advance: mo,
  /** Call this when an animation's priority changes. */
  sort(e) {
    if (er)
      V.onFrame(() => Er.sort(e));
    else {
      const t = Ae.indexOf(e);
      ~t && (Ae.splice(t, 1), eu(e));
    }
  },
  /**
   * Clear all animations. For testing purposes.
   *
   * ☠️ Never call this from within the frameloop.
   */
  clear() {
    Ae = [], qt.clear();
  }
};
function rv() {
  qt.forEach($l), qt.clear(), V(mo);
}
function $l(e) {
  Ae.includes(e) || eu(e);
}
function eu(e) {
  Ae.splice(
    ov(Ae, (t) => t.priority > e.priority),
    0,
    e
  );
}
function mo(e) {
  const t = Zr;
  for (let n = 0; n < Ae.length; n++) {
    const r = Ae[n];
    er = r.priority, r.idle || (Ms(r), r.advance(e), r.idle || t.push(r));
  }
  return er = 0, Zr = Ae, Zr.length = 0, Ae = t, Ae.length > 0;
}
function ov(e, t) {
  const n = e.findIndex(t);
  return n < 0 ? e.length : n;
}
var sv = {
  transparent: 0,
  aliceblue: 4042850303,
  antiquewhite: 4209760255,
  aqua: 16777215,
  aquamarine: 2147472639,
  azure: 4043309055,
  beige: 4126530815,
  bisque: 4293182719,
  black: 255,
  blanchedalmond: 4293643775,
  blue: 65535,
  blueviolet: 2318131967,
  brown: 2771004159,
  burlywood: 3736635391,
  burntsienna: 3934150143,
  cadetblue: 1604231423,
  chartreuse: 2147418367,
  chocolate: 3530104575,
  coral: 4286533887,
  cornflowerblue: 1687547391,
  cornsilk: 4294499583,
  crimson: 3692313855,
  cyan: 16777215,
  darkblue: 35839,
  darkcyan: 9145343,
  darkgoldenrod: 3095792639,
  darkgray: 2846468607,
  darkgreen: 6553855,
  darkgrey: 2846468607,
  darkkhaki: 3182914559,
  darkmagenta: 2332068863,
  darkolivegreen: 1433087999,
  darkorange: 4287365375,
  darkorchid: 2570243327,
  darkred: 2332033279,
  darksalmon: 3918953215,
  darkseagreen: 2411499519,
  darkslateblue: 1211993087,
  darkslategray: 793726975,
  darkslategrey: 793726975,
  darkturquoise: 13554175,
  darkviolet: 2483082239,
  deeppink: 4279538687,
  deepskyblue: 12582911,
  dimgray: 1768516095,
  dimgrey: 1768516095,
  dodgerblue: 512819199,
  firebrick: 2988581631,
  floralwhite: 4294635775,
  forestgreen: 579543807,
  fuchsia: 4278255615,
  gainsboro: 3705462015,
  ghostwhite: 4177068031,
  gold: 4292280575,
  goldenrod: 3668254975,
  gray: 2155905279,
  green: 8388863,
  greenyellow: 2919182335,
  grey: 2155905279,
  honeydew: 4043305215,
  hotpink: 4285117695,
  indianred: 3445382399,
  indigo: 1258324735,
  ivory: 4294963455,
  khaki: 4041641215,
  lavender: 3873897215,
  lavenderblush: 4293981695,
  lawngreen: 2096890111,
  lemonchiffon: 4294626815,
  lightblue: 2916673279,
  lightcoral: 4034953471,
  lightcyan: 3774873599,
  lightgoldenrodyellow: 4210742015,
  lightgray: 3553874943,
  lightgreen: 2431553791,
  lightgrey: 3553874943,
  lightpink: 4290167295,
  lightsalmon: 4288707327,
  lightseagreen: 548580095,
  lightskyblue: 2278488831,
  lightslategray: 2005441023,
  lightslategrey: 2005441023,
  lightsteelblue: 2965692159,
  lightyellow: 4294959359,
  lime: 16711935,
  limegreen: 852308735,
  linen: 4210091775,
  magenta: 4278255615,
  maroon: 2147483903,
  mediumaquamarine: 1724754687,
  mediumblue: 52735,
  mediumorchid: 3126187007,
  mediumpurple: 2473647103,
  mediumseagreen: 1018393087,
  mediumslateblue: 2070474495,
  mediumspringgreen: 16423679,
  mediumturquoise: 1221709055,
  mediumvioletred: 3340076543,
  midnightblue: 421097727,
  mintcream: 4127193855,
  mistyrose: 4293190143,
  moccasin: 4293178879,
  navajowhite: 4292783615,
  navy: 33023,
  oldlace: 4260751103,
  olive: 2155872511,
  olivedrab: 1804477439,
  orange: 4289003775,
  orangered: 4282712319,
  orchid: 3664828159,
  palegoldenrod: 4008225535,
  palegreen: 2566625535,
  paleturquoise: 2951671551,
  palevioletred: 3681588223,
  papayawhip: 4293907967,
  peachpuff: 4292524543,
  peru: 3448061951,
  pink: 4290825215,
  plum: 3718307327,
  powderblue: 2967529215,
  purple: 2147516671,
  rebeccapurple: 1714657791,
  red: 4278190335,
  rosybrown: 3163525119,
  royalblue: 1097458175,
  saddlebrown: 2336560127,
  salmon: 4202722047,
  sandybrown: 4104413439,
  seagreen: 780883967,
  seashell: 4294307583,
  sienna: 2689740287,
  silver: 3233857791,
  skyblue: 2278484991,
  slateblue: 1784335871,
  slategray: 1887473919,
  slategrey: 1887473919,
  snow: 4294638335,
  springgreen: 16744447,
  steelblue: 1182971135,
  tan: 3535047935,
  teal: 8421631,
  thistle: 3636451583,
  tomato: 4284696575,
  turquoise: 1088475391,
  violet: 4001558271,
  wheat: 4125012991,
  white: 4294967295,
  whitesmoke: 4126537215,
  yellow: 4294902015,
  yellowgreen: 2597139199
}, je = "[-+]?\\d*\\.?\\d+", tr = je + "%";
function Mr(...e) {
  return "\\(\\s*(" + e.join(")\\s*,\\s*(") + ")\\s*\\)";
}
var av = new RegExp("rgb" + Mr(je, je, je)), iv = new RegExp("rgba" + Mr(je, je, je, je)), cv = new RegExp("hsl" + Mr(je, tr, tr)), lv = new RegExp(
  "hsla" + Mr(je, tr, tr, je)
), uv = /^#([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/, dv = /^#([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})([0-9a-fA-F]{1})$/, fv = /^#([0-9a-fA-F]{6})$/, mv = /^#([0-9a-fA-F]{8})$/;
function hv(e) {
  let t;
  return typeof e == "number" ? e >>> 0 === e && e >= 0 && e <= 4294967295 ? e : null : (t = fv.exec(e)) ? parseInt(t[1] + "ff", 16) >>> 0 : _e && _e[e] !== void 0 ? _e[e] : (t = av.exec(e)) ? (Et(t[1]) << 24 | // r
  Et(t[2]) << 16 | // g
  Et(t[3]) << 8 | // b
  255) >>> // a
  0 : (t = iv.exec(e)) ? (Et(t[1]) << 24 | // r
  Et(t[2]) << 16 | // g
  Et(t[3]) << 8 | // b
  ja(t[4])) >>> // a
  0 : (t = uv.exec(e)) ? parseInt(
    t[1] + t[1] + // r
    t[2] + t[2] + // g
    t[3] + t[3] + // b
    "ff",
    // a
    16
  ) >>> 0 : (t = mv.exec(e)) ? parseInt(t[1], 16) >>> 0 : (t = dv.exec(e)) ? parseInt(
    t[1] + t[1] + // r
    t[2] + t[2] + // g
    t[3] + t[3] + // b
    t[4] + t[4],
    // a
    16
  ) >>> 0 : (t = cv.exec(e)) ? (Ea(
    Ma(t[1]),
    // h
    zn(t[2]),
    // s
    zn(t[3])
    // l
  ) | 255) >>> // a
  0 : (t = lv.exec(e)) ? (Ea(
    Ma(t[1]),
    // h
    zn(t[2]),
    // s
    zn(t[3])
    // l
  ) | ja(t[4])) >>> // a
  0 : null;
}
function Jr(e, t, n) {
  return n < 0 && (n += 1), n > 1 && (n -= 1), n < 1 / 6 ? e + (t - e) * 6 * n : n < 1 / 2 ? t : n < 2 / 3 ? e + (t - e) * (2 / 3 - n) * 6 : e;
}
function Ea(e, t, n) {
  const r = n < 0.5 ? n * (1 + t) : n + t - n * t, o = 2 * n - r, s = Jr(o, r, e + 1 / 3), i = Jr(o, r, e), c = Jr(o, r, e - 1 / 3);
  return Math.round(s * 255) << 24 | Math.round(i * 255) << 16 | Math.round(c * 255) << 8;
}
function Et(e) {
  const t = parseInt(e, 10);
  return t < 0 ? 0 : t > 255 ? 255 : t;
}
function Ma(e) {
  return (parseFloat(e) % 360 + 360) % 360 / 360;
}
function ja(e) {
  const t = parseFloat(e);
  return t < 0 ? 0 : t > 1 ? 255 : Math.round(t * 255);
}
function zn(e) {
  const t = parseFloat(e);
  return t < 0 ? 0 : t > 100 ? 1 : t / 100;
}
function Ca(e) {
  let t = hv(e);
  if (t === null) return e;
  t = t || 0;
  const n = (t & 4278190080) >>> 24, r = (t & 16711680) >>> 16, o = (t & 65280) >>> 8, s = (t & 255) / 255;
  return `rgba(${n}, ${r}, ${o}, ${s})`;
}
var rn = (e, t, n) => {
  if (B.fun(e))
    return e;
  if (B.arr(e))
    return rn({
      range: e,
      output: t,
      extrapolate: n
    });
  if (B.str(e.output[0]))
    return Es(e);
  const r = e, o = r.output, s = r.range || [0, 1], i = r.extrapolateLeft || r.extrapolate || "extend", c = r.extrapolateRight || r.extrapolate || "extend", u = r.easing || ((l) => l);
  return (l) => {
    const f = gv(l, s);
    return pv(
      l,
      s[f],
      s[f + 1],
      o[f],
      o[f + 1],
      u,
      i,
      c,
      r.map
    );
  };
};
function pv(e, t, n, r, o, s, i, c, u) {
  let l = u ? u(e) : e;
  if (l < t) {
    if (i === "identity") return l;
    i === "clamp" && (l = t);
  }
  if (l > n) {
    if (c === "identity") return l;
    c === "clamp" && (l = n);
  }
  return r === o ? r : t === n ? e <= t ? r : o : (t === -1 / 0 ? l = -l : n === 1 / 0 ? l = l - t : l = (l - t) / (n - t), l = s(l), r === -1 / 0 ? l = -l : o === 1 / 0 ? l = l + r : l = l * (o - r) + r, l);
}
function gv(e, t) {
  for (var n = 1; n < t.length - 1 && !(t[n] >= e); ++n)
    ;
  return n - 1;
}
var xv = {
  linear: (e) => e
}, on = /* @__PURE__ */ Symbol.for("FluidValue.get"), Ot = /* @__PURE__ */ Symbol.for("FluidValue.observers"), we = (e) => !!(e && e[on]), pe = (e) => e && e[on] ? e[on]() : e, Na = (e) => e[Ot] || null;
function vv(e, t) {
  e.eventObserved ? e.eventObserved(t) : e(t);
}
function sn(e, t) {
  const n = e[Ot];
  n && n.forEach((r) => {
    vv(r, t);
  });
}
var tu = class {
  constructor(e) {
    if (!e && !(e = this.get))
      throw Error("Unknown getter");
    bv(this, e);
  }
}, bv = (e, t) => nu(e, on, t);
function Gt(e, t) {
  if (e[on]) {
    let n = e[Ot];
    n || nu(e, Ot, n = /* @__PURE__ */ new Set()), n.has(t) || (n.add(t), e.observerAdded && e.observerAdded(n.size, t));
  }
  return t;
}
function an(e, t) {
  const n = e[Ot];
  if (n && n.has(t)) {
    const r = n.size - 1;
    r ? n.delete(t) : e[Ot] = null, e.observerRemoved && e.observerRemoved(r, t);
  }
}
var nu = (e, t, n) => Object.defineProperty(e, t, {
  value: n,
  writable: !0,
  configurable: !0
}), Hn = /[+\-]?(?:0|[1-9]\d*)(?:\.\d*)?(?:[eE][+\-]?\d+)?/g, wv = /(#(?:[0-9a-f]{2}){2,4}|(#[0-9a-f]{3})|(rgb|hsl)a?\((-?\d+%?[,\s]+){2,3}\s*[\d\.]+%?\))/gi, Ia = new RegExp(`(${Hn.source})(%|[a-z]+)`, "i"), Av = /rgba\(([0-9\.-]+), ([0-9\.-]+), ([0-9\.-]+), ([0-9\.-]+)\)/gi, jr = /var\((--[a-zA-Z0-9-_]+),? ?([a-zA-Z0-9 ()%#.,-]+)?\)/, ru = (e) => {
  const [t, n] = yv(e);
  if (!t || ys())
    return e;
  const r = window.getComputedStyle(document.documentElement).getPropertyValue(t);
  if (r)
    return r.trim();
  if (n && n.startsWith("--")) {
    const o = window.getComputedStyle(document.documentElement).getPropertyValue(n);
    return o || e;
  } else {
    if (n && jr.test(n))
      return ru(n);
    if (n)
      return n;
  }
  return e;
}, yv = (e) => {
  const t = jr.exec(e);
  if (!t) return [,];
  const [, n, r] = t;
  return [n, r];
}, Wr, Ev = (e, t, n, r, o) => `rgba(${Math.round(t)}, ${Math.round(n)}, ${Math.round(r)}, ${o})`, ou = (e) => {
  Wr || (Wr = _e ? (
    // match color names, ignore partial matches
    new RegExp(`(${Object.keys(_e).join("|")})(?!\\w)`, "g")
  ) : (
    // never match
    /^\b$/
  ));
  const t = e.output.map((s) => pe(s).replace(jr, ru).replace(wv, Ca).replace(Wr, Ca)), n = t.map((s) => s.match(Hn).map(Number)), o = n[0].map(
    (s, i) => n.map((c) => {
      if (!(i in c))
        throw Error('The arity of each "output" value must be equal');
      return c[i];
    })
  ).map(
    (s) => rn({ ...e, output: s })
  );
  return (s) => {
    const i = !Ia.test(t[0]) && t.find((u) => Ia.test(u))?.replace(Hn, "");
    let c = 0;
    return t[0].replace(
      Hn,
      () => `${o[c++](s)}${i || ""}`
    ).replace(Av, Ev);
  };
}, js = "react-spring: ", su = (e) => {
  const t = e;
  let n = !1;
  if (typeof t != "function")
    throw new TypeError(`${js}once requires a function parameter`);
  return (...r) => {
    n || (t(...r), n = !0);
  };
}, Mv = su(console.warn);
function jv() {
  Mv(
    `${js}The "interpolate" function is deprecated in v9 (use "to" instead)`
  );
}
var Cv = su(console.warn);
function Nv() {
  Cv(
    `${js}Directly calling start instead of using the api object is deprecated in v9 (use ".start" instead), this will be removed in later 0.X.0 versions`
  );
}
function Cr(e) {
  return B.str(e) && (e[0] == "#" || /\d/.test(e) || // Do not identify a CSS variable as an AnimatedString if its SSR
  !ys() && jr.test(e) || e in (_e || {}));
}
var Cs = ys() ? ae : Mo, Iv = () => {
  const e = ge(!1);
  return Cs(() => (e.current = !0, () => {
    e.current = !1;
  }), []), e;
};
function au() {
  const e = de()[1], t = Iv();
  return () => {
    t.current && e(Math.random());
  };
}
var iu = (e) => ae(e, Sv), Sv = [];
function Sa(e) {
  const t = ge(void 0);
  return ae(() => {
    t.current = e;
  }), t.current;
}
var cn = /* @__PURE__ */ Symbol.for("Animated:node"), kv = (e) => !!e && e[cn] === e, ke = (e) => e && e[cn], Ns = (e, t) => tv(e, cn, t), Nr = (e) => e && e[cn] && e[cn].getPayload(), cu = class {
  constructor() {
    Ns(this, this);
  }
  /** Get every `AnimatedValue` used by this node. */
  getPayload() {
    return this.payload || [];
  }
}, Ir = class lu extends cu {
  constructor(t) {
    super(), this._value = t, this.done = !0, this.durationProgress = 0, B.num(this._value) && (this.lastPosition = this._value);
  }
  /** @internal */
  static create(t) {
    return new lu(t);
  }
  getPayload() {
    return [this];
  }
  getValue() {
    return this._value;
  }
  setValue(t, n) {
    return B.num(t) && (this.lastPosition = t, n && (t = Math.round(t / n) * n, this.done && (this.lastPosition = t))), this._value === t ? !1 : (this._value = t, !0);
  }
  reset() {
    const { done: t } = this;
    this.done = !1, B.num(this._value) && (this.elapsedTime = 0, this.durationProgress = 0, this.lastPosition = this._value, t && (this.lastVelocity = null), this.v0 = null);
  }
}, nr = class uu extends Ir {
  constructor(t) {
    super(0), this._string = null, this._toString = rn({
      output: [t, t]
    });
  }
  /** @internal */
  static create(t) {
    return new uu(t);
  }
  getValue() {
    const t = this._string;
    return t ?? (this._string = this._toString(this._value));
  }
  setValue(t) {
    if (B.str(t)) {
      if (t == this._string)
        return !1;
      this._string = t, this._value = 1;
    } else if (super.setValue(t))
      this._string = null;
    else
      return !1;
    return !0;
  }
  reset(t) {
    t && (this._toString = rn({
      output: [this.getValue(), t]
    })), this._value = 0, super.reset();
  }
}, rr = { dependencies: null }, Sr = class extends cu {
  constructor(e) {
    super(), this.source = e, this.setValue(e);
  }
  getValue(e) {
    const t = {};
    return Be(this.source, (n, r) => {
      kv(n) ? t[r] = n.getValue(e) : we(n) ? t[r] = pe(n) : e || (t[r] = n);
    }), t;
  }
  /** Replace the raw object data */
  setValue(e) {
    this.source = e, this.payload = this._makePayload(e);
  }
  reset() {
    this.payload && $(this.payload, (e) => e.reset());
  }
  /** Create a payload set. */
  _makePayload(e) {
    if (e) {
      const t = /* @__PURE__ */ new Set();
      return Be(e, this._addToPayload, t), Array.from(t);
    }
  }
  /** Add to a payload set. */
  _addToPayload(e) {
    rr.dependencies && we(e) && rr.dependencies.add(e);
    const t = Nr(e);
    t && $(t, (n) => this.add(n));
  }
}, Dv = class du extends Sr {
  constructor(t) {
    super(t);
  }
  /** @internal */
  static create(t) {
    return new du(t);
  }
  getValue() {
    return this.source.map((t) => t.getValue());
  }
  setValue(t) {
    const n = this.getPayload();
    return t.length == n.length ? n.map((r, o) => r.setValue(t[o])).some(Boolean) : (super.setValue(t.map(Rv)), !0);
  }
};
function Rv(e) {
  return (Cr(e) ? nr : Ir).create(e);
}
function ho(e) {
  const t = ke(e);
  return t ? t.constructor : B.arr(e) ? Dv : Cr(e) ? nr : Ir;
}
var ka = (e, t) => {
  const n = (
    // Function components must use "forwardRef" to avoid being
    // re-rendered on every animation frame.
    !B.fun(e) || e.prototype && e.prototype.isReactComponent
  );
  return ar((r, o) => {
    const s = ge(null), i = n && // eslint-disable-next-line react-hooks/rules-of-hooks
    Re(
      (x) => {
        s.current = zv(o, x);
      },
      [o]
    ), [c, u] = Ov(r, t), l = au(), f = () => {
      const x = s.current;
      if (n && !x)
        return;
      (x ? t.applyAnimatedValues(x, c.getValue(!0)) : !1) === !1 && l();
    }, d = new Pv(f, u), m = ge(void 0);
    Cs(() => (m.current = d, $(u, (x) => Gt(x, d)), () => {
      m.current && ($(
        m.current.deps,
        (x) => an(x, m.current)
      ), V.cancel(m.current.update));
    })), ae(f, []), iu(() => () => {
      const x = m.current;
      $(x.deps, (g) => an(g, x));
    });
    const p = t.getComponentProps(c.getValue());
    return /* @__PURE__ */ h.createElement(e, { ...p, ref: i });
  });
}, Pv = class {
  constructor(e, t) {
    this.update = e, this.deps = t;
  }
  eventObserved(e) {
    e.type == "change" && V.write(this.update);
  }
};
function Ov(e, t) {
  const n = /* @__PURE__ */ new Set();
  return rr.dependencies = n, e.style && (e = {
    ...e,
    style: t.createAnimatedStyle(e.style)
  }), e = new Sr(e), rr.dependencies = null, [e, n];
}
function zv(e, t) {
  return e && (B.fun(e) ? e(t) : e.current = t), t;
}
var Da = /* @__PURE__ */ Symbol.for("AnimatedComponent"), Bv = (e, {
  applyAnimatedValues: t = () => !1,
  createAnimatedStyle: n = (o) => new Sr(o),
  getComponentProps: r = (o) => o
} = {}) => {
  const o = {
    applyAnimatedValues: t,
    createAnimatedStyle: n,
    getComponentProps: r
  }, s = (i) => {
    const c = Ra(i) || "Anonymous";
    return B.str(i) ? i = s[i] || (s[i] = ka(i, o)) : i = i[Da] || (i[Da] = ka(i, o)), i.displayName = `Animated(${c})`, i;
  };
  return Be(e, (i, c) => {
    B.arr(e) && (c = Ra(i)), s[c] = s(i);
  }), {
    animated: s
  };
}, Ra = (e) => B.str(e) ? e : e && B.str(e.displayName) ? e.displayName : B.fun(e) && e.name || null;
function lt(e, ...t) {
  return B.fun(e) ? e(...t) : e;
}
var Xt = (e, t) => e === !0 || !!(t && e && (B.fun(e) ? e(t) : ve(e).includes(t))), fu = (e, t) => B.obj(e) ? t && e[t] : e, mu = (e, t) => e.default === !0 ? e[t] : e.default ? e.default[t] : void 0, Tv = (e) => e, Is = (e, t = Tv) => {
  let n = Gv;
  e.default && e.default !== !0 && (e = e.default, n = Object.keys(e));
  const r = {};
  for (const o of n) {
    const s = t(e[o], o);
    B.und(s) || (r[o] = s);
  }
  return r;
}, Gv = [
  "config",
  "onProps",
  "onStart",
  "onChange",
  "onPause",
  "onResume",
  "onRest"
], Yv = {
  config: 1,
  from: 1,
  to: 1,
  ref: 1,
  loop: 1,
  reset: 1,
  pause: 1,
  cancel: 1,
  reverse: 1,
  immediate: 1,
  default: 1,
  delay: 1,
  onProps: 1,
  onStart: 1,
  onChange: 1,
  onPause: 1,
  onResume: 1,
  onRest: 1,
  onResolve: 1,
  // Transition props
  items: 1,
  trail: 1,
  sort: 1,
  expires: 1,
  initial: 1,
  enter: 1,
  update: 1,
  leave: 1,
  children: 1,
  onDestroyed: 1,
  // Internal props
  keys: 1,
  callId: 1,
  parentId: 1
};
function Hv(e) {
  const t = {};
  let n = 0;
  if (Be(e, (r, o) => {
    Yv[o] || (t[o] = r, n++);
  }), n)
    return t;
}
function hu(e) {
  const t = Hv(e);
  if (t) {
    const n = { to: t };
    return Be(e, (r, o) => o in t || (n[o] = r)), n;
  }
  return { ...e };
}
function ln(e) {
  return e = pe(e), B.arr(e) ? e.map(ln) : Cr(e) ? Ie.createStringInterpolator({
    range: [0, 1],
    output: [e, e]
  })(1) : e;
}
function Fv(e) {
  for (const t in e) return !0;
  return !1;
}
function po(e) {
  return B.fun(e) || B.arr(e) && B.obj(e[0]);
}
function Lv(e, t) {
  e.ref?.delete(e), t?.delete(e);
}
function Qv(e, t) {
  t && e.ref !== t && (e.ref?.delete(e), t.add(e), e.ref = t);
}
var Zv = {
  default: { tension: 170, friction: 26 }
}, go = {
  ...Zv.default,
  mass: 1,
  damping: 1,
  easing: xv.linear,
  clamp: !1
}, Jv = class {
  constructor() {
    this.velocity = 0, Object.assign(this, go);
  }
};
function Wv(e, t, n) {
  n && (n = { ...n }, Pa(n, t), t = { ...n, ...t }), Pa(e, t), Object.assign(e, t);
  for (const i in go)
    e[i] == null && (e[i] = go[i]);
  let { frequency: r, damping: o } = e;
  const { mass: s } = e;
  return B.und(r) || (r < 0.01 && (r = 0.01), o < 0 && (o = 0), e.tension = Math.pow(2 * Math.PI / r, 2) * s, e.friction = 4 * Math.PI * o * s / r), e;
}
function Pa(e, t) {
  if (!B.und(t.decay))
    e.duration = void 0;
  else {
    const n = !B.und(t.tension) || !B.und(t.friction);
    (n || !B.und(t.frequency) || !B.und(t.damping) || !B.und(t.mass)) && (e.duration = void 0, e.decay = void 0), n && (e.frequency = void 0);
  }
}
var Oa = [], Uv = class {
  constructor() {
    this.changed = !1, this.values = Oa, this.toValues = null, this.fromValues = Oa, this.config = new Jv(), this.immediate = !1;
  }
};
function pu(e, { key: t, props: n, defaultProps: r, state: o, actions: s }) {
  return new Promise((i, c) => {
    let u, l, f = Xt(n.cancel ?? r?.cancel, t);
    if (f)
      p();
    else {
      B.und(n.pause) || (o.paused = Xt(n.pause, t));
      let x = r?.pause;
      x !== !0 && (x = o.paused || Xt(x, t)), u = lt(n.delay || 0, t), x ? (o.resumeQueue.add(m), s.pause()) : (s.resume(), m());
    }
    function d() {
      o.resumeQueue.add(m), o.timeouts.delete(l), l.cancel(), u = l.time - V.now();
    }
    function m() {
      u > 0 && !Ie.skipAnimation ? (o.delayed = !0, l = V.setTimeout(p, u), o.pauseQueue.add(d), o.timeouts.add(l)) : p();
    }
    function p() {
      o.delayed && (o.delayed = !1), o.pauseQueue.delete(d), o.timeouts.delete(l), e <= (o.cancelId || 0) && (f = !0);
      try {
        s.start({ ...n, callId: e, cancel: f }, i);
      } catch (x) {
        c(x);
      }
    }
  });
}
var Ss = (e, t) => t.length == 1 ? t[0] : t.some((n) => n.cancelled) ? Dt(e.get()) : t.every((n) => n.noop) ? gu(e.get()) : Me(
  e.get(),
  t.every((n) => n.finished)
), gu = (e) => ({
  value: e,
  noop: !0,
  finished: !0,
  cancelled: !1
}), Me = (e, t, n = !1) => ({
  value: e,
  finished: t,
  cancelled: n
}), Dt = (e) => ({
  value: e,
  cancelled: !0,
  finished: !1
});
function xu(e, t, n, r) {
  const { callId: o, parentId: s, onRest: i } = t, { asyncTo: c, promise: u } = n;
  return !s && e === c && !t.reset ? u : n.promise = (async () => {
    n.asyncId = o, n.asyncTo = e;
    const l = Is(
      t,
      (v, b) => (
        // The `onRest` prop is only called when the `runAsync` promise is resolved.
        b === "onRest" ? void 0 : v
      )
    );
    let f, d;
    const m = new Promise(
      (v, b) => (f = v, d = b)
    ), p = (v) => {
      const b = (
        // The `cancel` prop or `stop` method was used.
        o <= (n.cancelId || 0) && Dt(r) || // The async `to` prop was replaced.
        o !== n.asyncId && Me(r, !1)
      );
      if (b)
        throw v.result = b, d(v), v;
    }, x = (v, b) => {
      const A = new za(), w = new Ba();
      return (async () => {
        if (Ie.skipAnimation)
          throw un(n), w.result = Me(r, !1), d(w), w;
        p(A);
        const E = B.obj(v) ? { ...v } : { ...b, to: v };
        E.parentId = o, Be(l, (P, D) => {
          B.und(E[D]) && (E[D] = P);
        });
        const C = await r.start(E);
        return p(A), n.paused && await new Promise((P) => {
          n.resumeQueue.add(P);
        }), C;
      })();
    };
    let g;
    if (Ie.skipAnimation)
      return un(n), Me(r, !1);
    try {
      let v;
      B.arr(e) ? v = (async (b) => {
        for (const A of b)
          await x(A);
      })(e) : v = Promise.resolve(e(x, r.stop.bind(r))), await Promise.all([v.then(f), m]), g = Me(r.get(), !0, !1);
    } catch (v) {
      if (v instanceof za)
        g = v.result;
      else if (v instanceof Ba)
        g = v.result;
      else
        throw v;
    } finally {
      o == n.asyncId && (n.asyncId = s, n.asyncTo = s ? c : void 0, n.promise = s ? u : void 0);
    }
    return B.fun(i) && V.batchedUpdates(() => {
      i(g, r, r.item);
    }), g;
  })();
}
function un(e, t) {
  Vt(e.timeouts, (n) => n.cancel()), e.pauseQueue.clear(), e.resumeQueue.clear(), e.asyncId = e.asyncTo = e.promise = void 0, t && (e.cancelId = t);
}
var za = class extends Error {
  constructor() {
    super(
      "An async animation has been interrupted. You see this error because you forgot to use `await` or `.catch(...)` on its returned promise."
    );
  }
}, Ba = class extends Error {
  constructor() {
    super("SkipAnimationSignal");
  }
}, xo = (e) => e instanceof ks, Vv = 1, ks = class extends tu {
  constructor() {
    super(...arguments), this.id = Vv++, this._priority = 0;
  }
  get priority() {
    return this._priority;
  }
  set priority(e) {
    this._priority != e && (this._priority = e, this._onPriorityChange(e));
  }
  /** Get the current value */
  get() {
    const e = ke(this);
    return e && e.getValue();
  }
  /** Create a spring that maps our value to another value */
  to(...e) {
    return Ie.to(this, e);
  }
  /** @deprecated Use the `to` method instead. */
  interpolate(...e) {
    return jv(), Ie.to(this, e);
  }
  toJSON() {
    return this.get();
  }
  observerAdded(e) {
    e == 1 && this._attach();
  }
  observerRemoved(e) {
    e == 0 && this._detach();
  }
  /** Called when the first child is added. */
  _attach() {
  }
  /** Called when the last child is removed. */
  _detach() {
  }
  /** Tell our children about our new value */
  _onChange(e, t = !1) {
    sn(this, {
      type: "change",
      parent: this,
      value: e,
      idle: t
    });
  }
  /** Tell our children about our new priority */
  _onPriorityChange(e) {
    this.idle || Er.sort(this), sn(this, {
      type: "priority",
      parent: this,
      priority: e
    });
  }
}, pt = /* @__PURE__ */ Symbol.for("SpringPhase"), vu = 1, vo = 2, bo = 4, Ur = (e) => (e[pt] & vu) > 0, Ue = (e) => (e[pt] & vo) > 0, Qt = (e) => (e[pt] & bo) > 0, Ta = (e, t) => t ? e[pt] |= vo | vu : e[pt] &= ~vo, Ga = (e, t) => t ? e[pt] |= bo : e[pt] &= ~bo, qv = class extends ks {
  constructor(e, t) {
    if (super(), this.animation = new Uv(), this.defaultProps = {}, this._state = {
      paused: !1,
      delayed: !1,
      pauseQueue: /* @__PURE__ */ new Set(),
      resumeQueue: /* @__PURE__ */ new Set(),
      timeouts: /* @__PURE__ */ new Set()
    }, this._pendingCalls = /* @__PURE__ */ new Set(), this._lastCallId = 0, this._lastToId = 0, this._memoizedDuration = 0, !B.und(e) || !B.und(t)) {
      const n = B.obj(e) ? { ...e } : { ...t, from: e };
      B.und(n.default) && (n.default = !0), this.start(n);
    }
  }
  /** Equals true when not advancing on each frame. */
  get idle() {
    return !(Ue(this) || this._state.asyncTo) || Qt(this);
  }
  get goal() {
    return pe(this.animation.to);
  }
  get velocity() {
    const e = ke(this);
    return e instanceof Ir ? e.lastVelocity || 0 : e.getPayload().map((t) => t.lastVelocity || 0);
  }
  /**
   * When true, this value has been animated at least once.
   */
  get hasAnimated() {
    return Ur(this);
  }
  /**
   * When true, this value has an unfinished animation,
   * which is either active or paused.
   */
  get isAnimating() {
    return Ue(this);
  }
  /**
   * When true, all current and future animations are paused.
   */
  get isPaused() {
    return Qt(this);
  }
  /**
   *
   *
   */
  get isDelayed() {
    return this._state.delayed;
  }
  /** Advance the current animation by a number of milliseconds */
  advance(e) {
    let t = !0, n = !1;
    const r = this.animation;
    let { toValues: o } = r;
    const { config: s } = r, i = Nr(r.to);
    !i && we(r.to) && (o = ve(pe(r.to))), r.values.forEach((l, f) => {
      if (l.done) return;
      const d = (
        // Animated strings always go from 0 to 1.
        l.constructor == nr ? 1 : i ? i[f].lastPosition : o[f]
      );
      let m = r.immediate, p = d;
      if (!m) {
        if (p = l.lastPosition, s.tension <= 0) {
          l.done = !0;
          return;
        }
        let x = l.elapsedTime += e;
        const g = r.fromValues[f], v = l.v0 != null ? l.v0 : l.v0 = B.arr(s.velocity) ? s.velocity[f] : s.velocity;
        let b;
        const A = s.precision || (g == d ? 5e-3 : Math.min(1, Math.abs(d - g) * 1e-3));
        if (B.und(s.duration))
          if (s.decay) {
            const w = s.decay === !0 ? 0.998 : s.decay, E = Math.exp(-(1 - w) * x);
            p = g + v / (1 - w) * (1 - E), m = Math.abs(l.lastPosition - p) <= A, b = v * E;
          } else {
            b = l.lastVelocity == null ? v : l.lastVelocity;
            const w = s.restVelocity || A / 10, E = s.clamp ? 0 : s.bounce, C = !B.und(E), P = g == d ? l.v0 > 0 : g < d;
            let D, M = !1;
            const k = 1, Y = Math.ceil(e / k);
            for (let z = 0; z < Y && (D = Math.abs(b) > w, !(!D && (m = Math.abs(d - p) <= A, m))); ++z) {
              C && (M = p == d || p > d == P, M && (b = -b * E, p = d));
              const Z = -s.tension * 1e-6 * (p - d), q = -s.friction * 1e-3 * b, T = (Z + q) / s.mass;
              b = b + T * k, p = p + b * k;
            }
          }
        else {
          let w = 1;
          s.duration > 0 && (this._memoizedDuration !== s.duration && (this._memoizedDuration = s.duration, l.durationProgress > 0 && (l.elapsedTime = s.duration * l.durationProgress, x = l.elapsedTime += e)), w = (s.progress || 0) + x / this._memoizedDuration, w = w > 1 ? 1 : w < 0 ? 0 : w, l.durationProgress = w), p = g + s.easing(w) * (d - g), b = (p - l.lastPosition) / e, m = w == 1;
        }
        l.lastVelocity = b, Number.isNaN(p) && (console.warn("Got NaN while animating:", this), m = !0);
      }
      i && !i[f].done && (m = !1), m ? l.done = !0 : t = !1, l.setValue(p, s.round) && (n = !0);
    });
    const c = ke(this), u = c.getValue();
    if (t) {
      const l = pe(r.to);
      (u !== l || n) && !s.decay ? (c.setValue(l), this._onChange(l)) : n && s.decay && this._onChange(u), this._stop();
    } else n && this._onChange(u);
  }
  /** Set the current value, while stopping the current animation */
  set(e) {
    return V.batchedUpdates(() => {
      this._stop(), this._focus(e), this._set(e);
    }), this;
  }
  /**
   * Freeze the active animation in time, as well as any updates merged
   * before `resume` is called.
   */
  pause() {
    this._update({ pause: !0 });
  }
  /** Resume the animation if paused. */
  resume() {
    this._update({ pause: !1 });
  }
  /** Skip to the end of the current animation. */
  finish() {
    if (Ue(this)) {
      const { to: e, config: t } = this.animation;
      V.batchedUpdates(() => {
        this._onStart(), t.decay || this._set(e, !1), this._stop();
      });
    }
    return this;
  }
  /** Push props into the pending queue. */
  update(e) {
    return (this.queue || (this.queue = [])).push(e), this;
  }
  start(e, t) {
    let n;
    return B.und(e) ? (n = this.queue || [], this.queue = []) : n = [B.obj(e) ? e : { ...t, to: e }], Promise.all(
      n.map((r) => this._update(r))
    ).then((r) => Ss(this, r));
  }
  /**
   * Stop the current animation, and cancel any delayed updates.
   *
   * Pass `true` to call `onRest` with `cancelled: true`.
   */
  stop(e) {
    const { to: t } = this.animation;
    return this._focus(this.get()), un(this._state, e && this._lastCallId), V.batchedUpdates(() => this._stop(t, e)), this;
  }
  /** Restart the animation. */
  reset() {
    this._update({ reset: !0 });
  }
  /** @internal */
  eventObserved(e) {
    e.type == "change" ? this._start() : e.type == "priority" && (this.priority = e.priority + 1);
  }
  /**
   * Parse the `to` and `from` range from the given `props` object.
   *
   * This also ensures the initial value is available to animated components
   * during the render phase.
   */
  _prepareNode(e) {
    const t = this.key || "";
    let { to: n, from: r } = e;
    n = B.obj(n) ? n[t] : n, (n == null || po(n)) && (n = void 0), r = B.obj(r) ? r[t] : r, r == null && (r = void 0);
    const o = { to: n, from: r };
    return Ur(this) || (e.reverse && ([n, r] = [r, n]), r = pe(r), B.und(r) ? ke(this) || this._set(n) : this._set(r)), o;
  }
  /** Every update is processed by this method before merging. */
  _update({ ...e }, t) {
    const { key: n, defaultProps: r } = this;
    e.default && Object.assign(
      r,
      Is(
        e,
        (i, c) => /^on/.test(c) ? fu(i, n) : i
      )
    ), Ha(this, e, "onProps"), Jt(this, "onProps", e, this);
    const o = this._prepareNode(e);
    if (Object.isFrozen(this))
      throw Error(
        "Cannot animate a `SpringValue` object that is frozen. Did you forget to pass your component to `animated(...)` before animating its props?"
      );
    const s = this._state;
    return pu(++this._lastCallId, {
      key: n,
      props: e,
      defaultProps: r,
      state: s,
      actions: {
        pause: () => {
          Qt(this) || (Ga(this, !0), Ut(s.pauseQueue), Jt(
            this,
            "onPause",
            Me(this, Zt(this, this.animation.to)),
            this
          ));
        },
        resume: () => {
          Qt(this) && (Ga(this, !1), Ue(this) && this._resume(), Ut(s.resumeQueue), Jt(
            this,
            "onResume",
            Me(this, Zt(this, this.animation.to)),
            this
          ));
        },
        start: this._merge.bind(this, o)
      }
    }).then((i) => {
      if (e.loop && i.finished && !(t && i.noop)) {
        const c = bu(e);
        if (c)
          return this._update(c, !0);
      }
      return i;
    });
  }
  /** Merge props into the current animation */
  _merge(e, t, n) {
    if (t.cancel)
      return this.stop(!0), n(Dt(this));
    const r = !B.und(e.to), o = !B.und(e.from);
    if (r || o)
      if (t.callId > this._lastToId)
        this._lastToId = t.callId;
      else
        return n(Dt(this));
    const { key: s, defaultProps: i, animation: c } = this, { to: u, from: l } = c;
    let { to: f = u, from: d = l } = e;
    o && !r && (!t.default || B.und(f)) && (f = d), t.reverse && ([f, d] = [d, f]);
    const m = !He(d, l);
    m && (c.from = d), d = pe(d);
    const p = !He(f, u);
    p && this._focus(f);
    const x = po(t.to), { config: g } = c, { decay: v, velocity: b } = g;
    (r || o) && (g.velocity = 0), t.config && !x && Wv(
      g,
      lt(t.config, s),
      // Avoid calling the same "config" prop twice.
      t.config !== i.config ? lt(i.config, s) : void 0
    );
    let A = ke(this);
    if (!A || B.und(f))
      return n(Me(this, !0));
    const w = (
      // When `reset` is undefined, the `from` prop implies `reset: true`,
      // except for declarative updates. When `reset` is defined, there
      // must exist a value to animate from.
      B.und(t.reset) ? o && !t.default : !B.und(d) && Xt(t.reset, s)
    ), E = w ? d : this.get(), C = ln(f), P = B.num(C) || B.arr(C) || Cr(C), D = !x && (!P || Xt(i.immediate || t.immediate, s));
    if (p) {
      const z = ho(f);
      if (z !== A.constructor)
        if (D)
          A = this._set(C);
        else
          throw Error(
            `Cannot animate between ${A.constructor.name} and ${z.name}, as the "to" prop suggests`
          );
    }
    const M = A.constructor;
    let k = we(f), Y = !1;
    if (!k) {
      const z = w || !Ur(this) && m;
      (p || z) && (Y = He(ln(E), C), k = !Y), (!He(c.immediate, D) && !D || !He(g.decay, v) || !He(g.velocity, b)) && (k = !0);
    }
    if (Y && Ue(this) && (c.changed && !w ? k = !0 : k || this._stop(u)), !x && ((k || we(u)) && (c.values = A.getPayload(), c.toValues = we(f) ? null : M == nr ? [1] : ve(C)), c.immediate != D && (c.immediate = D, !D && !w && this._set(u)), k)) {
      const { onRest: z } = c;
      $(Kv, (q) => Ha(this, t, q));
      const Z = Me(this, Zt(this, u));
      Ut(this._pendingCalls, Z), this._pendingCalls.add(n), c.changed && V.batchedUpdates(() => {
        c.changed = !w, z?.(Z, this), w ? lt(i.onRest, Z) : c.onStart?.(Z, this);
      });
    }
    w && this._set(E), x ? n(xu(t.to, t, this._state, this)) : k ? this._start() : Ue(this) && !p ? this._pendingCalls.add(n) : n(gu(E));
  }
  /** Update the `animation.to` value, which might be a `FluidValue` */
  _focus(e) {
    const t = this.animation;
    e !== t.to && (Na(this) && this._detach(), t.to = e, Na(this) && this._attach());
  }
  _attach() {
    let e = 0;
    const { to: t } = this.animation;
    we(t) && (Gt(t, this), xo(t) && (e = t.priority + 1)), this.priority = e;
  }
  _detach() {
    const { to: e } = this.animation;
    we(e) && an(e, this);
  }
  /**
   * Update the current value from outside the frameloop,
   * and return the `Animated` node.
   */
  _set(e, t = !0) {
    const n = pe(e);
    if (!B.und(n)) {
      const r = ke(this);
      if (!r || !He(n, r.getValue())) {
        const o = ho(n);
        !r || r.constructor != o ? Ns(this, o.create(n)) : r.setValue(n), r && V.batchedUpdates(() => {
          this._onChange(n, t);
        });
      }
    }
    return ke(this);
  }
  _onStart() {
    const e = this.animation;
    e.changed || (e.changed = !0, Jt(
      this,
      "onStart",
      Me(this, Zt(this, e.to)),
      this
    ));
  }
  _onChange(e, t) {
    t || (this._onStart(), lt(this.animation.onChange, e, this)), lt(this.defaultProps.onChange, e, this), super._onChange(e, t);
  }
  // This method resets the animation state (even if already animating) to
  // ensure the latest from/to range is used, and it also ensures this spring
  // is added to the frameloop.
  _start() {
    const e = this.animation;
    ke(this).reset(pe(e.to)), e.immediate || (e.fromValues = e.values.map((t) => t.lastPosition)), Ue(this) || (Ta(this, !0), Qt(this) || this._resume());
  }
  _resume() {
    Ie.skipAnimation ? this.finish() : Er.start(this);
  }
  /**
   * Exit the frameloop and notify `onRest` listeners.
   *
   * Always wrap `_stop` calls with `batchedUpdates`.
   */
  _stop(e, t) {
    if (Ue(this)) {
      Ta(this, !1);
      const n = this.animation;
      $(n.values, (o) => {
        o.done = !0;
      }), n.toValues && (n.onChange = n.onPause = n.onResume = void 0), sn(this, {
        type: "idle",
        parent: this
      });
      const r = t ? Dt(this.get()) : Me(this.get(), Zt(this, e ?? n.to));
      Ut(this._pendingCalls, r), n.changed && (n.changed = !1, Jt(this, "onRest", r, this));
    }
  }
};
function Zt(e, t) {
  const n = ln(t), r = ln(e.get());
  return He(r, n);
}
function bu(e, t = e.loop, n = e.to) {
  const r = lt(t);
  if (r) {
    const o = r !== !0 && hu(r), s = (o || e).reverse, i = !o || o.reset;
    return dn({
      ...e,
      loop: t,
      // Avoid updating default props when looping.
      default: !1,
      // Never loop the `pause` prop.
      pause: void 0,
      // For the "reverse" prop to loop as expected, the "to" prop
      // must be undefined. The "reverse" prop is ignored when the
      // "to" prop is an array or function.
      to: !s || po(n) ? n : void 0,
      // Ignore the "from" prop except on reset.
      from: i ? e.from : void 0,
      reset: i,
      // The "loop" prop can return a "useSpring" props object to
      // override any of the original props.
      ...o
    });
  }
}
function dn(e) {
  const { to: t, from: n } = e = hu(e), r = /* @__PURE__ */ new Set();
  return B.obj(t) && Ya(t, r), B.obj(n) && Ya(n, r), e.keys = r.size ? Array.from(r) : null, e;
}
function Xv(e) {
  const t = dn(e);
  return B.und(t.default) && (t.default = Is(t)), t;
}
function Ya(e, t) {
  Be(e, (n, r) => n != null && t.add(r));
}
var Kv = [
  "onStart",
  "onRest",
  "onChange",
  "onPause",
  "onResume"
];
function Ha(e, t, n) {
  e.animation[n] = t[n] !== mu(t, n) ? fu(t[n], e.key) : void 0;
}
function Jt(e, t, ...n) {
  e.animation[t]?.(...n), e.defaultProps[t]?.(...n);
}
var _v = ["onStart", "onChange", "onRest"], $v = 1, eb = class {
  constructor(e, t) {
    this.id = $v++, this.springs = {}, this.queue = [], this._lastAsyncId = 0, this._active = /* @__PURE__ */ new Set(), this._changed = /* @__PURE__ */ new Set(), this._started = !1, this._state = {
      paused: !1,
      pauseQueue: /* @__PURE__ */ new Set(),
      resumeQueue: /* @__PURE__ */ new Set(),
      timeouts: /* @__PURE__ */ new Set()
    }, this._events = {
      onStart: /* @__PURE__ */ new Map(),
      onChange: /* @__PURE__ */ new Map(),
      onRest: /* @__PURE__ */ new Map()
    }, this._onFrame = this._onFrame.bind(this), t && (this._flush = t), e && this.start({ default: !0, ...e });
  }
  /**
   * Equals `true` when no spring values are in the frameloop, and
   * no async animation is currently active.
   */
  get idle() {
    return !this._state.asyncTo && Object.values(this.springs).every((e) => e.idle && !e.isDelayed && !e.isPaused);
  }
  get item() {
    return this._item;
  }
  set item(e) {
    this._item = e;
  }
  /** Get the current values of our springs */
  get() {
    const e = {};
    return this.each((t, n) => e[n] = t.get()), e;
  }
  /** Set the current values without animating. */
  set(e) {
    for (const t in e) {
      const n = e[t];
      B.und(n) || this.springs[t].set(n);
    }
  }
  /** Push an update onto the queue of each value. */
  update(e) {
    return e && this.queue.push(dn(e)), this;
  }
  /**
   * Start the queued animations for every spring, and resolve the returned
   * promise once all queued animations have finished or been cancelled.
   *
   * When you pass a queue (instead of nothing), that queue is used instead of
   * the queued animations added with the `update` method, which are left alone.
   */
  start(e) {
    let { queue: t } = this;
    return e ? t = ve(e).map(dn) : this.queue = [], this._flush ? this._flush(this, t) : (Mu(this, t), wo(this, t));
  }
  /** @internal */
  stop(e, t) {
    if (e !== !!e && (t = e), t) {
      const n = this.springs;
      $(ve(t), (r) => n[r].stop(!!e));
    } else
      un(this._state, this._lastAsyncId), this.each((n) => n.stop(!!e));
    return this;
  }
  /** Freeze the active animation in time */
  pause(e) {
    if (B.und(e))
      this.start({ pause: !0 });
    else {
      const t = this.springs;
      $(ve(e), (n) => t[n].pause());
    }
    return this;
  }
  /** Resume the animation if paused. */
  resume(e) {
    if (B.und(e))
      this.start({ pause: !1 });
    else {
      const t = this.springs;
      $(ve(e), (n) => t[n].resume());
    }
    return this;
  }
  /** Call a function once per spring value */
  each(e) {
    Be(this.springs, e);
  }
  /** @internal Called at the end of every animation frame */
  _onFrame() {
    const { onStart: e, onChange: t, onRest: n } = this._events, r = this._active.size > 0, o = this._changed.size > 0;
    (r && !this._started || o && !this._started) && (this._started = !0, Vt(e, ([c, u]) => {
      u.value = this.get(), c(u, this, this._item);
    }));
    const s = !r && this._started, i = o || s && n.size ? this.get() : null;
    o && t.size && Vt(t, ([c, u]) => {
      u.value = i, c(u, this, this._item);
    }), s && (this._started = !1, Vt(n, ([c, u]) => {
      u.value = i, c(u, this, this._item);
    }));
  }
  /** @internal */
  eventObserved(e) {
    if (e.type == "change")
      this._changed.add(e.parent), e.idle || this._active.add(e.parent);
    else if (e.type == "idle")
      this._active.delete(e.parent);
    else return;
    V.onFrame(this._onFrame);
  }
};
function wo(e, t) {
  return Promise.all(t.map((n) => wu(e, n))).then(
    (n) => Ss(e, n)
  );
}
async function wu(e, t, n) {
  const { keys: r, to: o, from: s, loop: i, onRest: c, onResolve: u } = t, l = B.obj(t.default) && t.default;
  i && (t.loop = !1), o === !1 && (t.to = null), s === !1 && (t.from = null);
  const f = B.arr(o) || B.fun(o) ? o : void 0;
  f ? (t.to = void 0, t.onRest = void 0, l && (l.onRest = void 0)) : $(_v, (g) => {
    const v = t[g];
    if (B.fun(v)) {
      const b = e._events[g];
      t[g] = ({ finished: A, cancelled: w }) => {
        const E = b.get(v);
        E ? (A || (E.finished = !1), w && (E.cancelled = !0)) : b.set(v, {
          value: null,
          finished: A || !1,
          cancelled: w || !1
        });
      }, l && (l[g] = t[g]);
    }
  });
  const d = e._state;
  t.pause === !d.paused ? (d.paused = t.pause, Ut(t.pause ? d.pauseQueue : d.resumeQueue)) : d.paused && (t.pause = !0);
  const m = (r || Object.keys(e.springs)).map(
    (g) => e.springs[g].start(t)
  ), p = t.cancel === !0 || mu(t, "cancel") === !0;
  (f || p && d.asyncId) && m.push(
    pu(++e._lastAsyncId, {
      props: t,
      state: d,
      actions: {
        pause: fo,
        resume: fo,
        start(g, v) {
          p ? (un(d, e._lastAsyncId), v(Dt(e))) : (g.onRest = c, v(
            xu(
              f,
              g,
              d,
              e
            )
          ));
        }
      }
    })
  ), d.paused && await new Promise((g) => {
    d.resumeQueue.add(g);
  });
  const x = Ss(e, await Promise.all(m));
  if (i && x.finished && !(n && x.noop)) {
    const g = bu(t, i, o);
    if (g)
      return Mu(e, [g]), wu(e, g, !0);
  }
  return u && V.batchedUpdates(() => u(x, e, e.item)), x;
}
function Fa(e, t) {
  const n = { ...e.springs };
  return t && $(ve(t), (r) => {
    B.und(r.keys) && (r = dn(r)), B.obj(r.to) || (r = { ...r, to: void 0 }), Eu(n, r, (o) => yu(o));
  }), Au(e, n), n;
}
function Au(e, t) {
  Be(t, (n, r) => {
    e.springs[r] || (e.springs[r] = n, Gt(n, e));
  });
}
function yu(e, t) {
  const n = new qv();
  return n.key = e, t && Gt(n, t), n;
}
function Eu(e, t, n) {
  t.keys && $(t.keys, (r) => {
    (e[r] || (e[r] = n(r)))._prepareNode(t);
  });
}
function Mu(e, t) {
  $(t, (n) => {
    Eu(e.springs, n, (r) => yu(r, e));
  });
}
var tb = h.createContext({
  pause: !1,
  immediate: !1
}), nb = () => {
  const e = [], t = function(r) {
    Nv();
    const o = [];
    return $(e, (s, i) => {
      if (B.und(r))
        o.push(s.start());
      else {
        const c = n(r, s, i);
        c && o.push(s.start(c));
      }
    }), o;
  };
  t.current = e, t.add = function(r) {
    e.includes(r) || e.push(r);
  }, t.delete = function(r) {
    const o = e.indexOf(r);
    ~o && e.splice(o, 1);
  }, t.pause = function() {
    return $(e, (r) => r.pause(...arguments)), this;
  }, t.resume = function() {
    return $(e, (r) => r.resume(...arguments)), this;
  }, t.set = function(r) {
    $(e, (o, s) => {
      const i = B.fun(r) ? r(s, o) : r;
      i && o.set(i);
    });
  }, t.start = function(r) {
    const o = [];
    return $(e, (s, i) => {
      if (B.und(r))
        o.push(s.start());
      else {
        const c = this._getProps(r, s, i);
        c && o.push(s.start(c));
      }
    }), o;
  }, t.stop = function() {
    return $(e, (r) => r.stop(...arguments)), this;
  }, t.update = function(r) {
    return $(e, (o, s) => o.update(this._getProps(r, o, s))), this;
  };
  const n = function(r, o, s) {
    return B.fun(r) ? r(s, o) : r;
  };
  return t._getProps = n, t;
};
function rb(e, t, n) {
  const r = B.fun(t) && t;
  r && !n && (n = []);
  const o = ue(
    () => r || arguments.length == 3 ? nb() : void 0,
    []
  ), s = ge(0), i = au(), c = ue(
    () => ({
      ctrls: [],
      queue: [],
      flush(b, A) {
        const w = Fa(b, A);
        return s.current > 0 && !c.queue.length && !Object.keys(w).some((C) => !b.springs[C]) ? wo(b, A) : new Promise((C) => {
          Au(b, w), c.queue.push(() => {
            C(wo(b, A));
          }), i();
        });
      }
    }),
    []
  ), u = ge([...c.ctrls]), l = ge([]), f = Sa(e) || 0;
  ue(() => {
    $(u.current.slice(e, f), (b) => {
      Lv(b, o), b.stop(!0);
    }), u.current.length = e, d(f, e);
  }, [e]), ue(() => {
    d(0, Math.min(f, e));
  }, n);
  function d(b, A) {
    for (let w = b; w < A; w++) {
      const E = u.current[w] || (u.current[w] = new eb(null, c.flush)), C = r ? r(w, E) : t[w];
      C && (l.current[w] = Xv(C));
    }
  }
  const m = u.current.map(
    (b, A) => Fa(b, l.current[A])
  ), p = gt(tb), x = Sa(p), g = p !== x && Fv(p);
  Cs(() => {
    s.current++, c.ctrls = u.current;
    const { queue: b } = c;
    b.length && (c.queue = [], $(b, (A) => A())), $(u.current, (A, w) => {
      o?.add(A), g && A.start({ default: p });
      const E = l.current[w];
      E && (Qv(A, E.ref), A.ref ? A.queue.push(E) : A.start(E));
    });
  }), iu(() => () => {
    $(c.ctrls, (b) => b.stop(!0));
  });
  const v = m.map((b) => ({ ...b }));
  return o ? [v, o] : v;
}
function ju(e, t) {
  const n = B.fun(e), [[r], o] = rb(
    1,
    n ? e : [e],
    n ? [] : t
  );
  return n || arguments.length == 2 ? [r, o] : r;
}
var ob = class extends ks {
  constructor(e, t) {
    super(), this.source = e, this.idle = !0, this._active = /* @__PURE__ */ new Set(), this.calc = rn(...t);
    const n = this._get(), r = ho(n);
    Ns(this, r.create(n));
  }
  advance(e) {
    const t = this._get(), n = this.get();
    He(t, n) || (ke(this).setValue(t), this._onChange(t, this.idle)), !this.idle && La(this._active) && Vr(this);
  }
  _get() {
    const e = B.arr(this.source) ? this.source.map(pe) : ve(pe(this.source));
    return this.calc(...e);
  }
  _start() {
    this.idle && !La(this._active) && (this.idle = !1, $(Nr(this), (e) => {
      e.done = !1;
    }), Ie.skipAnimation ? (V.batchedUpdates(() => this.advance()), Vr(this)) : Er.start(this));
  }
  // Observe our sources only when we're observed.
  _attach() {
    let e = 1;
    $(ve(this.source), (t) => {
      we(t) && Gt(t, this), xo(t) && (t.idle || this._active.add(t), e = Math.max(e, t.priority + 1));
    }), this.priority = e, this._start();
  }
  // Stop observing our sources once we have no observers.
  _detach() {
    $(ve(this.source), (e) => {
      we(e) && an(e, this);
    }), this._active.clear(), Vr(this);
  }
  /** @internal */
  eventObserved(e) {
    e.type == "change" ? e.idle ? this.advance() : (this._active.add(e.parent), this._start()) : e.type == "idle" ? this._active.delete(e.parent) : e.type == "priority" && (this.priority = ve(this.source).reduce(
      (t, n) => Math.max(t, (xo(n) ? n.priority : 0) + 1),
      0
    ));
  }
};
function sb(e) {
  return e.idle !== !1;
}
function La(e) {
  return !e.size || Array.from(e).every(sb);
}
function Vr(e) {
  e.idle || (e.idle = !0, $(Nr(e), (t) => {
    t.done = !0;
  }), sn(e, {
    type: "idle",
    parent: e
  }));
}
Ie.assign({
  createStringInterpolator: ou,
  to: (e, t) => new ob(e, t)
});
var Cu = /^--/;
function ab(e, t) {
  return t == null || typeof t == "boolean" || t === "" ? "" : typeof t == "number" && t !== 0 && !Cu.test(e) && !(Kt.hasOwnProperty(e) && Kt[e]) ? t + "px" : ("" + t).trim();
}
var Qa = {};
function ib(e, t) {
  if (!e.nodeType || !e.setAttribute)
    return !1;
  const n = e.nodeName === "filter" || e.parentNode && e.parentNode.nodeName === "filter", {
    className: r,
    style: o,
    children: s,
    scrollTop: i,
    scrollLeft: c,
    viewBox: u,
    ...l
  } = t, f = Object.values(l), d = Object.keys(l).map(
    (m) => n || e.hasAttribute(m) ? m : Qa[m] || (Qa[m] = m.replace(
      /([A-Z])/g,
      // Attributes are written in dash case
      (p) => "-" + p.toLowerCase()
    ))
  );
  s !== void 0 && (e.textContent = s);
  for (const m in o)
    if (o.hasOwnProperty(m)) {
      const p = ab(m, o[m]);
      Cu.test(m) ? e.style.setProperty(m, p) : e.style[m] = p;
    }
  d.forEach((m, p) => {
    e.setAttribute(m, f[p]);
  }), r !== void 0 && (e.className = r), i !== void 0 && (e.scrollTop = i), c !== void 0 && (e.scrollLeft = c), u !== void 0 && e.setAttribute("viewBox", u);
}
var Kt = {
  animationIterationCount: !0,
  borderImageOutset: !0,
  borderImageSlice: !0,
  borderImageWidth: !0,
  boxFlex: !0,
  boxFlexGroup: !0,
  boxOrdinalGroup: !0,
  columnCount: !0,
  columns: !0,
  flex: !0,
  flexGrow: !0,
  flexPositive: !0,
  flexShrink: !0,
  flexNegative: !0,
  flexOrder: !0,
  gridRow: !0,
  gridRowEnd: !0,
  gridRowSpan: !0,
  gridRowStart: !0,
  gridColumn: !0,
  gridColumnEnd: !0,
  gridColumnSpan: !0,
  gridColumnStart: !0,
  fontWeight: !0,
  lineClamp: !0,
  lineHeight: !0,
  opacity: !0,
  order: !0,
  orphans: !0,
  tabSize: !0,
  widows: !0,
  zIndex: !0,
  zoom: !0,
  // SVG-related properties
  fillOpacity: !0,
  floodOpacity: !0,
  stopOpacity: !0,
  strokeDasharray: !0,
  strokeDashoffset: !0,
  strokeMiterlimit: !0,
  strokeOpacity: !0,
  strokeWidth: !0
}, cb = (e, t) => e + t.charAt(0).toUpperCase() + t.substring(1), lb = ["Webkit", "Ms", "Moz", "O"];
Kt = Object.keys(Kt).reduce((e, t) => (lb.forEach((n) => e[cb(n, t)] = e[t]), e), Kt);
var ub = /^(matrix|translate|scale|rotate|skew)/, db = /^(translate)/, fb = /^(rotate|skew)/, qr = (e, t) => B.num(e) && e !== 0 ? e + t : e, Fn = (e, t) => B.arr(e) ? e.every((n) => Fn(n, t)) : B.num(e) ? e === t : parseFloat(e) === t, mb = class extends Sr {
  constructor({ x: e, y: t, z: n, ...r }) {
    const o = [], s = [];
    (e || t || n) && (o.push([e || 0, t || 0, n || 0]), s.push((i) => [
      `translate3d(${i.map((c) => qr(c, "px")).join(",")})`,
      // prettier-ignore
      Fn(i, 0)
    ])), Be(r, (i, c) => {
      if (c === "transform")
        o.push([i || ""]), s.push((u) => [u, u === ""]);
      else if (ub.test(c)) {
        if (delete r[c], B.und(i)) return;
        const u = db.test(c) ? "px" : fb.test(c) ? "deg" : "";
        o.push(ve(i)), s.push(
          c === "rotate3d" ? ([l, f, d, m]) => [
            `rotate3d(${l},${f},${d},${qr(m, u)})`,
            Fn(m, 0)
          ] : (l) => [
            `${c}(${l.map((f) => qr(f, u)).join(",")})`,
            Fn(l, c.startsWith("scale") ? 1 : 0)
          ]
        );
      }
    }), o.length && (r.transform = new hb(o, s)), super(r);
  }
}, hb = class extends tu {
  constructor(e, t) {
    super(), this.inputs = e, this.transforms = t, this._value = null;
  }
  get() {
    return this._value || (this._value = this._get());
  }
  _get() {
    let e = "", t = !0;
    return $(this.inputs, (n, r) => {
      const o = pe(n[0]), [s, i] = this.transforms[r](
        B.arr(o) ? o : n.map(pe)
      );
      e += " " + s, t = t && i;
    }), t ? "none" : e;
  }
  // Start observing our inputs once we have an observer.
  observerAdded(e) {
    e == 1 && $(
      this.inputs,
      (t) => $(
        t,
        (n) => we(n) && Gt(n, this)
      )
    );
  }
  // Stop observing our inputs once we have no observers.
  observerRemoved(e) {
    e == 0 && $(
      this.inputs,
      (t) => $(
        t,
        (n) => we(n) && an(n, this)
      )
    );
  }
  eventObserved(e) {
    e.type == "change" && (this._value = null), sn(this, e);
  }
}, pb = [
  "a",
  "abbr",
  "address",
  "area",
  "article",
  "aside",
  "audio",
  "b",
  "base",
  "bdi",
  "bdo",
  "big",
  "blockquote",
  "body",
  "br",
  "button",
  "canvas",
  "caption",
  "cite",
  "code",
  "col",
  "colgroup",
  "data",
  "datalist",
  "dd",
  "del",
  "details",
  "dfn",
  "dialog",
  "div",
  "dl",
  "dt",
  "em",
  "embed",
  "fieldset",
  "figcaption",
  "figure",
  "footer",
  "form",
  "h1",
  "h2",
  "h3",
  "h4",
  "h5",
  "h6",
  "head",
  "header",
  "hgroup",
  "hr",
  "html",
  "i",
  "iframe",
  "img",
  "input",
  "ins",
  "kbd",
  "keygen",
  "label",
  "legend",
  "li",
  "link",
  "main",
  "map",
  "mark",
  "menu",
  "menuitem",
  "meta",
  "meter",
  "nav",
  "noscript",
  "object",
  "ol",
  "optgroup",
  "option",
  "output",
  "p",
  "param",
  "picture",
  "pre",
  "progress",
  "q",
  "rp",
  "rt",
  "ruby",
  "s",
  "samp",
  "script",
  "section",
  "select",
  "small",
  "source",
  "span",
  "strong",
  "style",
  "sub",
  "summary",
  "sup",
  "table",
  "tbody",
  "td",
  "textarea",
  "tfoot",
  "th",
  "thead",
  "time",
  "title",
  "tr",
  "track",
  "u",
  "ul",
  "var",
  "video",
  "wbr",
  // SVG
  "circle",
  "clipPath",
  "defs",
  "ellipse",
  "foreignObject",
  "g",
  "image",
  "line",
  "linearGradient",
  "mask",
  "path",
  "pattern",
  "polygon",
  "polyline",
  "radialGradient",
  "rect",
  "stop",
  "svg",
  "text",
  "tspan"
];
Ie.assign({
  batchedUpdates: Fu,
  createStringInterpolator: ou,
  colors: sv
});
var gb = Bv(pb, {
  applyAnimatedValues: ib,
  createAnimatedStyle: (e) => new mb(e),
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  getComponentProps: ({ scrollTop: e, scrollLeft: t, ...n }) => n
}), Nu = gb.animated;
const xb = "Save changes", vb = "Cancel", bb = "Review", wb = "OBJECT", Ab = "Changed", yb = 440, Eb = {
  tension: 320,
  friction: 34
}, Mb = 250, jb = 200;
function Rw({
  pendingCount: e,
  pendingItems: t,
  saving: n = !1,
  onSave: r,
  onCancel: o,
  saveLabel: s = xb,
  cancelLabel: i = vb,
  viewUnsavedLabel: c = bb,
  pendingKeyLabel: u = wb,
  pendingValueLabel: l = Ab,
  dimmed: f = !1,
  disabled: d = !1,
  expandMaxHeight: m = yb,
  expandSpringConfig: p = Eb,
  barTransitionMs: x = Mb,
  chevronTransitionMs: g = jb
}) {
  const [v, b] = de(!1), A = e > 0, w = ue(
    () => Array.isArray(t) && t.length > 0,
    [t]
  ), E = ue(
    () => (t ?? []).some((k) => !!k.section),
    [t]
  ), C = ue(() => {
    const k = /* @__PURE__ */ new Map();
    return (t ?? []).forEach((Y) => {
      const z = Y.section?.trim() || "General", Z = k.get(z) ?? [];
      Z.push(Y), k.set(z, Z);
    }), Array.from(k.entries()).map(([Y, z]) => ({
      section: Y,
      items: z
    }));
  }, [t]), [P, D] = ju(() => ({
    maxHeight: 0,
    opacity: 0,
    config: p
  }));
  if (ae(() => {
    A || b(!1);
  }, [A]), ae(() => {
    const k = v && w;
    D.start({
      maxHeight: k ? m : 0,
      opacity: k ? 1 : 0,
      config: p
    });
  }, [v, w, D, m, p]), typeof document > "u")
    return null;
  const M = e === 1 ? "1 unsaved change" : `${e} unsaved changes`;
  return Lu(
    /* @__PURE__ */ a.jsx(
      "div",
      {
        className: H(
          "fixed bottom-3 left-3 right-3 z-[35] transform-gpu transition-all md:bottom-6 md:left-[calc(18rem+1.5rem)] md:right-6",
          A ? "translate-y-0 opacity-100 pointer-events-auto" : "translate-y-6 opacity-0 pointer-events-none"
        ),
        style: { transitionDuration: `${x}ms` },
        children: /* @__PURE__ */ a.jsxs("div", { className: "relative", children: [
          /* @__PURE__ */ a.jsx(
            Nu.div,
            {
              style: {
                maxHeight: P.maxHeight,
                opacity: P.opacity
              },
              className: "overflow-hidden",
              children: /* @__PURE__ */ a.jsx(
                "div",
                {
                  className: H(
                    "pointer-events-auto mx-auto w-[calc(100%-7rem)] overflow-auto rounded-t-[28px] border border-b-0 border-border/70 shadow-[0_20px_48px_-28px_rgba(15,23,42,0.5)] backdrop-blur",
                    f ? "bg-muted/80 opacity-80 shadow-none" : "bg-card/95"
                  ),
                  children: /* @__PURE__ */ a.jsxs("table", { className: "w-full text-sm", children: [
                    /* @__PURE__ */ a.jsx("thead", { className: "sticky top-0 bg-muted/70", children: /* @__PURE__ */ a.jsxs("tr", { className: "border-b border-border/70", children: [
                      /* @__PURE__ */ a.jsx("th", { className: "px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground", children: u }),
                      /* @__PURE__ */ a.jsx("th", { className: "px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground", children: l })
                    ] }) }),
                    /* @__PURE__ */ a.jsx("tbody", { children: C.map((k, Y) => /* @__PURE__ */ a.jsxs(Gu, { children: [
                      E ? /* @__PURE__ */ a.jsx("tr", { className: "border-b border-border/70 bg-muted/45", children: /* @__PURE__ */ a.jsx(
                        "td",
                        {
                          colSpan: 2,
                          className: "px-4 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-muted-foreground",
                          children: k.section
                        }
                      ) }) : null,
                      k.items.map((z, Z) => /* @__PURE__ */ a.jsxs(
                        "tr",
                        {
                          className: "border-b border-border/60 last:border-b-0",
                          children: [
                            /* @__PURE__ */ a.jsx("td", { className: "px-4 py-2.5 align-top font-medium text-foreground", children: z.key }),
                            /* @__PURE__ */ a.jsx("td", { className: "px-4 py-2.5 align-top text-muted-foreground", children: z.value })
                          ]
                        },
                        `${k.section}-${z.key}-${z.value}-${Z}`
                      ))
                    ] }, `${k.section}-${Y}`)) })
                  ] })
                }
              )
            }
          ),
          /* @__PURE__ */ a.jsxs(
            "div",
            {
              className: H(
                "pointer-events-auto relative flex min-h-[3.75rem] items-center justify-between gap-3 rounded-[999px] border border-border/70 px-4 py-2 shadow-[0_18px_40px_-20px_rgba(15,23,42,0.4)] backdrop-blur-md",
                f ? "bg-muted/80 opacity-80 shadow-none" : "bg-card/96"
              ),
              children: [
                /* @__PURE__ */ a.jsxs("div", { className: "flex min-w-0 shrink items-center gap-2.5 text-sm", children: [
                  /* @__PURE__ */ a.jsxs("span", { className: "relative flex size-2.5 shrink-0", children: [
                    /* @__PURE__ */ a.jsx("span", { className: "absolute inline-flex h-full w-full animate-ping rounded-full bg-amber-400 opacity-60" }),
                    /* @__PURE__ */ a.jsx("span", { className: "relative inline-flex size-2.5 rounded-full bg-amber-400" })
                  ] }),
                  /* @__PURE__ */ a.jsx("span", { className: "truncate font-medium text-foreground", children: M })
                ] }),
                /* @__PURE__ */ a.jsxs("div", { className: "flex shrink-0 flex-wrap items-center justify-end gap-1.5", children: [
                  w ? /* @__PURE__ */ a.jsxs(
                    te,
                    {
                      type: "button",
                      variant: "outline",
                      size: "sm",
                      disabled: n || d,
                      onClick: () => b((k) => !k),
                      children: [
                        /* @__PURE__ */ a.jsx(Fg, { className: "size-4" }),
                        c,
                        /* @__PURE__ */ a.jsx(
                          io,
                          {
                            className: H(
                              "size-4 transition-transform",
                              v ? "rotate-0" : "rotate-180"
                            ),
                            style: { transitionDuration: `${g}ms` }
                          }
                        )
                      ]
                    }
                  ) : null,
                  o ? /* @__PURE__ */ a.jsxs(
                    te,
                    {
                      type: "button",
                      variant: "outline",
                      size: "sm",
                      onClick: o,
                      disabled: n || d,
                      children: [
                        /* @__PURE__ */ a.jsx(wn, { className: "size-4" }),
                        i
                      ]
                    }
                  ) : null,
                  /* @__PURE__ */ a.jsxs(
                    te,
                    {
                      type: "button",
                      size: "sm",
                      onClick: r,
                      disabled: n || d,
                      children: [
                        n ? /* @__PURE__ */ a.jsx(kl, { className: "size-4 animate-spin" }) : /* @__PURE__ */ a.jsx(Xg, { className: "size-4" }),
                        n ? "Saving…" : s
                      ]
                    }
                  )
                ] })
              ]
            }
          )
        ] })
      }
    ),
    document.body
  );
}
function Pw({
  columns: e,
  rows: t,
  rowKey: n,
  loading: r = !1,
  loadingMessage: o = "Loading...",
  emptyMessage: s = "No records found.",
  page: i,
  pageSize: c,
  pageSizeOptions: u = [10, 25, 50],
  onPageChange: l,
  onPageSizeChange: f,
  onRowClick: d,
  paginationMode: m = "client",
  totalRowCount: p,
  sortBy: x = null,
  sortOrder: g = "asc",
  onSortChange: v,
  alignPaginationToLeft: b = !1,
  onLoadingChange: A
}) {
  const w = Yu(), E = ge(null), C = ge(null), [P, D] = de(null), M = m === "server", k = M ? p ?? t.length : t.length, Y = Math.max(1, Math.ceil(k / c)), z = Math.min(i, Y - 1), Z = z * c, q = M ? t : t.slice(Z, Z + c), T = k === 0 || q.length === 0 ? 0 : Math.min(Z + q.length, k), R = z > 0, j = T < k, O = T === 0 ? "No results" : M && j ? `Showing ${Z + 1}-${T}` : `Showing ${Z + 1}-${T} of ${k}`, N = M && j ? `Page ${z + 1}` : `Page ${z + 1} of ${Y}`, S = Math.max(3, Math.min(c || 5, 8));
  ae(() => {
    if (A)
      return A(w, r, o), () => {
        A(w, !1);
      };
  }, [r, o, w, A]);
  const I = (y) => {
    if (!v)
      return;
    v(y, x === y && g === "asc" ? "desc" : "asc");
  };
  return ae(() => {
    const y = () => {
      if (!E.current)
        return;
      const X = E.current.closest("[data-detail-tab-content], [data-table-viewport]");
      if (!X) {
        D(null);
        return;
      }
      const _ = X;
      if (!_)
        return;
      const J = E.current.getBoundingClientRect(), oe = _.getBoundingClientRect(), ee = Math.max(0, J.top - oe.top), ne = _.clientHeight, le = C.current?.offsetHeight ?? 80, he = 16;
      if (ne <= 0) {
        D(null);
        return;
      }
      const fe = ne - ee - he;
      if (fe < 180) {
        const Tu = Math.max(40, fe - le - 8);
        D(Math.max(0, Tu));
        return;
      }
      const Bu = Math.max(100, Math.floor(fe - le - 8));
      D(Bu);
    };
    y();
    const G = window.requestAnimationFrame(y);
    window.addEventListener("resize", y);
    const Q = new ResizeObserver(y);
    E.current?.parentElement && Q.observe(E.current.parentElement);
    const W = E.current?.closest("[data-detail-tab-content], [data-table-viewport]");
    return W && Q.observe(W), Q.observe(document.documentElement), () => {
      window.cancelAnimationFrame(G), window.removeEventListener("resize", y), Q.disconnect();
    };
  }, []), /* @__PURE__ */ a.jsxs(
    "div",
    {
      ref: E,
      className: "flex min-h-0 flex-col overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_18px_40px_-28px_rgba(0,0,0,0.35)]",
      children: [
        /* @__PURE__ */ a.jsx(
          "div",
          {
            className: "min-h-0 overflow-auto",
            style: P ? { maxHeight: `${P}px` } : void 0,
            children: /* @__PURE__ */ a.jsxs(ps, { containerClassName: "overflow-x-visible", children: [
              /* @__PURE__ */ a.jsx(Zx, { children: /* @__PURE__ */ a.jsx(Mt, { className: "hover:bg-transparent", children: e.map((y) => /* @__PURE__ */ a.jsx(
                Jx,
                {
                  className: H("sticky top-0 z-[1] bg-card text-center", y.className),
                  children: y.sortable && v ? /* @__PURE__ */ a.jsxs(
                    "button",
                    {
                      type: "button",
                      onClick: () => I(y.id),
                      className: "inline-flex w-full cursor-pointer items-center justify-center gap-1 text-center text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground",
                      children: [
                        /* @__PURE__ */ a.jsx("span", { children: y.header }),
                        x === y.id ? g === "asc" ? /* @__PURE__ */ a.jsx(io, { className: "size-3.5" }) : /* @__PURE__ */ a.jsx(ns, { className: "size-3.5" }) : /* @__PURE__ */ a.jsx(io, { className: "size-3.5 opacity-30" })
                      ]
                    }
                  ) : y.header
                },
                y.id
              )) }) }),
              /* @__PURE__ */ a.jsx(gs, { children: r ? Array.from({ length: S }).map((y, G) => /* @__PURE__ */ a.jsx(Mt, { className: "hover:bg-transparent", children: e.map((Q) => /* @__PURE__ */ a.jsx(dt, { className: Q.cellClassName, children: /* @__PURE__ */ a.jsx("div", { className: "h-4 w-full animate-pulse rounded bg-muted/60" }) }, `${Q.id}-skeleton-${G}`)) }, `loading-skeleton-${G}`)) : q.length === 0 ? /* @__PURE__ */ a.jsx(Mt, { className: "hover:bg-transparent", children: /* @__PURE__ */ a.jsx(
                dt,
                {
                  colSpan: e.length,
                  className: "py-10 text-center text-sm text-muted-foreground",
                  children: s
                }
              ) }) : q.map((y) => /* @__PURE__ */ a.jsx(
                Mt,
                {
                  className: H("odd:bg-transparent even:bg-muted/20", d && "cursor-pointer"),
                  onClick: d ? () => d(y) : void 0,
                  children: e.map((G) => /* @__PURE__ */ a.jsx(dt, { className: G.cellClassName, children: G.render(y) }, G.id))
                },
                n(y)
              )) })
            ] })
          }
        ),
        r && /* @__PURE__ */ a.jsx("div", { className: "sr-only", "aria-live": "polite", children: o }),
        /* @__PURE__ */ a.jsxs(
          "div",
          {
            ref: C,
            className: H(
              "flex-shrink-0 flex flex-col gap-3 border-t border-border/70 bg-card px-4 py-3 sm:flex-row sm:items-center",
              b ? "sm:justify-start" : "sm:justify-between"
            ),
            children: [
              /* @__PURE__ */ a.jsx("div", { className: "text-sm text-muted-foreground", children: O }),
              /* @__PURE__ */ a.jsxs("div", { className: H("flex flex-col gap-3 sm:flex-row sm:items-center", b ? "sm:ml-4" : ""), children: [
                /* @__PURE__ */ a.jsxs("label", { className: "flex items-center gap-2 text-sm text-muted-foreground", children: [
                  /* @__PURE__ */ a.jsx("span", { children: "Rows" }),
                  /* @__PURE__ */ a.jsx(
                    Jl,
                    {
                      value: c,
                      onValueChange: (y) => f(Number(y)),
                      options: u.map((y) => ({ value: String(y), label: String(y) })),
                      className: "h-9",
                      searchPlaceholder: "Search rows..."
                    }
                  )
                ] }),
                /* @__PURE__ */ a.jsxs("div", { className: "flex items-center gap-2", children: [
                  /* @__PURE__ */ a.jsxs(
                    te,
                    {
                      type: "button",
                      variant: "outline",
                      size: "sm",
                      onClick: () => l(z - 1),
                      disabled: !R,
                      children: [
                        /* @__PURE__ */ a.jsx(Rg, { className: "size-4" }),
                        "Previous"
                      ]
                    }
                  ),
                  /* @__PURE__ */ a.jsx("span", { className: "min-w-20 text-center text-sm text-muted-foreground", children: N }),
                  /* @__PURE__ */ a.jsxs(
                    te,
                    {
                      type: "button",
                      variant: "outline",
                      size: "sm",
                      onClick: () => l(z + 1),
                      disabled: !j,
                      children: [
                        "Next",
                        /* @__PURE__ */ a.jsx(It, { className: "size-4" })
                      ]
                    }
                  )
                ] })
              ] })
            ]
          }
        )
      ]
    }
  );
}
const Ds = "2", Za = "returnUrl", Rs = "ui.detailViewMode", Ao = `${Rs}:latest`, yo = "detail-view-mode-change", Iu = {
  floating: !1,
  collapsed: "0"
}, Eo = /* @__PURE__ */ new Map();
function Su() {
  const [e, t] = de(
    () => typeof window > "u" ? "/" : window.location.pathname
  );
  return ae(() => {
    const n = () => t(window.location.pathname);
    return window.addEventListener("popstate", n), () => window.removeEventListener("popstate", n);
  }, []), e;
}
function Cb(e) {
  return e ? e.length > 1 && e.endsWith("/") ? e.slice(0, -1) : e : "/";
}
function Nb(e) {
  return e === "1" || e === Ds ? e : "0";
}
function Ib(e) {
  const t = e.floating === !0;
  return {
    floating: t,
    collapsed: t ? Nb(e.collapsed) : "0"
  };
}
function or(e, t) {
  return e?.floating === t.floating && e.collapsed === t.collapsed;
}
function ku(e) {
  const t = Cb(e), n = t.split("/").filter(Boolean);
  if (n[0] !== "management")
    return t;
  if (n[1] === "classes" && n[2]) {
    const r = n[3], o = n[4];
    return o && (r === "schedules" || r === "sessions" || r === "tests" || r === "feedbacks") ? `/management/classes/${n[2]}/${r}/${o}` : `/management/classes/${n[2]}`;
  }
  return (n[1] === "schedules" || n[1] === "sessions" || n[1] === "tests" || n[1] === "feedbacks") && n[2] ? `/management/${n[1]}/${n[2]}` : t;
}
function sr(e) {
  return Eo.get(e) ?? null;
}
function Sb() {
  typeof window > "u" || window.dispatchEvent(new Event(yo));
}
function Du(e) {
  const t = e.trim();
  if (!t)
    return null;
  const n = typeof window > "u" ? "http://localhost" : window.location.origin;
  try {
    return new URL(t, n);
  } catch {
    return null;
  }
}
function Ru(e, t) {
  const n = Du(e);
  if (!n)
    return e;
  const r = n.searchParams.get(Za)?.trim();
  return r && t > 0 && n.searchParams.set(
    Za,
    Ru(r, t - 1)
  ), `${n.pathname}${n.search}${n.hash}`;
}
function kb(e) {
  return Ru(e, 3);
}
function Pu(e) {
  return sr(
    `${Rs}:${ku(e)}`
  ) ?? sr(Ao);
}
function Ja(e, t) {
  const n = Ib(t), r = `${Rs}:${ku(e)}`, o = !or(
    sr(r),
    n
  ), s = !or(
    sr(Ao),
    n
  );
  return Eo.set(r, n), Eo.set(Ao, n), (o || s) && Sb(), n;
}
function Db(e) {
  return Pu(e) ?? Iu;
}
function Rb(e) {
  return kb(e);
}
function Ow(e) {
  const t = Du(Rb(e));
  return t ? {
    pathname: t.pathname,
    search: t.search,
    hash: t.hash
  } : null;
}
function Pb(e = {}) {
  const t = Su(), n = e.enabled ?? !0, r = e.pathname ?? t, [o, s] = de(0);
  return ae(() => {
    if (!n || typeof window > "u")
      return;
    const i = () => {
      s((c) => c + 1);
    };
    return window.addEventListener(
      yo,
      i
    ), () => {
      window.removeEventListener(
        yo,
        i
      );
    };
  }, [n]), ue(() => n ? Db(r) : Iu, [n, r, o]);
}
function Ob(e = {}) {
  const t = Su(), n = e.enabled ?? !0, r = e.pathname ?? t, o = Pb({
    enabled: n,
    pathname: r
  }), [s, i] = de(o);
  ae(() => {
    i(
      (u) => or(u, o) ? u : o
    );
  }, [o]), ae(() => {
    n && (or(Pu(r), s) || Ja(r, s));
  }, [n, r, s]);
  const c = Re(
    (u) => {
      n && i(Ja(r, u));
    },
    [n, r]
  );
  return {
    ...s,
    isCollapsed: s.collapsed !== "0",
    hiddenCollapsed: s.collapsed === Ds,
    setMode: c
  };
}
const Xr = {
  tabPillSpring: {
    tension: 300,
    friction: 26
  },
  tabContentFadeOutMs: 100,
  tabContentFadeInMs: 200
};
let Kr = {
  tabPillSpring: { ...Xr.tabPillSpring },
  tabContentFadeOutMs: Xr.tabContentFadeOutMs,
  tabContentFadeInMs: Xr.tabContentFadeInMs
}, zb = 0;
function Bb() {
  return typeof window > "u" ? !1 : window.__DETAIL_TABS_DEBUG__ ? !0 : new URLSearchParams(window.location.search).get("detailTabsDebug") === "1";
}
function Ee(e, t, n) {
  Bb() && console.debug(`[DetailTabs:${e}] ${t}`, {
    path: typeof window > "u" ? void 0 : `${window.location.pathname}${window.location.search}`,
    ...n
  });
}
function Tb() {
  return {
    tabPillSpring: { ...Kr.tabPillSpring },
    tabContentFadeOutMs: Kr.tabContentFadeOutMs,
    tabContentFadeInMs: Kr.tabContentFadeInMs
  };
}
const Ou = zt(null), Gb = zt(!1);
function zw({
  children: e,
  className: t = "w-full max-w-[1700px] space-y-4",
  isBackground: n,
  onCloseFloating: r
}) {
  const o = gt(Gb), s = n ?? o, i = Ob({ enabled: !s }), c = s ? !1 : i.floating, u = s ? !1 : i.isCollapsed, l = s ? !1 : i.hiddenCollapsed, f = (x, g, v = !1) => {
    if (s)
      return;
    const b = g ? v ? Ds : "1" : "0";
    i.setMode({
      floating: x,
      collapsed: b
    });
  }, d = () => {
    s || !r || r();
  }, m = () => {
    f(!1, !1, !1);
  }, p = () => {
    f(!0, !1, !1);
  };
  return ae(() => {
    if (typeof document > "u" || !c)
      return;
    const x = document.body.style.overflow;
    return document.body.style.overflow = "hidden", () => {
      document.body.style.overflow = x;
    };
  }, [c]), /* @__PURE__ */ a.jsx(
    Ou.Provider,
    {
      value: {
        floating: c,
        collapsed: u,
        hiddenCollapsed: l,
        isBackground: s,
        expandToNormalView: m,
        switchToFloatingView: p
      },
      children: /* @__PURE__ */ a.jsxs(
        "section",
        {
          className: H(
            c ? "fixed inset-0 z-50" : "w-full",
            c && l ? "pointer-events-none" : "",
            c && u && !l ? "pointer-events-none" : ""
          ),
          children: [
            c && !u ? /* @__PURE__ */ a.jsx(
              "button",
              {
                type: "button",
                "aria-label": "Close details",
                onClick: d,
                className: "absolute inset-0 bg-background/45 backdrop-blur-md"
              }
            ) : null,
            c && l ? /* @__PURE__ */ a.jsx(
              te,
              {
                type: "button",
                size: "sm",
                variant: "outline",
                "aria-label": "Restore details panel",
                onClick: () => f(!0, !0, !1),
                className: "pointer-events-auto absolute right-0 top-1/2 z-20 h-28 w-7 -translate-y-1/2 rounded-l-md rounded-r-none border-r-0 bg-background text-[10px] font-semibold uppercase tracking-[0.14em] shadow-md [writing-mode:vertical-rl]",
                children: "Details"
              }
            ) : null,
            /* @__PURE__ */ a.jsxs(
              "div",
              {
                "data-floating-collapsed": c && u ? "true" : "false",
                className: H(
                  t,
                  c ? u ? l ? "floating-detail pointer-events-none absolute right-0 top-1/2 z-10 w-[44dvw] min-w-[34rem] max-w-[70rem] -translate-y-1/2 translate-x-[calc(100%-20px)] !h-[76dvh] overflow-hidden rounded-2xl border border-border/70 bg-background p-4 shadow-2xl" : "floating-detail pointer-events-auto absolute right-3 top-1/2 z-10 w-[44dvw] min-w-[34rem] max-w-[70rem] -translate-y-1/2 !h-[76dvh] overflow-visible rounded-2xl border border-border/70 bg-background p-4 shadow-2xl" : "floating-detail absolute left-1/2 top-1/2 z-10 w-[85dvw] -translate-x-1/2 -translate-y-1/2 !h-[76dvh] overflow-visible rounded-2xl border border-border/70 bg-background p-4 shadow-2xl" : ""
                ),
                children: [
                  c && u ? l ? null : /* @__PURE__ */ a.jsxs(a.Fragment, { children: [
                    /* @__PURE__ */ a.jsx(
                      te,
                      {
                        type: "button",
                        size: "icon",
                        variant: "outline",
                        "aria-label": "Expand details panel",
                        onClick: () => f(!0, !1),
                        className: "absolute -left-4 top-1/2 z-20 h-8 w-8 -translate-y-1/2 rounded-full bg-background opacity-100 shadow-md",
                        children: /* @__PURE__ */ a.jsx(It, { className: "h-4 w-4 rotate-180" })
                      }
                    ),
                    /* @__PURE__ */ a.jsx(
                      te,
                      {
                        type: "button",
                        size: "icon",
                        variant: "outline",
                        "aria-label": "Fully collapse details",
                        onClick: () => f(!0, !0, !0),
                        className: "absolute -left-4 top-[calc(50%+2.5rem)] z-20 h-8 w-8 -translate-y-1/2 rounded-full bg-background opacity-100 shadow-md",
                        children: /* @__PURE__ */ a.jsx(It, { className: "h-4 w-4" })
                      }
                    )
                  ] }) : null,
                  c && !u ? /* @__PURE__ */ a.jsx(
                    te,
                    {
                      type: "button",
                      size: "icon",
                      variant: "outline",
                      "aria-label": "Collapse details",
                      onClick: () => f(!0, !0, !1),
                      className: "absolute -right-4 top-1/2 z-20 h-8 w-8 -translate-y-1/2 rounded-full bg-background opacity-100 shadow-md",
                      children: /* @__PURE__ */ a.jsx(It, { className: "h-4 w-4" })
                    }
                  ) : null,
                  e
                ]
              }
            )
          ]
        }
      )
    }
  );
}
function Bw({
  items: e,
  current: t
}) {
  return /* @__PURE__ */ a.jsxs("div", { className: "flex flex-wrap items-center gap-2 text-base", children: [
    e.map((n, r) => /* @__PURE__ */ a.jsxs("div", { className: "contents", children: [
      n.onClick ? /* @__PURE__ */ a.jsx(
        te,
        {
          type: "button",
          variant: "link",
          size: "sm",
          className: "h-auto px-0 text-base no-underline hover:no-underline",
          onClick: n.onClick,
          children: n.label
        }
      ) : /* @__PURE__ */ a.jsx("span", { className: "text-base", children: n.label }),
      /* @__PURE__ */ a.jsx("span", { className: "text-muted-foreground", children: ">" })
    ] }, `${n.label}-${r}`)),
    /* @__PURE__ */ a.jsx("h2", { className: "min-w-0 break-words text-xl font-semibold tracking-tight", children: t })
  ] });
}
function Tw({
  breadcrumbs: e,
  actions: t,
  className: n
}) {
  const r = gt(Ou), o = r?.floating && !r.collapsed, s = r && !r.floating;
  return /* @__PURE__ */ a.jsxs(
    "div",
    {
      "data-detail-header": !0,
      className: H(
        "flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between",
        n
      ),
      children: [
        /* @__PURE__ */ a.jsx("div", { className: "min-w-0 flex-1", children: e }),
        t ? /* @__PURE__ */ a.jsxs("div", { className: "flex shrink-0 items-center gap-2 self-start", children: [
          o ? /* @__PURE__ */ a.jsxs(
            te,
            {
              type: "button",
              size: "sm",
              variant: "outline",
              onClick: () => r.expandToNormalView(),
              children: [
                /* @__PURE__ */ a.jsx(va, { className: "mr-2 h-4 w-4" }),
                "Expand View"
              ]
            }
          ) : null,
          s ? /* @__PURE__ */ a.jsxs(
            te,
            {
              type: "button",
              size: "sm",
              variant: "outline",
              onClick: () => r.switchToFloatingView(),
              children: [
                /* @__PURE__ */ a.jsx(ba, { className: "mr-2 h-4 w-4" }),
                "Floating View"
              ]
            }
          ) : null,
          t
        ] }) : o || s ? /* @__PURE__ */ a.jsxs("div", { className: "flex shrink-0 items-center gap-2 self-start", children: [
          o ? /* @__PURE__ */ a.jsxs(
            te,
            {
              type: "button",
              size: "sm",
              variant: "outline",
              onClick: () => r.expandToNormalView(),
              children: [
                /* @__PURE__ */ a.jsx(va, { className: "mr-2 h-4 w-4" }),
                "Expand View"
              ]
            }
          ) : null,
          s ? /* @__PURE__ */ a.jsxs(
            te,
            {
              type: "button",
              size: "sm",
              variant: "outline",
              onClick: () => r.switchToFloatingView(),
              children: [
                /* @__PURE__ */ a.jsx(ba, { className: "mr-2 h-4 w-4" }),
                "Floating View"
              ]
            }
          ) : null
        ] }) : null
      ]
    }
  );
}
function Gw({
  children: e,
  className: t = "grid gap-4 rounded-3xl border border-border/70 bg-card p-5 shadow-[0_20px_50px_-30px_rgba(0,0,0,0.45)] md:grid-cols-4"
}) {
  return /* @__PURE__ */ a.jsx("div", { className: t, children: e });
}
function Yw({ label: e, value: t }) {
  return /* @__PURE__ */ a.jsxs("div", { className: "space-y-1", children: [
    /* @__PURE__ */ a.jsx("div", { className: "text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground", children: e }),
    /* @__PURE__ */ a.jsx("div", { className: "text-sm font-medium", children: t })
  ] });
}
function Yb({
  children: e,
  className: t = "rounded-3xl border border-border/70 bg-card p-5 shadow-[0_18px_40px_-28px_rgba(0,0,0,0.35)]"
}) {
  return /* @__PURE__ */ a.jsx("section", { className: t, children: e });
}
function Hw({
  label: e,
  value: t
}) {
  return /* @__PURE__ */ a.jsxs(Yb, { children: [
    /* @__PURE__ */ a.jsx("div", { className: "text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground", children: e }),
    /* @__PURE__ */ a.jsx("p", { className: "mt-2 whitespace-pre-wrap break-words text-sm leading-6", children: t?.trim() || "-" })
  ] });
}
function Fw({
  rows: e,
  className: t,
  tableClassName: n,
  labelColumnClassName: r
}) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      className: H(
        "overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_18px_40px_-28px_rgba(0,0,0,0.35)]",
        t
      ),
      children: /* @__PURE__ */ a.jsx(
        ps,
        {
          className: H(
            "min-w-[40rem] table-fixed",
            n
          ),
          children: /* @__PURE__ */ a.jsx(gs, { children: e.map((o, s) => /* @__PURE__ */ a.jsxs(
            Mt,
            {
              className: H(
                "hover:bg-transparent",
                o.rowClassName
              ),
              children: [
                /* @__PURE__ */ a.jsx(
                  dt,
                  {
                    className: H(
                      "border-r border-border/60 px-4 py-3 align-middle text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground",
                      r ?? "w-[220px]",
                      o.labelClassName
                    ),
                    children: o.label
                  }
                ),
                /* @__PURE__ */ a.jsx(
                  dt,
                  {
                    className: H(
                      "px-4 py-3 align-middle text-sm",
                      o.valueClassName
                    ),
                    children: o.value
                  }
                )
              ]
            },
            o.key ?? `detail-row-${s}`
          )) })
        }
      )
    }
  );
}
function Lw({
  className: e,
  classKey: t,
  onOpenClass: n,
  buttonLabel: r = "Open class"
}) {
  const o = t?.trim() || "";
  return /* @__PURE__ */ a.jsxs("div", { className: "flex min-w-0 flex-wrap items-center gap-2", children: [
    /* @__PURE__ */ a.jsx("span", { className: "break-words", children: e?.trim() || "-" }),
    /* @__PURE__ */ a.jsx(
      te,
      {
        type: "button",
        size: "sm",
        variant: "outline",
        onClick: n,
        disabled: !o,
        children: r
      }
    )
  ] });
}
function Qw({
  label: e = "Class",
  classKey: t,
  onOpenClass: n
}) {
  const r = t?.trim() || "";
  return /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1", children: [
    /* @__PURE__ */ a.jsx("span", { children: e }),
    /* @__PURE__ */ a.jsx(
      te,
      {
        type: "button",
        size: "icon",
        variant: "ghost",
        className: "h-5 w-5 p-0 text-muted-foreground hover:text-foreground",
        onClick: n,
        disabled: !r,
        children: /* @__PURE__ */ a.jsx(It, { className: "h-3.5 w-3.5" })
      }
    )
  ] });
}
function Zw({
  title: e,
  children: t
}) {
  return /* @__PURE__ */ a.jsxs("aside", { className: "rounded-3xl border border-border/70 bg-card p-5 shadow-[0_18px_40px_-28px_rgba(0,0,0,0.35)]", children: [
    /* @__PURE__ */ a.jsx("div", { className: "text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground", children: e }),
    /* @__PURE__ */ a.jsx("div", { className: "mt-3 flex flex-col gap-2", children: t })
  ] });
}
function Hb({
  tabs: e,
  activeTab: t,
  onChange: n
}) {
  const r = ge(/* @__PURE__ */ new Map()), o = ge(null), s = ge(++zb), i = ge(!1), c = e.find((d) => d.value === t)?.value ?? e[0]?.value, [u, l] = ju(() => ({
    left: 0,
    width: 0
  })), f = Re(
    (d) => {
      if (!c)
        return Ee(s.current, "updatePill:no-active-tab", {
          activeTab: t,
          resolvedActiveTab: c,
          animate: d,
          tabs: e.map((g) => g.value)
        }), !1;
      const m = r.current.get(c);
      if (!m)
        return Ee(s.current, "updatePill:no-button", {
          activeTab: t,
          resolvedActiveTab: c,
          animate: d,
          availableButtons: Array.from(r.current.keys())
        }), !1;
      const p = m.offsetWidth;
      if (p <= 0)
        return Ee(s.current, "updatePill:zero-width", {
          activeTab: t,
          resolvedActiveTab: c,
          animate: d,
          offsetLeft: m.offsetLeft,
          offsetWidth: p,
          rect: m.getBoundingClientRect().toJSON()
        }), !1;
      const x = Tb();
      return Ee(s.current, "updatePill:start", {
        activeTab: t,
        resolvedActiveTab: c,
        animate: d,
        offsetLeft: m.offsetLeft,
        offsetWidth: p,
        buttonRect: m.getBoundingClientRect().toJSON(),
        tabListRect: o.current?.getBoundingClientRect().toJSON()
      }), l.start({
        left: m.offsetLeft,
        width: p,
        immediate: !d,
        config: {
          tension: x.tabPillSpring.tension,
          friction: x.tabPillSpring.friction,
          clamp: !0
        }
      }), !0;
    },
    [t, l, c, e]
  );
  return Mo(() => {
    let d = 0, m = 0;
    const p = 8, x = () => {
      const g = f(i.current);
      Ee(s.current, "layout-effect:measure", {
        attempts: m,
        measured: g,
        initialized: i.current
      }), !g && m < p && (m += 1, d = requestAnimationFrame(x));
    };
    return x(), i.current = !0, () => {
      d && cancelAnimationFrame(d);
    };
  }, [f]), ae(() => {
    Ee(s.current, "render-state", {
      activeTab: t,
      resolvedActiveTab: c,
      tabs: e.map((d) => d.value)
    });
  }, [t, c, e]), ae(() => {
    const d = () => {
      Ee(s.current, "window:resize"), f(!0);
    };
    return window.addEventListener("resize", d), () => {
      window.removeEventListener("resize", d);
    };
  }, [f]), ae(() => {
    if (typeof ResizeObserver > "u")
      return;
    const d = o.current, m = c ? r.current.get(c) : null;
    if (!d && !m)
      return;
    const p = new ResizeObserver(() => {
      Ee(s.current, "resize-observer"), f(!0);
    });
    return d && p.observe(d), m && p.observe(m), () => {
      p.disconnect();
    };
  }, [c, f]), ae(() => {
    if (typeof document > "u" || !("fonts" in document))
      return;
    let d = !1;
    return document.fonts.ready.then(() => {
      d || (Ee(s.current, "fonts:ready", {
        status: document.fonts.status
      }), f(!0));
    }), () => {
      d = !0;
    };
  }, [f]), /* @__PURE__ */ a.jsx("div", { className: "overflow-x-auto rounded-2xl border border-border/70 bg-card/70 p-1 drop-shadow-md", children: /* @__PURE__ */ a.jsxs("div", { ref: o, className: "relative flex min-w-max gap-2", children: [
    /* @__PURE__ */ a.jsx(
      Nu.div,
      {
        "aria-hidden": "true",
        className: "pointer-events-none absolute left-0 top-0 h-full rounded-xl bg-primary shadow-sm",
        style: {
          left: u.left,
          width: u.width,
          opacity: u.width.to(
            (d) => d > 0 ? 1 : 0
          )
        }
      }
    ),
    e.map((d) => /* @__PURE__ */ a.jsx(
      "button",
      {
        ref: (m) => {
          m ? r.current.set(d.value, m) : r.current.delete(d.value), Ee(s.current, "button:ref", {
            tabValue: d.value,
            mounted: !!m,
            activeTab: t,
            resolvedActiveTab: c,
            offsetLeft: m?.offsetLeft,
            offsetWidth: m?.offsetWidth
          }), m && d.value === t && f(i.current);
        },
        type: "button",
        onClick: () => {
          Ee(s.current, "button:click", {
            tabValue: d.value,
            activeTab: t,
            resolvedActiveTab: c
          }), d.value !== c && n(d.value);
        },
        className: H(
          "relative z-10 shrink-0 cursor-pointer whitespace-nowrap rounded-xl px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] transition-colors duration-150",
          c === d.value ? "text-primary-foreground" : d.hasPendingChanges ? "font-bold text-primary hover:bg-primary/10" : "text-muted-foreground hover:text-accent-foreground"
        ),
        children: d.label
      },
      d.value
    ))
  ] }) });
}
function Jw({
  tabs: e,
  activeTab: t,
  onChange: n,
  children: r,
  className: o,
  contentClassName: s
}) {
  return /* @__PURE__ */ a.jsxs("section", { className: H("flex min-h-0 flex-col", o), children: [
    /* @__PURE__ */ a.jsx(
      "div",
      {
        "data-detail-tabs-wrap": !0,
        className: "shrink-0 overflow-visible pb-2",
        children: /* @__PURE__ */ a.jsx(
          Hb,
          {
            tabs: e,
            activeTab: t,
            onChange: n
          }
        )
      }
    ),
    /* @__PURE__ */ a.jsx(
      "div",
      {
        "data-detail-tab-content": !0,
        className: H(
          "mt-4 flex min-h-0 flex-1 flex-col",
          s
        ),
        children: r
      }
    )
  ] });
}
function Fb(e) {
  return Number.isFinite(e) ? Math.max(0, e) : 0;
}
function Ww({
  topLabel: e,
  primaryLabel: t,
  secondaryLabel: n,
  percent: r,
  bottomLabel: o,
  bottomLeftLabel: s,
  bottomRightLabel: i
}) {
  const c = Fb(r), u = c > 100, l = Math.min(100, c);
  return /* @__PURE__ */ a.jsxs("div", { className: "flex min-w-[220px] flex-col gap-1.5", children: [
    e ? /* @__PURE__ */ a.jsx("div", { className: "truncate text-[11px] text-muted-foreground", children: e }) : null,
    /* @__PURE__ */ a.jsxs("div", { className: "flex items-center justify-between gap-3 text-xs text-muted-foreground", children: [
      /* @__PURE__ */ a.jsx("span", { className: "truncate", children: t }),
      n ? /* @__PURE__ */ a.jsx("span", { className: "shrink-0 tabular-nums", children: n }) : null
    ] }),
    /* @__PURE__ */ a.jsx("div", { className: "h-2 overflow-hidden rounded-full bg-muted/70", children: /* @__PURE__ */ a.jsx(
      "div",
      {
        className: `h-full rounded-full transition-[width] ${u ? "bg-destructive" : "bg-primary"}`,
        style: { width: `${l}%` }
      }
    ) }),
    s || i ? /* @__PURE__ */ a.jsxs("div", { className: "flex items-center justify-between gap-3 text-[11px] text-muted-foreground", children: [
      /* @__PURE__ */ a.jsx("span", { className: "truncate", children: s }),
      /* @__PURE__ */ a.jsx("span", { className: "truncate text-right", children: i })
    ] }) : o ? /* @__PURE__ */ a.jsx("div", { className: "truncate text-[11px] text-muted-foreground", children: o }) : null
  ] });
}
function Uw({
  open: e,
  onOpenChange: t,
  trigger: n,
  onClear: r,
  onApply: o,
  children: s,
  className: i,
  gridClassName: c
}) {
  return /* @__PURE__ */ a.jsxs(qo, { open: e, onOpenChange: t, children: [
    /* @__PURE__ */ a.jsx(Xo, { asChild: !0, children: n }),
    /* @__PURE__ */ a.jsx(Ko, { children: /* @__PURE__ */ a.jsx(
      _o,
      {
        align: "start",
        side: "bottom",
        sideOffset: 6,
        className: H(
          "z-20 w-[min(calc(100vw-2rem),72rem)]",
          "outline-none data-[state=closed]:pointer-events-none",
          "data-[state=open]:animate-in data-[state=closed]:animate-out",
          "data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0",
          "data-[state=open]:zoom-in-95 data-[state=closed]:zoom-out-95"
        ),
        children: /* @__PURE__ */ a.jsxs(
          "div",
          {
            className: H(
              "rounded-lg border border-border/60 bg-background px-4 pb-4 pt-3 shadow-lg",
              i
            ),
            children: [
              /* @__PURE__ */ a.jsxs("div", { className: "mb-3 flex items-center gap-2.5", children: [
                /* @__PURE__ */ a.jsx("span", { className: "text-[11px] font-semibold uppercase tracking-widest text-muted-foreground/50", children: "Filters" }),
                /* @__PURE__ */ a.jsx("div", { className: "h-px flex-1 bg-border/40" })
              ] }),
              /* @__PURE__ */ a.jsx(
                "div",
                {
                  className: H(
                    "grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(min(18rem,100%),1fr))] [&>*]:min-w-0",
                    c
                  ),
                  children: s
                }
              ),
              (r || o) && /* @__PURE__ */ a.jsxs("div", { className: "mt-4 flex items-center justify-end gap-2 border-t border-border/30 pt-3", children: [
                r && /* @__PURE__ */ a.jsx(te, { type: "button", size: "sm", variant: "ghost", onClick: r, children: "Clear all" }),
                o && /* @__PURE__ */ a.jsx(te, { type: "button", size: "sm", onClick: o, children: "Apply filters" })
              ] })
            ]
          }
        )
      }
    ) })
  ] });
}
const Vw = ar(function({
  open: t,
  onToggle: n,
  activeCount: r = 0,
  className: o,
  onClick: s,
  ...i
}, c) {
  const u = r > 0, l = (f) => {
    s?.(f), f.defaultPrevented || n?.();
  };
  return /* @__PURE__ */ a.jsxs(
    te,
    {
      ref: c,
      type: "button",
      variant: t || u ? "secondary" : "outline",
      onClick: l,
      className: H("gap-1.5", o),
      ...i,
      children: [
        /* @__PURE__ */ a.jsx(ex, { className: "size-4" }),
        "Filters",
        u && /* @__PURE__ */ a.jsx("span", { className: "flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1.5 text-[11px] font-semibold leading-none text-primary-foreground", children: r })
      ]
    }
  );
});
function qw(e, t, n = "") {
  return e.get(t) ?? n;
}
function Xw(e, t, n, r = 0) {
  const o = e.get(t);
  if (!o)
    return n;
  const s = Number.parseInt(o, 10);
  return Number.isNaN(s) || s < r ? n : s;
}
function Kw(e, t, n) {
  const r = e.get(t);
  return r === "1" ? !0 : r === "0" ? !1 : n;
}
function _w(e, t) {
  const n = new URLSearchParams(e);
  return Object.entries(t).forEach(([r, o]) => {
    if (o == null || o === "" || o === !1) {
      n.delete(r);
      return;
    }
    if (o === !0) {
      n.set(r, "1");
      return;
    }
    n.set(r, String(o));
  }), n;
}
function $w(e) {
  if (!(e instanceof Error))
    return !1;
  const t = e.message.toLowerCase();
  return t.includes("403") || t.includes("forbidden") || t.includes("insufficient permission") || t.includes("insufficient permissions");
}
const Lb = "edited_at is older than the current value. refresh data and retry with a newer edited_at";
function eA(e) {
  return e instanceof Error ? e.message.toLowerCase().includes(Lb) : !1;
}
function tA({ ...e }) {
  return /* @__PURE__ */ a.jsx(Ci, { "data-slot": "sheet", ...e });
}
function nA({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Ni, { "data-slot": "sheet-trigger", ...e });
}
function rA({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(mn, { "data-slot": "sheet-close", ...e });
}
function Qb({
  ...e
}) {
  return /* @__PURE__ */ a.jsx(Ii, { "data-slot": "sheet-portal", ...e });
}
function Zb({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Si,
    {
      "data-slot": "sheet-overlay",
      className: H(
        "fixed inset-0 z-50 bg-black/50 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:animate-in data-[state=open]:fade-in-0",
        e
      ),
      ...t
    }
  );
}
function oA({
  className: e,
  children: t,
  side: n = "right",
  showCloseButton: r = !0,
  ...o
}) {
  return /* @__PURE__ */ a.jsxs(Qb, { children: [
    /* @__PURE__ */ a.jsx(Zb, {}),
    /* @__PURE__ */ a.jsxs(
      ki,
      {
        "data-slot": "sheet-content",
        className: H(
          "fixed z-50 flex flex-col gap-4 bg-background shadow-lg transition ease-in-out data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:animate-in data-[state=open]:duration-500",
          n === "right" && "inset-y-0 right-0 h-full w-3/4 border-l data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-sm",
          n === "left" && "inset-y-0 left-0 h-full w-3/4 border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-sm",
          n === "top" && "inset-x-0 top-0 h-auto border-b data-[state=closed]:slide-out-to-top data-[state=open]:slide-in-from-top",
          n === "bottom" && "inset-x-0 bottom-0 h-auto border-t data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom",
          e
        ),
        ...o,
        children: [
          t,
          r && /* @__PURE__ */ a.jsxs(mn, { className: "absolute top-4 right-4 rounded-xs opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:outline-hidden disabled:pointer-events-none data-[state=open]:bg-secondary", children: [
            /* @__PURE__ */ a.jsx(wn, { className: "size-4" }),
            /* @__PURE__ */ a.jsx("span", { className: "sr-only", children: "Close" })
          ] })
        ]
      }
    )
  ] });
}
function sA({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "sheet-header",
      className: H("flex flex-col gap-1.5 p-4", e),
      ...t
    }
  );
}
function aA({ className: e, ...t }) {
  return /* @__PURE__ */ a.jsx(
    "div",
    {
      "data-slot": "sheet-footer",
      className: H("mt-auto flex flex-col gap-2 p-4", e),
      ...t
    }
  );
}
function iA({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Di,
    {
      "data-slot": "sheet-title",
      className: H("font-semibold text-foreground", e),
      ...t
    }
  );
}
function cA({
  className: e,
  ...t
}) {
  return /* @__PURE__ */ a.jsx(
    Ri,
    {
      "data-slot": "sheet-description",
      className: H("text-sm text-muted-foreground", e),
      ...t
    }
  );
}
function Jb(e) {
  if (!e || typeof e != "object") return null;
  const t = e;
  return typeof t.appsheet_key != "string" || typeof t.title != "string" ? null : {
    appsheet_key: t.appsheet_key,
    title: t.title,
    description: typeof t.description == "string" ? t.description : null,
    priority: typeof t.priority == "string" ? t.priority : null,
    start_time: typeof t.start_time == "string" ? t.start_time : null,
    end_time: typeof t.end_time == "string" ? t.end_time : null,
    created_at: typeof t.created_at == "string" ? t.created_at : null
  };
}
async function Wb() {
  const e = await at(
    `${Ln.baseURL}/users/me/notifications`,
    { method: "GET", headers: { Accept: "application/json" }, credentials: "include" }
  );
  if (!e.ok)
    throw new Error(`Failed to fetch notifications: ${e.status} ${e.statusText}`);
  const t = await e.json();
  return (Array.isArray(t) ? t : Array.isArray(t?.notifications) ? t.notifications : []).map(Jb).filter((r) => r !== null);
}
async function Ub(e) {
  try {
    await at(Qu(e), {
      method: "PATCH",
      headers: { Accept: "application/json", "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ dismissed: !0 })
    });
  } catch {
  }
}
const Vb = 300 * 1e3;
function qb(e) {
  const t = Date.now();
  return !(e.start_time && new Date(e.start_time).getTime() > t || e.end_time && new Date(e.end_time).getTime() < t);
}
const zu = zt(null);
function lA({ children: e }) {
  const { isAuthenticated: t } = Ll(), [n, r] = de([]), [o, s] = de(/* @__PURE__ */ new Set()), i = ge(!1), c = Re(async () => {
    if (!i.current) {
      i.current = !0;
      try {
        const g = await Wb();
        r(g);
      } catch {
      } finally {
        i.current = !1;
      }
    }
  }, []), u = Re(() => {
    c();
  }, [c]);
  ae(() => {
    if (!t) {
      r([]), s(/* @__PURE__ */ new Set());
      return;
    }
    c();
    const g = setInterval(u, Vb), v = () => {
      c();
    };
    return window.addEventListener("focus", v), () => {
      clearInterval(g), window.removeEventListener("focus", v);
    };
  }, [t, c, u]);
  const l = Re((g) => {
    s((v) => /* @__PURE__ */ new Set([...v, g])), Ub(g);
  }, []), f = n.filter(
    (g) => qb(g) && !o.has(g.appsheet_key)
  ), d = f.filter((g) => g.priority === "IMMEDIATE"), m = f.filter((g) => g.priority === "URGENT"), p = f.filter((g) => g.priority === "INFORMATIVE"), x = f.length;
  return /* @__PURE__ */ a.jsx(
    zu.Provider,
    {
      value: { immediate: d, urgent: m, informative: p, unreadCount: x, dismiss: l, refetch: u },
      children: e
    }
  );
}
function Ps() {
  const e = gt(zu);
  if (!e) throw new Error("useNotifications must be used within NotificationProvider");
  return e;
}
function uA() {
  const { informative: e, unreadCount: t, dismiss: n } = Ps(), [r, o] = de(!1), s = (i) => {
    !i && e.length > 0 && e.forEach((c) => n(c.appsheet_key)), o(i);
  };
  return /* @__PURE__ */ a.jsxs(qo, { open: r, onOpenChange: s, children: [
    /* @__PURE__ */ a.jsx(Xo, { asChild: !0, children: /* @__PURE__ */ a.jsxs(
      "button",
      {
        type: "button",
        className: "relative flex h-8 w-8 items-center justify-center rounded-md border border-border/70 bg-muted/45 text-foreground transition-colors hover:bg-muted",
        "aria-label": `Notifications${t > 0 ? `, ${t} unread` : ""}`,
        children: [
          /* @__PURE__ */ a.jsx(Ig, { className: "size-4" }),
          t > 0 && /* @__PURE__ */ a.jsx("span", { className: "absolute -right-1.5 -top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-destructive px-1 text-[10px] font-bold leading-none text-destructive-foreground", children: t > 99 ? "99+" : t })
        ]
      }
    ) }),
    /* @__PURE__ */ a.jsx(Ko, { children: /* @__PURE__ */ a.jsxs(
      _o,
      {
        align: "end",
        sideOffset: 8,
        className: H(
          "z-50 w-80 rounded-xl border border-border/70 bg-popover shadow-lg outline-none",
          "data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:slide-in-from-top-2",
          "data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:slide-out-to-top-2",
          "duration-150"
        ),
        children: [
          /* @__PURE__ */ a.jsxs("div", { className: "flex items-center justify-between border-b border-border/60 px-4 py-3", children: [
            /* @__PURE__ */ a.jsx("p", { className: "text-sm font-semibold text-foreground", children: "Notifications" }),
            e.length > 0 && /* @__PURE__ */ a.jsx(
              "button",
              {
                type: "button",
                onClick: () => {
                  e.forEach((i) => n(i.appsheet_key)), o(!1);
                },
                className: "text-xs text-muted-foreground transition-colors hover:text-foreground",
                children: "Clear all"
              }
            )
          ] }),
          e.length === 0 ? /* @__PURE__ */ a.jsx("div", { className: "px-4 py-8 text-center text-sm text-muted-foreground", children: "No new notifications" }) : /* @__PURE__ */ a.jsx("ul", { className: "max-h-80 divide-y divide-border/50 overflow-y-auto", children: e.map((i) => /* @__PURE__ */ a.jsxs("li", { className: "flex items-start gap-3 px-4 py-3", children: [
            /* @__PURE__ */ a.jsxs("div", { className: "min-w-0 flex-1", children: [
              /* @__PURE__ */ a.jsx("p", { className: "text-sm font-medium leading-snug text-foreground", children: i.title }),
              i.description && /* @__PURE__ */ a.jsx("p", { className: "mt-0.5 line-clamp-2 text-xs text-muted-foreground", children: i.description })
            ] }),
            /* @__PURE__ */ a.jsx(
              "button",
              {
                type: "button",
                onClick: () => n(i.appsheet_key),
                className: "mt-0.5 shrink-0 text-muted-foreground transition-colors hover:text-foreground",
                "aria-label": "Dismiss",
                children: /* @__PURE__ */ a.jsx(wn, { className: "size-3.5" })
              }
            )
          ] }, i.appsheet_key)) })
        ]
      }
    ) })
  ] });
}
function dA() {
  const { immediate: e, dismiss: t } = Ps(), n = e[0] ?? null;
  return n ? /* @__PURE__ */ a.jsx(rs, { open: !0, children: /* @__PURE__ */ a.jsxs(
    os,
    {
      showCloseButton: !1,
      onEscapeKeyDown: (r) => r.preventDefault(),
      onInteractOutside: (r) => r.preventDefault(),
      className: "sm:max-w-md",
      children: [
        /* @__PURE__ */ a.jsxs(ss, { children: [
          /* @__PURE__ */ a.jsxs("div", { className: "mb-1 flex items-center gap-2", children: [
            /* @__PURE__ */ a.jsxs("span", { className: "inline-flex items-center gap-1.5 rounded-full border border-destructive/30 bg-destructive/15 px-2.5 py-0.5 text-xs font-semibold text-destructive", children: [
              /* @__PURE__ */ a.jsx(Sl, { className: "size-3" }),
              "Important"
            ] }),
            e.length > 1 && /* @__PURE__ */ a.jsxs("span", { className: "text-xs text-muted-foreground", children: [
              "1 of ",
              e.length
            ] })
          ] }),
          /* @__PURE__ */ a.jsx(is, { children: n.title }),
          n.description && /* @__PURE__ */ a.jsx(cs, { children: n.description })
        ] }),
        /* @__PURE__ */ a.jsx(as, { children: /* @__PURE__ */ a.jsx(te, { onClick: () => t(n.appsheet_key), children: "Got it" }) })
      ]
    }
  ) }) : null;
}
function fA() {
  const { urgent: e, dismiss: t } = Ps();
  return e.length === 0 ? null : /* @__PURE__ */ a.jsx("div", { className: "space-y-2", children: e.map((n) => /* @__PURE__ */ a.jsxs(
    "div",
    {
      role: "alert",
      className: "flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-amber-800 dark:border-amber-400/25 dark:bg-amber-400/10 dark:text-amber-300",
      children: [
        /* @__PURE__ */ a.jsx(rx, { className: "mt-0.5 size-4 shrink-0 text-amber-500 dark:text-amber-400" }),
        /* @__PURE__ */ a.jsxs("div", { className: "min-w-0 flex-1", children: [
          /* @__PURE__ */ a.jsx("p", { className: "text-sm font-semibold leading-snug", children: n.title }),
          n.description && /* @__PURE__ */ a.jsx("p", { className: "mt-0.5 text-sm opacity-80", children: n.description })
        ] }),
        /* @__PURE__ */ a.jsx(
          "button",
          {
            type: "button",
            onClick: () => t(n.appsheet_key),
            className: "mt-0.5 shrink-0 opacity-60 transition-opacity hover:opacity-100",
            "aria-label": "Dismiss notification",
            children: /* @__PURE__ */ a.jsx(wn, { className: "size-4" })
          }
        )
      ]
    },
    n.appsheet_key
  )) });
}
export {
  Ln as API_CONFIG,
  Uw as AdvancedFiltersPanel,
  Vw as AdvancedFiltersToggle,
  Bl as AppTheme,
  bw as AuthProvider,
  Gb as BackgroundDetailViewContext,
  te as Button,
  Ml as Card,
  ew as CardAction,
  Nl as CardContent,
  bg as CardDescription,
  tw as CardFooter,
  jl as CardHeader,
  Cl as CardTitle,
  Mw as ColorModeIconDropdown,
  Gl as ColorModeSelect,
  Ds as DETAIL_HIDDEN_COLLAPSED_VALUE,
  Zw as DetailActionPanel,
  Bw as DetailBreadcrumbs,
  Yb as DetailCard,
  Qw as DetailClassHeaderLabel,
  Lw as DetailClassLinkValue,
  Fw as DetailFieldsTable,
  Tw as DetailHeader,
  Gw as DetailSummaryGrid,
  Yw as DetailSummaryItem,
  Jw as DetailTabbedSection,
  Hb as DetailTabs,
  Hw as DetailTextBlock,
  zw as DetailView,
  rs as Dialog,
  rw as DialogClose,
  os as DialogContent,
  cs as DialogDescription,
  as as DialogFooter,
  ss as DialogHeader,
  ax as DialogOverlay,
  sx as DialogPortal,
  is as DialogTitle,
  nw as DialogTrigger,
  us as DropdownMenu,
  iw as DropdownMenuCheckboxItem,
  fs as DropdownMenuContent,
  sw as DropdownMenuGroup,
  aw as DropdownMenuItem,
  yx as DropdownMenuLabel,
  ow as DropdownMenuPortal,
  Tl as DropdownMenuRadioGroup,
  St as DropdownMenuRadioItem,
  Ex as DropdownMenuSeparator,
  cw as DropdownMenuShortcut,
  lw as DropdownMenuSub,
  dw as DropdownMenuSubContent,
  uw as DropdownMenuSubTrigger,
  ds as DropdownMenuTrigger,
  vx as ForgotPassword,
  Ux as FormTableDialog,
  Wx as FormTableRow,
  dA as ImmediateNotificationDialog,
  ut as Input,
  Nt as Label,
  jw as LazyViewFallback,
  Iw as MultiSelectDropdown,
  uA as NotificationBell,
  lA as NotificationProvider,
  Mb as PENDING_CHANGES_BAR_DEFAULT_BAR_TRANSITION_MS,
  vb as PENDING_CHANGES_BAR_DEFAULT_CANCEL_LABEL,
  jb as PENDING_CHANGES_BAR_DEFAULT_CHEVRON_TRANSITION_MS,
  yb as PENDING_CHANGES_BAR_DEFAULT_EXPAND_MAX_HEIGHT,
  wb as PENDING_CHANGES_BAR_DEFAULT_PENDING_KEY_LABEL,
  Ab as PENDING_CHANGES_BAR_DEFAULT_PENDING_VALUE_LABEL,
  xb as PENDING_CHANGES_BAR_DEFAULT_SAVE_LABEL,
  Eb as PENDING_CHANGES_BAR_DEFAULT_SPRING_CONFIG,
  bb as PENDING_CHANGES_BAR_DEFAULT_VIEW_UNSAVED_LABEL,
  Dw as PatchRecordDialog,
  Rw as PendingChangesBar,
  yw as PendingChangesProvider,
  Ww as ProgressMetricCell,
  Aw as ResetPassword,
  Jl as SearchableSelect,
  wg as Separator,
  tA as Sheet,
  rA as SheetClose,
  oA as SheetContent,
  cA as SheetDescription,
  aA as SheetFooter,
  sA as SheetHeader,
  iA as SheetTitle,
  nA as SheetTrigger,
  ww as SignIn,
  Pw as SimpleDataTable,
  Yl as SitemarkIcon,
  tn as StatusBanner,
  ps as Table,
  gs as TableBody,
  kw as TableCaption,
  dt as TableCell,
  Sw as TableFooter,
  Jx as TableHead,
  Zx as TableHeader,
  Mt as TableRow,
  Qx as Textarea,
  fA as UrgentNotificationBanner,
  vg as buttonVariants,
  Nw as canAccessManagement,
  _b as configureApi,
  fw as formatDate,
  mw as formatUserTimestamp,
  Ow as getDetailReturnLocation,
  Kw as getQueryBoolean,
  Xw as getQueryNumber,
  qw as getQueryParam,
  vw as getTextareaRows,
  hs as getUserScopes,
  Fx as hasAnyUserScope,
  Cw as hasUserScope,
  eA as isOutdatedEditedAtConflictError,
  $w as isPermissionDeniedError,
  Pu as readStoredDetailViewMode,
  Db as resolveDetailViewMode,
  kb as stripDetailViewModeFromPath,
  pw as toBackendBoundary,
  xw as toBackendTimeValue,
  hw as toBackendTimestamp,
  gw as toDateTimeLocalValue,
  Rb as toDetailReturnUrl,
  _w as updateSearchParams,
  Ll as useAuth,
  zl as useColorMode,
  Ob as useDetailViewMode,
  Ps as useNotifications,
  Ew as usePendingChanges,
  Pb as useResolvedDetailViewMode,
  Ja as writeDetailViewMode
};
